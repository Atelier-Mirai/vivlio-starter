...
#### 既知の不具合（未修正）
- **sideimage 内の脚注URLが重複表示される** (`contents/00-preface.md` 謝辞・著者紹介): sideimage コンテナ内のリンクから生成された脚注URLが、PDF上で複数回表示される。Vivliostyle の `float: footnote` が、脚注参照（`<a href="#fnN">`）のあるページと脚注定義（`aside#fnN`）のあるページの両方に同じ脚注を表示する挙動が原因。sideimage 内では `aside` を参照と同じ段落直後に配置できない（CSS Grid レイアウトが壊れる）ため、`aside` が別ページに配置され重複が発生する。不正な脚注（`#fn5` 等）の表示は解消済み。詳細は `docs/footnote.md` を参照。
- **画像の配置（`align=left` / `align=center` / `align=right`）が乱れる** (`contents/25-cross-reference.md` §8.2): `align=left` による float が後続の `align=center` / `align=right` 画像に干渉し、各画像が意図した位置に配置されない。テキストの回り込みも崩れ、複数の配置指定が混在するとレイアウトが連鎖的に崩壊する。CSS の float 解除タイミングの問題と考えられるが、改修方法を検討中。
- **テーブル内リンクの脚注URLが重複表示される**: Markdown テーブルのセル内にリンク記法 `[text](url)` を含む場合、脚注化されたURLがページ下部に複数回（3回など）重複して表示される。テーブルが複数セルに同一URLを含む場合や、VFM のテーブル変換と脚注処理の相互作用が原因と考えられる。
...
### Fixed
- **付録の章番号レター（A/B/C...）が catalog.yml の順番と一致しない問題を修正** (`lib/vivlio/starter/cli/common.rb`): `appendix_number_to_letter` が章番号から直接レターを計算していたため（90→A, 91→B, ...）、付録が 91 から始まるプロジェクトでは `vs build 91` が「付録 B」と表示されていた。catalog.yml の付録一覧での順番に基づいてレターを割り当てるよう変更し、91 が最初の付録であれば「付録 A」と正しく表示されるようになった。全章ビルド時の目次と本文の表示も一致するようになった。
