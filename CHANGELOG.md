...
- **Step 8 既存 preview サーバー再利用（方策D）**: `PageMappingExtractor` が起動時にポート応答を確認し、既に `vivliostyle preview` が動いていれば起動・停止をスキップするようにしました（約 3s 短縮）。`vivliostyle_persistent_server_spec.md` に計測結果と方策評価を記載。
