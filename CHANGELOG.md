...
## 既知の不具合（Known Issues）
> 2026-06-14: `docs/specs/build-output-bugfix-spec.md` に基づき、印刷PDF本文欠落・付録採番・付録図表番号・EPUB奥付/索引併合・中間生成物残存・テーブル内数式を修正（下記 Fixed 参照）。EPUB の扉絵・段落数式は Kindle 互換性の課題があり保留（`docs/specs/epub-kindle-compatibility-report.md`）。

- **EPUB の扉絵（frontispiece/ornament）が表示されない**（保留）: 扉絵は CSS 背景（`background-attachment: fixed` + 固定ページ寸法前提）で描画されるため、リフロー型 EPUB では表示されない（画像実体は EPUB に同梱済み）。Kindle では背景画像・position 重ね合わせ・`vh` 全面がいずれも不安定で、確実なのはインライン img の上下並べのみ。Amazon 出稿を視野に方式未決のため保留。詳細・選択肢は `docs/specs/epub-kindle-compatibility-report.md`。
- **EPUB で段落・ブロック数式が生 LaTeX のまま露出する**（保留）: VFM は数式を `data-math-typeset` span（生 LaTeX）として出力し Vivliostyle の MathJax がレンダリング時に組版する設計のため、PDF では描画されるが EPUB 出力には焼き込まれない。temml 等で MathML 化すれば主要リーダーでは描画できるが、Kindle は MathML が不安定で崩れ得る。Kindle 確実化には画像化が要り規模大のため方式未決で保留。詳細は同報告書。
- **EPUB の本文 † マーク数が同時ビルドする target に依存する**（保留・ターゲット整合性テストで検出）: 用語集オートリンクの脚注記号「†」は、`pdf` を併せてビルドすると Step 8（backlink dedup）が「同一 PDF ページ内の 2 回目以降の †」を削除し、その結果が共有 HTML 経由で EPUB にも反映される。一方 `epub` 単体ビルドは Step 8 を実行しないため † が全て残る。よって `targets: epub` 単体と `targets: pdf, epub` とで EPUB の † 数が変わる。リフロー型 EPUB にページ概念はなく本来この差は生じるべきでない。dedup は PDF ページ依存処理のため、EPUB を共有 HTML の dedup 影響から切り離す修正（HTML スナップショット／EPUB 用再生成等）が必要だが規模があり、`build-output-bugfix-spec.md` ⑦ として別途検討。本文の実体テキスト自体は同一。
- **EPUB の表紙画像が一回り小さく見える**（要実機確認）: EPUB カバー（1600×2560, 比 1:1.6）と本文 A4（比 1:1.414）のアスペクト比差による表示上の見え方の可能性。欠陥か表示仕様かを Kindle Previewer 等で確認のうえ判断する。
