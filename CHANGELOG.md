...
### Added
- [Medium] **索引ライブラリの持ち運び（`vs index:export` / `vs index:import`）を追加**: 作者が書いた用語集の定義（`[g]`）と reject 一覧を、書籍をまたいで引き継げるようにした（`IndexCommands::IndexLibrary` を新設）。`index_glossary_terms.yml` 等はこの本固有の情報（出現章・行、バックリンク、`source`）と作者の資産（定義・棄却判断）が混在するため、**持ち運べる分だけ**を `index_library.yml`（`version: 1`）へ抜き出す。`export` は用語集[g]の `term`/`yomi`/`definition` と reject 一覧のみを term 昇順・冪等で書き出し（固有情報は除外）、`import` は追記マージで既存語を既定で温存（`--prefer-import` でライブラリ優先）・採用済み語は reject に追加しない。既定パスは `book.yml` の `index_glossary.library.path`（`export_to`/`import_from` で個別上書き可、`~` 展開）で設定でき、引数省略で使える。パス解決は「引数 > 設定 > 組み込み既定」。ルート routing に `index:export`/`index:import` を登録、`vs index` 概要ヘルプも更新。仕様は [index-library-portability-spec.md](docs/specs/index-library-portability-spec.md)（yomi 個人辞書は Phase 2）。回帰テスト（export の[g]限定/固有情報除外/冪等・import の追記/既存優先/`--prefer-import`/reject 衝突スキップ/パス解決）を追加。マニュアル（`33-index-glossary.md`）に説明を追記。
