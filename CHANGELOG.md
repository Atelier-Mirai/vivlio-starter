...
- [Medium] **`vs doctor` が壊れた inkscape を「✅ OK」と誤報告する不具合**: 存在チェック（`command_exists?`）が「ファイルが在り実行ビットが立つ」だけを見ていたため、削除済みアプリ本体を exec するラッパー（在るのに `exit 126` で起動失敗）を見抜けなかった。`command_runnable?` を新設し、`--version` の実起動で終了ステータスまで確認する機能チェックへ変更（inkscape に適用）。壊れたツールは MISSING として報告され、`--fix` の再インストール対象になる。
