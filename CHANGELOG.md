...
- **配る辞書そのものをリリースゲートで見張るようにした**（`DC-01`・`scripts/dict_conflicts.rb`・`rake test:manual` → `rake test:release`）。`ML-01` が「この本の原稿に指摘が無い」を見るのに対し、こちらは**辞書の側**を見る。辞書は scaffold に載って全ての本へ渡るので、矛盾を抱えたまま出荷すると著者は消せない指摘を踏む——直すと別のルールが鳴り、戻すと元のルールが鳴る。この本に用例の無い語でも刺さるため、原稿の検査では捕まらない。所要 2 秒。
  ...
  守る約束は 1 つ。**自前の辞書が「こう書け」と言う綴りは、他のどの規則からも叱られてはならない。** 検査は 3 つを見る。

  1. `spellcheck-tech-word` が叱らないか
  2. 自前辞書どうし・自分自身と逆を向いていないか
  3. 同じ `expected` を別ファイルが宣言していないか（下記）

  1 は**上流の実装を実際に呼ぶ**。自前で規則を真似ると癖まで写し取れないためで、実際 `spellcheck-technical-word` は「直し先が元の綴りの先頭に一致する規則」を自ら黙らせる。`エディター => エディタ` のような末尾長音を削る項目は**一度も発火しない**。この癖を知らずに正規表現で数えて、実害の無い衝突を 54 件も拾ったあとで気づいた。

- **原稿の文章品質をリリースゲートで見張るようにした**（`ML-01`・`rake test:manual` → `rake test:release`）。リポジトリのコード（`ruby -Ilib bin/vs`）で `vs lint` を実走させ、指摘ゼロを確かめる。同じ並びの `MB-03` が「ビルドの警告ゼロ」を見張っているのと対で、こちらは**出荷する本そのもの**の状態を見る。所要 9 秒。

  `rake test` には入れない——原稿を書いている最中は当然のように赤くなり、コードの回帰でないもので通常テストが落ちると赤の意味が薄れる。`rake lint` のような別タスクも作らない。`vs lint` と同じことを別の名前で提供するだけで、`config-key-criteria-guidelines.md` の「重複した入口か」にそのまま当たる。

  **逃げ道（allowlist）は置かない。** `MB-03` には `allowed_warnings.yml` があるが、lint 側はいま 0 件で、最初から例外表を用意すると「0 件を保つ」規律がすぐ緩む。抑止が要る箇所は原稿へ `<!-- no-lint -->` を置く——理由がその場に残るほうが、後から読める。失敗時は指摘の中身と直し方を assert メッセージに載せる。

- **抑止コメントに短い記法を入れた**。いちばん書く「次の行だけ」を、いちばん短い形にした。
  ...
  本書での実績は次の行だけが 24 回、範囲が 7 回で、最頻の用法がいちばん長い綴りだった。範囲指定を `-start` / `-end` にしたのは、区間の指定であることを名前で示すためである（Prettier の `prettier-ignore-start` / `-end` と同じ流儀。`disable` / `enable` は状態の切り替えに読め、閉じ忘れても名前が違和感を残さない）。**旧記法（`vs-lint-disable-next-line`・`vs-lint-disable`・`vs-lint-enable`）も引き続き受け付ける**ので、既存の原稿を書き換える必要はない。新旧が対になっていてもよい。

  定義は `NotationGuard` に一本化した。同じ正規表現が独自校正・スペルチェック・textlint への変換の 3 箇所に散っており、今回それが不具合の温床だったことも分かった（下記）。

- **抑止コメントの閉じ忘れを指摘するようにした**（`unclosed-suppression`）。`<!-- no-lint-start -->` を閉じ忘れると、そこから原稿の終わりまで指摘が**すべて**黙る。黙っていることは「指摘が出ない」という形でしか現れないので、著者は気づけない。開いたまま原稿が終わっていたら、開いた行を挙げる。

- **弱い表現の検出（`ja-no-weak-phrase`）を独自ルールで置き換えた**。上流のルールは形態素の並びで見るため、**同じ「かもしれません」でも直前の語形で当たったり外れたりしていた**——MeCab は「したかもしれません」を `かも`（副助詞）と切るが、「するかもしれません」は `か`＋`も` に割るので、1 語の `かも` を探す上流の規則から漏れる。本書でも 93 章の「将来的に正式対応するかもしれません」だけが素通りし、96 章の「生まれていなかったかもしれません」は指摘されていた。

  上流のルールは 5 パターン（`かも。`・`かもしれ`・`思う`・`思います`・`可能性を示唆している`）しか持たずオプションも無いので、`SUPERSEDED_TEXTLINT_RULES` に載せて切り、ProseChecker 側へ文字列で見る同名のルールを置いた（`sentence-length`・`ja-no-mixed-period` と同じやり方）。ルール名を揃えてあるので、`disabled_rules: [ja-no-weak-phrase]` はそのまま効く。

  「思う」は句点か読点が続く形だけを見る——「思うように動かない」は弱い表現ではない。**`--fix` はしない**。言い切れるかどうかは書き手にしか分からず、謝辞の「生まれていなかったかもしれません」のように弱さが意図された文もある。

- **文体の混在（`no-mix-dearu-desumasu`）の指摘に、挙がった語を添えるようにした**。これまでは固定の要約「「である」と「です・ます」が混在しています。」だけを出しており、**どの語がどちらの側で挙がったのかが落ちていた**。97 章の指摘を調べるのに `textlint --format json` を手で叩いて 394 行 119 桁の「である。」を突き止める必要があり、上流が最初から持っている情報を要約の段で捨てていたことになる。

  上流は 2 行目に `=> "ですます"調 の文体に、次の "である"調 の箇所があります: "である。"` と書く。ここから多数派と挙がった語を読み、`「である。」が「です・ます」の中に混在しています` と組み立てる。**文言から読むので逆向き**（である調の中の「です。」）**も同じ形で出る**。読み取れなければ従来の固定文に戻すので、上流が文言を変えても指摘は落とさない。

  「箇条書き:」「本文:」の接頭辞を外す判断はそのままで、場所の違いは引き続き 1 行に畳む。

- **`vs lint` が英文のまま返していた指摘を日本語にした**（`max-comma`）。上流ルールの `This sentence exceeds the maximum count of comma. Maximum is 3.` を「一つの文に半角カンマが多すぎます（上限 3 個）。読点「、」で区切るか文を分けてください（数字の桁区切りも数えます）」と出す。日本語の原稿を書いている著者に英文だけが返るのは、それだけで指摘が読み飛ばされる。

  **上限値は文面から持ち越す。** 「多すぎます」に丸めると、あと何個減らせばよいのかが分からなくなる。上流が文面を変えたら訳せないが、そのときも素通しで英文のまま出す——訳し損ねた英文が出るほうが、指摘そのものを取り落とすよりずっとよい。

  「数字の桁区切りも数えます」を添えたのは、**和文でこの規則に当たるのが桁区切り（`2,894 × 4,092 px`）か欧文の語の並びしかない**ためである（本書の 2 件がその両方だった）。書いておかないと、読点を 1 つしか打っていない著者が理由を探すことになる。

- **`config/.textlintrc.yml` に `max-ten`（読点）と `max-comma`（半角カンマ）の上限を書き出した**。どちらもプリセットが黙って持っていた値（3）で、書かないと調整できることが著者に見えない。値は変えていないので指摘は増減しない。

- **`vs lint` の指摘に出る出現行を、クリックでエディタに開けるようにした**（`book.yml` の `lint.line_links`）。`行: 223` を押すとその行が開く。指摘を読んでから原稿を開き、行を探す往復が要らなくなる。

  **方式を選べるようにしたのは、端末がリンクを見つける道が二通りあり、どちらも一長一短だからである。** `compact`（既定）はハイパーリンク（OSC 8）で、飛び先を URI に隠せるので表示は `行: 223` のまま薄い青で済む。ただしリンクの装飾は端末が決めるもので、下線を付ける端末では外せない。`path` は `行: contents/10-intro.md:223` と素のテキストで書く方式で、端末は**画面に見えている文字列**を読んでパスと行を割り出す——だから下線は付かない代わりに、パスを省くことはできない。「短く書いて下線も付けない」は原理的に選べないため、どちらを我慢するかを著者に委ねた。`off` で素の行番号に戻る。

  飛び先は `file://<絶対パス>:<行>` と書く。`#L223` や `?line=223` も試したが、数行ずれた位置に着地した（Zed で実測）。色は 256 色の薄い青にした——基本色（`\e[34m`）はテーマ側で塗り替えられ、背景に沈む配色があるため。`NO_COLOR` が立っていれば色だけを外し、リンクは残す。

  **出力先が端末でなければ、どの設定でも素の行番号に戻す。** `vs lint > report.txt` のようにファイルへ落とすときや、並列ビルド・構造化出力で行を配列へ溜めるときに、読めない制御文字がログへ紛れて差分の突き合わせを壊すのを避けるためである。

- **数と「つ」の表記を見る独自ルール `kansuji-counter-suffix` を追加した**。本書の方針は「数は漢数字（`一つ` `二つ目` `もう一つ`）、記号の個数は算用数字（`空白 2 つ` `3 つのバッククォート`）」で、原稿には `ひとつ`・`一つ`・`1つ`・`1 つ` が混ざっていた（1 だけで 32 箇所）。算用数字とひらがなで書いた数には漢数字を、記号の個数を漢数字やひらがなで書いた箇所には算用数字を薦める。

  記号の個数かどうかは、**数の直前か直後に記号の名前があるか**だけで見ている。名前は `空白` `改行` `バッククォート` `波かっこ` `縦棒` など記号の意味しか持たない語に限り、記号だけを書いたインラインコード（`` `---` ``）も含める。`ハッシュ`（Ruby の Hash）・`パイプ`・`ドット`・`文字` は載せない——試作で「1 つのハッシュで」を黙らせたため。同じく英数字を含むインラインコード（ファイル名）は記号と見なさない。文の意味は読んでいないので、「直前の 1 つだけ」が `---` を指すような文脈依存の箇所は判定できず、`<!-- vs-lint-disable-next-line -->` で黙らせる。本書全体で 50 件を指摘し、そのうち文脈依存で外したのは 4 件だった。**`--fix` はしない**——判定を外した箇所へ「空白二つ」を黙って書き込むことになるため。`ひとつづき`（一続き）は数ではないので拾わず、ひらがなは `ひとつ`〜`みっつ` に限る（`やっつける` `むっつり` に紛れるため）。数を算用数字で書く方針（JTF の「1つ」など）の著者は `disabled_rules` で切る。
...
- **「することも／はできる」を冗長と言わないようにした**（`ja-no-redundant-expression` の `dict1`・`dict2`）。この辞書は「すること**が**できる」→「できる」の冗長さを狙ったものだが、が・は・もを同じ扱いにしており、意味を持つ助詞まで削れと言っていた。本書の 10 件を一つずつ見ると、**正しい指摘は「が」の 1 件だけ**だった。

  - **「も」（7 件）はすべて意味を運んでいた。**「または、`bundled/` プレフィックスを付けて明示的に指定する**ことも**できます」の「も」は「別のやり方として、こうもできる」の意味で、`指定できます` に削ると唯一の方法に読める。7 件とも選択肢を足す文だった
  - **「は」（3 件）は否定の焦点を示していた。**「リストと図の両方で `@css-table-rotate` を定義する**ことは**できません」は自然な日本語で、1 件は「コピーしたり検索したり**することは**できなくなります」——削ると「〜たり〜たりする」の形が崩れる

  ルールの辞書ごとの除外設定（`dictOptions.dictN.allows`）で「することも」「することは」だけを外し、本当に冗長な「することができます」「することが可能です」は引き続き指摘する。本書の指摘は 100 → 90 件。

- **前後に空きのある ` / ` を `slash-between-japanese` の対象から外した**。「直後に続くブロック要素（コードブロック / テーブル / 画像）」に「`・` か全角 `／` に」と指摘していたが、`・` に直すと**意味が変わる**。本書では「・」が「と」、「 / 」が「どれか一つ」を表していた——和文の並列は中黒が 544 件で、空きありのスラッシュは全件が「どれか一つ」の意味だった（上の例は直後に来るどれかが対象、`章番号 / 範囲` は引数がどちらかの形、`高品質 / 標準` はどちらかを選ぶ）。三つ並べて `・` にすると「全部が対象」と読める。

  残る案の全角 `／` との違いは字幅だけで、前後に空きがあれば半角でも間合いが取れる。この規則が欧文の `EPUB / Kindle` を見逃しているのと同じ理由である（詰めると一語に見える、の裏返し）。詰まって見えるのは空きのない `有効/無効` の形（片側だけ空いた形を含む）に限るので、そちらは引き続き指摘する。見出しも「並べるなら `・`、どれか一つなら前後に空きを入れた ` / ` か全角 `／` に」と、**意味で直し方が分かれる**ことを示す文言に改めた。

  **`・` の並びの中の `/` も対象から外した。** `文字数・行数・文/節統計` は「文字数」「行数」「文/節統計」の 3 項目で、`・` より `/` を強く結ぶ書き分けである（`索引/用語集辞書`・`設定キーを足す/やめる` も同じ形）。判定は左右の捕獲（各 6 字まで）に `・` があるかだけを見る。文単位に広げる案も試したが、「リンク・画像の基本検証を有効/無効にする」のように**無関係な `・` と同居しただけの `/` まで黙る**——`リンク・画像` は「基本検証」に掛かるだけで、`有効/無効` とは関係が無い。この違いは文法を読まないと分からない。

  窓の外にある `・`（`運転免許・IT資格（基本情報技術者/応用情報技術者）`）は指摘されるが、**見逃すより検出して著者に委ねる**ほうを採った。lint の役目は候補を示すことで、まれな誤検出は `<!-- vs-lint-disable-next-line -->` で受ければよい。見逃しを選ぶのは、対比ルールの「のように」（実測 10 件中 10 件が誤検出）のように**誤検出が常態化するとき**に限る。6 字窓の本書での誤検出は 0 件だった。本書の指摘は 15 → 0 件（本文の 5 件は書き直し、`・` の並びの中の 3 件は通るようになった）。

- **「使うべきではない言葉」を叩く上流の項目を打ち消した**。`spellcheck-tech-word` が内蔵する辞書に `(?:…|最強|常識的|最も優れた|知らない|知らなさそう|酷い)` という項目があり、**`expected` が置換語ではなくメッセージ文**（`使うべきではない言葉なので修正してください`）だった。同ルールは `expected` をそのまま修正テキストに使うため、`--fix` を当てると「手酷い失敗でした」が**「手使うべきではない言葉なので修正してください失敗でした」**になる（実測）。

  そもそも語の選び方の良し悪しという価値判断で、綴りの誤りでも表記ゆれでもない。文脈も見ないので「Rust は知らない」と「知らない記法」を区別できず、「常識的な判断をすると」のようなごく普通の文まで叩く。**本書にも生きた指摘が 1 件あった**（`95-import.md:111`「知らない記法を黙って通しません」）。六語まとめて打ち消した。誇大表現を見たいなら、`--fix` を持たない独自ルール（`kansuji-counter-suffix` と同じ立場）として実装するのが筋である。

- **`チェーン => チェイン` を逆に倒した**。ツールチェーン・ブロックチェーン・デイジーチェーンはいずれも長音で定着している。上流の辞書は直せないので、`BUILTIN_ALLOWLIST` で一方の口を塞ぎ、逆向きの `チェイン => チェーン` を `prh_web_technology.yml` に置いて**正しい綴りを主張する**形にした。単に黙らせると、誤って `チェイン` と書いたときに何も言われなくなる。

- **誤検出しうる辞書項目を総当たりで洗った**。実在語 61,386 語（SudachiDict 由来＋**本書から MeCab で抜いた複合名詞 2,616 語**）に対し、辞書の見出し語が「語の内側」に現れるかを機械的に調べ、候補を実物の textlint で検証した。上流の候補 17 件のうち 8 件が実際に発火し、5 件は対処済み（`マスター画`・`レビューア`・`ビューワー`・`対処方`・`既定`）、新たに上の 2 件が見つかった。`薔薇 => バラ` も当たるが `薔薇色 → バラ色` は許容範囲なので見送った。**自前の辞書は白**——候補 49 件はいずれも先読みを落とした抽出の誤警報で、実物では 1 件も発火しない（`テーマスタイル`・`カスタマイズ`・`レンダリング結果` で確認）。
...
- **OCR の誤認識補正を校正辞書から切り離し、`config/ocr_corrections.yml` へ移した**（`vivlio-starter-pdf` 1.1.3 と対）。`textlint_rewrite.yml` に置いていた 6 項目は `vs pdf:read` の読み取り結果を直すためのものだったが、校正辞書は**全原稿**に当たるため、自分で書いた文章まで巻き添えにしていた。実測すると `Al => AI`（OCR が AI をエルの小文字と読み違える補正）が**アルミニウムの元素記号 `Al` と人名 `Al Gore` を書き換え**、`語芸 => 語彙` が**「言語芸術」を「言語彙術」に**していた。prh 由来の指摘は自動修正を持つので、`vs lint --fix` が黙って壊す種類の不具合である。雛形に入っていたため、新しい本すべてが抱えていた。
  ...
  **しかも補正は 2026-08-04 から一度も効いていなかった。** プラグイン側の読み込み先が `config/textlint_prh.yml` と決め打ちで、`6bbc2007`「校正まわりの設定ファイル名を対にする」でこのファイルが `textlint_rewrite.yml` へ改名された際に取り残されていた。ファイルが無ければ空の表を当てて素通りするので、例外も警告も出ない。つまり**校正の側でだけ当たり、肝心の読み取りには当たっていなかった**。
  ...
  走査の誤りと表記の方針は別物なので、表を分けた。当たるのは OCR したページのテキストだけになる。読み取る本ごとに癖が違うので、著者が育てられるよう YAML のままにしてある（`expected` ＋ `patterns`、`/…/` で正規表現）。一字だけの置き換えを書くと正しく読めている箇所まで巻き添えにするため（`力 => カ` は「力学」を「カ学」にする）、**語の形で書く**よう雛形と 94 章に明記した。`Al => AI` は既定に入れない——`Al` が AI なのかアルミニウムなのかは文脈でしか決まらず、機械に任せる項目ではない。

- **半角かっこの除外を、除外リストから辞書そのものへ移した**。`半角かっこ => 全角かっこ` は `textlint_rewrite.yml`（本書自身の辞書）の項目で、中身を問わず `/\((.+?)\)/` と当てていた。`(1)` `(a)` `(Matz)` の除外を著者の除外リスト側で書き足していたが、**本来は辞書のパターンが広すぎるという不具合**であり、除外リストで覆う筋のものではない。パターンに先読みを足して、中身が半角英数字だけのかっこ（`(1)` `(a)` `(Matz)`、カンマ区切りの座標 `(0, 0)` `(1000, 1000)` を含む）を辞書の側で外した。全角にすると左に空きができ、行頭では字下げに見える。座標は数学の書き方でもある。

  除外したい形・したくない形の両方を `specs` に書いてある（`発売は(2026年)です => 発売は（2026年）です` は従来どおり）。prh は辞書を読み込むたびに `specs` を自己検査するので、パターンを触って期待が崩れればその場で lint が止まる。

- **`従って => したがって` を接続詞に限定した**（`/(?<!に)従って/`）。公用文は「したがって」を仮名で書く接続詞に挙げているが、**「動詞の『従う』は漢字で書く」と但し書きを添えている**。辞書はその区別を持たず、`dir` で指定した向き**に従って**伸びます」のような動詞まで叩いていた（本書の 3 件はすべて動詞で、接続詞の用例は 0 件だった）。

  直前の格助詞「に」を後読みで見る。動詞の「従って」は「〜に従って」の形を取り、接続詞は文頭に立つので「に」が付かない——この一点で分かれる。同じ辞書の `ゆえに`（`/(?<!事)故に/`）と同じ手口。接続詞が変換されること・動詞が変換されないことは `specs` に書いてあり、prh が辞書の読み込みごとに自己検査する（期待を壊すとその場で lint が止まることを実測で確認した）。

- **`並びに => ならびに` を辞書から外した**。内閣訓令「公用文における漢字使用等について」は接続詞を原則仮名と定めたうえで、**「ただし，次の４語は，原則として，漢字で書く。及び／並びに／又は／若しくは」**と明記している。辞書が公用文と逆を向いていた（この項目は上流の ics.media 辞書由来で、js-primer ではない）。

  加えて、本書で当たっていた 1 件は接続詞ですらなかった——「対象は『障害者』という**並び**に限られ」は名詞＋格助詞で、`並びに`（＝および）ではない。辞書には `/([横縦])?並びに/` と `regexpMustEmpty` で `横並びに`・`縦並びに` を除く工夫があるが、`という並びに` のような形は防げない。

- **`かっこ => カッコ`・`括弧 => カッコ`・`をおす => を押す` を辞書から外した**（`prh_open_close.yml`）。三つとも JavaScript Primer の校正辞書由来で、上流の ics.media 辞書には無い（ics.media はむしろ `お勧め`・`おすすめ` → **オススメ** と、逆のカタカナ化を定めている。辞書どうしで意見が割れる＝一般則ではなく各書の取り決め、という証拠でもある）。

  `括` も `弧` も常用漢字で、公用文にカタカナ化を求める定めは無い。本書は `括弧` を除外リストへ入れて漢字で通していたが、片方を止めながらもう片方（`かっこ`）で叩かれ続けるのは筋が通らないので、ルールごと外して除外リストの項目も消した。`をおす => を押す` は**部分一致で文を壊す**実害があった——「図はコードフェンスで囲むこと**をおす**すめします」が `ことを押すすめします` になる。辞書の但し書き（`〜をおす -> 〜を押す`）からも部分一致を避けようとした痕跡が見えるが、防ぎきれていない。そもそも「おす」を仮名で書く著者はほとんどいない。

- **`分かる => わかる` を逆向きにした**（`わかる => 分かる`）。仮名で書くよう定められているのは助詞・助動詞・補助動詞・形式名詞・指示代名詞で（`位→くらい` `～て見る→てみる` `事→こと` `通り→とおり`）、「分かる」はその一覧に無い。それどころか文化審議会「公用文作成の考え方」（令和 4 年）は別表で **`解る、判る → 分かる`** と、漢字へ寄せるよう明記している。仮名も「広く一般に向けた解説・広報等においては、読み手に配慮し…仮名で書いたり」と認められてはいるが、原則ではなく例外の位置づけ。本書の実態も漢字 14 対仮名 5 で漢字が多数派だった。

  従来の定義は `pattern: 分かる` という文字列一致で、**終止形しか見ていなかった**（`分かります` `分からない` は素通り）。反転にあたって `/わか(る|り|ら|っ)/` の正規表現にし、別語の「分かれる」と「若い・沸かす」を巻き込まないことを specs で固定した。原稿は 5 箇所を漢字へ揃えた。

- **`付ける => つける` を辞書から外した**（`prh_open_close.yml`）。ひらくのは補助動詞・形式名詞・副詞であって、「付与する」という実質動詞の `付ける` は漢字で書くのが原則側にある。この項目は**一般則ではなく、JavaScript Primer の校正辞書で「つけるに統一する」と決めたもの**だった（上流の ics.media / テキスト校正くんの辞書には無い。同辞書では `漬けるだけ` と並べてあり、同音異字を一方へ寄せる意図が見える）。この辞書の 116 項目のうち 75 項目には理由が書かれているが、これだけ空欄だった。

  ルールとしても揃っていない。対象が `付ける` だけで `付く` は入っていないため、`` `{.bordered}` を付けると細い枠が付きます `` が `をつけると細い枠が付きます` になり、同じ動詞が一文の中で不揃いになる。辞書自身の説明文（`prh_cho_on.yml` の「語尾が -er で終わる語彙には長音を**付けます**」）も漢字で書いている。本書の 22 件はすべて `{.bordered}` `@ラベル` `label` を目的語に取る実質動詞で、漢字と仮名の比も 50 対 2 だった。

- **上流ルールの取りこぼしを打ち消す仕組みを `vs lint` 自身に持たせた**（`LintRunner::BUILTIN_ALLOWLIST`）。`config/textlint_allowlist.yml` は**著者が「この本ではこう書く」と決めた語**を並べる場所で、ツールの不具合を書き写す場所ではない。新しい本を作るたび同じ 1 行を書かせるのもおかしい。実行時 textlintrc の `filters.allowlist.allow` へ注ぎ込むので、**著者の除外リストはそのまま合算され**、フィルタ経由なので `--fix` にも効く（textlint はフィルタで消えた指摘の修正を当てない）。

  最初の項目は**半角パーセント**である。`spellcheck-tech-word` が内蔵する上流辞書（`technical-word-rules` の `%([^0-9]) => ％$1`）は `%` の直後が和文のとき全角を促すが、技術書では半角 `%` が普通で、`shift-y=N%` のように CSS の値でもある（**本書は半角 `%` が 79 箇所・全角 `％` が 0 箇所**）。直後に何が続くかは見ないパターンにした（`10%・` `0%（` のほか、`**両裾の 10%**から` のように強調の閉じが挟まる形もあり、綴りを数え上げてもきりがないため）。本書で 11 件が消え、同ルールの他の指摘（`mecab => MeCab` など）は残る。

  ここへ書くのは**項目単位で切れないルール**に限る。辞書の項目そのものが誤っているなら辞書を直せばよく、`spellcheck-tech-word` は `context.options` を読まないため設定では手が届かない、という事情が理由である。**本の方針は書かない**——`マスター => マスタ` のような長音の扱いは逆の方針を採る本が実在するので、著者の除外リストの側に置く。

- **複合語の継ぎ目で起きる誤検出を、語単位から形単位へ改めた**。prh も `spellcheck-tech-word` も**語の切れ目を見ずに部分文字列で照合する**。日本語は語を空白で区切らないので、複合語の継ぎ目に見出し語と同じ並びが偶然現れ、別の語として叩かれる。**「プレビューアプリ」の中の「レビューア」が校閲者と読まれ「レビュアにせよ」と言われる**のが典型である。

  語を 1 つずつ除外リストへ足す運用は追いつかない。実測すると、5 語を登録してあるにもかかわらず**同型の指摘が本書に 4 件残っていた**——`プレビューアプリ`（2 件・登録してあったのは `レビューワークフロー`）、`マスターファイル`・`マスターは`（登録してあったのは `マスター画像`）。上流の項目は `マスター([^.])` と「続く任意の 1 字」を叩くので、語を数え上げてもきりがない。

  出どころで三つに振り分けた。**上流の継ぎ目の誤検出**（`対処方 => 対処法`、`レビューア|レビューワー => レビュア`）は `BUILTIN_ALLOWLIST` へ形で書いた（`/対処方法/` `/プレビューア/` `/レビューワーク/`）。**本の方針**（`既定` は和語で通す・長音は残す）は著者の除外リストに残し、`マスター画像` は `/マスター/` に改めて複合語ごと外れるようにした。**自前の辞書の不具合**（`prh_idiom.yml` の `アボガド => アボカド` がアボガドロ定数に当たる）は辞書を直した（`/アボガド(?!ロ)/`＋`specs`）。本書の指摘は 126 → 122 件。

  **丸数字は除外リストへ入れない。** 技術書では `(1)` を使うのが一般で、ルールの指摘は正しい。本書が `①` を書くのは `:::{.showcase}` の注釈を本文から指す 2 か所だけ——図の上には丸バッジが描かれ `alt` 属性にも「① 愛用のバイオリン」と集約されるので、読者が図で見ているのと同じ記号で呼ぶ必要がある。例外なので `<!-- vs-lint-disable-next-line -->` で 1 行ずつ抑えた。

- **`spellcheck-tech-word` の `skip` 設定を雛形から外した**。ルールはオプションを一切読まない（最新の 5.0.0 でも `reporter(context)` に options の引数が無く、README にオプションの節も無い）。`skip: []` と書いてあると「ここに書けば除外できる」と読めるため、`spellcheck-tech-word: true` に改めた。指摘を止める窓口は全ルール共通で `config/textlint_allowlist.yml`。

- **一文の長さを和文で測るようにした**（`sentence-length` を独自ルールへ移した）。textlint の同名ルールは**記法を外したあとの素の文字数**を数える。バッククォートは落ちるのに中身は残るため、`` `:::{.sideimage-right}` `` のようなインラインコードが 21 字として効いていた。設定の `skipPatterns` では届かない——数える時点でコードだった痕跡が無いためで、実測でも本書全体への効果は 0 件だった。

  インラインコード・丸かっこの中・欧文を数えない `japanese_length`（`long-parenthetical` が使っていたもの）で測る。本書の指摘 24 件を測り直すと 69・65・52… で、どれも読んで長い文ではなかった（3041 文の和文の長さは中央 27 字・平均 29 字・最大 94 字）。既定の上限 100 字は「これまで書いてきたどの文よりも長い」ところに立つ歯止めになる。`book.yml` の `lint.sentence_length_max` で変えられ、`0` で検査しない点は従来どおり。**キーの意味が「記法を外した文字数」から「和文の文字数」へ変わる**ので、上限を書いているプロジェクトは実質的に緩くなる。

  あわせて、`book.yml` の上限を実行時 textlintrc へ書き戻していた経路（`SENTENCE_LENGTH_PRESETS` / `sentence_length_rule`）を撤去した。上限は `ProseChecker.check` が `parenthetical_max` と同じ形で受け取る。

- **文末の句点を見る独自ルール `missing-period` を追加し、textlint の `ja-no-mixed-period` を置き換えた**。あちらは段落の末尾が句点でなければ一律に叩き、**日本語の体言止めを知らない**。実測すると本書の指摘 39 件のうち 13 件がそれで、`**用途**: PDF閲覧、電子配布` のような定義、図に添えるキャプション（`大きな余白（1.5rem）で配置`）、読点で終えて次のブロックへ続ける書き方、VFM の 2 段ヘッダー表の行、結びの `{彌榮|いやさか}` まで「句点を付けよ」と言っていた。どれも直しようがない。

  見分けは**段落の最後の 1 文字**で付く。用言（動詞・形容詞・助動詞）の終止形は必ずひらがなで終わり（`…する`・`…ない`・`…した`・`…です`・`…ます`）、体言止めは名詞で終わるので漢字・カタカナ・英数字・閉じかっこになる。長音記号 `ー` は入れない——`…のメンバー` を用言と読み違えるため。置き換えの結果は 39 件 → 26 件で、**新しく出た指摘は 0 件**だった（段落の切り方が textlint と食い違っていないことの確認）。

  箇条書き・見出し・表・引用で始まる段落は見ない（項目に句点を付けないのは一般の作法で、textlint 側も `ListItem` を最初から除外していた）。段落全体が強調ひとつの行（`**独自の装飾を追加する**`。組版では小見出しとして組まれる）も同じ。`NotationGuard` を通してから読むので、`:::{.output}` / `:::{.terminal}` の中身も対象外になる。助詞で終わる形（`…という自然な習慣で`）は指摘が残るが、助詞の一覧を抱えるより `<!-- vs-lint-disable-next-line -->` で抑えるほうが軽いと判断した（本書で 2 件）。`--fix` はしない——句点を足すのか体言止めに直すのかは著者が決めること。
...
- **画像に枠を引く `.bordered` を追加した**。`![](x.webp){.bordered}` または `:::{.bordered}` で囲む。キャプション付きの画像（`<figure>`）には既定で `0.5mm` の枠が付くが、キャプションのない画像と `:::{.sideimage-*}` の中の画像は figure にならないため枠が効かない。白背景のスクリーンショットや図版は枠が無いと紙面の余白に溶けて輪郭が消えるので、明示して引けるようにした（Re:VIEW の `//image[…][…][border=on]` の受け先でもある。実測: `//image` 39 件・`//sideimage` 55 件）。あわせて `cross_reference_processor.rb` が `{.bordered}` を解いておきながら `<figure>` へ出していなかった取りこぼしを直した（`align_class` → `class_attr`）。

- **`**強調**` だけで一段落をなす行を、小見出しとして組むようにした**。原稿で `**ビルド時の警告**` のように書かれた行は文中の強調ではなく小さな見出しとして働いているが（本書で 98 件）、本文と同じ大きさ・同じ行頭位置で色が違うだけだったため、前の段落と地続きに見えていた（実測: 小見出しも本文も x=84.19）。後処理の `PostProcess.mark_strong_headings!` が `strong-heading` クラスを付け、上に半行（`0.5lh`）あけて字下げを打ち消す。行頭が版面左端へ出るので、h3 と本文の中間層として働く。直後がコードのものは `code-caption` の担当なのでここでは対象外。

- **補足を小さめに組む `.small` を追加した**。本筋から外れる添え書きを `<small>` の生タグで書かずに済ませるためのもの。`:::{.small}` で囲むか、1 段落なら段落末に `{.small}` と書く。上下を 1 行分（`1lh`）あけて本文から切り離し、左右を 2em 内側へ寄せ、文字を 0.88em に落とす。**行送りは本文のまま保つ**——行送りは `--base-line-height` の絶対値が版面の行グリッドを作っており、ここで変えるとその部分だけ行位置がずれて隣ページと小口が揃わなくなる。文字だけ小さくすれば行数は動かない。上下の余白を `lh` の整数倍にしているのも同じ理由で、mm や em で半端に空けると以降の行位置がずれる。段落末の記法を後処理（`PARAGRAPH_CLASS_RULES`）へ置いたのは、段落内の Markdown を生かすため——VFM は段落末の `{.class}` を解釈せず（見出し末尾だけは解釈する）、`{.text-*}` の前処理経路は中身を生 HTML 化するので `to_f` のようなインラインコードが壊れる。

- **`rake test:notation` を追加し、リリースゲートへ組み込んだ**。記法網羅フィクスチャ（`test/vivlio_starter/fixtures/notation/`）は 2026-08-11 から置かれていたが、回す仕組みが無く眠っていた。隔離した一時プロジェクトで実ビルドし、**VFM 単体が組めた記法がパイプラインを通っても残っているか**を突き合わせる。壊れ方を先に知らなくてよいのが要点で、参照リンク・タスクリスト・字下げコードブロックの 3 件はこの 1 本で同時に捕まった。
- **実ビルドを伴うテストで PDF プレビューを開かないようにした**（`VS_NO_PDF_PREVIEW`）。著者の `vs build` は従来どおり成果物を開く——目的が「見ること」だから。抑えるのは機械が検査する場面だけで、ゲートは 14 回以上ビルドするため、放っておくと Preview が次々に開いて走行中の画面へ手を出すことになる。Rake のテストタスクが自動で立てるので、利用者が設定する必要はない。

- **`vs lint` に記法の取り違えを見る 3 ルールを追加**（`markdown-notation-collision-spec.md` §5〜§7）。いずれもビルドは成功するため、紙面を見るまで気づけない類の事故を書いた時点で伝える。`stray-index-markup` は索引記法とまぎらわしい `[g]` `[eV]`（読みなし・半角 1〜2 文字だけを見る。`[CSS]` `[PDF]` のような 3 文字は索引語として自然なので黙る）、`indented-code-block` は非対応の 4 スペース字下げ（リストの続きは除外し、ブロックの先頭 1 行だけ報告）、`setext-heading` は改ページのつもりが見出しになる `---` / `===`。3 つとも `lint.disabled_rules` と `<!-- vs-lint-disable -->` で切れ、`--fix` はしない（正解が二択で、決められるのは著者だけのため）。原稿は第 8 章に「記法の取り違え」の節を新設。
...
- **`prh_open_close.yml` が自分自身と逆を向いていた**（`どういうとき`）。「`どういう時` → `どういうとき`」と「`どういう(?!こと)` → `どのような`」が同居しており、前者が勧めた綴りを後者が叱っていた（上流の `spellcheck-tech-word` も同じ指摘をする）。前者は不要——`どういう` → `どのような` と、一般則の `(…|な|の)時` → `とき` が順に効いて `どのようなとき` に落ち着くので、削除した。探り用の原稿で 2 段の連鎖を実測している。

- **同梱の辞書どうしが逆を向いていた**（`インターフェイス`）。`prh_web_technology.yml` は「`インターフェース`・`インタフェース` → `インターフェイス`」と言い、上流の `spellcheck-tech-word` は「`インターフェイス` → `インタフェース`」と言う。どちらに直しても他方に叱られるため、著者からは直しようがなかった。

  採ったのは `インターフェイス`。テクニカルコミュニケーター協会「外来語（カタカナ）表記ガイドライン 第 3 版」が、長音を「ー」で書く原則の**例外**として `インターフェイス（interface）[59.1%]` を名指しで挙げている（数字は利用者アンケートの支持率で、内閣告示第二号 2Ⅲ3 注 2 の「『エイ』と書く慣用がある場合はそれによる」に当たる）。上流の出どころは WEB+DB PRESS 編集部の用語統一ファイルで、JIS 寄りの雑誌のハウススタイルである。上流側の口を `BUILTIN_ALLOWLIST` で塞いだ。JIS 表記で通す本は `prh_web_technology.yml` の項目を逆へ向ければよい。

  同じ形の衝突は `チェーン` で一度踏んでおり、二度目だったので**総当たりで数えた**。自前 836 規則と上流 555 規則を突き合わせ、逆を向いている組み合わせは 58 件。うち 54 件は末尾長音の軸で `lint.trim_long_vowel` が吸収しており、残る 4 件のうち実際に指摘が出るのは `チェーン` と `インターフェイス` の 2 件だけだった（`サーバーサイド` と カタカナ中黒の除去は、上流の実装が「直し先が元の綴りの先頭に一致する規則」を自ら黙らせるため発火しない）。

- **`vs import` のメッセージで「Vivlio Starter」を「Vivlio」と略記していた**（`re_renderer.rb`・`re_inline.rb`・`import.rb`・`stray_page_break_check.rb` の計 9 行）。「Vivlio に対応する記法がありません」「Vivlio の --- は改ページです」など、取り込み中に著者が読む文言である。95 章の出力例はこの文言を引き写していたため、原稿だけを直すと実物と食い違う。両方を揃えた。

- **`ruby-build`・`ruby-vips` を `Ruby` へ直すよう促さないようにした**（`BUILTIN_ALLOWLIST`）。上流の項目は `\bRuby\b`（大文字小文字を無視）で、正規表現の `\b` はハイフンを語の切れ目とみなすため、**小文字が正しい名前**の頭に当たっていた。gem や Homebrew の formula には `ruby-` で始まるものが多く（`ruby-lsp`・`ruby-prof` など）、語を一つずつ足しても追いつかないので継ぎ目の形（`/ruby-[a-z]/`）で書いた。素の `ruby` は引き続き `Ruby` へ直すよう促される。

- **記法を説明しただけで、そこから先の校正が止まっていた**。抑止コメントを**行の一部でも**一致すれば有効と見なしていたため、`` `<!-- vs-lint-disable -->` `` のようにインラインコードで書いた**例示**を本物の指示と取り違えていた。31 章は記法そのものを解説する章なので、201 行の説明文から先で独自ルールが黙り、**8 件の指摘が埋もれていた**（交ぜ書き 4 件・数の表記 3 件ほか）。行全体がマーカーのときだけ効かせるよう直した。

  スペルチェック側（`tokenizer.rb`）だけは以前から行全体で見ており、そこには「説明文を本物と解釈しないため」という注記まであった。**同じ判断が 3 箇所に書き写され、そのうち 2 箇所が古いまま取り残されていた**ことになる。定義を `NotationGuard` へ一本化した。
...
- **同梱辞書の「すぎる => 過ぎる」を逆向きに直し、「事」「時」の書き分けを足した**（`prh_open_close.yml`）。「実質的な意味を持つ語は漢字、補助的・形式的な用法はひらがな」という原則で、この辞書は既に `下さい => ください`・`して頂く => していただく`・`出来る => できる`（ただし完成を表す `出来上がる` は漢字のまま）を持っている。**「すぎる」だけが逆を向いていた。**

  程度を表す「〜すぎる」（長すぎる・少なすぎる・詰め込みすぎる）はひらがな、本来の動詞（時間が過ぎる・期限を過ぎた）は漢字にする。判定は**直前の一文字**で足りる——程度の「すぎる」は形容詞語幹・形容動詞語幹・動詞連用形に付くが、本動詞の前には助詞（が・を・は・も）が立つ。`通り過ぎる`・`行き過ぎる` は方向や逸脱を表す複合動詞なので除く。本書の 12 件が基準どおりに揃い、「約 2.3 年が過ぎる」だけが漢字で残った。

  あわせて形式名詞の `こと`・`とき` を足した（`する事 => すること`・`な時 => なとき`）。直前が連体形か「な」「の」のときだけ当て、後ろに熟語を作る字が続くものは外す——さもないと「関する事項」が「すること項」に壊れる。`仕事`・`返事`・`時間`・`当時`・`考え事`・`願い事` が変わらないことを確かめた。どの項目にも両方向の `specs` を添えてあるので、向きが崩れたら lint の起動時に落ちる。
...
- **同梱辞書の「プレフィックス => プリフィックス」を逆向きに直した**（`textlint_dictionaries/prh_cho_on.yml`）。IT 分野の慣用は「プレフィックス」で、IT 用語辞典の見出し語も、IPv6 の「プレフィックス長」も、CSS の「ベンダープレフィックス」もこちらである（「プリフィックス」は電話のプリフィックス番号の文脈で生きている表記）。辞書は 2018 年の生成元をそのまま引き継いでいて、本書の原稿を少数派の側へ引き寄せていた。**黙らせるのではなく向きを逆にした**——正しい綴りを主張するのが辞書の仕事だからである。
  ...
  あわせて英単語を拾う `/prefix/i` を外した。この辞書 1,200 項目で**唯一の「英単語 => カタカナ」**という外れた項目で、`vendor prefix` のように欧文のまま書いた語まで書き換えてしまう。

- **半角かっこの除外を「中身が半角文字だけ」まで広げた**。もともとの狙いは「中身が機械の綴りなら全角にしない」（`(1)` `(a)` `(Matz)` `(0, 0)`）だが、`(token => count)` のような式や対応の表記が漏れていた。先読みを `[ -~]*`（半角文字だけ）へ広げた。

  **「英単語を並べた補足だけは全角に」という対の規則も試したが、やめた。** 本書は地の文の補足を全角で書くので（`（vs build）`・`（textlint）`・`（KPF）` など 174 箇所）、`(Kindle Previewer 3)` は拾うべき不統一に見えた。しかし当該箇所を見ると `51-doctor.md` の表の 1 列目で、`pdfinfo (poppler)`・`gs (Ghostscript)`・`rsvg-convert (librsvg)` と**同じ「コマンド名 (提供元)」の形**に揃っていた。地の文の補足と、コマンド名へ添える併記は、どちらも「半角の英字」で**字面では区別できない**。空白の有無で分ける規則は、表の中で唯一 2 語以上だったこの 1 件だけを叩く誤検出になる。全角と半角の両方で書かれている語は本書で `poppler` の 1 語だけで、それも地の文（`（poppler）`）と表（`(poppler)`）の使い分けだった。

- **「リーダビリティー」が「リーダービリティー」と指摘されていたのを直した**。上流の `リーダ([^ー]) => リーダー$1`（読み手・指導者の意）が、可読性の用語「リーダビリティー」の頭に当たっていた。別語なので、長音の文体をどちらに決めた本でも誤検出になる。`BUILTIN_ALLOWLIST` に `/リーダビリティ/` を足した。

  あわせて `TRIM_LONG_VOWEL_ALLOWLIST` の取りこぼしを直した。**上流の「末尾に長音を足す」項目は 3 件ではなく 5 件**で、`メーカ(?!ー) => メーカー` と `リーダ([^ー]) => リーダー$1` が漏れていた。見出し語に `ー` を含まないもので絞って数えたためで、`リーダ` `メーカ` は語幹自体が `ー` を含む。`trim_long_vowel` を有効にした本が、`リーダ` `メーカ` と書いて長音を足すよう促されることは無くなった。

- **`:::{.output}` の中の長い行が、枠の右端で黙って切り落とされていたのを直した**（`chapter-common.css`）。31 章の出力例を `:::{.output}` へ移したところ、lint の出力例の行末（「…になりま」の「す」など）が紙面から消えた。PDF の本文テキストにも残らないので、読者は欠けたことにすら気づけない。原因は `prism.css` が `<pre>` だけでなく**中の `<code>` にも `white-space: pre`** を指定していることで、`code.css` が `pre` 側を `pre-wrap` にしても `<code>` は折り返さない。行番号付きのコードブロックは別の組み方なので影響せず、出力例の枠の中の素の `<pre><code>` だけが引っかかっていた。`.terminal`・`.diagram` と同じく枠内で折り返す（`pre-wrap` は連続空白を保ち、はみ出すときだけ折り返すので、収まっている行の桁揃えは変わらない）。本書で切れていたのはこの 2 行だけだった。

- **31 章の出力例を `:::{.output}` で囲んだ**。`:::{.output}` は lint の章より後に導入されたため、素のコードフェンスで書かれ、行番号が付いていた。ツールの出力 14 個を囲み、あわせて**実物の出力と合わせた**——記法の取り違えの 3 例は末尾の直し方の案内（「（コードなら … と囲む／…）」）を省いた抜粋で、先頭の空白も実物と違っていた。エラーの 2 例には実物どおり 🔴 と 2 行目の例を足した。集約表示の例の 1 行目は、既定で切られている `ja-space-between-half-and-full-width` から、既定で有効な `missing-period` に差し替えた。

- **`閏年 => うるう年` を辞書から外した**（`prh_open_close.yml`）。交ぜ書きの独自ルール（`mazegaki`）は逆向きの `うるう年 => 閏年` を提案するので、**どちらで書いても片方が叩き、`vs lint --fix` のたびに入れ替わっていた**。31 章の交ぜ書きの説明「`うるう年`（閏年）」が `--fix` で「`うるう年`（うるう年）」に壊れて発覚した。この項目は js-primer にだけあり（ics.media の辞書には無い）、9/16 の辞書統合で取り込まれていた。交ぜ書き辞書 2,063 語と prh の 857 項目を機械的に突き合わせ、逆向きの組はこの 1 つだけであることを確かめた。

- **31 章（文章校正）を、これまでの lint の変更に追いつかせた**。`disabled_rules` を「出力段で指摘を取り除く方式」と説明していたが、いまは textlint の設定の段で切るので `--fix` にも効く。例に挙げていた `prh`（`一つ → ひとつ`）はすでに辞書から外していたので、逆を向いたルールの例を `arabic-kanji-numbers` と `kansuji-counter-suffix` に差し替えた。ほかに、見出しの注記（`␣`・`（半角 → 全角）`）、`--fix` が書き換えないもの（抑止・記法・出力例の囲み）と表示されても直らない指摘（ふりがなの親文字）、`trim_long_vowel` が著者の `textlint_rewrite.yml` を対象にしないこと、`textlint_rewrite.yml` には「この本の表記」だけを書くことを足した。表記揺れ辞書の正規表現の例は OCR の誤読（`ログラミング`）から `/ユーザ(?!ー)/` と `specs` の書き方へ、除外リストの例は lint があらかじめ処理するようになった `対処方法`・`/^:::\{/` から `既定`・`/マスター/` と継ぎ目の誤検出の書き方へ改めた。`book.yml` の `trim_long_vowel` のコメントにも、著者の辞書は対象外であることを書き添えた。

- **違いが空白や字幅だけの指摘に、見出しで注記を添えるようにした**。上流の「全角かっこの前後の空白を消せ」は `" ） => ）"` を出すが、表示を作るときに前後の空白を切り詰めていたため、**消すべき空白が画面から消え、「） => ）」と同じ字が並んで見えていた**（著者の報告で判明）。半角かっこを全角へ直す指摘も、`(2026年) => （2026年）` の違いが字幅だけで見分けにくい。「実際 => 期待」の違いが空白だけなら空白を `␣` で見せて「（空白を削除）」「（空白を追加）」を、字幅だけ（NFKC で同じになる）なら「（半角 → 全角）」「（全角 → 半角）」を添える。

  ```
  [spellcheck-tech-word] ␣） => ）（空白を削除）
  [prh] (Kindle Previewer 3) => （Kindle Previewer 3）（半角 → 全角）
  [prh] CSS組版 => CSS␣組版（空白を追加）
  ```

  実際側にも ` => ` が現れうる（`(token => count) => （token => count）`）ので、区切りの候補を順に試し、空白か字幅の違いとして説明がつく切り方を採る。違いが見て分かる言い換え（`ユーザ => ユーザー`）には付けない。

  あわせて、この「） => ）」を生んでいた誤検出を直した。**直前の変更で解析パスのラベルを消すようにしたため**、`（@auto / @omakase）` が `（ /  ）` になり、`）` の前に原稿に無い空白が生まれていた。ラベルは消さずに、形の同じ無害な語（`@label`）へ置き換える——`@` で始まる半角の 1 語という形を保てば、空白やかっこの規則から見た姿は原稿と同じになる。指摘の前後を全件で突き合わせ、変化はこの 1 件の消滅と 7 件の見出しの注記だけであることを確かめた。

- **`vs lint --fix` と表示の食い違いを網羅的に洗い、残っていた 5 件を直した**。前の 5 件（下記）を受けて、解析パスで働く抑止を全部数え上げ（14 種）、修正パスに対応する守りがあるかを一つずつ実測した。

  - **`trim_long_vowel` が上流辞書に効いていなかった**（表示に出ないのに `--fix` が直す——同じ型の 6 例目）。`ディレクタ` → `ディレクター`、`ベンダ` → `ベンダー`。末尾長音の抑止は prh の同梱辞書なら写しから項目を落とせるが、`spellcheck-tech-word` の辞書には手が届かず、出力のメッセージが「X => Xー」の形かどうかを見て表示から落としていた。**既定で有効な設定なので全書籍が該当する**。あわせて `メンバ([^ー]) => メンバー$1` は後続の 1 字を巻き込むので「X => Xー」の形にならず、長音を省く文体を選んでいても指摘・修正されていた。上流 22 項目のうち末尾の長音を足す 3 項目（`ディレクタ`・`ベンダ`・`メンバ`）を、設定が有効なときだけ実行時 textlintrc の除外へ注ぐ（`TRIM_LONG_VOWEL_ALLOWLIST`）。
  - **ふりがなを壊していた**。`{沢山|たくさん}` が `{たくさん|たくさん}` になる。解析パスは親文字を地の文として残すので「沢山 => たくさん」は表示に出るが、修正は記法の内側へ当たっていた。ふりがなを振った時点で著者は漢字を選んでいるので、修正パスでは記法ごと退避する（指摘は表示に残るので、直すなら著者が記法ごと書き換える）。
  - **相互参照のラベルを壊していた**。`@ruby-sample` が `@Ruby-sample` に、`@pageref:javascript-intro` が `@pageref:JavaScript-intro` になり参照先が見つからなくなる（出力例の外でも起きる）。ラベルは識別子で地の文ではないので、解析パスでは形の同じ無害な語（`@label`）へ置き換え、修正パスでは退避する。綴りはビルドの置換器（`CrossReferenceProcessor::ReferenceReplacer`）と**同じ定義**を使う——数式と同じ流儀で、別に書くと lint が守る範囲とビルドが参照として扱う範囲がずれる。メールアドレスはこの定義で除かれる。
  - **独自校正が出力例の囲みの中を直していた**。`:::{.output}` の中の「だ円」が「楕円」になる。textlint 側は出力例を中和・退避するのに、自動修正を持つ独自校正（交ぜ書き・康煕部首）だけは中まで指摘し、直していた。出力例の判定を `NotationGuard.machine_data_lines` として公開し、解析・修正・独自校正が同じ答えを使うようにした。
  - **`trim_long_vowel` が著者自身の規則まで消していた**。本書の `textlint_rewrite.yml` にある `ユーザ => ユーザー` が、長音を足す形なので写しから落とされ、同じ項目の `ユーザーさん`（敬称）の検出まで巻き添えになっていた。この設定が黙らせたいのは同梱の長音辞書の一般則で、著者が「この本ではこう書く」と決めた表記ではない。項目を落とすのを `config/textlint_dictionaries/` の辞書に限った。本書では例文の 1 件（`31-lint.md` の「ユーザ」）が指摘されるようになった。

  **表示の段での除外を撤去した**（`TextlintFormatter` の `disabled_rules`・`trim_long_vowel`）。どちらも実行時 textlintrc で効くようになったので、残っていた表示の段の除外は**設定の取りこぼしを隠す**働きしかしない。外せば取りこぼしは表示に出て、`--fix` と一致する。表示と修正の食い違いは、見逃しの中でもいちばん気づきにくい形である。表示の段で落とすのは次行抑止の行だけになった（修正パスも同じ行を退避している）。

  **不変条件を実物の textlint で確かめる統合テストを足した**（`fix_invariant_test.rb`）。記法と抑止を網羅した見本原稿に、出荷している雛形の設定で `vs lint --fix` を当て、期待値と突き合わせる。単体テストは textlint を差し替えて動かすので、この種の食い違いはここでしか捕まらない。**守りを 1 つずつ外した版で、7 つとも正しい理由で落ちることを確かめてある**（ラベル → `@Ruby-sample`、長音 → `ディレクター`、ふりがな → `{たくさん|たくさん}`、復元順 → `VSMATH0001`、`disabled_rules` → `1つ`、出力例 → `だ円`、著者の辞書 → `ユーザ` のまま）。textlint が無い環境では skip する。

- **`vs lint --fix` が `lint.disabled_rules` を無視していた**。`disabled_rules` に `arabic-kanji-numbers` を書いた本で、`vs lint` は無指摘なのに `vs lint --fix` が「一つ前の節」を「1つ前の節」へ書き換え、直後の `vs lint` で `kansuji-counter-suffix` の指摘が復活していた（著者の報告で判明）。`TextlintFormatter` は切ったルールの指摘を**表示から落とすだけ**で、`--fix` は textlint に原稿を直接直させるため、設定で切っていないルールの修正はそのまま当たる。実行時 textlintrc でルールを切るようにした（`LintRunner#apply_disabled_rules!`）。名前の読み方は表示側に合わせてあり、短縮名（`arabic-kanji-numbers`）は読み込んでいる全プリセットで、完全名（`ja-technical-writing/arabic-kanji-numbers`）はそのプリセットだけで、プリセットの外のルール（`prh`・`spellcheck-tech-word`）はルールごと切る。プリセットに無い名前を書いても textlint は黙って無視する（実測）。

- **`vs lint --fix` が出力例の囲み（`:::{.output}` `:::{.terminal}` `:::{.showcase}`）の中を書き換えていた**。解析パスは `NotationGuard` がこれらを中和するので指摘は表示に出ないが、修正用の退避（`mask_for_fix`）は数式と属性記法しか守っておらず、**見えない指摘が `--fix` で当たっていた**。実測では 24 章の `:::{.output}` の中の `@ruby-sample` が `spellcheck-tech-word` に `@Ruby-sample` へ直され、**相互参照のラベルが壊れた**。出力例は実物の出力と一字一句合っていなければならない。囲みの地の文の行を行ごと目印へ退避するようにした。囲みの中のコードフェンスは退避しない——フェンスの行まで目印にすると、textlint がコードブロックと認識できず中身を地の文として直しにかかる。

- **`vs lint --fix` が、`<!-- vs-lint-disable-next-line -->` で守った行の数式を目印のまま書き戻していた**。「面積は $\pi r^2$ です」が「面積は VSMATH0000 です」になり、**数式が原稿から消える**（実測。同じファイルのどこかで他の修正が 1 件でも当たって書き戻しが起きれば発生する）。退避は重なる——数式を目印にしたあとで、その行ごと次行抑止の行として退避する。ところが復元を退避した順に行っていたため、内側の数式の目印は外側の退避に隠れたまま空振りし、外側を戻した時点で原稿に残っていた。復元を**退避の逆順**にした（`NotationGuard.restore_masked`）。値つきの属性記法（`{width=20%}`）も同じ経路で壊れていた。

  上の 3 件と、下の `vs-lint-disable-next-line`・`trim_long_vowel` の 2 件は、いずれも**表示では黙るのに `--fix` だけが当てる**という同じ型の不具合である。表示段で抑止したものは修正に効かない——設定で切るか、可逆な退避で守るしかない。確かめ方は捨て章に `vs lint` と `vs lint --fix` を当て、**`--fix` の差分が表示された指摘と一致するか**を見ればよい（`lint-false-positive-notes.md` §7）。

- **`vs lint --fix` が `<!-- vs-lint-disable-next-line -->` を無視していた**。著者が「ここは直すな」と書いた行がそのまま書き換えられていた（実測: `:::{.showcase}` の丸数字を守るコメントを置いた行が `①` → `（1）` にされ、図に描かれた丸バッジと本文の呼び名が食い違った）。原因は経路の違いで、textlint のコメントフィルタは `-next-line` を実装していないため（v1.3.0）、解析パスは出力段で行ごと落として辻褄を合わせていたが、`--fix` にはその段が無かった。修正パスの間だけ**行を丸ごと目印へ逃がす**ようにした（数式・属性記法と同じ仕掛けで、書き戻しの `restore_masked` が一括で戻す。行数も変わらない）。

- **`vs lint --fix` が `lint.trim_long_vowel` を無視していた**。末尾長音を省く文体を選んでいても、`--fix` が `ベクタ` を `ベクター` へ、`コンテナ` を `コンテナー` へ直していた。textlint はフィルタで消えた指摘の修正は当てないが、この設定はフィルタではなく独自の出力段の判定だったため素通りしていた。実行時設定が**辞書そのものから「X => Xー」の項目を落とした写し**を読むようにして、解析と修正のどちらにも同じように効かせる。落とすのは末尾長音の形だけで、綴りの誤りを直す項目は残す（実測: `prh_cho_on.yml` の 321 項目のうち 314 件が該当し、残る 7 件は `プリフィックス`・`クォート`・`ガベージコレクション` など)。
...
- **補助スクリプトを `scripts/` へ集めた**（`copy_to_scaffold.rb`・`count_lines.rb`）。`dict_conflicts.rb` を足すにあたって置き場を決めた。今後は `ruby scripts/copy_to_scaffold.rb` と綴る。
  ...
  参照は 91 箇所・46 ファイルあり（うち `docs/archives/` が 40 本、`CHANGELOG.md` が 13 箇所）、すべて書き換えた。過去のエントリを書き換えることにためらいがあったが、archives の「要 `ruby scripts/copy_to_scaffold.rb` 同期」は**今日読んだ人がそのまま実行する手順**なので、古いパスのまま残すほうが害が大きい。旧版は git に残る。

  スクリプト側は、自分の位置からルートを辿るように直した（`ROOT = File.expand_path('..', __dir__)`）。カレントディレクトリに依存しないので、どこから起動しても同じ結果になる。gem に同梱されるのは `{bin,lib}/**/*` と 4 つの文書だけなので、配布物への影響は無い。

- **`:::{.output}` と `:::{.terminal}` の中身を校正の対象から外した**（`NotationGuard` の機械データ・ブロックへ追加）。中に置くのは実行結果・ログと `$ コマンド`、つまり**機械が出した文字列**であって著者の書いた日本語ではない。校正の指摘は「著者が直せること」でなければ意味がないが、出力例の句点や表記を著者が直すわけにはいかない。しかも記法の解説では**同じ文字列をすぐ上のフェンスにも書く**ので、フェンスの中は除外され、その下に置いた表示例だけが叩かれる——片方だけ指摘される状態だった（実測: 本書で 3 件。`:::{.output}` に置いた相互参照の変換結果が `ruby => Ruby`・`"表示を行う"は冗長` と指摘されていた）。`column` / `note` / `tip` のような囲みは入れない。普通の文章を書く場所なので、一律に外すと本物の誤りを見逃す。

- **`vs lint` の指摘を、原稿 1 ファイルにつき 1 つの表にまとめた**。これまでは textlint の指摘（`📄 … (textlint)`）と Ruby で実装した独自校正の指摘（`📄 … (校正)`）が別々のブロックに分かれ、同じ原稿の見出しが 2 度現れていた。検査の実装が 2 つに分かれているのは都合であって、著者にとっては同じ「日本語校正」である——完了サマリーでも 1 つの件数へ合算していたのに、表だけが分かれていた。分かれていると、どちらを先に直すのか・その原稿に全部でいくつあるのかが読み取れない（実測: 22 章は `slash-between-japanese` 6 件だけが 35 件の下にぶら下がっていた）。

  ブロックのラベルは `(日本語校正)` と `(スペルチェック)` に改めた。完了サマリーの内訳と同じ語なので、表と件数がそのまま対応する。並べ替えは混ぜたあとで一度だけ行うため、集約（`TextlintFormatter.aggregate_json` / `ProseChecker.aggregate`）は畳んだ行を返すだけに変え、順序と出現行の整形は従来どおり `FindingRows.arrange` が受け持つ。

  スペルチェックの表だけは出現行を同じ行に置いたまま残した。**形を揃えると、行数が倍になるか、読めない幅になるかのどちらかにしかならない**——日本語校正の指摘は文の形をしているので横に伸びる（実測: 本書 161 件のラベルは中央 36 桁・最大 164 桁で、同じ行に置くと 177 桁に達する）。「その表のラベルが全部 N 桁以内なら 1 行」という内容しだいの規則も測って捨てた。しきい値 32 桁で 22 章中 1 章だけが 1 行形式に倒れ、同じ実行の中で表の形が変わってしまう。揃えるのは表の形ではなく**表の中の列**で、`行:` の位置をその表の最長ラベルに合わせるようにした（従来は 28 桁の決め打ちで、`notation-implemention-guide => …` のような長い語が来た行だけ右へずれて列が崩れていた。綴り誤りの語は長さを見越せない）。

- **一文の長さを、丸かっこの中を除いて数えるようにした**（`sentence-length` の `skipPatterns`）。読者はかっこの中を読み飛ばして本筋を追えるので、「一文の読みにくさ」を測るには外すほうが実態に合う。本書の指摘 66 件のうち **30 件は補足のぶんで上限を超えていた**もので（`（バージョン 0.31.2）`・`（KFX / Enhanced Typesetting）` のような添え書き）、切り替えで 48 件から 24 件に減った。あわせて `lint.sentence_length_max` の反映を「`max` だけ差し替え」に直した——設定ごと置き換えていたため、上限を書いた著者の手元でだけ除外指定が消えていた。

  かっこの中だけが伸びる書き方は、独自ルール **`long-parenthetical`** が受ける。**句点で区切り、インラインコードと欧文を除いた和文**で 60 字を超えたら指摘する。生の長さで測ると `（yellow / orange / red / …）` のような**値の列挙**が上位を占め、字数で叩いても直しようがない——実測すると、この数え方で本書の最長は 39 字（40 字超は 0 件）だった。既定の 60 字は、その最長から十分離した高さである。`--fix` はしない（文を分けるか、かっこの外へ出すかは著者が決めること）。

- **冗長表現の指摘から `【dict2】` の表示を落とした**。`ja-no-redundant-expression` が付けるこの印はルール内部のパターン番号（`dict1`〜`dict6`）で、本来は 2 行目の解説 URL（`…#dict2`）と対になっている。`vs lint` は先頭行だけを見せるため、著者の手元には意味の読み取れない記号だけが残っていた。

- **`ja-no-successive-word`（同じ語の連続）を既定で切った**。本書全体の 21 件のうち 20 件が誤検出だった。叩いていたのは記号を重ねる表記で、表の評価欄の `★★☆☆☆`（18 件）、三点リーダーを 2 つ重ねる「……」（1 件、日本語の正しい書き方）、伏せ字の「○○」（1 件）。「ですです」のような本物の打ち間違いは 1 件も無く、当たりは 42 章の「〜のいずれ**かか**確認」だけで、これは「〜のいずれかになっているか確認」へ直した。このルールは `preset-ja-technical-writing` にだけ入っているので、`config/.textlintrc.yml` の 1 か所で切っている。
...
- **prh の表記ゆれ辞書を分野別に整理した**。従来は `icsmedia.yml` が 7 つの辞書を読み込み、そこへ js-primer（JavaScript 入門書）の辞書が重なる形で、**何が効いているのかを設定から読み取れなかった**。実際、同じ語を複数の辞書が抱え、しかも**逆向きに直すもの**があった——`先程 => さきほど` と `さきほど => 先程`、`ハンドラ => ハンドラー` と `ハンドラ => リスナー関数`、`おすすめ => オススメ` と `おすすめ => お勧め`。どちらに直しても別の辞書が指摘するので、著者はその語を書けなくなる。

  整理の方針は「1 つの語は 1 つの辞書が受け持つ」。誤字・ひらく漢字・冗長・重言・カタカナ表記・固有名詞・技術用語の 7 辞書に分け、本書固有の表記は `textlint_rewrite.yml` にまとめた。使う辞書は `.textlintrc.yml` に全部並べ、二段構えの読み込みをやめている。JavaScript Primer 専用の辞書（`js_primer.yml`）は、あの本の用語選択・章名・サンプルアプリ名が中心で、一般の書籍に要る項目は 1 割ほどしかなかったため、それらを分野別の辞書へ移して削除した。規則は 913 件から 786 件になり、**辞書をまたぐ食い違いは 19 組から 5 組**（残るのは「サーバ」と「サーバサイド」のような、直した結果が同じになる包含関係）へ減った。

  **本書の表記も、この過程で決めた**——数は漢数字（`1度 => 一度`）、**先程・ハンドラ・お勧め**、完成を表す「出来上がる」は漢字のまま（可能の「できる」だけを開く）、コマンド名は `` `vivlio-starter` `` とインラインコードで書く（地の文の綴りはブランド名「Vivlio Starter」へ直す）。原稿も 7 箇所を揃えた。誤検出が多い項目も落としている（`事 => こと` は「返事」「約束事」まで、`Destructuring assignment` の項目は直前の 1 文字を消す壊れた作りだった）。整理の前後で本書全体の `vs lint` の出力が変わらないことを、語を並べた検査文と全章の実行で確かめている。

- **スペルチェックが記法を語として読んでいたのを直した**（`Tokenizer`）。3 つある。(1) インラインコードの判定を自前の `` /`[^`]*`/ `` から `Masking`（コード領域解釈の唯一の実装）へ移した——バッククォートを 1 つずつ数えるため、記法を解説する可変長のコードスパン（```` ```mermaid ````）で対が食い違い、**同じ行の後ろにある地の文までコード扱いが外れて**いた。(2) 相互参照ラベルの除去を `@pageref:見出しラベル` のようにコロンで続く形まで広げた。(3) 脚注のラベル `[^id]` を落とすようにした（定義行の説明文は検査対象のまま）。あわせて URL の除去をラベルより先に回した——`@qr:https://…` は先にラベルを削ると URL と認識されなくなる。

- **図の記法 `mermaid` をスペルチェックの辞書へ入れた**。図を書く道具なので、同類の `PlantUML` と同じ `software-tools.txt` に置く。実行ファイルの `mmdc` と npm パッケージの `mermaid-cli` は、`qpdf` や `textlint` と同じく本書が使う外部ツールなので `vivlio-starter-terms.txt` に置いた。同じ整理で、gem 名（`Prawn` `CombinePDF` `pdf-reader`）・組版用語（`recto` `verso`）・略語（`ICC` `IME`）・mermaid の図の種別も、それぞれの辞書へ入れている。**本書のスペルチェックの指摘は 54 件から 0 件になった。**

- **人名の辞書を拡充した**（`famous-people.txt`。193 語 → 970 語）。もとは姓だけを並べた辞書で、`Einstein` は通るのに `Albert` が未知語になっていた。技術書は人物をフルネームで書くので、姓しか知らない辞書は**必ず片方を叩く**（提案も的外れになる——`Albert` に返ってくるのは、綴りが一番近い辞書収録語のカナダの州名 `Alberta` だった）。

  収録も計算機科学と近代物理に偏っており、アルキメデスとユークリッドは居るのにアリストテレスとプラトンが居ない、ニュートンは居るのにケプラーが居ない、という状態だった。古代・イスラム世界・科学革命から現代までを分野ごとに補い、姓と名を対で載せる方針に改めた。あわせて、重複していた 39 行と、人物でない語（`Erlang` `Airbnb` `OpenAI` など。いずれも `products.txt` などに既出）を落とし、方針を先頭のコメントに書いた。本書のスペルチェックの指摘は 54 件から 46 件になり、消えたのはすべて実在の人名だった。

- **「通り」と「始める」の指摘を、当たるべき用法だけに絞った**（`prh_open_close.yml`）。どちらも文字列一致だったため、**直すと誤りになる箇所まで叩いていた**。「通り」を開くのは形式名詞のときだけなので、直前に「の」か「た」がある形に限る（`/(の|た)通り/`）——連濁する複合語（`予想通り` `文字通り`）は置換すると「予想とおり」になり、道の名前（`中央大通り`）は語ごと壊れ、数え方の `二通り` も巻き込んでいた。「始める」は動詞なので開かず、副詞の `始めに` だけを見る（`書き始める` は指摘され `書き始めます` は素通りする、という活用形任せの当たり方も直った）。あわせて、動詞の「通ります」を守っていた許可リストの 1 行が不要になったので外した。本書の指摘は 334 件から 307 件になった。

  本書の表記も決めた——**数え方の「通り」は漢数字**（`2 通り` → `二通り`）で、副詞の「ひととおり」も漢字に揃える。二通り・三通りと並べたときに一だけ姿が変わるのを避けるためで、`一つ` `二つ` と同じ扱いである（`textlint_rewrite.yml`）。原稿も 2 か所を直した。

- **`vs lint` が fancy list（`(1)` `a.` `(iv)`）をリストとして読むようにした**（`NotationGuard`）。textlint の Markdown パーサは Pandoc 由来のこの記法を知らず、**リストを段落として読んでいた**。すると箇条書きのために外してある検査が素通りし、`ja-no-mixed-period` が項目の末尾に句点を要求する（`(1) 括弧付き数字の項目` に「。」を付けよ、の類。標準の `-` や `1.` なら同ルールは `ListItem` を最初から除外して黙る）。検査用の一時ファイル上でマーカーだけを `-` へ読み替え、本文と行番号はそのままにする。マーカーの綴りは前処理の `parse_list_marker` を使う——lint が見る範囲とビルドがリストとして組む範囲をずらさないためで、`B. Russell は…`（空白 1 つ）のように前処理がリストにしない行は素のまま渡す。`--fix` は別経路なので原稿は書き換わらない。本書の指摘は 336 件から 334 件になった（21 章の 2 件）。

- **半角英数字だけを囲む半角かっこを、そのままでよいことにした**（`config/textlint_allowlist.yml`）。表記辞書は半角かっこを全角へ直すよう薦めるが、これは和文の中でのバランスを見た規則で、`(1)` `(a)` のような箇条書きの番号や `まつもとゆきひろ氏(Matz)` のような英字の添え書きには当てはまらない——全角にすると左に空きができ、行頭では字下げに見える。許可リストの正規表現を数字だけから半角英数字へ広げ、`--fix` でも書き換わらないようにした。

- **かっこの隣の空白の指摘から、区切りのために置いた空白を外した**（`space-around-brackets`）。textlint の `ja-no-space-around-parentheses` は「かっこの隣に空白があれば指摘」としか言えず、本書で指摘された 18 件はすべて区切りの空白だった——`**DTP ソフト**（InDesign） — 紙面を…` のダッシュ、`` `@prop-list` → 「表 4-2」 `` の矢印、`**参照:** 「インストール詳細」` の強調の閉じなど。しかも textlint の自動修正が効くため、`vs lint --fix` がこの空白を削って `（InDesign）— 紙面を` と詰めていた。

  そこで独自ルールに置き換え、空白の反対側が区切り記号（`—` `–` `→` `-` `:` `：` `/`）・表のセル境界・強調の閉じ（`**`）・行末のときは黙るようにした。かっこの内側の空白（`（ 補足 ）`）と、和文の中でかっこの外側に入った空白（`ここで 「引用」`）は従来どおり指摘する。強調の開きの手前（`「引用」 **太字**`）も指摘に残す。半角の `[` `]` はリンク・脚注・索引語の記法なので見ない。切り替え後の本書の指摘は 0 件。textlint 側のルールは `SUPERSEDED_TEXTLINT_RULES` で常に切り、自動修正はしない。
...
- **スラッシュの指摘を「和文どうしのときだけ」に絞った**（`slash-between-japanese`）。textlint の `ja-no-space-around-slash` は「スラッシュの前後に空きを入れるな」としか言えず、**本書で 270 件ある `EPUB / Kindle` のような欧文の並列**まで叩いていた。詰めると一語に見えて読みにくく、これは日本語の技術書で広く使われる書き方である。さらに `しきい周波数 / Hz` は**単位を表す除算**で、`・` に置き換えると意味が変わる。
  ...
  そこで判定を「**両側が和文のときだけ**」に変え、`メリット / デメリット`・`有効/無効` のように和文を並べた箇所だけを指摘して `・` か全角 `／` を薦める（空きの有無は問わない）。条件は原稿の実測で決めた——空きつきのスラッシュ 297 件のうち、両側が和文は 16 件、片側だけ和文は 11 件で、後者には上記の単位表記が含まれていた。textlint 側のルールは `SUPERSEDED_TEXTLINT_RULES` として**常に切る**（著者が `disabled_rules` に書く必要はない）。自動修正はしない——`・` と全角 `／` のどちらが合うかは文脈で決まるため。

- **`vs lint` の集約表示を、件数の多い順・同数なら最初の出現行の早い順に並べるようにした**。従来は件数だけを並べ替えの鍵にしていたが、**Ruby の `sort_by` は安定ではない**ため、同数の行（実測では 1 件の指摘が 10 行以上並ぶ）が実行ごとに違う順で出ていた。著者は原稿を上から直していくので、同じ重さの指摘は紙面の並びで出したほうが追える——第 2 の鍵に最初の出現行を置いた。textlint・独自校正・スペルチェックの 3 つの集約はそれぞれ別に実装されているので、3 箇所とも同じ規則に揃えている。あわせて、3 箇所に複製されていた整形（出現行の畳み込み・10 行での打ち切り・並べ替え）を `Lint::FindingRows` へ寄せた。`MAX_SHOWN_LINES = 10` も 3 回定義されていたのが 1 つになる。**何を 1 件と数えるかは各検査に残した**——スペルチェックは語が出た行数、他の 2 つは指摘の個数で数えており、ここを共通化すると片方の意味が変わってしまう。集めるまでは各検査、見せ方は共通、という切り分けである。

- **定義リストの `<dl>` 化を前処理から後処理へ移した**。これまでは pre_process が Markdown 段階で検出ブロックを Kramdown へ渡し、`<dl class="def-list">` を作っていた。**その結果、定義リストの中身が以降のすべての処理から隠れていた**——Kramdown は VFM 固有の記法を知らないのでルビ `{漢字|よみ}` をそのまま素通しし、索引スキャナ（ビルド Step 4）は生 HTML を「触らない領域」として飛ばすため `[語|読み]` も残る。実ビルドで検証したところ、定義リストの中でだけ **ルビと索引が紙面に記法のまま出ていた**（数式・キー表示・強調・インラインコードは、変換の順序がたまたま前だったため効いていた）。

  VFM が組み終えた HTML を受け取ってから `<dl>` にすれば、この問題は構造的に消える。`hardLineBreaks: true` の VFM は定義リストのブロックを 1 つの `<p>` にまとめて行を `<br>` で区切るので（`<p>用語A<br>: 説明<br>続きの行</p>`）、後処理でこれを `<dt>` / `<dd>` へ組み直す。字下げは VFM の出力では失われているため、**継続行かどうかは「次の行が定義行か」で判別する**。同じ実ビルドでルビ・索引・数式・キー表示・強調・インラインコードのすべてが効くことを確かめた。

  **字下げの情報だけは前処理に残した。** VFM は行頭の空白を落とすので、HTML になった時点では「字下げした継続行」と「新しい用語」が見分けられない——どちらも `<br>` 区切りの 1 行になり、「次の行が定義行か」でも決められない（どちらも次は定義行）。そこで前処理では**幅ゼロの WORD JOINER（U+2060）を継続行の頭に置くだけ**にし、HTML 化はしない。行の中の記法はそのまま VFM と索引スキャナが処理する。印は後処理が取り除くが、万一取りこぼしても紙面には何も出ない。

  `markdown-notation-collision-spec.md` の `.text-*` と同じ「前処理で生 HTML 化すると中の記法が死ぬ」構図で、直し方も同じ——**HTML 化はできるだけ後ろへ置く**。記法網羅フィクスチャにも定義リストの検体を足し、説明の中でルビ・索引・強調・インラインコードが効いていることを `rake test:notation` の不変条件にした。

- **`vs import` を `.re` 直変換にした**（`re-direct-import-spec.md`）。これまでは Re:VIEW Starter 同梱の Ruby に `rake markdown` させ、その Markdown を追従変換し、**さらに `.re` を読み直して失われた構造を復元**していた。この 3 段階をやめ、`.re` から直接 Vivlio の Markdown を書き出す。

  **なぜやったか（＝寿命）。** 同梱のスクリプトは `gem install review --version 2.5` とバージョンを固定しており、Re:VIEW Starter の開発は停止している。Ruby が進んで 2.5 が動かなくなった時点で、旧方式の `vs import` は丸ごと死ぬ。直変換なら著者の `.re` さえあれば何年後でも取り込める。

  **構造の復元が要らなくなった。** Re:VIEW の markdownbuilder は `//sideimage` を平坦化して囲みの終わりを残さず、`//list` と `//output` を同じフェンスへ落として種別を残さず、`//clearpage` を空行 2 つにする。だから「原稿の何番目の囲みが Markdown の何行目か」を位置で当てる復元コードが 3 本（508 行）必要だった。原文を読めば構造はそこにあるので、当て推量ごと消えた。**閉じフェンスを開きと誤認して 68 箇所を壊した事故**（rc.3 で修正）も、他人の Markdown を後から直す構造そのものが原因だった。

  **今より良くなった点。** `//list[][yume.c][file=source/star2/yume.c,1]` は Re:VIEW がコードを紙面へインライン展開してしまうが、直変換では ```` ```include:star2/yume.c ```` へ落とす。`source/` は `codes/` へコピーされるので**コードの置き場所が 1 箇所に保たれ、コードを直せば紙面に反映される**（実測 299 箇所）。

  **知らない記法は黙って通さない。** Re:VIEW と Starter が定義する 154 種を「変換する 114 / 指定が落ちる 29 / 対応概念が無い 11」に仕分け、後ろ 2 つは `file:line` 付きで 🟡・🔴 を出す。記法ごとに畳んで「どの記法が何箇所・最初の 3 箇所はどこか」を示し、最後に変換サマリ（変換した件数と残した件数）を出す。**静かに終わった＝全部うまくいった、と誤解させない。**

  **必須ファイルが減った。** 同梱スクリプトを動かさないので、必須は `catalog.yml` と `contents/*.re` だけになり、`lib/ruby/review-markdownmaker.rb` の存在チェックは外した。ただし**対応しているのは Starter の記法**である——実装後に Re:VIEW 2.5.0 の `compiler.rb` と突き合わせたところ、命令の集合は Starter ⊃ 本体（本体の 101 種はすべて Starter が持つ）だが、**行頭の `-` は Starter の拡張**で、本体では段落になる。未知の命令ではないので 🔴 も出ず、黙って意味が変わる。本体からの移行は要望が出た時点で改めて調べる。

  **catalog.yml に載っている章だけを変換する。** Re:VIEW では `contents/` に置いただけの `.re` は原稿ではない（実測: `book_c` は 26 個のうち catalog にあるのが 7 個で、残りは書きかけ・没の章）。全走査すると没原稿が本に混ざる。

  **ラベル ID を本全体で一意にする。** Re:VIEW は章内で一意ならよいが Vivlio は全書で一意（実測: 10 冊中 7 冊が重複を持っていた。`tbl1` を 3 章で使うなど）。章をまたいで重複した ID だけ `01-intro-tbl1` の形へ改名し、参照側も追随させる。重複しなかった ID は著者が覚えている名前のまま残す。

  **段落の改行は行末で判断する。** Re:VIEW は段落内の行を連結して組み、Vivlio は `hardLineBreaks` で改行をそのまま出す。実測 1,540 箇所の改行のうち **86.1% は文の切れ目**（`。` `？` `」` で終わる）で、ここは著者が一文一行で書いた形なのでそのまま残す。**文の途中で折り返した 214 箇所だけ連結**して Re:VIEW での組み上がりに戻す。

  検証は著者の実原稿 11 冊と Starter の記法解説ガイド、計 12 プロジェクト・76,000 行超を通して行い、例外なく完走することを確かめた。残った 🔴 は `@<balloon>`・`//embed`・`@<large>` 系・辞書の無い `@<w>` 系だけで、いずれも仕様どおり原文を残している。

- **`.small` の余白をブロックのときだけに限った**。`margin-block` / `margin-inline` を `div.small` / `p.small` へ移し、`.small` 共通には `font-size` だけを残した。`<span class="small">` のようにインラインへ当たると `margin-inline: 2em` が文中に隙間を作ってしまうため。`vs import` が `@<small>{…}` を文中に置く経路で必要になったが（実測 111 件のうち 90 行が文の途中）、インラインへ当てたときに崩れるのは元から潜在していた不整合である。

- **PDF しおりの抽出対象 HTML の選び方を 1 箇所へ寄せた**。閲覧用（`PdfMerger#add_outline_to_output_pdf!`）と入稿用（`PrintPdfBuilder#add_outline!`）に、章番号での絞り込みと特殊ページの追加が**同じ内容で二重に書かれていた**。片方だけ直せば同じ本なのに 2 つの PDF でしおりの構成が食い違うため、`Build::Utilities.outline_target_htmls` へ統合した。前付・奥付を拾わないのは従来どおりで、これらは `OutlineExtractor` が `page_range_*` で別途扱う。
...
- **`vs lint` が画像の幅指定を「表記ゆれ」と誤って指摘し、`--fix` が記法を壊していたのを直した**。`![](logo.webp){width=20%}` の半角 `%` に `spellcheck-tech-word` が当たり、「`%}` を `％}` へ」と指摘していた。これは**自動修正を持つ指摘**なので、`vs lint --fix` をかけると `{width=20％}` に書き換わり、**幅指定が効かなくなって画像が原寸で出る**（複製で実証）。原稿には値つきの属性記法が 73 箇所あり、`{width=40% align=center}` のような併記も含む。
  ...
  直し方は数式と同じ構図である。解析パスでは `NotationGuard` が属性記法を中和して指摘そのものを出さず（G5）、修正パスでは目印へ退避して `--fix` に触らせない（`mask_for_fix`）。**`--fix` には解析パスの中和が効かない**——textlint に原稿を直接直させるため、守りたいものは退避するしかない。`$(1/2)πr³$` が `$（1/2）πr³$` になる不具合と同型で、守る対象に属性記法が入っていなかったという漏れだった。地の文の `20%` は従来どおり指摘する（本物の指摘は消さない）。
...
- **EPUB / Kindle の図版が、ビルドした機械の OS 書体で焼かれていた不具合を修正**（`type3-font-embedding-notes.md` §9.6）。`rsvg-convert`（librsvg）は **data: URI の @font-face を読まない**——素材 SVG と書体を埋め込んだ派生 SVG をラスタライズした PNG がバイト単位で一致する（実測）。macOS では `FONTCONFIG_FILE` で同梱書体を登録しても見えない（Homebrew の pango が CoreText を使うため。`font-family` 自体は効いており `Courier New` は別出力になる）。つまり焼いた図には**そのとき使った機械の既定書体**が焼き付き、別の Mac で組めば別の書体、Linux の CI なら DejaVu になって**成果物が再現しなかった**。しかも EPUB は本文書体を埋め込まない（`embed_fonts?` は `false`・本文は読者の書体で組まれる）ので、図が拠れる書体は図自身が持つものしかない。そこで **EPUB にも PDF と同じ派生 SVG をそのまま配る**ようにした（`stage_author_svg_for_epub!`）。EPUB 3.3 で `image/svg+xml` はコアメディアタイプで、Apple Books と Kobo は WebKit 系なので data: URI の @font-face を解する。効かない端末に当たっても端末既定の書体で字は出るだけで、対策前と同じ状態に戻る。Kindle は KFX が SVG を扱えないのでここで焼く（透過があれば PNG・無ければ JPEG。WebP 経路と同じ規則）。書体が OS 依存になるのは見出し画像が既に受け入れている割り切りと同じで、この経路で新たに悪くなるものはない。実測（全章ビルド）: PDF の Type 3 が 0 件、EPUB 内の図版 14 枚が全件サブセットを内包。

- **著者の `images/` を機械が書き換えるのをやめた**。Techbook モードは `images/**/*.svg` を rsvg-convert で 350dpi の WebP へ焼き、`<img src>` をそちらへ向けていた。Type 3 は防げていたが、ベクタを失ううえ書体は OS のものになり、しかも**素材のディレクトリへ生成物を置いていた**（`image-format-per-target-spec.md` §3.5 の「素材を機械が黙って書き換えない」に反する）。`ImageOptimizer.optimize_images!` のラスタライズ対象を絵文字だけに絞り、`Techbook::Processor#rewrite_svg_references!` を `stylesheets/` の生成物に限定した。後者を限定しないと、過去のビルドが `images/` に残した `.webp` を掴んで元の挙動へ戻ってしまう。図版は PDF・EPUB ともベクタのまま運ばれ、Kindle だけが EPUB 枝で焼かれる——**フレーバが判る場所で焼く**ほうが、素材の隣に形式を 1 つ置いて全ターゲットで使い回すより素直である。

- **コード書体の実体を探せていなかった不具合を修正**。`HackGen35 Console NF` の実体は `stylesheets/fonts/hackgen35/` にあるが（page-settings.css が実ファイルを直に指しているだけで、Google Fonts 側の slug 規則には従っていない）、`SvgFontEmbedder#font_path` は `fonts/<slug>/`＝`fonts/HackGen35_Console_NF/` しか見ていなかった。slug で引けないときは中身のファイル名から引き当てる。等幅で組んだ図だけサブセットを作れず Type 3 が残る、という形で表に出る。

- **コードブロックの直前にある段落が、キャプションでなくても見出し扱いされていた不具合を修正**。`code.css` の `p:has(+ pre)` / `p:has(+ figure > pre)` が「コードの直前の段落」を隣接だけでキャプションと見なしており、本書では **403 件の普通の文章が字下げ・両端揃え・本文書体を失っていた**（意図した対象は `**設定例**` のように強調だけで一段落をなす 16 件）。判定を後処理の `PostProcess.mark_code_captions!` へ移し、「段落全体が `<strong>` ひとつ」かつ「直後がコードブロック」の両方を満たすものにだけ `code-caption` クラスを付ける。CSS では「段落全体が強調か」を表現できない——`:only-child` は要素の子しか見ず、地の文を含む段落も通してしまう——ので、隣接の推測ではなく構造の確定に置いた。画像の `<figure>` を巻き込まないよう、figure は `language-` クラス付きに限る。副産物として 16 件のキャプションが `.code-caption` の `break-after: avoid` を得ている——旧ルールはこれを持たず、キャプションだけがページ末に取り残される穴が残っていた。

- **ビルド時にも単位・記号とまぎらわしい手動マークアップを知らせるようにした**（同 §11-3）。`vs index:auto` の R9（半角 1〜2 文字は辞書へ登録せず警告）は辞書を守るが紙面は守っておらず、`[g]` が警告なしに索引へ載っていた。紙面へ出る直前にも同じしきい値で見張る。**弾かずに警告だけ**——`[g]` が本当に索引語のつもりかは機械に判らず、弾けば正しい手動マークアップまで巻き込むため。語ごとに 1 行へ畳むので、同じ語が何箇所にあっても 1 件で済む。

- **会話文の `avatar` の綴りを間違えると、警告なしにアバターが全部消えていた不具合を修正**。`TalkRegistry.parse_avatar_mode` は `auto` を先に見たあと残りを `Common.truthy?` へ渡すだけで、取り得る値を宣言した `AVATAR_MODES` を**一度も参照していなかった**。そのため `config/talk.yml` に `avatar: onn` と 1 文字打ち間違えるだけで `:off` が確定し、全話者のアバターが理由も示されず消えていた（`:::{.talk avatar=onn}` のブロック指定も同じ）。**フォールバック先が最も目立たない状態**だったぶん、失敗が最大限静かになる形だった。受理する綴りを `AVATAR_MODE_SPELLINGS` に明記して照合し、未知の綴りは 🟡 で既定へ戻す。兄弟の `style` は以前から同じ流儀で警告していたので、その非対称を解消したことになる。`avatar: on` / `off` は YAML（Psych の YAML 1.1）が真偽値へ解決するため、`true` / `false` も同じモードの綴りとして受ける——ここを狭めると既存の `talk.yml` が壊れる。

- **`vs clean --cache` が旧版の `_glossarypage.html` を消し残していた不具合を修正**。索引ページだけが名指しで削除され、用語集ページに対がなかった。`--cache` 単独指定は `clean_build_artifacts`（`LEGACY_ROOT_PATTERNS` の `'*.html'`）を通らないため、P4 以前のビルドがルートへ残した用語集ページだけが生き残っていた。

- **参照リンクと GFM のタスクリストが索引マークアップに化けて壊れていた不具合を修正**（同 §3・§4）。`[本文][ref]` と `[ref]: url` は両方の角括弧が索引語になってリンクが消え、`- [ ]` `- [x]` は VFM へ届く前にマーカーが索引語へ化けていた。参照リンクの判定根拠は CommonMark そのもの——`[foo]` が参照リンクになるのは同じ文書に `[foo]: url` の定義があるときだけ。タスクリストは紙面へチェックボックスとして組み、枠は罫線で描く（`☑` `☐` は同梱の本文書体に無く、使うと Type 3 混入の経路になる）。Kindle は罫線を解さないため `□` / `■` の文字へ落とす。原稿は第 3 章にタスクリストの節を新設し、`---` の前後に空行が要ることと、コードブロックがフェンスのみであることを追記。

## [1.0.0-rc.3] - 2026-08-21

### Added

- **```` ```math ```` で「これは数式だ」と宣言できるようにした**。仕様: `plain-math-notation-spec.md` §6.5
  - 自動判定には越えられない線がある。``cos θ = 1 - θ²/2! + …`` は `θ` と `²` があるので数式と分かるが、``cos = 1 - 2 + 4 - 6`` は英字・数字・記号だけで `total = price - tax` と字面が同じで、**機械には決められない**。判定を保守的に保つ以上こちらは落とすほかなく、著者が明示する手段が要る。
  - 言語欄に `math` と書いたコードブロックは、**判定を一切通さず**ディスプレイ数式として組む。素の言語なしフェンスに掛かる 4 条件（行数・実行結果らしさ・関係演算子・数式らしさ）はどれも適用しない。宣言であって推測ではないため。
  - **綴りは GitHub に合わせた。** 同じ書き方でディスプレイ数式が出るので、著者が新しく覚えることが無い。Prism の言語名（`latex` / `tex` / `context`）とは衝突せず、前処理でフェンスごと `$$…$$` へ置換するので Prism まで届かない。```` ```latex ```` は「LaTeX のソースを見せる」用途なのでコードのまま扱う。
  - 各行を `$$` で囲まなくても `=` の位置で揃う。著者がすでに `\\` や `\begin{aligned}` を書いていれば、その行組みをそのまま渡す。

- **著者が素で書いた数式を TeX へ起こすようにした**（`√n`・`x²`・`H₂O`・`a^(p-1) mod p`・`Σx²`）。仕様: `plain-math-notation-spec.md`（第 1 段階＝変換器）
  - **これは親切機能である前に、現に起きている無言のデータ欠落の修理**である。MathJax の TeX パーサは素の表記を**エラーにしない**まま壊す。実測: `x²+y²` → `𝑥+𝑦`（上付きが消える）、`H₂O` → `𝐻𝑂`、`ａ＋ｂ` → 何も描かれない、`a・b` → `𝑎𝑏`（中黒 U+30FB が消える）、`2^(1/3)` → `2` の右肩に `(` だけ、`√2` → 根号の横線が無い、`Σx` → `𝛴𝑥`（斜体の変数）、`tanθ` → `𝑡𝑎𝑛𝜃`。ビルドは成功し警告も出ないので、著者は完成した本を見るまで気づかない。
  - **原稿に LaTeX は一度も書かない。** TeX はビルド中だけの中間表現で、著者が読み書きするのは常に素の表記。`a^(p-1)` の `a` を `b` に直す、というごく普通の編集がそのままできる。
  - **変換するのは実測で壊れている表記だけ。** ギリシャ文字・`≡ ≈ ≒ ≠ ≦ ∈ → ° ∞ ... ! −`・`·`(U+00B7)・`∑`(U+2211)・`∫`・`f'(x)` は**そのままで正しく描かれる**ことを 1 つずつ確認して表から外した（`π` は `\pi` と完全に同一の SVG になる）。触らない記号は壊しようがないので、規則を増やさないこと自体が安全側に効く。**消えるのは `・`(U+30FB) であって `·`(U+00B7) ではない**——この 2 つは見た目が似ていて挙動が違う。
  - **TeX の特殊文字を退避する。** `50% + 1` は `%` から後ろがコメント扱いで `50` になり、`a#b`・`a&b` は TeX エラーになる。`〜` は**半角 `~` にしてはならない**——TeX の `~` は改行しない空白で、範囲を表す `g^1〜g^(p-1)` が `g^1 g^(p-1)` に化ける（`\sim` を使う）。
  - **既存の TeX は 1 文字も変えない。** ゲート（素の表記のしるし）が無ければ走査すらしない。`a^2` は TeX としてそのまま正しいのでしるしにせず、壊れる形（引数が 2 文字以上の `a^560`、丸括弧を引数にした `a^(p-1)`）だけを拾う。**1 文字でも変わると数式 SVG のキャッシュキーが割れて既刊原稿が丸ごと再描画される**ので、ここは本書の現行数式を並べたテストで守っている。
  - **全体拒否をしない**（`MathTextRenderer` と逆）。素の表記と TeX は混在しうる（`\frac{1}{2}√2`）ので、解釈できない字は原文のまま次へ送る。閉じ括弧が無い `√(x` も原文を失わずそのまま通る。
  - **`alt` は著者の原文のまま**にし、TeX は `data-vs-tex` で渡す。`alt` の消費者は 2 人いて必要とするものが逆だった——Kindle のテキスト化は TeX が要り、章扉リードの焼き込みは人が読める文字列が要る。`alt` を TeX にすると章扉に `\sqrt{2}` と焼かれ、素のままにすると Kindle が式を拒否して px 固定 SVG へ落ちる。
  - **Kindle の追従性も一緒に改善する。** `$x²+y²$` は起こす前だと `²` で `MathTextRenderer` に拒否され SVG のままだったが、いまはテキスト化されてフォントサイズに 100% 追従する。
  - 検証: 実原稿コーパス（`workbook` の 166 ファイル＋`problems.yml`）から集めた **329 式を MathJax で描画し、TeX エラー 0 件・`<text>` 混入 0 件**。変換前後のグリフ比較で **115 式が改善・79 式は元から正しく無変換・退行 0**。
- **バッククォートで書かれた数式を、原稿を書き換えずに数式として組むようにした**。仕様: `plain-math-notation-spec.md` §6
  - **数式記法を知らない著者は、バッククォートを数式デリミタの代用に使う**——等幅で本文と区別が付くからである。実測（`workbook` 166 ファイル＋`problems.yml`）でインラインコードスパン 3,422 出現のうち **361 出現（11%）が数式**、言語なしフェンス 546 件のうち **70 件がディスプレイ数式**だった。残る 89% は本物のコードなので判定が要る。
  - **判定は「コードのしるしがあれば拒否」を先に置く。** 逆向き（数式のしるしがあれば数式）にすると `array[i] * 2` のようなコードを拾う。フェンスは `Masking.replace_top_level_fences` で先に処理し、数式でないものは退避してからインライン走査に入る（フェンスの中のコードスパンを拾わないため）。
  - **複数行のディスプレイは `\begin{aligned}` で関係演算子の位置を揃える。** 1 行ずつ独立した `$$` にすると `X = …` に続く `= …` の導出が揃わない。
  - **1 文字の綴りは式にしない。** 本書 `32-metrics.md` の「`・` で連結し」で踏んだ——記号そのものを説明している箇所を数式にしていた。実測でこの規則が落とすのはコーパス全体で `λ` 1 種だけ。
  - **推測で拾う経路では日本語を数式にしない**（明示的な `$…$` は組む）。`攻撃側の戦闘力 = (投入兵士数 × 訓練度)` を数式画像にしても得るものは無く、それを機械が黙って決めてよい判断でもない。
  - 逃げ道: 確実に数式にしたいときは `$…$` で明示、確実にコードのままにしたいときは言語付きフェンス（```` ```ruby ````）に置く。
  - 検証: 54 / 166 ファイルが変換され、インライン 326 箇所・ディスプレイ 17 箇所が数式になった。起こした **275 式を MathJax へ通して TeX エラー 0 件・`<text>` 混入 0 件**。誤検出 0。
- **Kindle の数式を作り直した**（インラインは 3 段に落とす／ディスプレイは PNG／日本語入り SVG に書体を埋め込む）。仕様: `plain-math-notation-spec.md` §3-2
  - **インラインの落とし先を 3 段にした。** ①`MathTextRenderer` が通れば HTML、②通らなくても**素の表記で書かれていれば著者の原文テキスト**、③LaTeX で書かれた式だけ SVG。素の表記はそのまま読める形で書かれているので、変換を挟む必要が無い。**実測でフォントサイズに追従できる式が 20% → 100%**（`workbook` コーパス 280 種）。
    - これは**上の変換器が入れた退行の解消**でもある。`sin(x)` → `\sin(x)`・`mod` → `\bmod` と起こす一方、`MathTextRenderer` は `\sin` も `\bmod` も知らない（ホワイトリストが LaTeX で書かれた原稿だけを見て作られたため）ので、**変換前ならテキスト化できていた式が SVG へ落ちていた**（実測 34 種）。個別に足すのではなく、落とし先を変えて経路ごと無くした。
    - **判定は「変換されたか」ではなく「素の表記で書かれたか」。** `a × b ÷ c` は変換が要らない（記号がそのまま正しく描かれる）が、原文はそのまま読める。取り違えると追従率が 27 ポイント落ちる。正しい判定は **TeX の構文（`\command`・`{}`）を含まないこと**。
  - **ディスプレイ数式は Kindle で PNG にする。** `Σ` や `∫` を含む大きな式を独立行に組むのがディスプレイ数式の意図なので、テキスト化せず組版を保つ。KFX の SVG 対応が不確実な点にも効く。**クリーン EPUB はベクタのまま**——数式 SVG はパスだけで安全で、Kobo・Apple では拡大に強い利点が残る。`rsvg-convert` 不在時は SVG のまま警告して続行。
  - **日本語を含む数式 SVG に書体を埋め込んだ。** MathJax は自分のフォントに無い字（CJK）を `<text font-family="serif">` で出す。`<img>` 参照の SVG は独立文書で `@font-face` が届かないため、**PDF に Type 3 が混入していた**（実測 +2 件・OS の明朝）。`SvgFontEmbedder` を当てて対照と同数（CID TrueType）まで戻した。
    - **縮退の落とし穴**: 書体を解決できない環境で `font-family` だけ専用名へ書き換えると、存在しないファミリを指すだけで従来（`serif`）より悪くなる。埋め込みに失敗したら名前も変えない。
    - キャッシュキーは**日本語を含む式だけ書体を混ぜる**（書体を替えたら焼き直す必要があるため）。含まない式のキーは書体に依存しないまま保ち、本書の既存数式 34 件でキー変化 0 件を確認。

- **交ぜ書き辞書を 2 段構成にし、1,912 語を足した**（150 → 2,062 語）。仕様: `mazegaki-two-tier-spec.md`
  - **分かれ目は「前後の制約だけで誤検出を外せるか」だった。**「き損（毀損）」は後続を `なねじ` で外せば「引き損なう」が消えるので正規表現で足りる。「う回（迂回）」は外せない——「使う回数」「行なう回数」「という回答」の直前はひらがなにも漢字にもなる。`使う|回数` と形態素が切れることを見るしか区別する手立てがない。**この 2 つを同じ辞書に混ぜると、MeCab が無い環境で後者が誤検出を出す**ので分けた。
  - 第 1 層 `MazegakiDictionary::ALL` 150 語は MeCab 不要（いまの振る舞いをそのまま保つ）。第 2 層は `MECAB_ONLY` 3 語＋ `lint/data/mazegaki.tsv` 1,912 語で、**MeCab があるときだけ**足す。無ければ静かに黙る（導入案内は索引機能がすでに出しており、lint でも鳴らすと二重になる）。
  - 第 2 層の判定は 3 条件の AND——**開始が形態素の頭**（`細かい書き方`→かい書 を防ぐ）、**終了が形態素の切れ目**（`成果物ごとに`→物乞、`引っかかった`→引っ掻 を防ぐ）、**両端が助詞・助動詞でない**（`RS のしきい値`、`呼び出し側が`）。走査は形態素の頭からだけ辞書を引く（2,000 語を `Regexp.union` にすると遅い）。
  - `--fix` は第 2 層を**判定した位置だけ**置換する。同じ綴りが同じ行の別の場所に出ても、そちらが条件を満たすとは限らないため。第 1 層の `gsub` より先に、後ろから当てる。
  - データは SudachiDict の正規化表記の派生物（Apache-2.0）。**種別「かな書き」は使えなかった**——17,439 語で検出 25,814 箇所、`し→四` 7,674 件・`する→為る` 5,526 件。SudachiDict の正規化は検索のための同一視であって書き言葉の正誤ではない。絞り込みの 6 条件と実測値は `lint/data/filter.rb` の冒頭にあり、`rake mazegaki:build` で作り直せる。
  - 本書 200 万字での実測は**誤検出 0**。
- **辞書へ 12 語追加**: `えん下→嚥下`・`下かん→下疳`・`骨髄しゅ→骨髄腫`・`ほう章→褒章`、ガード付きで `後じん→後塵`（「最後じんわり」を外す）・`は種→播種`（「後書き（99）は種別が」）・`合し→合祀`（「結合し直した」）、MeCab 前提で `う回→迂回`・`語い→語彙`・`り患→罹患`。
  - `う回`・`語い`・`り患` は「制約で救えない」として長く落としていた語だが、**MeCab を前提にすれば覆る**（正規表現のみ 10 問中 4 問で誤検出、MeCab 併用で 10/10）。
  - 採らなかった語も理由ごと記録した。`語ろ→語呂`（「語ろうとする」）・`御す→御簾`（「部下を御する」）・`交し→交趾`（「言葉を交して」）・`りん子→綸子`（人名）は、**交ぜ書き表記がそのまま別語として読める**型。同音異義ではなく同表記異読なので、形態素解析でも落とせない。`そ上` は俎上とも遡上とも読めて行き先が定まらない。

- **`vs upgrade` が「別の場所を変えた競合」を 3-way マージで自動合流するようにした**（`upgrade-three-way-merge-spec.md`）: これまで**雛形と著者が同じファイルを変えていれば、どこを変えたかに関わらず「競合」**で、`y`（著者の変更を捨てる）か `n`（雛形の改良を取り逃す）の二択だった。あなたがスタイルシートの末尾に自分の規則を足し、雛形は先頭のほうを改良した——という**両立できるはずの場面**まで二択になっていた。
  - 実測（rc.1 のプロジェクトを現行へ更新）: `stylesheets/preface.css` が**合流**になり、雛形側の 2 箇所（`--preface-h3-marker` の追加と `content` の差し替え）が入りつつ、著者の 3 行がそのまま残った。衝突マーカーの混入は 0。
  - **同じ場所を変えていたときだけ「競合」として尋ねる。** このときは適用すれば著者の変更が本当に消えるので、そう名指しできるようになった（従来はどの競合でも断定できなかった）。
  - **祖先（前回追従時の雛形）の中身は lock から復元できない**——`config/scaffold.lock` はハッシュしか持たないため「変わったか」は分かっても「どこが変わったか」は分からない。調達元は 2 つで、**インストール済みの旧版 gem**（`gem update` は旧版を消さない）と、**`.cache/vs/scaffold-base/`**（`vs new` / `vs upgrade` の完了時に控える写し・114 ファイル 1.2MB）。どちらも無ければ従来どおり 2-way の競合として尋ねる。
  - **祖先は「lock のハッシュと中身が一致したもの」だけ採る。** 祖先を間違えたマージは「衝突なく合流した」という顔で誤った中身を書き、合流は尋ねずに適用するので**気づく機会がどこにもない**。版の取り違え・`gem cleanup` 済み・古い保存基準といった事故を、経路ごとの場合分けではなく内容そのもので一律に弾く。
  - **保存基準は lock と歩調を合わせる。** 競合を `n` でスキップしたファイルの基準まで新版へ進めると、次回は「雛形側は何も変えていない」判定になり、**雛形の改良を黙って捨てた合流**が起きる。複製するのは「lock が現在の雛形と一致すると記録したファイル」だけにした（回帰テストあり）。
  - マージ本体は **`git merge-file`** に任せた。自前の diff3 は境界条件が多く、枯れた実装のほうが安い。`git` が無い環境では静かに 2-way へ落とす（エラーにしない）。**衝突マーカーが著者のファイルへ紛れ込むことはない**——衝突なく合流できたときだけ書き込む。
  - 合流も上書きなので、従来どおり `.cache/vs/upgrade-backup/` へ退避してから適用する。

- **`vs lint` に日本語固有の 2 ルールを足した**（`lint-japanese-prose-rules-spec.md`）: 交ぜ書き（`mazegaki`）と、二通りに読める対比（`ambiguous-comparison`）。どちらも textlint のルールでは扱えない。
  - **交ぜ書き**は熟語の一部だけをかなで書いた表記を「`だ円 => 楕円`」の形で指摘し、`vs lint --fix` で置換する。辞書は 140 語。由来は 2 つで、常用漢字表に入ったのにかな書きが残っているもの（`完ぺき`・`破たん`・`進ちょく`）と、いまも表外漢字のもの（`だ円`・`しきい値`・`べき乗`・`ぜい弱`）。
    - **語は「他の語の一部として現れない」ものだけを載せた。** `う回`（迂回）は「使う回数」に、`語い`（語彙）は「日本語いろは」に当たるので入れていない。救えるものは前後を制約した——`べき乗` は「予約すべき乗り換え」を避けるため助動詞「べき」が付く動詞終止形（う段）の直後を外し、`き損` は「引き損なう」、`き裂` は「引き裂く」、`かく乱` は「とにかく乱暴」、`だ円` は「囲んだ円」、`ら致` は「〜から致します」を外す。**制約の理由は 1 行ずつコメントに残した**（消すと静かに誤検出へ戻るため）。
    - **危険な綴りは実際の文字列で確かめる。** `かく乱` を一度「書く乱れた字に当たる」として落としたが、これは誤りだった——「書く乱れ」の「書」は漢字なので `かく乱` の並びにならない。頭の中で「当たりそう」と判断すると、当たらない語まで落とす。
    - **かな書きが主流になっている語も指摘する**（`改ざん` → 改竄、`せん断` → 剪断、`宝くじ` → 宝籤）。常用漢字表が 2000 字余りに収まらないために交ぜ書きが広まった語で、漢字表記そのものは正しい——JIS が「せん断」を採るのも表内に収めるためであって「剪断」が誤りだからではない。**除外の線引きは書き手の方針であって、辞書の側で決めるものではない**という整理にした（この基準は 2 度動いた末の結論で、当初は「世間の標準表記は載せない」、次に「規格が定める語だけ外す」だった）。
    - **交ぜ書きでない語も 1 つだけ同居させた**（`障害者` → 障碍者）。漢字から漢字への置き換えなので定義からは外れるが、`障がい` → 障碍 を採る著者にとって `障害者` だけ素通りするのは中途半端である。ルール ID も分けない——`disabled_rules: [mazegaki]` で切ったのに一部の指摘だけ残るほうが意外な振る舞いになる。対象は文字列「障害者」に限り、「システム障害」「障害物競走」は書き換えない。
    - **「この語はかなで通す」は `config/textlint_allowlist.yml` に書けば語ごとに黙る。** 辞書は gem 側にあって著者が削れないので、語単位で拒否する道を用意した。窓口は textlint と同じファイルに一本化してある（`book.yml` の `lint.disabled_terms` は「指摘したくない語句は config/textlint_allowlist.yml に書きます」という理由で廃止済みで、独自ルールだけ別の場所を見ると著者が二度学ぶことになる）。**黙らせた語は `--fix` の置換対象からも外れる**——指摘しない語を直すのは筋が通らないうえ、「黙っているのに原稿が書き換わる」のは最も分かりにくい壊れ方になる。
      - 判定は **語全体が覆われたときだけ**（textlint の allowlist と同じ）。部分一致にすると、除外リストにある `括弧` の 1 行（「括弧 => カッコ」を止めるためのもの）が「かぎ括弧 => 鉤括弧」まで消してしまう。
  - **二通りに読める対比**は「B は A と同様に X しない」の形を指摘する。「A も X しない」のか「A とは違って B は X しない」のかが文脈だけでは決まらず、読者が反対の意味で受け取っても気づけない。自動修正はしない（どちらが X するのかは著者にしか分からない）。
    - **「のように」は対象から外した。** 当初は含めていたが、本書 27 ファイルで **10 件検出・10 件すべて誤検出**だった（「`@titlepage` のように文字が続く場合は展開されません」など、例示・限定・様態の用法）。格助詞「と」を伴う 3 表現に絞って再測し 0 件。読点や文末を条件に足す案も試したが、条件を積むほど脆くなるだけだった。
    - **段落の切れ目に箇条書き・表・見出しを加えた。** 辞書を広げて 263 ファイル（本書＋`docs/`）へ当て直したところ、**比較表現を 1 つも含まない行**が挙がった。箇条書きの項目どうし・表のセルどうしを 1 文へ連結していたためで、隣り合うだけの行で「比較 → 否定」が成立していた（5 件）。指摘が指す行の説明がつかないときは、段落の作り方を疑うとよい。
  - **prh 辞書ではなく gem 本体に置いた。** `config/` は著者が編集するファイルで `vs upgrade` が上書きできず、辞書方式では新規プロジェクトにしか届かない。加えて prh の指摘はすべて `ruleId: prh` なので、`lint.disabled_rules` で切ると表記揺れ辞書ごと死ぬ。独自ルールなら `mazegaki` / `ambiguous-comparison` を個別に切れる。
  - **「ら抜き言葉」は実装していない——すでに動いていた。** `preset-japanese` に `no-dropping-the-ra` が含まれており、「見れる」「集めれる」は以前から `[no-dropping-the-ra]` として挙がる。原稿 `31-lint.md` に効いている旨を書いた。
- **入稿用 PDF を `vs pdf:compress` にかけようとしたら引き止めるようにした**（`print-pdf-compress-guard-spec.md`）: 圧縮すると Ghostscript が画像を 150 dpi へ落とし全画像を JPEG へ再エンコードするため、印刷所が求める 300〜350 dpi を下回る（実測: 97.2 MB → 10.6 MB・中央値 564 ppi → 150 ppi）。原稿はこの危険を文章で塞いでいたが（`45-utility.md`「入稿用データには使用しないでください」）、**文章で書いてあるということは機械が止めていないということ**である。
  - **止めない。尋ねる。** 出力は `_compressed` 付きで**上書きしない**ので、事故は「圧縮版を入稿してしまう」ことであって原本の破壊ではない。入稿以外の用途で配りたいなど、意図して圧縮する場面もありうる。非対話（パイプ・CI）では尋ねられないので、警告だけ出して続行する。
  - **警告には理由と数値を入れた。**「使わないでください」だけでは、なぜ駄目なのかが分からず著者は次も同じことをする。
  - **判定は 2 つの手掛かりの OR。** ファイル名は `"#{project.name}_print"` の**前方一致**で見る——`include_version: false` なら `_v1.0.0` が付かず、手元には過去のバージョンでビルドしたものも残るので、厳密一致では取りこぼす。著者がリネームしている場合はページボックス（塗り足しがあると MediaBox ≠ TrimBox）で拾う。`crop_marks: false` ＋ リネーム済みの両方が重なると検出できないが、判定を緩めると閲覧用にまで警告が出て狼少年になるため、仕様上の限界として受け入れる。
  - **ビルド経路は既に三重に守られていた**ので触っていない。圧縮対象が閲覧用の結合済み PDF だけであること、圧縮ステップの条件が `t.pdf &&` で始まること、入稿用の導出元が中間生成物であることによる。塞いだのは手動コマンドの経路だけである。
- **PDF へ渡す画像を JPEG の派生へ差し替えるようにした**（`Build::DerivedImage`・`image-format-per-target-spec.md` §3.1）: PDF は WebP を格納できず（ISO 32000 に該当するフィルタが無い）、渡された Chromium はデコードして Flate へ入れ直す。可逆圧縮では写真がほとんど縮まないため、**素材 280 KB が PDF 内で 1,963 KB になっていた**。JPEG なら `DCTDecode` でストリームがそのまま入る。実測: 24 章の単章ビルドで **7,148,170 → 4,957,964 バイト（−30.6%）**、`pdfimages` の `enc` 列も `image`（Flate）から `jpeg` へ変わる。
  - **著者の `images/` には触れない。** 派生は `.cache/vs/derived/pdf/` にだけ作り、PDF 枝がステージングする `pdf/` の HTML だけがそこを指す。`html/` の原本を読む EPUB / Kindle には現れないので、**WebP が最適な EPUB 側は変換ゼロのまま**である。既存プロジェクトも移行作業なしで効く。
  - **透過は PDF に限って白へ落とす。** PDF の地は紙の白で、`@page` に背景色の指定が無い——**透過部分はどのみち白く出るので、見た目が 1 ピクセルも変わらないまま JPEG が使えるようになる**。本書の花の見本 12 枚は PDF 内 **37.22 MB → 6.31 MB**。
    - **ただし地色を持つブロックの中では透過を保つ。** 透過を落とす根拠は「PDF の地は紙の白」だったが、**囲みブロックには背景色がある**——実装直後の紙面を拡大したところ、コラムの緑地に絵文字の白い矩形が浮いていた。`img` の祖先に地色クラス（`column` `memo` `notice` `tip` `terminal` `book-card` `talk-body` ほか）があれば透過を残す。祖先をたどる必要から、差し替えを Nokogiri ベースへ改めた。
      - **フラット化したものと透過を残したものは別ファイルで持つ**（`_alpha` 接尾辞）。同じ絵が地色のブロックの中と外の両方に出ることがあり、片方で上書きしてはならない。
      - **透過を残すときは JPEG を候補から外す**（持てないため）。サイズを比較せず PNG で決める。
      - 代償は小さい。全章で 27,304,871 → 27,312,818 バイト（**+0.03%**）。
    - **EPUB / Kindle では捨てない。** あちらは `html/` の原本（素材そのもの）を読む。**Kindle 端末のダークモードは背景を黒にするが画像は反転しない**ため、白で塗ってあると矩形が浮く。
    - **テーマ素材（`stylesheets/images/`）も捨てない。** CSS 背景で入るため `<img>` を経由せず対象外になるが、それが正しい——章扉の生成は `-trim` で余白を落とす際に**透過を「どこまでが絵か」の手掛かりに使っている**（`image_generator.rb:176-182`）。白で潰すと絵の中の白まで巻き込みうる。
  - **形式は色数では決められなかった。** 透過が無いものは JPEG と PNG を両方作って小さいほうを採る（実測 2026-08-17・本書 78 件: 色数 2,701 の写真は JPEG が、色数 2,731 の図は PNG が勝つ）。サイズの勝敗は「写真か図か」とほぼ一致するので、この比較は画質の判定も兼ねている——写真は PNG で膨らみ、文字入りの図は PNG で縮むためである。
  - **`:::{.showcase}` の合成画像も JPEG で埋めるようにした。** 図解注釈は元画像＋注釈を 1 枚の SVG に焼き、元画像を base64 の data URI で抱えるが、**これが PNG 固定だった**。PNG は可逆なので写真では縮まず、PDF 内で 1 枚 1,849 KB・全章 5 枚で 9.2 MB を Flate のまま占めていた。写真判定（`photographic?`）と JPEG 出力は EPUB 向けのラスター化で既に持っていたので、同じ基準を data URI にも適用しただけである。実測 **1,849 KB → 268 KB（約 1/7）**。SVG が中身を抱える以上、キャッシュキーの版も上げないと古い PNG 版が残る（`v2` → `v3`）。
  - **紙面に対して過剰な画素数を、組んでから測って縮めるようにした**（`shrink oversized images` ステップ・§3.6）。上限だけでは救えない無駄がある——**同じ素材が用途によって違う大きさで置かれる**ためで、本書の第 12 章は章扉用の 2048×2048 をカタログとして版面の半分に並べており、668 ppi（必要な 350 ppi の 1.9 倍）になっていた。素材としては何も間違っておらず、悪いのは置き場所のほうなので、上限では検出できない。
    - **測るのは組版の後でなければならない。** 表示幅は表の列幅や `width` 指定で決まり、原稿からは分からない。`vs build` は索引のページ番号確定のため既に PDF を 2 回組んでいるので、**新しいパスを増やさずに** 1 回目と 2 回目の間へ挟んだ。
    - **素材の特定は要らなかった。** `pdfimages` はどのファイルから来たかを教えないが、必要なのは「画素数 W×H の素材には何 px あれば足りるか」だけなので、画素数をキーに ppi の最小値（＝最も大きく表示されている場面）を採れば決まる。同じ画素数の素材が複数あっても、最も大きく表示されているものに合わせるので 350 ppi を下回らない。
    - **段階へ切り上げる**（480 / 800 / 1280 / 2048）。1,073px 要るなら 800 ではなく 1280——足りない解像度は後から取り戻せず、印刷では粗さとして残る。段階にすると同じ素材が似た大きさで何箇所に置かれても 1 つの派生を使い回せる。段階を超えるときだけ必要画素数そのものへ丸める（版面全幅に置かれた 4K 素材が 2048px で打ち止めになるのを防ぐ）。
    - **`width` / `height` 属性で intrinsic size を凍結する。** 画素数を変えてもレイアウトは動かないことを実測で確かめてあるが（42 章で 2048px → 1280px に差し替え、全 16 ページ 1,249 語が同一座標）、素材の寸法が不揃いだと表の列幅配分がわずかに動く。属性で与えればその余地も消える。
    - 索引を使わず Step 8 が走らない本では、測定結果が `.cache/vs/derived/` に残るので**次回のビルドから**効く。
  - **クリーン EPUB では PNG / JPEG を WebP へ寄せるようにした。** EPUB 3.3 では image/webp がコアメディアタイプで、同じ絵なら 3 つの形式でもっとも小さい（実測: WebP 280 KB / JPEG 441 KB）。本書は素材が WebP なので対象は**ビルド生成物**（showcase の合成ラスタ・mermaid 図）の 7 件だけだったが、それでも **1,580 KB → 857 KB（−46%）**。素材を PNG / JPEG で置いている本ではさらに効く。
    - **差し替えた元ファイルだけをパッケージから落とす。** Kindle が WebP を丸ごと外すのは KFX が読めないからで、外さなければ壊れる。クリーン EPUB にとって PNG / JPEG の除去は最適化にすぎず、著者の CSS が `images/` の画像を参照していれば壊す側に回る。
    - **ローカライズより後に走らせなければならない。** 資産のコピーは HTML 書き換えの後に走るので、順序を誤ると削除が空振りし、未参照の重複が残って**逆に +876 KB 太る**（実測して気づいた）。
  - **EPUB / Kindle にも版面基準の縮小を効かせた。** リフローでは「組んでから測る」ができない——ページが確定せず、表示幅は端末・フォントサイズ・向きで変わる。しかし**版面に対する割合は組版系によらない**ので、PDF 側で測った割合をそのまま読者の画面幅へ掛ければよい。`width=30%` の指定も 2 列に並べた表も、PDF の測定に既に現れている。実測: **EPUB 30.5 MB → 18.1 MB（−40.6%）**、epubcheck は 0 エラー / 0 警告。
    - 画面幅は EPUB **2048px**（iPad Pro 12.9"。ここを超える表示先が事実上ない）、Kindle **1024px**（既存値。Amazon が入稿 KPF の画像をすべて JPEG へ再エンコードして配信するため 1600px でも過剰、という実測に基づく）。
    - 版面の 48.1% を占める花の見本は **2048px → 985px** になった。章扉は全面に敷かれるので割合 1.0 となり、上限だけが効いて 2880×4153 → 1420×2048。
    - **表紙だけは縮めない。** ストアが表紙に固有の寸法を求めており（KDP の推奨は長辺 2560px で、既定の 1600×2560 はまさにその値）、本文と同じ基準で落とすと要件を割る。
    - **測定結果は世代をまたいで引き継ぐ。** 2 回目以降のビルドは 1 回目から縮小版で組まれるため、素材の画素数のエントリが測定から消える。EPUB / Kindle は素材から派生を作るので、そのキーが引けないと縮小が効かなくなる。
  - **`vs resize --high` の上限を 2000 → 2048px にした。** 2 の冪で派生の段階の最大値と一致するため、素材を段階と同じ画素数で持てばビルド側の縮小が「もう小さいので不要」と判定してそのまま使える。2000 のままだと、2048×2048 で配られている同梱のテーマ素材が `resize:high` を通すたびに 48px だけ削られていた。
  - **全章の合計: 97.2 MB → 31.0 MB（−68.1%）。** 総ページ数 346 は不変、🔴🟡 とも 0 件。残る Flate 10.3 MB はほぼ全部が「触らないと決めたもの」——表紙 6.4 MB（350 ppi で適正）、章扉 3.0 MB と節絵 0.66 MB（テーマ素材）である。
  - 章扉・節絵（`stylesheets/images/` のテーマ素材）は CSS 背景で入るため `<img>` を経由せず、自然に対象外になる。表示解像度も 350〜368 ppi で既に適正である。
- **区切り線のつもりで置かれた `---` を `vs preflight` が拾うようにした**（`Guards::StrayPageBreakCheck`）: 標準の Markdown で `---` は水平線だが、**Vivlio Starter では改ページ**である。素直に区切りのつもりで書くとそこでページが終わり、白紙が生まれる。2026-08-10 に判型を B5 → A4 へ変えたとき全 26 章に 4 件の取り残しが見つかった件（`PLANNED.md`）の検出側。**版面が狭いうちは 1 ブロックで埋まる割合が大きく目立たなかった**——判型を変えると紙面の前提が変わるので、変えた直後こそ走らせたい。
  - **指摘するのは 2 つだけ**。(a) 後ろに内容が無い（白紙が 1 枚増えるだけで、誤検知の余地がない）(b) 見出しの直前（そこでページが切れる）。どちらも「行番号・直後の見出し名・何が起きるか」を添え、意図した改ページなら `@pagebreak` と書けばよいことまで案内する。
  - **指摘しないものを決めるほうが本体だった。** 節見出し（h2）の直前は、h2 が自動改ページする設定なら `PageBreakNormalizer` が正規化するうえ、原稿（`22-extentions.md`）が「著者は気にせず書いて構いません」と明記している——ここで警告すると**本の説明と機能が矛盾する**。同じ理由で、連続した `---`（意図的な空白ページ）・後ろに本文が続く改ページ・`@pagebreak` は対象外。`section_pagebreak` の判定は正規化と同じ述語（`Common.section_pagebreak_enabled?`）を見るので、設定を変えれば指摘も追随する。
...
- **交ぜ書きの辞書が 2 段になったことを原稿と `vs doctor` に反映した**: 31 章は「`う回`（迂回）のように他の語の一部として現れる綴りは載せていません」と書いていたが、その `う回` はいま MeCab があれば拾える。**実装が原稿を追い越していた**ので、辞書が環境で変わること（MeCab があれば約 1,900 語増える／無ければ黙るだけで誤検出は出ない）を書き足し、収録しない語の例を `そ上`（俎上とも遡上とも読めて行き先が定まらない）へ差し替えた。
  - `vs doctor` の mecab の説明も「索引機能用」のままだったので「索引の読み推測・交ぜ書き検出用」へ。51 章の導入ツール表も同じ。
- **`vs resize` に `--medium` を足し、ヘルプに具体的な数値を出すようにした**: 3 つのプリセットが原稿の表に並んでいるのに「標準だけはオプションで書けない」状態で、`vs resize --medium` はエラーになっていた。既定と同じ動作だが、書けて通るほうが著者を戸惑わせない。
  - あわせて `--high 高品質（quality=90 / 最大 2048px）` のように**中身をヘルプへ出した**。ヘルプを見ただけでどれを選ぶか決められる。
  - **数値は `WEBP_PRESETS` から組む**（`ResizeCommand.preset_help`）。書き写すとプリセットを動かしたときに黙って食い違う——`--high` の上限は同日 2000 → 2048 へ変えたばかりで、そのとき原稿 2 箇所も追いかけて直した。ヘルプが正典から外れないことをテストで固定した（RS-06）。
- **未使用の `RESIZE_DESC`（74 行）を撤去した**: `high` / `medium` / `low` / `default` 4 つ分のヘルプ文を抱えていたが、定義箇所以外に参照がゼロだった。実際のヘルプは `resize_command.rb` が別に持っており、二重管理になっていた。かつてのサブコマンド形式の名残と見られる。
- **`vs build` / `vs preflight` から `--no-resize` を撤去した**: 実測でメリットがゼロだと確かめた。かつては初回ビルドの SVG ラスタライズ（本書の twemoji 3,714 件で約 56 秒）を飛ばせたが、**同梱の WebP が効いて、`vs new` した新規プロジェクトの初回ビルドでも 0.06 秒で終わる**（実測: 3,746 件すべて `up-to-date` でスキップ）。`FileUtils.cp` は SVG と WebP を同じ mtime でコピーし、判定が `>=` なので必ずスキップ側に落ちる。
  - 一方、指定すると Chromium が SVG 内のパスを Type 3 フォントとして埋め込み、**入稿できない PDF が黙って出来る**（実測: 単章で 22 件混入）。得るものが無く、失うものが大きいオプションだった。
  - 名前も実態と乖離していた——`resize` の語から Type 3 対策を飛ばすことは読み取れない。**`vs resize` が実は WebP 変換だったのと同じ不整合**である。
  - **代替は用意しない。** SVG のラスタライズは Type 3 対策であって、速度と引き換えにする性質のものではない（そもそも 0.06 秒である）。`output.pdf.techbook: false` にすればラスタライズも止まるが、あれは**絵文字の見せ方を決める設定**——カラー絵文字を Twemoji 画像へ差し替えるか、システムの絵文字をそのまま使うか（後者は Twemoji のクレジット表記が要らない代わりに入稿適性を失う）——であって、速度のための設定ではない。
- **`vs build` から画像品質プリセット（`--high` / `--medium` / `--low`）を撤去した**: 画像の WebP 変換をやめたことで行き先を失い、指定しても何も起きない状態になっていた。`vs resize` 側の同名オプションは従来どおり効く（あちらは素材そのものを変換するコマンドなので、品質の指定に意味がある）。`--no-resize` は残るが、いま飛ばすのは **Techbook モードの SVG ラスタライズ**だけである——Chromium が SVG 内のパスを Type 3 フォントとして PDF へ埋め込むのを避ける処理なので、**入稿用 PDF では使ってはならない**。
- **原稿の画像参照を、実在するファイルへ直接解決するようにした**（`image-format-per-target-spec.md` §3.1）: これまで `.png` / `.jpg` の参照を**無条件で `.webp` へ読み替えて**いた。存在しない webp を指す HTML が出来上がり、`vs build` の Step 1 が `vs resize` を呼んで webp を作ることでかろうじて成立していた——つまり **`vs resize` の自動実行は、画質やサイズのためではなく、この読み替えの前提を作るためだった**。
  - **`vs build` は `vs resize` を呼ばなくなった。** 素材はそのまま使い、ターゲットごとの派生をビルドが `.cache/` に作る方式へ移したので、`images/` に `.webp` を並置する必要が消えた。**著者の `images/` に生成物が増えなくなる。**
  - **拡張子の取り違えはこれまでどおり吸収する。** `myawesome.jpg` を置いて `![](myawesome.png)` と書いても拾える。ただし仕組みが変わった——読み替えではなく、書かれた拡張子で探して無ければ拡張子違いを探す形になった。探索順は **png / jpg を webp より先**にする（同名の原本が残っていれば、そちらから派生を作るほうが劣化が一段で済む）。
  - `vs resize` は**素材そのものを軽くしたい著者の明示コマンド**として残る。4K 写真を大量に置くと Git リポジトリが膨らむので、そこを縮めたい場面はある（§3.5 の「素材は機械が黙って書き換えない」の出口）。
- **画像の WebP 変換から「22 倍の崖」を外し、変換を並列で走らせるようにした**: 全プリセットが `-define webp:method=6`（ImageMagick の最高圧縮）を使っていたが、**5 → 6 で時間が 22 倍に跳ねる一方、縮むのはサイズ 1.5% だけ**だった（実測 2026-08-16・本書の扉絵 6 枚で 2.38 秒 → 52.78 秒）。`method` は画質パラメータではなく「圧縮を探す徹底度」で、`quality` を固定している以上、上げても見た目は変わらない（PSNR の差 0.0006 dB）。libwebp / ImageMagick の既定である **4** へ戻した。選んだ理由はコミットにもコメントにも残っておらず、少なくとも 2026-05-21 の時点で入っていた。
  - **遅くなるのは透過を持つ絵に偏る。** 同じ扉絵から透過を外すと 10.89 秒 → 0.35 秒（**31 倍**）で、ピクセル数は無関係だった（800px へ縮めても 7.24 秒）。libwebp がアルファチャンネルの探索まで徹底するためである。**スクリーンショット（透過なし・376 色）では 6 でも 0.15 秒/枚**と症状が出ないので、章扉のようなイラストを持つ本でだけ初回ビルドが遅くなる、気づきにくい形になっていた。
  - **変換を並列化した**（`ResizeCommands.each_in_parallel`）。1 件ごとに magick / rsvg-convert のプロセスを起こす形で Ruby 側は待つだけなので、スレッドで足りる。実測: スクリーンショット 60 枚が **7.83 秒 → 1.57 秒**（5.0 倍）、twemoji の SVG→WebP 60 件が **1.45 秒 → 0.24 秒**（6.0 倍）。並列度は CPU コア数（上限 8・`VIVLIO_IMAGE_CONCURRENCY` で上書き可）。上限を設けるのは magick が 1 プロセスあたり画像 1 枚を丸ごとメモリに載せるためである。
  - 規模で言うと、**同梱 twemoji 3,700 件の初回変換が約 5.6 分 → 約 56 秒**。1 ページに 3〜4 枚のスクリーンショットを入れた 300 ページの本（約 1,000 枚）なら 150 秒 → 25 秒になる。いずれも `.webp` が元画像より新しければ mtime 判定で丸ごとスキップされるので、効くのは初回だけである。
  - **変換に失敗しても 1 件目で打ち切らなくなった。** 並列では順に止められないという事情もあるが、著者にとっても「あと何枚直せばよいか」が一度で分かるほうがよい。
  - SVG → **lossless** WebP 側の `method=6` は据え置いた。可逆圧縮では画質が定義上変わらず、対象の絵文字が小さいため 4 と 6 で時間もサイズも同じだった（0.02 秒・472 バイト）。
- **用語集リンク（†）を章ごとの初出だけに絞った**（`build-mode-parity-spec.md` §4.1）: これまで**全出現に †** が付いていた（実測: `01-computer-journey.md` の `HTML` 5 箇所すべて）。技術書の慣習としては過剰で紙面が騒がしく、加えて単章ビルドとの一致を妨げていた。**実測: 本書で 571 個 → 167 個（70.8% 減）。**
  - **「書籍全体の初出」ではなく「章ごとの初出」を採る。** 全体の初出にすると、単章ビルドではその章が第 1 章でなくても † が付き、プレビューと本番が食い違う。章ごとなら両モードで一致し、第 5 章から読み始めた読者も用語集へ飛べる。
  - **通し番号 `@term_occurrence` は流用できない。** あれは書籍全体の出現回数で、索引アンカー `idx-<hash>-<n>` の一意性を担っているため章でリセットできない。「この章で † を付けた用語」を別に持ち、章の走査の入口で開き直す。
  - 索引タグ（`index-term`）は従来どおり**全出現に付く**。索引のページ番号は出現ごとに要るもので、† とは目的が違う。`<dfn>` / `<span>` の使い分け（初出＝定義箇所）も書籍全体のまま——索引ページで定義箇所を太字にするためのメタデータで、`base.css` が透明なアンカーとして組むため見た目には出ない。
  - 副作用として、Step 8 の**本文 † 重複排除（同一ページ内の 2 回目以降を消す処理）は常に 0 件**になる。撤去は紙面の目視確認で規則が確定してから（`PLANNED.md`）。
- **`vs upgrade` の競合 diff に、どちらが自分のファイルか示す凡例を出すようにした**: `-` / `+` だけを並べており、**「gem 側が ○○ でプロジェクト側が ×× ということが分からない」**（実測 2026-08-12）。標準の unified diff にある `--- a/` `+++ b/` に相当する行が無かった。パスではなく**役割**を書く——著者に必要なのはファイル名ではなく、どちらが残るかである。
  - `凡例  - いまのあなたのファイル  /  + 新しい雛形（適用すると + の側になります）` を diff の先頭へ。記号の意味だけでは「で、どちらが残るのか」が残るので、適用後どうなるかまで書く。
- **`vs upgrade` の競合 diff に色を付けた**: `-` と `+` は記号だけの区別で、流し読みで取り違えやすい。削除行を赤・追加行を緑・ハンク見出しをシアンにした。
  - **色は「diff の生成」ではなく「出力」の関心事**として分けた。`unified_diff` / `render_hunks` は素のテキストを返し続け、色を乗せるのは `print_diff` の中だけ。diff の中身を検査するテスト群が色の有無に影響されない。
  - **記号の判定に `lstrip` を使ってはならない。** CSS のカスタムプロパティを含む文脈行（`    --preface-h3-marker: …`）が、先頭文字を見ると削除行と区別できず赤く塗られる。表示行は自前で組み立てているので、**位置（添字 3）で判定する**。回帰テストあり。
  - `NO_COLOR`（no-color.org の事実上の標準）・`TERM=dumb`・非 tty（パイプ／リダイレクト）では付けない。判定は `$stdout` と `ENV` を引数で受け取るので、テストが global state を差し替えずに済む。
  - 本コードベースで ANSI エスケープを使う初めての箇所。他所で要るまでは upgrade の diff 表示に閉じておく。
- **単章ビルドを全章ビルドのステップ表から導出するようにした（単位の軸を導入）**: 単章と全章でステップ表が 2 つあり、どちらも手で維持されていた。片方への追加がもう片方へ届かないため「単章では ○○ だったが全章では ×× だった」が繰り返し起き、そのたびに個別の補償を足していた（実測: 補償コード 5 箇所・3 ファイル）。原因は実装の不注意ではなく**構造**にある。
  - 相（`:shared` / `:pdf` / `:epub` / `:join`＝どの出力枝が必要とするか）と**直交する第 2 の軸「単位」**をステップ表に足した。`unit: :chapter` は 1 章だけ読めば答えが出るもの、`unit: :whole_book` は全章そろわないと答えが出ないもの（索引ページ・目次・部扉・PDF 結合）。**単章モードは `:chapter` の行を導出して走らせる**ので、モードごとの一覧を手で持たなくなった。
  - **既定の向きが反転した。** これまでは「単章に足し忘れる」が既定だったが、いまは「単章にも走る」が既定で、外すときだけ `SINGLE_MODE_SKIP` に**理由つきで**宣言する。表に無い逸脱はテストが落とす。現在の逸脱は `techbook post-process` の 1 件のみ（Type 3 対策は入稿用の関心事でプレビューには不要・著者判断）。
  - 単章専用だった `build_target_sections_html` を撤去した。中身は `SectionBuilder.preprocess_sections!` ＋ `convert_sections_html!` と等価で、**同じ処理の別実装**が差分の温床になっていた。
  - 実測: 同じ章を単章・全章の両方で組み、章 HTML を突き合わせたところ**差分は章番号（第4章 / 第1章）のみ**。索引タグ 29 件・用語集リンク 2 件・コンテナ div 1 件がいずれも一致する。章番号は catalog 内の位置で決まる全書籍情報なので、仕様どおり一致させない側に分類している。
  - 仕様書: `build-mode-parity-spec.md`

### Fixed

- **記法の説明が索引語として登録されていた**。仕様: `index-html-code-protection-spec.md`
  - 索引の「その他」に **`用語`** という項目が出ていた。早見表のセルに書いた ``[用語|読み]``——**索引記法そのものを読者へ見せている箇所**——が、用語「用語」・読み「読み」として登録されていたためである。読みが漢字なので五十音のどの行にも入らず、「その他」に落ちて紙面に出ていた。
  - 原稿はバッククォートで囲んでいる。ところが**索引スキャンは、表やコンテナが生 HTML へ変換された後に走る**——`:::{.long-table}` の表は前処理で `<table>` になり、セルは `<td><code>[用語|読み]</code></td>` になっている。保護は `Masking::INLINE_CODE_SPAN`（バッククォートを探す）だけだったので、`<code>` になった時点で効かない。エスケープしていた `\|` も `|` へ戻っており、裸のマークアップと区別が付かなかった。
  - `Masking` に `HTML_INLINE_CODE` を足し、スキャナの 2 か所（手動マークアップの処理と、辞書由来のタグ付け）で保護するようにした。**`/<[^>]+>/` より先に置く**——後にすると `<code>` と `</code>` が個別に退避され、中身を囲む対が消えて保護できない。
  - **同じ内容は、表現形式によらず同じ扱いになるべきである。** `` `foo` `` と `<code>foo</code>` は同じ意図で、前者を保護して後者を保護しない理由はない。前処理の変換順という実装の都合が紙面に漏れていた。
  - 実測: 索引語 176 → 173 種・出現 2,399 → 2,379 件。消えたのは `用語`・`!`・`id` の 3 語で、**いずれも辞書に登録が無い**（すべて記法見本からの誤登録）。`vs index:auto` は生の原稿を見るので保護が効いており、ビルド時のスキャンだけがすり抜けていた。

- **`vs index:apply` が、登録済みの語を手書きの定義文ごと消していた**。仕様: `index-apply-rejected-consistency-spec.md`
  - 原稿の加筆に合わせて `index:auto` → `index:apply` を回したら、**登録していない語が 3 件（`CSS`・`EPUB`・`Ruby`）辞書から消えた**。除外リストへ移ったのでもなく、単に無くなった。`apply` は「辞書を更新しました」と成功だけを報告する。
  - **原因は 2 段で、時限式に働いていた。** (1) `[-i]`（索引から外す／用語集には残す）が、**残す語まで除外リストへ書き込む**ため、`terms` と `rejected` の両方に載る矛盾が生まれる。(2) 次回以降の `apply` で「Section 4 同期」が、除外リストにあって `terms` にも居る語を `remove_term!` で消す。**1 回目は何も壊れて見えず、壊れるのは次回**——そのとき著者は「今回いじっていない語」が消えたことに気づけない。
  - 同じファイルの「孤立データ除去」には `[-i]` 後の `g` を守る意図がコメントで明記されており、**その 20 行後で矛盾していた**。
  - `[-i]` / `[-g]` は除外リストへ書かないようにした（フラグを全部失う語だけ送る）。Section 4 同期は向きを反転し、**定義文を持つ `terms` を正として除外リストの側を掃除する**——既に矛盾を抱えたプロジェクトも、次の `apply` で黙って正しい状態へ寄る。
  - **取り返しのつかない削除は黙って行わない。** `[r]` で定義文ごと消すときは、戻し方を添えて警告する。個別の分岐で経路を塞ぐより、この規律のほうが長持ちする（`remove_term!` を呼ぶ経路が将来また増えても気づける）。
  - 回帰テストは**同じレビューで 2 回 `apply` する**形にした（IA-07）。1 回だけ走らせるテストではこの形は捕まらない。修正を戻すと 4 件が落ちることを確認済み。

- **目次・索引・用語集が `lang="ja"` のベタ書きだった**。あわせて **kindlepreviewer の `-locale` を `book.language` から導出する**ようにした（`PLANNED.md` の「後日調査」から削除）
  - この 3 ページは `FrontmatterGenerator` を通らないため `<html lang>` を自前で組んでおり、**英語の本でもここだけ日本語と申告していた**（読み上げ・ハイフネーションが誤る）。判型・テーマ色に続く同じ系統の取り残し。
  - `-locale` は当面 `en` 固定のままだった。**専用のキーは設けない**（`config-key-criteria-guidelines.md` §2.2「重複した入口」）——`book.language` が既にあり、言語らしきキーが 2 つ並ぶと著者はどちらを設定すべきか迷う。しかも `-locale` が効くのは**変換ログと Previewer UI の表示言語**だけで、ログ CSV は一時ディレクトリごと捨てている（著者へはこちらが日本語で要約する）。
  - **`Common.book_language` を唯一の実装にした。** 消費者は `<html lang>`（章・目次・索引・用語集）・EPUB の `dc:language`・vivliostyle の config・`-locale` と散らばっており、「空なら ja」の規則を写すと必ずどれかが取り残される——現に取り残されていた。
  - 地域付き（`ja-JP`）は `lang` / `dc:language` では**そのまま保つ**（BCP 47 なので丸めてはならない）が、`-locale` では主要部分（`ja`）で解決する。Previewer が受け付ける 9 種（`en de fr it es zh ja pt nl`）に無ければ `en` へ落とし、変換自体は止めない。
  - **`ja` でもログ集計は無傷**を実測で確認した。CSV の文面は日本語になるが、`summarize_kpf_logs` が数えるコード（`W14016` 等）は変わらない（`en`・`ja` とも `Error=0 / Warning=1`）。

- **A4 以外の判型で目次・索引・用語集を確認した**（`PLANNED.md` の「後日調査」から削除）
  - 1.0.0-rc.2 でこの 3 ページに `book-settings.css` の link を足したが、**確認は A4 でしか行っていなかった**——本書が A4 なので、直っているかどうかを見た人が誰もいない状態だった。
  - B5（`b5_standard`）と A5（`a5_standard`）で全章を組み、**PDF の全ページが指定判型**であることを実測した（B5: 461 ページすべて 182×257mm / A5: 576 ページすべて 148×210mm）。目次・索引・用語集の紙面も確認し、2 段組・余白・テーマ色の帯とも崩れていない。A5 は版面が最も狭く、索引のページ番号列（`342−344, 418`）が 2 行に折り返しても溢れない。
  - 判型は**全ページの `MediaBox` を測って**判定した。3 ページだけ別の値になる不具合なので、当該ページを目で見るより網羅的で速い。

- **Kindle のインライン数式が読者のフォント拡大に追従しない問題を解消した**（`KNOWN_ISSUES.md` から削除）
  - 2026-07-19 の実装では「単純な式だけ」がテキスト化され、`\sqrt`・`\sum`・積分は SVG 画像のまま残って追従しなかった。いまは `√…`・`∑` ＋上下付き・`∫` ＋上下付きのテキストになる。さらに**素の表記で書かれた式は、テキスト化に失敗しても著者の原文テキストへ落ちる**ので、そちらも追従する。
  - 実測で **SVG に残るものは 0 件**（本書のインライン数式 53 件・`workbook` コーパス 280 種）。「表セル内に置かない」という運用回避も不要になった（表の中も同じ経路でテキスト化される）。
  - 残るのは **LaTeX で書かれた、かつテキストで表しようがない式**だけ（`x^{y^z}` の 2 段入れ子・入れ子分数・`\vec` 等の未対応コマンド）。同じ式を素の表記で書けば追従するので、実害のある縁ではないと判断した。
  - ディスプレイ数式が Kindle で画像のままなのは**意図した設計**（`Σ` や `∫` を独立行に組む意図を保つため。行を占有するので崩れない）。

- **数式の中の日本語が本文と違う書体で組まれていた**。仕様: `plain-math-notation-spec.md` §13.9
  - `SvgFontEmbedder` は図のラベル用に**見出し書体・Bold** を埋める。数式でもそれを使っていたため、`面積 = √(s(s−a)…)` の「面積」だけ周りと違う書体になっていた（本書なら本文が明朝・見出しがゴシック）。数式の中の日本語は**本文の続き**なので、本文書体・Regular を埋めるようにした。
  - 書体の実体を探す規則を役割で選び分けるようにした（同梱は `*Bold.ttf` / `*Regular.ttf`、Google Fonts はファイル名末尾のウェイト数値の最大 / 最小）。キャッシュキーに混ぜる書体名も本文書体へ。
  - **もとは「日本語ラベルを本文へ、右辺を数式へ分割する」案だった**が、着手前に測り直したところ前提が変わっていた——フォント埋め込みと「強いしるしがあれば日本語も数式にする」判定が入った結果、対象と見ていた 16 行の大半は分割しなくても既に組まれていた。残る実害は書体の不揃いだけで、こちらのほうが小さく、中央寄せの数式からラベルだけ外へ出す収まりの悪さも避けられる。

- **`vs lint` が数式を日本語の文として読んでいた**。仕様: `plain-math-notation-spec.md`（作業中に発見）
  - コードスパンは textlint が Code ノードとして飛ばすのに、**`$…$` の数式は素の文として読まれていた**。数式の中の半角括弧が prh に「全角にせよ」と指摘され、`--fix` が当たれば `$(4/3)πr³$` が `$（4/3）πr³$` になって**数式が壊れる**（実測）。素の表記を数式として組む機能が入り、著者が数式を書くほど踏みやすくなって表面化した。
  - `NotationGuard` に数式の退避を足した。**知識の置き場所はここ**——`Masking` は「コード領域解釈の唯一の実装」として前処理・索引・メトリクスから使われており、記法の知識を混ぜると lint と無関係な処理まで意味が変わる（同ファイルの冒頭コメントが定めている方針）。数式の綴りは `MathTransformer` の定数を参照する。lint が守る範囲とビルドが数式として扱う範囲がずれないようにするため。
  - **修正パスでも守る。** 記法ガードは非可逆なので `--fix` では通していないが、数式の退避は可逆なので修正パスにも入れた。地の文の括弧は今までどおり直り、数式の中だけが守られる。
  - 目印は実測で選んだ。インラインコード化は `spaceAroundCode` を誘発し、`X` のような 1 文字は全角と半角の間のスペース規則に触れる。素の `VSMATH0` はどちらも起こさず、`--fix` にも触られない。
  - 行数は保存する（複数行のディスプレイ数式は落とした改行を目印の後ろへ足す）。指摘の行番号がずれない。

- **語の途中に入った強調記法で、交ぜ書きの校正がすり抜けていた**。仕様: `inline-emphasis-word-split-spec.md`
  - **穴は両方向にあった。**「`結**合し**直した`」は前後を制約するガード（`(?<![一-龥])合し`）が `合` の直前に `*` を見てすり抜けて**誤検出**になり、「`だ**円**`」「`あいさつ**文**`」「`か**ぎ括弧**`」は語が記法で分断されて**取りこぼし**になっていた。原因は 1 つ——**判定の入力が、読者の見る文字列になっていなかった**。
  - `Masking.strip_emphasis` を足し、記法を外した文字列で判定して位置を元の行へ戻すようにした。対象は `**` `*` `__` `_` `***` `___` `~~`。
  - **規則はレンダラに合わせた。** 厳密な CommonMark なら `__太字__の語` は強調にならないが、VFM は `<strong>太字</strong>の語` を出す。`node -e` で VFM に実際に食わせて確かめ、アンダースコアの語中制限を **ASCII の英数字だけ**に限った（`snake_case_name` は壊さず、和文は外す）。
  - **`--fix` は語の内側に記法があるとき置換しない。**「`だ**円**`」を `楕円` にすると `**` が黙って消える——著者の書いた記法を lint が落とすのは、交ぜ書きを直さないより悪い。指摘は出るので手で直せる。記法が語の外なら保ったまま置換する（`**だ円**` → `**楕円**`）。
  - あわせて `--fix` を位置ベースへ作り替えた。以前は辞書を順に `gsub` していたので置換後の文字列が次のパターンに当たりえたが、いまは 1 回の走査で得た位置だけを後ろから置くので**収束が構造的に保証される**。
  - **二通りに読める対比（ambiguous-comparison）にも同じ穴があった**。実測で 6 例中 3 例が取りこぼし（`スレッド**と同様**に` は `と同様*` になって当たらない）。こちらは誤検出を生まない——比較の型は構文なので、記号が紛れると当たらなくなるだけ。段落を組み立てる前に**行ごとに**記法を外すことで直した。行の中で外すので、段落内の位置と行番号の対応はそのまま保たれる。
...
- **`<!-- vs-lint-disable-next-line -->` が textlint の指摘に効いていなかった**: 1 行だけ校正を抑止する記法が、**textlint 側にはまったく効かない**状態だった（囲む形の `vs-lint-disable` 〜 `vs-lint-enable` は効く）。原稿 `31-lint.md` が「一行だけ除外したいときは」として案内している手段が、動いていなかったことになる。
  ...
  - 独自ルール（`mazegaki` / `ambiguous-comparison`）側は元から自前で解釈していたため、**同じコメントが片方にしか効かない**ちぐはぐな状態が解消した。
- **テーマ画像の WebP 生成が、存在しないコマンドを呼んでいた**: `ThemeImageResolver#resolve_image_path` は `stylesheets/images/` に PNG / JPG しか無いとき WebP を作ろうとするが、その中身が `system("vs resize:high …")` だった——**`vs resize:high` というサブコマンドは登録されていない**（`root_command.rb` にあるのは `'resize'` の 1 つだけ）。存在しないサブコマンドを渡すと `vs` はヘルプを出して**正常終了する**ため、「WebP を生成します」とログが出たまま 1 枚も生成されていなかった。`ResizeCommands.execute_resize_high` を直接呼ぶよう直した（プロセスの起動も要らなくなる）。
  - **元をたどるとコメントの誤りだった。** `resize_command.rb` の冒頭が「提供コマンド: resize / resize:high / resize:medium / resize:low」と**存在しないものを列挙**しており、実装がその記述を信じた形である。コメントも正した。
  - 素材が既に WebP のプロジェクトでは通らない経路なので、長く露見しなかった。
- **同梱素材の `tsubaki` だけが 2052×2048 で、他の 11 枚（2048×2048）と揃っていなかった**: 正方形のつもりで作られた素材が横に 4px はみ出しており、著者へ配られる `stylesheets/images/bundled/` の原本からそうなっていた。
  - **4px が紙面を動かしていた。** 第 12 章は 12 枚を 2 列の表に並べているが、**テーブルの列幅は列単位に全行共通で決まる**ため、右列にある tsubaki 1 枚の不揃いが p4〜p7 の全行を 0.38pt（0.13mm）ずらしていた。表示解像度も列で 666 / 668ppi と食い違っており、正すと 668ppi に揃う。
  - 差し替え用の 2048×2048 PNG を、他の素材と同じ流儀（ロッシー VP8 ＋別チャンクのアルファ・`quality 90` / `method 4`・**縮小なし**）で WebP へ変換した。旧素材と同じサイズ帯に収まる（657,910 バイト / 旧 655,082 バイト）。**テーマ素材の変換に `vs resize:high` を使ってはならない**——`高精細` プリセットの `max_px` は 2000 なので、2048px の素材が黙って縮む。
  - `image-format-per-target-spec.md` §3.6 の検証中に発見した。
- **ビルドの中核である VFM を、誰も導入せず・誰も検査していなかった**: `vfm` は Markdown → HTML 変換そのものを担うのに、`vs doctor` の検査対象 17 種に入っておらず、`vs upgrade` の更新計画にも並ばず、README の導入手順は**別のパッケージ**を指していた。3 つとも独立に抜けていたため、揃っていない環境では「`vs doctor` がすべて OK と言うのにビルドが白紙になる」状態が成立する（2026-08-16 の点検で発覚。`release-1.0-considerations.md` A-2）。
  - **README が指していたのは無関係なパッケージだった。** `npm install --save-dev @vivliostyle/cli vfm` と書いていたが、npm の `vfm` は "Form Validation for Vue3"（実測: 本開発機にこの `vfm@1.5.0` が実際に入っていた＝この手順を踏んだ痕跡）。VFM は `@vivliostyle/vfm` である。加えて `--save-dev` はローカル導入で `vfm` コマンドが PATH に現れず、実装は**グローバル前提**で `system("vfm …")` と呼ぶ。**どちらの誤りも単独でビルドを壊す。**
  - **壊れ方が静かなのが問題の本体。** 変換はシェルのリダイレクト（`vfm foo.md > foo.html`）なので、コマンドが無くても **0 バイトの .html が残る**。さらに VFM は入力を読めなくても **exit 0 を返し、Node のスタックトレースを標準出力へ吐く**——それがそのまま章の HTML として保存される（実測）。旧実装は `system` の戻り値しか見ていないため後者は成功扱いになり、いずれも「中身のある章」として後続を素通りして、本文が白紙やエラーダンプのまま本が組み上がっていた。
  - **3 箇所を塞いだ。** (1) `Guards::VfmCheck` を新設し、`vs build` / `vs preflight` が本処理へ入る前に 🔴 と正しいパッケージ名で止める（直接ビルド `vs build foo.md` も VFM を通るので同じ Guard を通す）。(2) `vs doctor` の検査対象と `--fix` の導入対象、`vs upgrade` の更新計画（`ToolUpgrader::TOOLS`）へ `@vivliostyle/vfm` を追加。**追加した初回の `vs upgrade --dry-run` で 2.7.0 → 2.7.2 の未適用更新が実際に見つかった。** (3) 変換の成否を終了コードでなく**出力の中身**（VFM の出力は必ず `<!doctype html>` で始まる）で判定し、失敗時は壊れた .html を残さず 🔴 で章名を挙げて報告する。
- **`vs upgrade` が、著者の見ていない変更まで `y` で適用していた**: 実地の観察で、雛形側が 25 行目・著者側が 93 行目という**別の場所**を変えた状態で `vs upgrade` を実行したところ、画面に出たのは 25 行目だけで末尾は `… 残り 56 行` と切れていた。著者は「絵文字の変更なら良い」と判断して `y` を押し、**93 行目の自分の変更が消えた**。バックアップは残るが、失われたことに気づけない。
  - **真因は diff が「ハンク」になっていなかったこと。** 最初の変更から最後の変更までを、間の無変更行ごと 1 つの塊にしていた。切り捨てられた 56 行はほとんどが無変更行で、情報量がほぼ無いものが肝心の削除行を画面外へ押し出していた。つまり「プレビューが短すぎる」のではなく、**diff が無駄に長い**。
  - 標準の unified diff と同じ規則（間の無変更行が `CONTEXT_LINES × 2` を超えたら区切る）でハンクに分けた。ハンク見出しの行番号は著者の現物基準で数える（追加行では進めない）。**実測: 同じ条件の diff が 60 行超 → 16 行になり、両方の変更がプレビューに収まる。**
  - それでも切れる場合に備え、prompt の前に要約（`変更 2 箇所（+3 / -2 行）`）を出し、切り詰め時は `⚠️ [y] は表示していない箇所にも適用されます` を添える。**旧来の「残り 56 行（[d] で全文表示）」は「読まなくてよい続き」と読めてしまう**——`y` の及ぶ範囲についての情報が無かったことが、事故の最後の一押しだった。
  - **隠れているのが文脈行だけのときは警告しない。** 実測で、21 行の diff が上限 20 行を 1 行だけ超え、その 1 行が文脈行という場面を踏んだ。変更はすべて見えているのに警告が出ると狼少年になり、本当に変更が隠れている場面での警告まで軽く扱われる。
  - **中身が同じものを競合として尋ねなくなった。** 分類はすべて lock との比較で行っており、「現物と雛形が別経路で同じ内容に到達した」場合を見ていなかった。lock だけが取り残されている状態で diff は空になり、y/n のどちらを選んでも結果が変わらない問いを著者に投げていた。
  - 仕様書: `upgrade-conflict-review-spec.md`（3-way マージによる自動合流は `upgrade-three-way-merge-spec.md` へ分離）
- **`vs upgrade` が textlint 一式を毎回更新し続けていた**: `textlint-rule-no-doubled-conjunction@3.0.0` と**版を固定して**指定していたのに、更新判定は `npm outdated` にその名前があるかどうかだけを見ており、**固定した版が毎回「古い」と報告されていた**。更新しても固定版が入り直すだけで状態は変わらないため、`vs upgrade` のたびに 9 パッケージの再インストールが走る無限ループになっていた（実測: 約 20 秒/回。2 回連続で実行しても解消しない）。
  - **構造**: 版を固定したパッケージは更新対象から外す（`pinned_names`）。指定から名前を切り出す純粋関数にしたので、スコープ付き（`@scope/pkg@1.2.3`）も正しく扱える。これがないと、将来また誰かが固定したときに同じループが再発する。
  - **個別**: `no-doubled-conjunction` の固定を解除した。固定の理由はコメントにも git 履歴にも残っておらず、実物を比較したところ 3.0.0 → 3.0.1 の変更は**警告位置のずれの修正**（`report()` は現在の文を対象にするのに、位置の計算に前の文のトークンを使っていた）と README の加筆だけで、依存も engines も変わらない。`vs lint` の指摘箇所が正しい位置を指すようになるので、追随するほうが良い。
  - 実測: 3.0.1 導入後、`npm outdated` から textlint 系が消え、表示も他ツールと同じ「変更なし（npm・最新）」に揃った。
- **著者が明示した索引記法 `[用語]` が 1 つも索引に載っていなかった**: 前処理（Step 3）は索引スキャン（Step 4）より先に走るのに、`strip_index_markup!` が**無条件で**括弧を外していた。スキャナが記法を見る前に消えるため、`[404]` `[!DOCTYPE]` `[基本情報技術者]` `[引数|ひきすう]` はいずれも素通しになる。`[HTML]` だけ索引に載って見えたのは明示記法が効いたからではなく、**辞書による自動索引が裸の `HTML` を拾っていた**だけだった（実測: 章内 5 箇所の `HTML` は括弧の有無によらず全部タグ化されており、明示記法は 1 件も寄与していない）。
  - `strip_index_markup!` は**索引・用語集が無効なときだけ**の代替に絞った。有効なときは記法を温存し、スキャナがタグを付ける。
  - あわせて**単章ビルドでも索引タグ付けが走るようにした**（`IndexCommands.tag_chapters_for_build!`）。索引処理は「章ごとのタグ付け」と「索引ページ・ページ番号・主要参照」の 2 つの性質を併せ持つが、後者が全書籍を単位とすることを理由に前者まで単章から外していた。タグ付けだけなら章を絞っても正しく動き、全書籍を単位とする警告を呼ばないので `full_catalog_scope?` ガードが防いでいた誤検知も起きない。これで**単章ビルドがプレビューとして全章と同じ本文を出す**ようになり、前処理側でモードを判定する必要がなくなった（`build-mode-parity-spec.md` §4 の先取り実装）。
  - 述語 `index_enabled?` の実体は `Common` へ置いた。前処理から `IndexCommands` を参照すると `index/*` → `pre_process/*` の逆向き依存と噛み合って読み込み順が壊れるため。
  - 実測: 著者が並べた 14 個（`[HTML]` `[基本情報技術者]` `[コンピュータ]` `[引数|ひきすう]` `[404]` `[500]` `[!]` `[&&]` `[||]` `[|]` `[&]` `[!DOCTYPE]` `[<h1>]` `[</h1>]`）が**すべて索引語になり、索引ページに掲載される**ことを確認。単章ビルドでも同じ結果になる。
- **索引記法 `[<h1>]` が本物の章見出しになり、目次と PDF アウトラインを汚染していた**: 前処理の `strip_index_markup!` は索引スキャナが走らないビルドのために `[用語]` を素のテキストへ落とすが、**`CGI.escapeHTML` を通していなかった**。索引スキャナ側（`process_term`）は必ずエスケープするので、**同じ「索引語を表示する」処理が 2 箇所にあって規則が食い違っていた**形である。`[<h1>]` は生タグとして VFM へ渡り、`<h1>` 要素として組まれ、`[</h1>]` までの文章を丸ごと巻き込んで章見出しに化けていた（実測: 目次に「第2章 は、見出しタグです。」という章が現れ、本来の章題 `index-test` を置き換えていた）。
  - 素のテキスト表現を `IndexMarkup.plain_text` として**綴りの正典へ引き上げた**。読み（`|` 以降）を落とす規則も前処理にしか無かったので、あわせて寄せた。`notation-implementation-guide.md` §2 の「同じ責務のコードを書き始めたら基盤側に API を足すサイン」に従っている——`IndexMarkup` 自体が同じ理由で（3 箇所へコピーされた判定を集約するために）生まれたモジュールで、今回はその引き上げ切れていなかった続きにあたる。
  - **著者が明示した `[404]` `[!DOCTYPE]` が索引語にならない不具合は、これとは別**（前処理が索引スキャナより先に括弧を外している）。仕様書 `index-markup-plain-fallback-spec.md` に分けて記録し、本修正はエスケープのみを扱う。
- **引用（`>`）の中に書いたコードブロックが「地の文」として扱われていた**: `Masking` は Markdown のコード領域解釈の唯一実装で、索引付与・`vs lint`・`vs metrics`・`:::` の開閉ガード・画像パス正規化・相互参照・リンク検証など **20 箇所以上**がここを通る。ところがフェンス判定 `fence_marker` は `line.lstrip` した先頭しか見ないため、`> ```html` は先頭が `>` でフェンスと認識されず、**引用内のコードが丸ごと地の文に落ちていた**。
  - 症状は索引で表に出た。引用で示した AI との対話例のコードに `<span class="index-term">` が注入され、`<p id="message">この<span id="idx-…" class="index-term" data-yomi="てきすと">テキスト</span>が赤くなるはず</p>` と**タグがそのまま紙面に印字された**。分量集計にコードが混ざる・lint がコードを検査する・コード中の `:::` を数える、といった影響も同時に受けていた。
  - `BLOCKQUOTE_PREFIX`（`/\A[ ]{0,3}(?:>[ ]?)+/`）で引用記号を剥がしてから判定する。入れ子引用（`> >`）と `>` 前 3 文字までの字下げを許す。CommonMark はブロック引用内のフェンスドコードブロックを認めており、GitHub 等も描画するので、**正しい Markdown が壊れていた側**である。
  - **剥がすだけでは新たな回帰を生む。** フェンスの中に「引用つきフェンス」の解説（```` ```markdown ```` … `> ``` ` … ```` ``` ````）を書くと、内側の `> ``` ` が外側のフェンスを途中で閉じ、以降の本文がコード扱いになる。`scan_lines` が開始フェンスの引用の深さを覚え、**一致した行だけを閉じフェンスと認める**ことで防いだ。現行の原稿には該当箇所が無い（実測 0 件）が、記法を解説する本なら普通に書かれ得る。
  - 回帰テストを 3 件追加（`masking_test.rb` 18 → 21 件）: 引用内フェンスの除外・入れ子引用の除外・解説フェンスを引用フェンスに閉じさせないこと。
  - **本書（マニュアル）は引用の中にコードフェンスを書いていないため、露見しなかった。** 前書きの 📚（下記）と同じ構図で、自分の原稿にない書き方は検査を素通りする。発見は別プロジェクト（`vs new` 後の実書籍）のビルドから。
...
- **開発者向けガイド（61 章）を書き直した**: 1,095 行・42,489 字 → 444 行・17,212 字。冒頭で「フォークして改造・拡張したい開発者向け」と宣言しながら、実態は全機能のカタログになっていた。**他章と重複する約 400 行**——コマンド別解説 6 節（12・23・31〜33・43〜45・51 章）、`book.yml` / `catalog.yml` の解説（41・23 章）、著者向け記法の再掲（22・24・25・33 章）——を各章へ委ね、「どこに何があるか」と「手を入れるならどこを触るか」に絞った。章内の重複（gem のディレクトリ構造と `vs new` が作る構造が別々に「ディレクトリ構造／構成」として並ぶ、バージョン表が 2 箇所）も解消した。
  - **ビルドパイプラインの記述が実装から取り残されていた。** 原稿は Step 0〜14＋E・K の番号表だったが、実装は番号を撤去して**条件つき 1 枚のステップ表**になっており、`:shared` →（`:pdf` ∥ `:epub`）→ `:join` の 4 相で **PDF 枝と EPUB 枝が並列に走る**。番号を捨てた理由（実行条件で並びが変わるため番号が意味を失い、ログとドキュメントがずれる）と、ラベルがログ・計時・原稿の共通語彙であることを含めて書き直した。ステップを足す手順も「表に 1 行足して相を選ぶ」形で示した。
  - **事実の誤りを修正**: Ruby `4.0.3`（8/7 に対応表から外した版）、廃止済みキー `auto_approve_threshold`、コマンド対応表からの `preflight` / `upgrade` 欠落。
  - **開発者が最初に踏むのに書かれていなかったもの**を追加: `docs/specs/NOTES.md`（症状から引く知見メモの索引）、`scripts/copy_to_scaffold.rb` によるスキャフォールド同期（ルートが正本）、PDF プロバイダの MIT / AGPL 二系統、記法・設定キー・ビルドステップそれぞれの拡張手順。
  - **組版に合わせて表とツリー図を作り直した**。3 列 19 行のコマンド対応表は全セルが折り返して読めていなかったので、規則（`foo` → `samovar/foo_command.rb` ＋ `cli/foo.rb`）を本文で述べ、**例外 6 件だけの表**にした。ワークスペースのツリーは行内の日本語コメントが折り返していたため、構造だけを図に残して説明を表へ分けた。`rake` タスク表・基盤の表も第 1 列が折り返さない粒度へ整えた。
...
- [High] **索引が「出現箇所」ではなく「説明箇所」を指すようになった**（`index-main-reference-spec.md`・Phase 1〜6 完了）: 索引の役割は所在の網羅ではなく**説明の在り処への案内**なのに、全出現を同格のページ番号として並べていたため、頻出語ほどページ番号の壁になって索引としての価値が下がる逆転が起きていた（実測: 「用語集」は 13 章に 100 回出現し、腰を据えて説明しているのは 33 章だけ）。**その語を説明している章での初出**を主要参照として太字＋先頭に置き、副次参照はその後ろに続ける形にした。実測: `用語集 …… **196**, ii, 24, 27, 51, 102`。
  ...
  - **候補どまりにしているのは、自動判定だけでは頻出語を救えないから**。索引語 153 語のうち 120 語（78%）は何らかの見出しに現れるが、「ファイル」は **42 見出し**に当たる。当たりすぎて絞れないのは主要参照がいちばん必要な語のほうで、確定は著者に委ねるほかない（§1.2 実測）。
  - **候補を出す対象は「広く散らばっている語」に限る**（出現章数の比率 0.33 以上）。2〜3 章にしか出ない語なら索引のページ番号がそのまま案内として働く。この集合はビルド時の警告と同一にしてあり、警告を見た著者がレビューファイルを開けばそこに候補が並んでいる。警告は**要約 1 行**に留める——実測 30〜38 語あり、語ごとに出すとログが埋まるうえ、直す場はレビューファイルなのでそこから修正に進めない。
  - **この閾値は `book.yml` のつまみにしなかった。** 仕様は `main_reference_hint_ratio` として出す前提だったが、担う 2 つの役割のうち後者が破綻している。(1) 著者が 0.33 を 0.4 に変えるべきかを判断する材料が無い。(2) 唯一の実需だった「警告を止めたい」は `reference_style: all`（主要参照を丸ごと切る設定）が担うべきで、比率を `1.0` にして黙らせるのは意図が値から読めない。`all` のとき候補提示も促しも止めることで、つまみを 1 本減らしつつ無効化を意味の読める操作にした。
  - 参照の出し方を `index.reference_style`（`main_and_sub` / `main_only` / `all`）と `index.max_sub_references`（既定 8・0 で無制限）で選べる。**主要参照が指定されていない語はどの設定でも間引かない**——「まずここを読めばよい」を示せないまま出現箇所だけ削ると、その語が索引から実質消える。間引いた件数はビルド末尾に必ず出す。
  - **並べ替えは dedup より前でなければならない**。`BacklinkDeduplicator` は同一ページを指すリンクの DOM 上で最初の 1 本を残すため、順序が逆だと主要参照のほうが消える。
- [Medium] **索引の連続するページ番号を範囲表記にまとめた**（`index.page_range_min`・既定 3）: 「105, 106, 107」ではなく「105–107」と出す。ページ番号は CSS の `target-counter` がレンダ時に解決するので Ruby 側からは見えないが、**dedup 段だけが** `PdfPageMapExtractor` の anchor → 通しページ番号を持っているため、連続判定はそこでしかできない。
  - 畳んだ `<dd>` は区切りを**リテラルのテキストノード**で持つ形へ書き換え、`class="resolved"` を立てる。リンクを `<span class="page-range">` でくくると `a:not(:last-child)::after` という兄弟の前提が壊れるため、従来のカンマ生成規則を `dd:not(.resolved)` へ限定した。**EPUB / Kindle は dedup を通らない `html/` の原本から作る**ので、そちらは従来のフォールバックで組まれる。
  - **前付け（ローマ数字）と本文（アラビア数字）はまたがない**（「iv-12」という無意味な範囲ができる）。**主要参照も範囲に含めない**（太字の一点であることに意味がある）。`2` は範囲にしない——「12, 13」のほうが読みやすい。
- 辞書エントリ → 照合 Regexp の解釈を `TermPattern` へ集約した。同じ 3 行が `TermSpread` と `IndexCandidateExtractor` に重複しており、主要参照の候補算出で 3 箇所目になった。剥がし方がずれると「広さは数えるのに候補は出ない」という噛み合わない挙動になる。
...
- [Low] **表記揺れ辞書（prh）の使い方を `31-lint.md` に書いた**: `config/textlint_prh.yml` は「表記揺れ辞書（プロジェクト固有）」としてファイルの所在だけ挙げてあり、**登録の仕方が書かれていなかった**。`expected` / `patterns` の書き方、正規表現、`--fix` での一括修正、書名や引用は allowlist へ回すこと（著者の表記ではないので直してはいけない）を追記した。
  - あわせて `CSS 組版` を辞書へ登録した（原稿の実態は「CSS 組版」13 箇所・「CSS組版」4 箇所）。
- [Medium] **主要参照をフラグ欄 `[igm33]` で書けるようにした**（`index-main-reference-section-spec.md` Phase 4）: 語ごとに子行 `- 主要参照: 33` を足す形だったため、110 語の索引で **110 行増えて**一覧性が落ちていた。フラグの後ろに `m` と章番号を足す形にして、行を増やさずに書けるようにした（複数章は `[igm21,22]`）。
  - **`m?` は機械の推測、`m` は著者が確定した指定。** 自分で決めたものと機械の下書きが見分けられないと、レビューが「全部確認し直す」作業になる。実データで推測 66 語・確定 44 語が正しく分かれた。
  - 章名や節指定のような長い値は**子行に譲る**（`[igm21#Markdown とは]` は読めない）。子行はフラグ欄より優先する——後から書き足した細かい指定が勝つ。
  - **前提としてフラグの綴りを `TermLine` へ集約した。** フラグを読む正規表現が **9 箇所**に散っており（`parse_index_approved` `parse_unreject` `parse_yomi_changes` `term_blocks` …）、`m33` を足すと `(?:i|ig|gi|x)` が軒並みマッチしなくなる状態だった。以後フラグを増やすときも 1 箇所で済む。
  - `m` の分割は**最初の `m`** で行う。フラグ本体に `m` は使われていないので、値に `m` を含む章名（`21-markdown-tutorial`）が来ても壊れない。
- [Medium] **主要参照の候補が全語に出るようになり、推測が 45% から 65% になった**（`index-main-reference-section-spec.md` Phase 3）: 推測の指標を「見出しヒット数」から **`h1×3 + h2×2 + h3×1 + 章内の出現回数`** へ変えた。著者が手で決めた 44 語を正解として測った実測値である。
  - 旧実装が弱かったのは**見出しに出ない語に候補を出せない**ためで、44 語中 14 語が該当した。出現回数を足すと全語に候補を出せる——「予め設定しておく」にはこれが前提になる。実測: 未指定 66 語すべてに候補が出た。
  - **定義パターンの加点はしない。** 重み 0 / 3 / 8 のいずれでも 65% で変わらなかった。効かない要素を残すと、後から触る人が「効くはず」と誤解する。
  - 候補を出す対象を「広く散らばっている語」から**未指定の語すべて**へ広げた。フラグ欄で書けるようになり行数が増えないため、絞る理由が消えたため。除くのは「索引に出るページ番号が 1 つしかない語」だけ（実測 1 語）。
- [Medium] **主要参照で節を名指しできるようにした**（`index-main-reference-section-spec.md` Phase 2）: 自動で選ばれた節が意図と違うとき、`main: 21#Markdown とは` と書いて指定できる。
  - **文言で書く。** 節番号（`4-1`）は組版時に振られるもので原稿に無い。**レベルを問わない**（h2〜h6。実測で h4 は 56 個、h5 は 3 個ある）。**語を含まない見出しも指せる**——自動推測は語を含む見出しを探すが、手動指定は文言の一致だけを見るため
  - **空白を無視して比べる。** 「CSS 組版」と「CSS組版」のような揺れを吸収する。強調・インラインコード・ルビ・索引タグも均してから照合する
  - 同じ文言の見出しが複数あるときは `23#章の削除（vs delete）#基本的な使い方` と親を添えて絞る。**中間は省略できる**（祖先のいずれかに一致すればよい）。実測で章内の見出し重複は 655 個中 7 個（1%）なので、絞りが要るのは稀
  - **曖昧なら黙って最初を採らない**。候補を親つきで並べて promote する——意図と違えば索引が静かに間違った場所を指す
  - **見出しが見つからなければ知らせる。** 見出しの文言は推敲で変わる（「主な用途」→「さまざまな用途」）し、節ごと消えることもある。その章の見出し一覧と「近いもの」を添えて直し方を示す。**値は保存し、章単位の指定として先へ進む**——推敲の途中で組版が止まらないように
- [High] **索引候補の 3 分の 1 が記法や文の断片だったのを直した**: 定義パターン（「〜とは」「〜について」の直前を取る）が `.{2,20}` で**文の途中から機械的に 20 文字を切り出して**いたため、`た章のみです。 は本の**目次（章立て）` `| | ビルド対象の章リスト（章の順序` のような断片が候補になっていた。実測（本書 27 章）で候補 **4,053 件のうち 1,355 件（33%）**がこの類で、スコア分布と順位を歪め、`candidate_pool` を上げても本物の候補が出てこない原因になっていた。
  - 切り出しに使う文字を `TERM_CHAR`（空白・句読点・括弧・Markdown 記法を除く）へ限定し、根本で断片が生まれないようにした。`/` `.` `-` は許す——`PDF/X-1a` `Terminal.app` `10-20行目` を巻き込まないため。
  - 名詞連続から混ざる残骸には `valid_term?` の `JUNK_TERM_PATTERN` で二重に蓋をした。
  - 実測: 候補 **4,053 → 2,698 件**。記法・文断片は **0 件**になり、`PDF/X-1a` `CSS組版` などの正当な語は残っている。
- [Medium] **MeCab が 1 語と認識する複合語が候補から丸ごと漏れていたのを直した**: 候補抽出は「名詞が 2 つ以上連続したもの」だけを拾っていたため、**MeCab の辞書に 1 語として載っている専門用語が漏れる**。「特殊相対性理論」「一般相対性理論」は（「特殊」＋「相対性理論」で 2 語なので）拾えるのに、**「相対性理論」単体は拾えない**という非対称が起きていた。日本語を含む 5 文字以上の単独名詞を拾うようにした（実測でスコア 338.1 の推奨候補として出る）。
  - 英字のみの単独名詞は拾わない。実測 153 件中 136 件が `section` `table` `column` のような CSS クラス名・記法由来だった。「・」でつないだ並び（「ヘッダー・フッター・ノンブル」）も語ではなく列挙なので落とす。
- [Medium] **見直し候補（登録済みだが順位が目安語数の外に出た語）を一覧する手段が無かった**: `vs index:plan` / `vs index:auto` は「見直し候補 61 件・例: カラー / レベル / …他 56 語」と**件数と例だけ**を告げ、レビューファイルにも出していなかった。**外すべき語を見つける場がどこにも無い**状態で、これは実装漏れ。`## 1. 登録済み用語の確認` に「### 見直し候補」サブセクションを設けて全件出す。
  - 一般語と違って**既定は現状維持**（`[i]` のまま）。順位が低いことは「索引に要らない」を意味しない——章を絞った本では専門語でも出現が少ない。判断材料だけ示して決めるのは著者に委ねる。
  - 一般語と重なる語は一般語を優先し、**同じ語を 2 つの枠に出さない**。
- [Medium] **同じ警告が 3 回出ていたのをやめた**: 索引の主要参照の警告が、逐次ログの本文・build の「ほか警告 1 件」・preflight の「章に紐付かない指摘」と、1 件のまま 3 回顔を出していた。**本文を出すのは 1 回だけ**にする。
  - build の「ほか警告 N 件（詳細は `vs preflight` で確認できます）」を**撤去**した（`preflight-chapter-summary-spec.md` §2.4）。「リンク・画像以外の警告が埋もれないように」という趣旨だったが、それらの発生源も逐次ログで本文を出しているため、実際には常に再掲になっていた。しかも案内先の preflight でも件数しか出ないので、辿っても同じ件数に行き着くだけだった。
  - preflight の章別サマリーから「章に紐付かない指摘 🟡 警告 1 件」の行も**撤去**した。件数だけの行は、**何について警告されていて、どう直せばよいかを著者に伝えられない**。この枠に入る指摘（Guard 警告・索引まわり）はすべて発生源が本文で言い切っている（`🟡 主要参照が未指定の索引語が 55 語あります` ＋ `vs index:auto を実行すると…`）ので、表に件数を足すと同じ 1 件を二度語ることになる。章別サマリーは章の話に徹する。
  - `IssueRegistry` は集計専用に戻した。総括（`⚠️ Preflight 完了: 警告 N 件`）にだけ効かせ、**メッセージも件数も再表示しない**——「画面に 🟡 が出ているのに『良好』と総括しない」ための集計であって、再掲のためではない。
- [Medium] **`⚠️` `ℹ️` が端末で半角幅に描かれ桁が揃わなかったのを直した**: この 2 つだけが**単一の文字ではなく 2 コードポイント**（`U+26A0` ＋ `U+FE0F`、`U+2139` ＋ `U+FE0F`）で、`⚠` `ℹ` という文字記号（East Asian Width = N）を異体字セレクタ VS16 で絵文字化しているだけだった。**幅の数え方は規定されておらず端末実装に委ねられる**ため、絵文字の見た目で描きながら幅は 1 と数える端末がある。`⚠️` → **`❗`**（U+2757）、`ℹ️` → **`💡`**（U+1F4A1）へ置き換えた。どちらも単一コードポイントで EAW=W。
  - 置換先はいずれも**既にこのプロジェクトで同じ意味合いで使われていた**ものを選んだ（`❗` は `doctor.rb` の「まだ見つからないツールがあります」、`💡` は既存 2 箇所）。語彙を増やしていない。
  - 最終行の 3 段階が `✅` `❗` `❌` になり、**色が読めなくても形で区別できる**（チェック / 感嘆符 / バツ）。
  - コードベース全体を洗って該当はこの 2 つだけと確認した。検査スクリプトと使ってよい絵文字の条件を `terminal-output-notes.md` に残した——**新しいアイコンを足すときは必ず確認すること**。
- [Low] **`vs preflight` の進行表示が「ビルド中」と名乗っていたのを直した**: 原稿を検査するだけで何も組まないのに「ビルド中: …」と出ていたため、出力物ができると誤解させていた。preflight では「点検中: …」と出す。
- [Medium] **インラインコードの解釈違いで索引語が取りこぼされていたのを直した**: 索引スキャナだけが `Masking`（コード領域解釈の唯一の実装）を迂回し、独自パターン `` /`[^`]+`/ `` でインラインコードを判定していた。これは**単一バッククォート対しか見ない**ため 2 つの誤りを同時に起こす。(1) `` ``foo`bar[用語]`` `` のような N 連バッククォート対の**内側が露出**し、リテラル表示が目的のコードの中身が索引語になる。(2) 逆に `` `` `` を「バッククォート・空白・バッククォート」と食べてしまい、**次のバッククォートまでの地の文全体をコードとみなす**（過剰保護）。解釈を `Masking::INLINE_CODE_SPAN` へ一本化した。
  - **弱い箇所は 2 つあった**。`process_line`（記法 `[用語]` の検出）だけでなく `protect_untouchable_regions!` にも同じパターンがあり、後者は辞書由来の自動タグ付けと用語集の †リンク付与の**両方**から呼ばれる。片方だけ直しても半分しか塞がらない。
  - **実害は顕在だった**。着手時は「該当ケースは原稿に無く潜在的」と見ていたが、実原稿 27 章で突き合わせると 4 章 7 件が現に狂っていた。過剰保護の方向を数えていなかったのが見落としの原因（実例: `31-lint.md` の 1 行で「インライン」「コード」「スペース」の 3 語が索引から消え、本物のコード `` `true` `` は保護されていなかった）。
  - あわせて `process_line` の**手書き退避（ローカル Hash ＋ 独自トークン `VSCODE<n>`）を撤去**し、同ファイルの `LineMask` へ寄せた。素直に置き換えると索引語が 1 件も検出されなくなる——`Regexp.last_match` はフレームローカルなので、`gsub` を `LineMask#substitute!` の内側で呼ぶと `$~` はそちらのフレームに立ち、呼び出し側で定義したブロックからは常に nil に見える（既存 2 呼び出し元はブロック引数しか使わないため露呈していなかった）。`LineMask#substitute_match!` を足して **`MatchData` を明示的にブロックへ手渡す**形にすると制約が消える。詳細 → `index-code-protection-unification-spec.md`
- [Medium] **EPUB / Kindle で脚注が本文と見分けられなかったのを直した**: リフローには「ページ下部」が無いので、PDF が脚注領域へ降ろしている `aside` は参照した段落の直後にそのまま残る。ところが `.page-footnote` の `display: inline` がそのまま効き、**番号も区切りも無い、本文と地続きの文字列**として出ていた（実機で確認）。番号を実テキストで注入し、左罫＋一回り小さい文字の注記ブロックとして本文から切り分けた。番号を `::before` でなく実体にするのは Kindle(KFX) が擬似要素を解さないためで、クリーン EPUB も同じ経路に揃えてフレーバ分岐を増やしていない（`vs-adm-label` と同じ方式）。
- [Medium] **コード行番号の折返し位置が 1 行目と揃わなかったのを直した（PDF / EPUB / Kindle 共通）**: 真因は `base.css` の `*, *::before, *::after { box-sizing: border-box }`。番号欄の `width` / `inline-size` は **padding を含んだ総幅**なのに、content-box のつもりで「数字幅 ＋ 余白」と足し算していたため、総幅が `padding-left` より狭くなっていた（PDF は 0.8em・クリーン EPUB は 0.6em ぶん）。折返し行が右にずれて見えるが、実際にずれていたのは**1 行目のほう**。総幅を `padding-left` と同値（2.4em）に揃えた。F 案が最初から抱えていた欠陥で、PDF へ展開して 2 ターゲット同時に露見した。
  - Kindle は `::before` を解さず番号を実テキストで出すため、桁数を**ブロックごとの最大値から 3 桁固定へ**変更した。ブロックごとに桁数が変わると番号欄の実幅が変わり、CSS の固定値と対応しない。
- [Medium] **PDF のコード行番号の欄が広すぎたのと、縦罫が折返し行で途切れていたのを直した**: `pre.line-numbers { padding-left: 3.8em }` は絶対配置ガター時代の名残で、行ブロックが自前で番号欄を持つ今は**余白が二重**になりコードの開始位置が版面の内側へ大きく寄っていた。撤去して番号欄は等幅 3 桁ぶん（2.4em）に。番号右の縦罫は `::before` にしか付かない＝各論理行の 1 行目にしか出ないため折返しで途切れる。EPUB / Kindle は元から罫なしなので PDF も揃えて撤去した。
- [Medium] **脚注が参照と別のページに組まれることがあったのを直した**: 参照は p.83 なのに脚注本体が p.84 の下部、という状態が起きていた（ページ下部には空きがあり「一杯だから」ではない）。`aside` は `<p>` の中に置けないため段落の直後に置かれており、`float: footnote` の錨が「参照のある行」ではなく**段落の末尾**になっていたのが原因。段落がページ末尾に掛かると、1 件目を降ろした時点で残りが尽き 2 件目以降が次ページへ送られる。参照直後に既にある `span#fnN`（同内容・画面では非表示）を脚注フロートにして、錨を参照位置へ移した。入りきらないときは行ごと次ページへ送られるので、参照と脚注が離れない。
  - **インライン脚注 `^[…]` 固有の問題ではなかった。** 参照形式とインライン形式の順序を入れ替えて実測すると、送られるのは常に**後ろの 1 件**だった（最終 HTML では両者は同型の `aside` になり、由来の区別は残らない）。
  - 参照番号は既存の `<a class="footnote-ref"><sup>N</sup></a>` が描くので、`float: footnote` に自動付与される `::footnote-call` / `::footnote-marker` を消す。消さないと番号が二重になる（実測「11」「22」）。
  - `aside` は DOM に残したまま隠す（後処理の sideimage 変換・出現順の再番号付けが `aside` を辿るため）。**一律には隠せない**——`append_unused_footnotes_to_body!` が足す sideimage 由来の `aside` には対応する span が無く、隠すと脚注が消える。span を伴う経路にだけ `data-footnote-anchored` を付けて区別する。詳細 → `footnote-same-page-anchoring-spec.md`
- [Medium] **PDF のコード行番号が折返しでずれていたのを直した**: 論理行が折り返すと続きの表示行が次の番号を貰い、以降が 1 つずつ繰り上がって最後の論理行の番号が消えていた（原稿 3 行のコードで `:::` に番号が付かない）。`epub-code-line-numbers-spec.md` §0 は「PDF はページ幅固定のため崩れない・変更対象外」としていたが、**この前提が誤り**で、`code.css` が全 `pre` に `white-space: pre-wrap` を掛けているため PDF でも長行は折り返す。クリーン EPUB で解決済みだった症状が PDF に残っていた。
  - EPUB と同じ F 案（1 論理行 = 1 ブロック要素＋ぶら下げインデント）を PDF にも敷いた。PDF では `pre` を残して `code` の中身だけを `span.vs-code-line` へ組み直す（枠・背景・フォントの既存 CSS を活かすため）。変換は `pdf/` のコピーにだけ掛けるので、`html/` の原本を読む EPUB 経路は不変。
  - トークン `span` の行跨ぎを保つ分割ロジックは EPUB 側に既にあったので、`cli/code_line_blocks.rb` へ引き上げて共有した（2 度目の実装を作らない）。
  - **番号欄の総幅は `padding-left` と厳密に一致させること**。0.4em ずれただけで折返し行の頭が 1 行目の本文開始位置と揃わない（実測）。詳細 → `pdf-code-line-numbers-spec.md`
- [Medium] **インライン脚注 `^[短い補足]` が脚注にならなかったのを直した**: `^` が本文にそのまま出てブラケットが消え、脚注本体が本文へ流れ込んでいた（脚注末尾の「。」と本文の「。」が重なる）。VFM はインライン脚注に完全対応しており、壊していたのは前処理だった。`[…]` を索引マークアップとみなす実装が **3 箇所に重複し、どれも `^[` を除外していなかった**のが真因。`^` はブラケットの**外側**にあるため、参照脚注 `[^id]` を落とすための既存条件（中身が `^` で始まるか）には原理的に引っかからない。
  - 除外は**パターン側**でしか書けない。`skip_term?` はブラケットの中身しか受け取らず、`^[補足]` と `[補足]` が同一入力になるため。既にリンク記法 `[text](url)` を除外している `(?!\()` と対になる `(?<!\^)` を足した——**新しい判定の追加ではなく、既存の除外語彙に欠けていた 1 項目の補完**である。
  - 綴りの定義を `IndexMarkup`（`cli/index_markup.rb`）へ集約し、前処理・索引スキャナ・辞書登録の 3 箇所が参照する形にした。`Masking` には入れない（コード領域解釈の唯一の実装に記法の知識を混ぜない、という `NotationGuard` 確立の型に揃える）。`notation-implementation-guide.md` §2 の再実装禁止リストにも登録し、次に `[…]` を使う記法が来たときの寄せ先を明示した。
  - **辞書登録だけはパターンが 2 本必要**という落とし穴があった。読み付き `[用語|読み]` と読みなし `[用語]` を順に当てる方式で、読みなし側が `|` を除外していることが二重登録を防いでいる。汎用パターンへ一本化すると同じ語が両方で拾われる。共有すべきは**除外条件であってパターンの形ではない**。
  - 副次的に、`vs index:auto` がインライン脚注の本文を索引語として**辞書へ恒久登録する**問題も塞いだ（登録されると本文中の同じ文字列が全章で自動タグ付けされ、被害が残る）。現時点の原稿では `^[…]` がすべてコードフェンス内にあり辞書は汚れていなかったため、実害の出る前の予防となる。既存原稿の出力は変わらない。
- [Medium] **見出しを語の途中で折らないようにした**（`HeadingSegmenter` を新設）: 末尾 1 文字の泣き別れは前段の対応で消えたが、「コマンドラインオプ／ション」のように**語の途中で割れる**問題は残っていた。CSS の `word-break: auto-phrase` は文節境界で割るため、複合語（1 文節）の内部には分割点が無く、組版エンジンは文字間の任意の位置で折る。MeCab で語の境界を取り、**各語を `white-space: nowrap` で括る**ことで、折返しが語と語の境目でしか起きないようにした。実測: 「コマンドラインオプ／ション」→**「コマンドライン／オプション」**。
  - 処理は**テキストノード単位**で行う。見出しには索引語（`<span class="index-term">`）や用語集リンクが埋まっており、見出し全体を分割して組み直すと構造が壊れるため（要素の境目はもともと折返し候補なので分割の質は落ちない）。
  - 助詞・約物・閉じ括弧は直前の語へ、開き括弧は次の語へ接着する（「章の／管理」より「章の管理」、行末に「（」を残さない）。
  - EPUB / Kindle の合成画像も同じ語境界で行を詰める（`wrap_text_by_width` が語単位のパッキングへ）。1 語で幅を超える場合だけ従来の表示幅分割へ落ちる。
  - **MeCab が無い環境では末尾 2 文字だけを守る従来の挙動へ縮退する**。組版が環境で変わるのは避けたい性質だが、変わるのは折返し位置の自然さだけで、内容・寸法・ページ数には影響しない。MeCab は索引の読み推定で既に任意依存として使っており、可用性判定は `YomiInferrer` に集約している。
- [Medium] **EPUB / Kindle の章扉で章番号と章題が重なるのを直した**: 章題を画像高さの一定比に centering していたため、長い章題（「章の管理（create / delete / rename / renumber）」で 4 行）が上へ伸びて章番号に重なっていた（`epub_h1_kasanari.png` 実測）。**章番号 → 章題 → リードを上から順に積む**方式へ変え（PDF の章扉と同じ流れ）、行数がいくつでも重ならず下へ伸びるだけにした。リードの左寄りも解消。
- [Medium] **EPUB / Kindle で回転テーブルがページ境界で断ち切られるのを直した**: 回転テーブルの箱は PDF の版面高さ（`--rotate-table-height`・mm 固定）を前提に組まれているが、リフローのページ高さはリーダー任せなので、箱がページより高いと途中で切れて次ページにも続きが出ていた（`epub_rotate_table1/2.png` 実測）。EPUB では 1 画面に収まる高さ（`88vh`）へ置き換え、分割禁止と前後改ページを legacy 併記（`page-break-*`）で指示する。**実機での確認が必要**（`vh` の解釈はリーダー依存）。
- [Medium] **囲みブロック（コラム・ヒント・メモ・注記・注意）がページを跨いで分割されないようにした**: ラベル（`::before`）は `top: -6mm` の絶対配置で箱の上辺に貼り付くため、分割されると 2 枚目に**ラベルの無い枠だけ**が残り「何の囲みか分からない箱」になっていた（`pdf_memo.png` 実測）。5 種すべてに `break-inside: avoid` を付け、入りきらないときは次ページへ送る。1 ページに収まらない長さの囲みは avoid を守れず分割されるが、それは分割せざるを得ないケース。
- [Medium] **見出し末尾の 1 文字が次の行へ落ちる「泣き別れ」を防いだ**: 日本語の見出しは 1 語で分割点を持たないことが多く（「コマンドラインオプション」）、`word-break: auto-phrase` でも文節境界が無いため文字間で折れて末尾 1 文字が独り残っていた（`pdf_h2_nakiwakare.png` 実測）。`HeadingProcessor` が章題・節題の**末尾 2 文字を `white-space: nowrap` の span で括る**ことで、その 2 文字が分割できなくなり折返しが手前へ移る。**WORD JOINER（U+2060）を挟む方法は Vivliostyle が無視した**（HTML には入るが組版結果が変わらない）——自前の行分割を持つエンジンには制御文字より CSS で表現する方が確実。見出し末尾は素のテキストとは限らず、索引語（`<span class="index-term">`）や用語集リンクで終わることが多いため、**最も深い末尾のテキストノードまで降りて**括る（ここを見落とすと「オプション」のような索引語で終わる見出しだけ効かない）。h3 は `block-size` 固定の小見出しなので対象外。

- [Low] **漢字の配当学年を取り出す `KanjiLevels.grade_of` を追加した**（`furigana-level-spec.md` §3.2）: 同梱データは学年（1〜6）を持っているのに `level_of` がすべて `:kyoiku`（L0）へ潰しており、「小4向けの本なら小5以上にルビ」という**学年基準の判定ができなかった**。教育漢字なら学年を、それ以外は `nil` を返す入口を足した。`level_of` の戻り値は変えていないため `vs metrics` の集計・表示は不変。両者の一致は構造的に保証してある——`level_of` が `:kyoiku` を返す条件そのものを `grade_of` の非 nil に委ねたので、「`grade_of` が 1〜6 を返す字数の合計 ＝ `level_of` が `:kyoiku` を返す字数」がずれない。将来の `vs furigana`（半自動ルビ付与）が使う。
- [High] **`@` ディレクティブを 5 記法（7 種）追加した**（`at-directive-tier1-spec.md`）: 廃止で `@vspace` だけになっていた `@` 記法に、印刷本で必要になる 5 つを加えた。
  - **`@pageref:id`** — 参照先の**ページ番号つきリンク**。「**「インストール」（p.200）**」のように組まれる。あわせて**見出しにもラベルを付けられる**ようにした（`## インストール @install`。行末の `@id` は紙面・目次・柱に出ない）。ページ番号は CSS の `target-counter()` が組版時に注入するため PDF 解析は不要で、原稿を書き換えてページがずれても追従する。**EPUB / Kindle ではページ番号だけが自然に落ちてタイトルのリンクとして残る**——リーダーは `target-counter()` を解釈できず `content` 宣言ごと破棄し、Kindle は `::after` 自体を無視するため、劣化に Ruby コードを 1 行も要さない（CSS カスケードで構造的に成立する）。アンカーは見出しの**内側**に置く（前の行に置くと `break-before: page` でアンカーだけが前ページ末尾に落ちてページ番号が 1 ずれる）。
  - **`@pagebreak` / `:recto` / `:verso`** — 改ページと、部扉・章扉で使う**右起こし（奇数ページ開始）／左起こし**。引数なしは `---` と等価。EPUB / Kindle には左右ページの概念がないため単純改ページへ劣化する。引数の書き間違い（`@pagebreak:left` 等）はビルド前チェックが 🔴 で位置つきに指摘する。
  - **`@version` / `@title` / `@today`** — `book.yml` の `project.version` / `book.main_title` とビルド実行日を本文へ差し込む。`@titlepage` のような続き文字には反応せず、コードブロック内も素通しする。
  - **`@qr:URL`** — その場に QR コードの SVG（既定 18mm 角）を生成して挿入する。同一 URL は全章で 1 ファイルを共有し、再ビルドでも作り直さない。生成には pure Ruby / MIT の `rqrcode` を使う（ネイティブ拡張なし）。
  - **`@hspace:N`** — `@vspace` の水平版（字下げ）。単位を省略したときの既定は **em**（水平方向は文字数の感覚が自然なため。`@vspace` の既定 mm とは意図的に非対称）。
- [Medium] **節見出しの直前に書いた `---` / `@pagebreak` を自動で無効化し、空白ページが挟まらないようにした**（`page-break-control-spec.md` §2.1）: 節見出し（`##`）は既定でページの先頭から始まるため、「節の前は区切りを書く」という自然な習慣で書くと `---` の改ページと重なり、**空白ページが 1 枚挟まる**事故が頻発していた。後処理に `PageBreakNormalizer` を新設し、改ページマーカーの直後が h2 のときだけマーカーを無効化する。VFM は h2 ごとに `<section>` を作り、image スタイルでは h2 が `<article>` に包まれるため、隣接判定は兄弟比較では足りない——「文書順で次の内容要素」を親を遡り・ラッパーを降りて探す。`@pagebreak:recto` / `:verso` の直後の h2 は**マーカー側を勝たせる**（h2 の改ページを打ち消して合流させる）。h2 以外（段落・`h3`・図）が続く正当な改ページは触らない。**意図的な空白ページは `---` を 2 つ重ねれば従来どおり作れる**（h2 直前の 1 つだけが正規化される）。`page.section_page_break: false`（節で改ページしない設定）のときは冗長性が生じないため何もしない。正規化の発生は `--log` で確認できる。
- [Medium] **`vs metrics` の章別リストに「表現が単調」「やや難解」の 🟡 警告を出すようにした**（`metrics-quality-warnings-spec.md`）: これまで章別リストの警告は**分量だけ**で、語彙多様度（MATTR）と読解難度（建石式 RS）は詳細分析の評価文にしか現れなかったため、「どの章が単調か／難解か」を知るには章ごとの詳細を目視で追う必要があった。分量警告と同じ一覧性を与え、`第22章 … 🟡 やや長い・表現が単調` のように 1 つの 🟡 へ `・` で連結して表示する。**専用のしきい値は設けず、詳細分析が「表現が単調」「やや難解」と評価するのと完全に同一の条件**で発火させる（MATTR ≤ 0.5 のバンド定数 `Formatter::MATTR_MONOTONOUS_MAX` を表示と警告で共有し、難解側は既存の `metrics.readability.standard` を参照する）。二重のしきい値は必ず乖離事故を生むため、新しい設定キーは 1 つも増やしていない。誤検知対策として `metrics.exclude_chapters` の章に加え、**統計的に当てにならない章では判定しない**——総語数が `metrics.mattr_window`（既定 100）未満なら単調を、10 文未満なら難解を出さない。`vs metrics --warn` は品質警告だけの章も拾うようになり、「単調・難解と評価された章だけを一覧して推敲対象を決める」使い方ができる。`--json` / `--yaml` の各章 `stats` には `warnings` 配列を追加した（警告なし・除外章は空配列）。警告は**キャッシュに保存せず表示時に毎回導出する**ため、しきい値やラベル文言を変えても再解析なしで即反映され、キャッシュの `schema_version` にも影響しない。
- [Medium] **節（`##`）でページを改めるかを `page.section_page_break` で選べるようにした**（`page-break-control-spec.md` §2.2）: これまで節見出しは常にページの先頭から始まり、変えるには CSS を直接書き換えるしかなかった。`false` にすると節が本文の流れの中に続けて組まれ、ページ数を抑えたい薄い本・配布資料に対応できる（既定は `true` ＝現行挙動）。実装は `BookSettingsCss` が **`false` のときだけ**打ち消し規則を `book-settings.css` へ出す方式で、`true` では何も出さずテーマ CSS がそのまま生きる。打ち消しは元ルールのセレクタを**そのまま複製**する（後段読込なので同特異度なら後勝ちできるが、セレクタがずれると特異度負けして黙って効かなくなる）ため、セレクタが `stylesheets/` に実在することを回帰テストで固定した。PDF・EPUB・Kindle の全ターゲットに効く（Kindle KFX 向けの legacy `page-break-before` も併せて打ち消す）。実測では 1 章（節 4 つ）のビルドが 17 → 13 ページになった。`theme.style` は `image` / `simple` のどちらでも使える。
- [Medium] **ビルド中の進捗をスピナーで表示するようにした**（`command-feedback-spinner-spec.md` §2）: `vs build` は数十秒〜数分かかるが、既定ログレベルではステップ間の出力がなく**フリーズと区別が付かなかった**。端末実行時に `⠹ ビルド中: convert … (5/14)` のようにステップ名と進捗（現在/総数）を表示する。ステップ名はログ・計時と同じ `step.label` を使うため語彙が一致する。**表示条件は「TTY かつ既定ログレベル かつ `VS_DEBUG`・`VS_NO_SPINNER` が未設定」の全てを満たすときだけ**で、パイプ・リダイレクト・CI・`--log` 指定時は自動的に出さない（逐次ログやパイプ出力と干渉させないため）。`VS_NO_SPINNER=1` で明示的に無効化できる。ログが割り込む場合はスピナー行を消去してから出力するため、行が重ならない（`Common#emit` を全ログ出力の唯一の出口として新設し、そこに消去フックを 1 箇所だけ置いた）。例外時も `ensure` で行を消すので、🔴 メッセージがスピナーの残骸で汚れない。外部 gem は追加していない（スレッド＋`\r` 書き換えの自作・約 110 行）。
- [Medium] **副作用が目的のコマンドが、実行結果を既定ログレベルでも 1 行で報告するようにした**（`command-feedback-spinner-spec.md` §1）: `vs clean` などは処理内容が info レベルの `log_action` / `log_info` だけだったため、**既定では何も表示されずに終わり「効いたのか分からない」**状態だった。Public コマンド全 26 種を監査し、「出力そのものが結果になっているコマンド」（`metrics` / `lint` / `doctor` 等）と「既に報告があるコマンド」（`build` / `preflight` 等）を除いた 5 コマンドを是正した——`vs clean` は `✅ 削除しました（キャッシュ 2 件・生成物 12 件）`、`vs rename` は `✅ 11-workflow を 12-workflow に変更しました`（一括連番なら `✅ 3 章の連番を付け直しました`）、`vs resize` は `✅ 画像を最適化しました（WebP 24 件生成・最新のため据え置き 3 件）`（全件が最新なら `✅ 画像はすべて最新でした（1 件を確認）`）、`vs index:apply` は `✅ 辞書を更新しました（索引 2 件・用語集 0 件・リジェクト 0 件）`。続けて**残る全コマンドにも広げ、公開 24 コマンドすべてが `log_result` で結果を報告する**ようにした——`vs create` は `✅ 3 章を作成しました（12-intro, …）`、`vs delete` は `✅ 削除しました（文書 1 件・画像ディレクトリ 1 件）`、`vs import` は `✅ インポートしました（contents/ に 12 章）`、`vs open` は `✅ ….pdf を開きました`、`vs pdf:compress` は `✅ PDF を圧縮しました: ….pdf（28.8 MB）`、`vs pdf:pages` は `✅ 12 ページを画像化しました（book_images/）`、`vs pdf:rasterize` / `vs pdf:read` / `vs new` / `vs index:export` / `vs index:import` も同様。`vs doctor` は `✅ すべての必要ツールが見つかりました` ／ `⚠️ 不足しているツール: …（3 件）`、`vs metrics` は長い解析結果の末尾を `✅ 27 章を解析しました（総 262417 文字）` で締める。**実行したら必ず応答が返る**ことは、動作が同じでも利用者の安心感・信頼感につながる。`vs index:auto` の総括行は候補抽出の集計という性質から `log_summary`（🔍・次の操作を `detail:` で案内）を維持したが、同コマンドの辞書更新報告は `log_always("📝 …")` と**絵文字を手書き**していて `log_*` の体系外だったため `log_result` へ移した（`📝 辞書を更新しました` → `✅ 辞書を更新しました`）。なお `doctor` / `tool_upgrader` の `✅ … : OK` は診断項目の一覧であって最終結果ではないため、手書きのまま据え置いている。

**実績値（件数・対象名）を必ず含める**方針とし、何もすることがなかった場合も無音にせず報告する（`✅ 削除対象はありませんでした`）。clean / resize / delete の各関数は件数を戻り値で返すよう変更し、`CleanSummary` / `ResizeSummary` / `DeleteSummary`（いずれも `Data.define`）で内訳を返す。**ビルドからも呼ばれる処理は表示を Samovar 層で行う**——`execute_clean` は `vs build` の Step 0、`execute_resize_with_preset` は Step 1 が対象ディレクトリごとに呼ぶため、ドメイン層で報告するとビルド中に無関係な報告が何行も混ざる。`pdf:compress` は既存の `pipeline_mode?` 判定で分岐し、ビルド中は従来どおり info レベルに留める。
- [Medium] **`vs preflight` に章別エラー・警告サマリーを追加**（`preflight-chapter-summary-spec.md`）: 前処理はファイル単位（並列）で流れるため 🔴🟡 が章をまたいで時系列に混在し、数十件出ると「21 章に何件あるのか」が読み取れなかった。従来の逐次ログはそのまま流したうえで、**最後に章別サマリー表**（`📋 章別サマリー` ＋ 章ごとに `✅` / `🔴 エラー N 件・🟡 警告 M 件`）を出すようにした。**指摘ゼロの章も載せる**（検査したことの証明）が、全章 ✅ なら `✅ 全 28 章: 問題なし` の 1 行へ短縮する。章ラベルは原稿の H1 見出し（`<br>`・振り仮名 `{漢字|よみ}`・強調記号を除去し全角 2 幅換算で切り詰め）を使い、記号の縦位置を揃えた。あわせて最終行を **3 段階**（🔴 エラーあり ／ 🟡 **警告のみ**＝`⚠️ Preflight 完了: 警告 N 件（ビルドは可能です）`・新設 ／ ✅ 指摘なし）にし、`Common.log_result` に `status: :warning`（⚠️）を追加した。**終了コードの意味は変えていない**（詳細は下記 Fixed と KNOWN_ISSUES）。横断的な収集器 `IssueRegistry`（新規・`Issue`/`Counts` の `Data`・`Monitor` で並列前処理から同期）を一段上に設け、**各発生源は「章・行・重要度・カテゴリ・メッセージ」を record するだけ**にした（表示はしない＝二重表示の責務分離）。ブリッジした発生源は リンク・画像検証（`:image` / `:link`）・コードインクルード（`:code_include`）・クロスリファレンス（`:cross_reference`・重複ラベル 🔴／未定義参照・孤立ラベル 🟡）・QueryStream（`:query_stream`）・Guard 警告（`:guard`・章に紐付かないため専用行「章に紐付かない指摘」へ集約）の 5 つで、`LinkImageValidator` の内部構造（`ValidationReport`）は温存した。`vs build` 側は章別表を出さず、`print_summary` の後に **`🟡 ほか警告 N 件（詳細は vs preflight で確認できます）`** の 1 行だけ足す（リンク・画像以外の発生源の警告が埋もれないようにするため）。
- [Medium] **EPUB / Kindle のパッケージサイズを削減**（実測: EPUB 32.7MB → 28.8MB、KPF 74.1MB → 53.5MB、配信される KFX 34MB → 28MB）: 2 点の是正による。(1) **Kindle 向け画像の長辺上限を 1600px → 1024px** へ（`KINDLE_IMAGE_MAX_EDGE`）。この変換経路（`transcode_webp_images_for_epub!`）は `:kindle` からしか呼ばれないため、**クリーン EPUB は 1600px を維持**し iPad 等の高精細画面での画質を落とさない。Amazon は入稿 KPF の画像を**すべて JPEG へ再エンコードして配信する**（実測: KFX resources は 244 件すべて JPEG・透過は白へフラット化）ため、1600px のまま送っても端末表示に対して過剰で配信サイズだけが増えていた。(2) **未参照の twemoji 絵文字マスターを同梱から除外**。`restore_plain_emoji_for_epub!` がプレーン絵文字へ戻すため未参照になるが、除外条件が `twemoji/*.svg` と svg 限定だったため、実体である webp 3689 件がクリーン EPUB へ同梱され続けていた（Kindle は WebP 除外で偶然弾けていた）。形式を問わず直下を除外する。実際に参照される `twemoji/vs-techbook/`（囲み数字等）は従来どおり同梱する。
- [Low] **会話文のアバターを Kindle パッケージから除外**: Kindle は `decorate_talk_for_kindle!` が アバター `<img>` ごと除去して inline 形式へ組み替えるため、アバター画像はどのみち表示されない。WebP は拡張子で既に弾かれていたが、png/jpg のアバターは未参照のまま同梱されてパッケージを太らせていた。`localized_image?` が Kindle 限定で `images/characters/` と `images/talk-avatars/` をサブディレクトリごと除外する。
- [Medium] **会話文に簡易アバターの自動生成を追加**（`talk-auto-avatar-spec.md`）: アバターの絵を用意すること自体が執筆の障壁になるため、`avatar` を **`on` / `off` / `auto` の 3 値**へ拡張した。`auto` は「画像指定があればその画像、無ければ**話者色を地に表示名の頭 1 文字を白抜きした簡易アバター**を自動生成」する。`config/talk.yml` の `display.avatar: auto`（本全体）・話者ごとの `avatar: auto`・ブロック引数 `:::{.talk avatar=auto}` のいずれでも指定でき、解決順は「画像 > 話者の auto > display の auto」で `off` が全てに優先する。生成には同梱フォント Zen Kaku Gothic New Bold と `magick` を使い外部依存を増やさない（CJK・ラテンとも描画を実測確認）。地色の WCAG 相対輝度から文字色（白／濃灰）を自動選択するため、淡い話者色（yellow・lime 等）でも読める。生成物は**ビルド生成物**として `GeneratedAssetCache` に内容アドレスで永続キャッシュし、参照は `asset_prefix` を付けない（著者資産 `images/characters/` とは規約が異なる）。`magick` 不在時は 🟡 でアバターなしへ縮退しビルドは止めない。新規: `TalkAvatarGenerator`、`ThemeColor.luminance`。
- [Medium] **会話文（対話）記法 `:::{.talk}` と `config/talk.yml` を新設**（`characters-dialogue-spec.md` / `talk-display-options-spec.md`）: 旧 `【先生】`/`【生徒】` ハードコード方式を廃し、著者が CSS を書かずに話者を増やせる方式へ刷新した。`:::{.talk}` ブロック内に **「キー: 発話」**（半角ローマ字キー＋半角コロン）を並べて書き、行頭を字下げすると直前の発話の続きになる。設定は任意ファイル `config/talk.yml` にまとめ、**表示設定（`display:`）と話者定義（トップレベル）の 2 部構成**とした。話者は**簡易形（値＝色）／詳細形（`name` / `color` / `avatar` / `side`）**の両対応。表示は **`style`（`chat` 吹き出し / `inline` 「名前：発話」）・`name`・`avatar`** の 3 軸で制御し、`display:` の既定をブロック引数（`:::{.talk style=inline name=off}`）が上書きする。PDF・クリーン EPUB の `chat` は**アバターの下に話者名を積む話者列＋尻尾つきの吹き出し**（`side` で左右振り分け）。**Kindle(KFX) は flex・擬似要素・`var()` を解さないため、EpubBuilder が DOM ごと `inline` 形式へ組み替えて劣化**させる（アバターと左右振り分けは省き、`name=off` でも話者名は必ず表示する——名前を落とすと話者を判別する手がかりが皆無になるため）。話者色は生成 CSS（`book-settings.css`）へ `--talk-c-<key>` と Kindle 用リテラル（`.talk-name` / `.talk-sep` / `strong`）を焼き込み、ソース CSS は不変に保つ（P3 方式）。未定義キー・`talk.yml` 不在・アバター不在・不明なオプション・`style=inline` かつ `name=off`（話者が判別不能になる）は修正例つきの警告で案内し、ビルドは止めない。雛形（scaffold）にはコメントアウトした記入例のみを置き、既定キャラクターは提供しない。新規: `TalkRegistry`（表示設定と話者の正規化・side 自動割当・色検証）、`MarkdownTransformer.convert_talk_blocks`、`EpubBuilder#decorate_talk_for_kindle!`、`components.css`／`chapter-common.css` の会話文スタイル。原稿 22 章「拡張記法」の会話文節を新記法で復活させた。

- [High] **設定ファイルを経由しない直接ビルド `vs build myawesome.md` を新設**: 書き捨てのメモや企画書を、`vs new` でプロジェクトを作らずに 1 枚だけ組版できるようにした。`vs build` に**実在する `.md` ファイル 1 件**を渡したときだけ直接モードへ分岐し（章の basename は拡張子を含まないため従来指定と衝突しない）、`config/book.yml`・`config/catalog.yml` を一切読まずにカレントディレクトリへ `<元ファイル名>.pdf` を生成する（macOS では自動で開き、完了時にフル/単章ビルドと同じ `📚 … を作成しました (N.Ns)` を表示する。所要時間はワークスペース組み立てを含む実時間）。テーマカラーは `--theme blue` / `--theme=blue` / `--theme '#e91e63'`（色名は `theme.color` と同じ語彙・HEX 可・既定 yellow・`=` 区切りも可）で指定し、未知の値は候補と実例を添えて 🔴 で止める。実装は**一時ワークスペース方式**——`Dir.mktmpdir` に「最小プロジェクト相当」（stylesheets へのエントリ単位シンボリックリンク＋既定 CONFIG＋原稿 1 章＋参照画像）を組み立て、chdir して既存の `:single` パイプラインをそのまま流すため、前処理・VFM 変換・Vivliostyle 呼び出し・リネームの実証済みコードを 100% 再利用する（`Common` のパス定数がすべて相対のため無改修で成立）。CONFIG は YAML を読まずに既定値スキーマ＋版面プリセット（B5）から組み立てる新 API `Common.build_direct_configuration` で用意し、`Common.install_configuration!`（`reload_configuration!` と共通の唯一の差し替え口）で載せ、終了時に必ず元へ戻す。`RootCommand#ensure_project_context!` はコマンド自身の `projectless?` を見るようになり、プロジェクト外でも起動できる。出力は**閲覧用 PDF のみ**・装飾は **simple 固定**・原稿は常に**本章**扱い（`00-`/`99-` でも前書き/後書きにしない）。画像は入力ファイルと同じ場所からの相対パスで解決し（プロジェクトの章を指定したときはその章の `images/` も探す）、同伴した png/jpg はワークスペース内で WebP へ変換する。`codes/` インクルード・QueryStream・クロスリファレンス・索引/用語集は非対応。`--no-resize` 等プロジェクト前提のオプションは 🟡 で無視を通知し、通常ビルドでの `--theme` は `book.yml` の編集を案内する。`--log=debug` 時は一時ワークスペースを残してパスを表示する。ビルドは symlink 先（著者のプロジェクト／gem 同梱 scaffold）を一切書き換えない——唯一の書き込み先である `stylesheets/fonts/google-fonts.css` はワークスペース側の実体コピーへ逃がしている。原稿 44 章・README に追記。仕様書: [direct-build-spec.md](docs/archives/direct-build-spec.md)。
- [Medium] **図・アスキーアート用の枠 `:::{.diagram}` を新設**: 罫線ツリー・構成図など、桁揃えの要るテキスト図を「行番号なしの囲み枠」で組めるようにした。見た目は `.output` と同じ細枠のエイリアスだが、意味づけが「実行結果」ではなく「図」で、Kindle でも `.output` の【OUTPUT】のようなラベルは付かない。div 化は既存の汎用コンテナ機構が担うため Ruby 追加はなく、CSS（root→scaffold 同期）と行番号免除の 1 箇所のみ。原稿 21 章（Markdown フレーバーの系統図＝印欧祖語アナロジー）・22 章（`.diagram` リファレンス節）で使用。当初は `.figure` の名で新設したが、既存の図版スタイル `:is(.figure, figure)` とクラス名が衝突し、枠内に `ul`/`img` を置くと図版用の絶対配置・枠線が誤適用される潜在バグがあったため、公開前に `.diagram` へ改名した（調査報告: docs/specs/figure-class-name-collision-report.md）。
...
- **`a4_custom` プリセットの行送りを 1.5 → 1.6 にした**: A4 は 1 行が約 44 字と長い（B5 は 37 字）のに、`a4_*` は行送りを B5 の 1.7 より**詰めて** 1.5 にしていた。日本語組版では行が長いほど行間を広げるほうが目が迷いにくく、逆を行っていたことになる。本書を A4 で組み直したときの「詰まった印象」の正体がこれで、1.6 へ戻しても +7 ページで済んだ（`a4_compact` 340 → `a4_custom` 347）。大部の本を書く人がそのまま使える叩き台になるよう、判断の理由をプリセットのコメントにも残した。`a4_standard` / `a4_airy` / `a4_compact` は従来の値のまま。

- **索引辞書から `contexts` を外した**: `config/index_glossary_terms.yml` に保存していた文脈の抜粋は、**書いた瞬間から腐るだけのデータ**だった。読むのはレビューファイルの生成だけで、そこは stale を捨てて本文から拾い直す。書くほうは `index:apply` が用語と読みしか解析しないため（`review_markdown_generator.rb` の `parse_index_approved`）更新の機会がなく、初回登録時の抜粋が `updated_at` だけ押されながら残り続けていた。**実測: 321 件中 127 件（39.6%）が現原稿と一致しない**。ビルド経路は `contexts` を一度も読んでいない。
  - `save_terms!` が `backlink_sources`・`score` を落とすのと同じ棚へ移した。旧辞書に残っていても次の保存で黙って消えるので、著者の手作業は要らない。**辞書は 109,967 → 46,767 バイト（−57%）・2,595 → 1,720 行**。
  - 代償は拾い直しの時間だが、`enrich_terms_with_context` で +0.10 秒、`vs index:auto` 全体 4.5 秒に対して 2%。実測で索引ページが 1 バイトも変わらないことを確認した（全 513 ページ・前後で同一）。
  - **文脈は原稿全体から拾う**。章を指定した実行（`vs index:auto 33`）でも、指定章に出てこない用語は他の章から抜粋を取ってくる。欄が空だと「原稿から消えた語」と見分けが付かず、著者は索引に残すかを決められない——判断材料を欠かさないことを、キャッシュを持たないことより優先した。**実測: 指定章だけから拾うと 171 語中 46 語しか埋まらないが、原稿全体からなら 171 語すべてが埋まる（+0.27 秒）**。指定章を先頭に置いて走査するので、いま推敲している章での使われ方が最優先で出る。
  - 指定外の章から来た抜粋には出どころを注記する。**「（走査対象外）」と「（catalog 未登録）」を書き分ける**——前者は本に載る章なので直す点は無いが、後者はその章が本に入らないということで、その語には索引のページ番号が付かない（章を戻すか語を外すかの判断が要る）。ビルド時の警告が「catalog 未登録の X に出現」と言い分けているのと同じ区別を、レビューの段階でも見せる。旧称の「（catalog 外）」は部分実行では catalog 内の章にも付いてしまい、実態と合っていなかった。
  - 登録語が走査した章に出てこない場合の注記も 3 通りに分けた。`[原稿に出現しません]`（死語＝語を外す）・`[catalog 未登録の章に出現]`（章を戻すか語を外す）・`[走査対象外の章に出現]`（直す点は無い）。行き先の違う 3 つを 1 つの文言で済ませていた。
  - **原稿に 1 回も出てこない語にスコアが付いていた**。`ScoringEngine#observe` は `tf` がゼロなら黙るが、その手前の `mark`（技術用語らしい綴りへの性質ボーナス）は語の形だけで通るため、**出現ゼロの語が「スコア: 15.0」と表示され**、`[原稿に出現しません]` へ振り分けられなかった。`score_terms` は返り値の説明に「原稿に出現しない語は含まない」と書いてあり、実装が追いついていなかった形。`tf.zero?` なら記録しない。本書の登録語 171 はすべて原稿に出現するため、帯の内訳は動かない（実測）。
  - 33 章に「登録した用語が原稿から消えたとき」「章を `catalog.yml` から外したとき」の 2 節を足した。どちらも運用で必ず起きるのに、辞書が自動で減らないことも、外した章の語がどう扱われるかも書いていなかった。**紙面での消え方が登録先で違う**ことも表にした——索引は行ごと消えるが、用語集は説明文が残って `→ p.` の行だけが消える（定義は本文の何ページに出るかとは独立した価値があるため掲載を維持する）。索引だけの語は紙面に痕跡が残らないので `vs build` も黙る、という非対称も明記した。
  - **「[原稿に出現しません]」が誤って出ていた**のも直した。この注記はスコアが取れない登録語に付くが、スコアは走査した章からしか算出されないため、部分実行では**他の章に堂々と出ている語にまで付いていた**（実測 38 件）。しかも直下の文脈行がその語の使用例を見せるので、著者は矛盾した 2 行を突き合わせることになる。文脈が拾えたかどうかで「[走査対象外の章に出現]」と書き分けた——前者は外す判断へ、後者は残す判断へ導く別物なので。
- **手動マークアップの実例を 33 章に置いた**: 記法（`[用語|読み]`）はコードブロックの中で説明していたが、本書のどこにも実際に使われた箇所がなかった——物理学の例文から外した 4 語がそれまでの唯一の使用例だった。「実例——この章に仕込んであります」の節で `[五十音順|ごじゅうおんじゅん]` を地の文に置き、巻末の索引から引けることを本文で案内している。実ビルドで確認: 索引の「五十音順」は `211, 212, 222, 226` を指し、例文のあるページ（211〜212）が含まれる。
- **物理学の例文に使った語を索引から外した**: `アインシュタイン` / `ニュートン` / `相対性理論` / `一般相対性理論` / `特殊相対性理論` / `重力` の 6 語。96 章のショーケースは組版の見本として物理学の文章を借りているだけで、本書の索引から引く語ではない。辞書から削除し、`index:auto` が再び候補に挙げないよう棄却リストへ移した。うち 4 語は 96 章の手動マークアップ（`[ニュートン|にゅーとん]` など）由来で、記法が残っていると `index:auto` のたびに再登録されるため、地の文へ戻した。索引語は 173 → 167 語（目安 131〜181 語の範囲内）。
- **`vs upgrade` の解説を「環境の診断と更新」の章へ集約した**（12・51 章）: 70 行の解説が「新規プロジェクトの作成（vs new）」に置かれ、51 章からはそこへ参照を飛ばしていた。新規作成の章に既存プロジェクトの更新手順があるのは筋が通らないので 51 章へ移し、章題を「環境の診断と更新（vs doctor / vs upgrade）」に改めた。doctor の `--yes` 節が「使い分け」の配下に潜り込んでいた構造の乱れもここで直した。
- **重複していた節を整理した**（21・33 章）: 21 章の導入は Markdown 一般の箇条書きが 95 行続き、「特徴」と「なぜ選ばれるのか」は中身が重複していた（要点 5 項目＋標準化の歩みへ圧縮）。33 章の「実用的なワークフロー」は章の冒頭と丸ごと同じ内容、「自動抽出機能」は前半と重なるうえ「スコアの絶対値では決めない」と書いた直後に「閾値を設定することで調整できます」と続いていた。
- **章の雛形を骨組みだけに戻し、記法の見本帳を別ファイルへ分けた**: `templates/chapter.md` は使える記法を実例つきで並べたチートシートを兼ねており 600 行を超えていた。`vs create` するたびにこれが章として生成されるので、著者は書き始める前に大量の見本を消すことになる。前書き（34 行）・付録（28 行）・後書き（28 行）と揃えて、章 1 本ぶんの骨組み（75 行）へ戻した。記法の実例は `templates/markdown_snippet.md` として残す——`vs create` では使われず、`catalog.yml` にも載らないので、置いたままでも本には出ない。「あの記法どう書くのだったか」を引くためのファイル。
- **`vs new` した著者の手元に置かれる README を分けた**: これまではルートの `README.md` をそのまま配っていたため、著者のプロジェクトに gem のライセンス・開発者向け情報・Changelog へのリンクが並んでいた。読む人が違えば書くことも違う——配布用は「あなたの本のプロジェクト」として、最初に動かす手順・ディレクトリの意味・よく使うコマンド・困ったときの入口に絞る（`docs/scaffold-README.md`）。ルートの README は gem のリポジトリを訪れた人向けのまま。
- **文体を他章へ揃えた**（24 章ほか）: 強調の前後にスペースを入れる（24 件）、半角コロンで文を終える、「行なう」「出来る」を使う、ハード改行の途中で改行する——といった癖がこの章に集中していた。全角コロン終止は 33 章に 18 件・43 章に 9 件あったので句点へ。「デフォルト」は本書が使う「既定」へ統一し、校正の提案（「既定 => デフォルト」ほか 4 語）は allowlist で止めた。

- **`vs metrics` の章一覧で、半角を含む見出しの行だけ左へずれていた**（`terminal-output-notes.md` §1）: 幅の判定 `display_width_for_char` が「ASCII でなければ 2 桁」という粗い実装で、**EAW=Ambiguous の記号まで全角と数えていた**。切り詰め記号 `…`（U+2026）がまさにそれで、端末では 1 幅に描かれるのに 2 桁として計算されるため、その行だけ埋め草が 1 桁足りなくなる。全角だけの見出しは 2 桁刻みで枠いっぱいに詰まるので誤差が出ず、**半角混じりの見出し（「第21章 Markdown 執筆チュート…」）だけが 1 桁短く見える**という現れ方をした。EAW=A を 1 幅として扱う（`AMBIGUOUS_WIDTH_CHARS`）よう改め、全 26 章が同じ桁で揃うことを実測で確認した。矢印なども同じ棚に載せてある——`terminal-output-notes.md` §1 が「EAW=A は端末で 1 幅が期待値」と決めており、絵文字の選定基準と同じ根拠に立つ。
- **校正が動詞の「通ります」を「とおり」へ直せと指摘していた**: prh 辞書 `js_primer.yml` の `- expected: とおり` は `pattern: 通り` の**文字列マッチ**で、形式名詞（「次のとおり」）だけでなく動詞「通る」の連用形（「経路を通ります」「テストが通りません」）まで拾っていた。品詞が違うので言い換えは成立せず、著者は毎回無視することになる。外部由来の辞書は上流の資産としてそのまま持ち、例外は `config/textlint_allowlist.yml` の「誤検出を抑えたい語」へ 1 行足す方針を採った（`"対処方法"` と同じ棚）。`"/通り(ます|ません|ました|抜け|過ぎ)/"` と正規表現で動詞の活用だけを外すため、**名詞用法への指摘は残る**——実測: 61 章の動詞 2 件が消え、22 章「2 通りあり」「次の通りです」の 2 件は据え置き。allowlist はマッチした語句の範囲だけを免除するので、同じ行の `sentence-length` などは従来どおり出る（11 件で不変）。
...
- **配布物にこの本固有の値が入っていた**: `scripts/copy_to_scaffold.rb` がテンプレート化していたのは 5 キーだけで、`vs new` した著者の `book.yml` に `series: "「技術書典20 新刊」"` `release: "令和八年四月二十六日"` `contact: "contact@atelier-mirai.net"` が初期値として並んでいた。いずれも任意項目なので空にして配る（`{{ }}` にしないのは `vs new` の質問を増やさないため）。埋め忘れより、他人の連絡先が初期値で入っているほうが害が大きい——著者が気づかず奥付へ載せうる。扉絵の既定も配布物側だけ `sakura` へ。
...
- [High] **必要な Ruby を 4.0 以上から 3.4 以上へ緩めた**（`release-1.0-considerations.md` A-1）: **Ruby 4.0 固有の機能は 1 つも使っていなかった。** Prism のバージョン指定パースで全 421 ファイルを解析したところ 4.0.0 / 3.4.0 / 3.3.0 いずれも構文エラー 0 件で、実際の下限を決めていたのは `it`（暗黙ブロック引数・1,024 箇所）**単独**だった。Ruby 3.4.10 で `rake test` を実走し、2,399 件すべて通過することを確認している。
  ...
  - Ruby 3.4 は 4.0 と並ぶ現行メンテナンス対象であり、RubyKaigi 2026 の調査（約 400 回答）でも本番利用は 3.4 が最大勢（3.4 約 145 / 4.0 約 140）。本 gem の利用者は Rails を運用する開発者ではなく技術同人誌の書き手で、手元の Ruby を能動的に上げない層が想定されるため、`>= 4.0` は間口を不必要に狭めていた。
  - `it` を捨てれば 3.2 まで下げられるが、`ruby-development-standard.md` の方針（最新イディオムで書く・互換分岐を残さない）と衝突するため採らない。同標準の対象バージョン表記も 3.4+ へ改めた。
...
- [High] **分量の判定を本文基準にし、プリセットを実測値へ入れ替えた**（`chapter-volume-calibration-data.md`）: 判定に渡していたのは生の文字数（コード・記法込み）で、**コードが多い章ほど「分量が十分」と判定される**歪みがあった。既定値のほうも導出が存在せず、生と地の文のどちらで数えた値かすら決められていなかった（初出コミット `a926b56f`・2026-01-16）。刊行済み技術書を章 128 件・節 408 件まで実測して引き直した。
  - **判定も表示も本文（コードと記法を除いた地の文）で数える。** `ChapterParser` が `Analyzer.prose_length` を使う。
  - **プリセットを 5 段にした**（`min` は 10・`ideal` は 25〜75・`max` は 90 パーセンタイル）。**選ぶ軸は総本文字数**——ページ数は判型・余白・書体・図版量で動くため単独では決まらない。

    | プリセット | 総本文 | 参考ページ | `chapter.min` | `chapter.ideal` | `chapter.max` |
    |---|---|---|---|---|---|
    | `compact` | 〜3.5 万字 | 20〜50 | 800 | [1,100, 3,400] | 4,200 |
    | **`handy`**（新設） | 3.5〜6.5 万字 | 50〜100 | 2,900 | [3,400, 5,700] | 6,800 |
    | `standard` | 6.5〜9 万字 | 100〜200 | 4,200 | [4,800, 8,500] | 9,700 |
    | `commercial` | 9〜15 万字 | 200〜350 | 6,400 | [7,800, 14,600] | 17,800 |
    | **`heavy`**（新設） | 15 万字〜 | 350〜 | 9,200 | [11,000, 16,000] | 19,600 |

  - **節の基準はプリセット共通**（`min` 400 / `ideal` [1,000, 2,800] / `max` 4,000）。実測が 200 頁以上の本からしか採れておらず、規模別に分ける根拠が無いため。なお `section.max` は従来値 4,000 と実測が一致した。
  - **章の分量を決めるのは本の厚さでなく章数である。** 総量がほぼ同じ 3 冊で 1 章あたりが 3 倍違うため、「技術書なら 1 章 ○○ 字」とは言えない。プリセットは外部の目安として使う位置づけ。
  - キャッシュは `CACHE_SCHEMA_VERSION` を 5 → 6 に上げて再計算させる。
- [High] **コードフェンス内の `## ` を節の切れ目と誤認していたのを直した**: 記法を解説する原稿ではフェンス内に `## 見出し` の実例が現れる。そこで節を切ると**閉じフェンスが開きフェンスとして解釈され、後続の地の文がコード扱いで消える**。本書の `21-markdown-tutorial` は節の本文合計が 6,303 → 2,195 字に化け、`## 第2レベル見出し` という記法の実例が幻の節として一覧に出ていた（他に `22-extentions`・`24-cross-reference`・`33-index-glossary`・`45-utility` でも誤検出）。見出しの判定を `Masking.each_prose_line`（フェンス解釈の唯一の実装）に委ね、コード外の行だけを対象にする。
- [High] **その本自身の章の中央値と比べる `metrics.use: relative` を追加した**（`chapter-volume-calibration-data.md` §7.2.0）: 機能ごとに章を立てるリファレンス寄りの本では章が細かくなり、刊行書の実測から引いた絶対のプリセットには構造的に届かない（本書は 18 章中 12 章が該当）。**章立ての粒度は著者の設計判断**であって直すべき欠陥ではないので、自分の本の中で目立って短い章・長い章だけを挙げる見方を選べるようにした。
  - 中央値の **0.65 倍未満 → 💡 加筆検討 ／ 0.80〜1.20 倍 → ✅ ／ 1.50 倍超 → 💡 やや長い**。本書では該当が 12 章から 8 章へ減る。
  - 倍率は 2 通りの導出（確定プリセットの形／刊行書 128 章の比の分布）が一致した値で、**`min` と `max` は互いに逆数**（1 ÷ 1.5 ≒ 0.65）。刊行書に当てると「短い」10%・「長い」10% と両裾が釣り合う。
  - 中央値を採るのは**分量判定の対象となる章だけ**（前書き・付録・後書きは短く、混ぜると基準が下へ引っ張られる）。**章が 5 つ未満**なら中央値が 1 章の増減で動くため、総本文字数から選んだ絶対のプリセットへ自動で切り替わる。
  - **節は相対にしない。** 節はもともと大きさのばらつきが大きく、相対にすると刊行書ですら半分が該当し、普通の長さの節まで「やや長い」と出る。
- [High] **`metrics.use` を変えても古い判定がキャッシュから配られていたのを直した**: 分量の判定は文字数と `book.yml` の設定から決まる派生値なのに、キャッシュへ保存していた。キャッシュの鮮度は**原稿の mtime でしか見ていない**ため、プリセットを変えても原稿を触っていない章は古い判定を出し続ける。相対モードでは「💡 加筆検討」と「✅」が同時に出るという矛盾した表示になって露見した。判定はキャッシュに載せず、再構築時に現在のしきい値から算出し直す。
- [High] **章別リストの棒グラフの縮尺がずれていたのを直した**: 事前スキャン（`light_scan_placeholder`）が棒の基準となる最大値を**生の文字数**で求めていたのに、棒を描く値は本文基準だった。いちばん長い章が半分の長さで描かれていた。事前スキャンも本文基準にそろえる（相対モードの中央値もここから採る）。
- [Medium] **`ideal` を ✅ として復活させた**: `metrics.*.chapter.ideal` / `section.ideal` は `ConfigLoader` が `ideal_min`/`ideal_max` へ展開してはいたものの、`check_volume` が `min` と `max` しか読まないため**どこからも使われない死蔵設定**だった（`ideal` を実際に消費していたのは漢字比率・平均語長・TTR の語彙難度だけ）。理想的な範囲に収まっている章・節へ ✅ を付ける。
  - `第31章 文章校正（vs lint） [##..........]  5,481 文字 ✅`
  - 「足りない／多すぎる」だけを伝えると、**直すところが無い章には何も返らず、判定されたのかどうかが分からない**。ちょうどよいことも伝える。
  - 文言は **`metrics.labels.just_right`（既定「丁度良い」）**。分量の判定が `too_short`（加筆検討）/ `just_right`（丁度良い）/ `too_long`（やや長い）の 3 状態で対になる。`ideal` という名にしないのは、`chapter.ideal` / `section.ideal` が範囲を表す**数値キー**で語が衝突するため。
  - **分量（💡 / ✅）は必ず先、文章の質（🤔）は後ろ**に並べる。同じ軸の判定が行の中で位置を変えると、読み手が軸を追えなくなる。
  - 分量判定の対象外の章（`exclude_chapters`・既定では前書き・付録・後書き）には ✅ も出さない。
  - 判定は `Formatter` が自身の `thresholds` から描画時に行う。`ChapterMetrics` / `SectionMetrics` に項目を足すと構築箇所 16 か所とキャッシュ形式に波及するため。
- [Medium] **`vs metrics` の指摘を系統ごとの記号に分けた**（`chapter-volume-calibration-data.md` §7.3）: 分量と文章の質を 1 つの 🟡 に合成していたが、**分量の指摘は「直すべき欠陥」ではなく提案**である。実測では章立ての粒度が細かい本ほど該当が増え（本書は 18 章中 12 章）、その大半は直す必要のない設計判断に由来する。🟡 では強すぎるので**分量は 💡、文章の質は 🤔**に分けた。
  - `第22章 拡張記法リファレンス [############] 28,849 文字 💡 やや長い ／ 🤔 表現が単調`
  - 同じ系統が複数なら `・` で連結し、系統をまたぐときは `／` で並べる。節（`##`）には分量しか付かないので常に 💡 になる。
  - どちらの記号も単一コードポイント・EAW=W で、`terminal-output-notes.md` §1 の桁揃えの規則を満たす。💡 は `lint` / `doctor` / `upgrade` がすでに「ヒント」の意味で使っており、語彙が揃う。
- [Medium] **一般語の案内を「説明箇所があるか」で組み直した**: 「外す／残す／章を指定」の 3 択には、**索引には不向きだが用語集には向く語**の逃げ道がなかった。書名や副題そのもの（本書における「Vivlio Starter」）は広く出るが腰を据えた説明箇所を持たず、主要参照を指定しようにも指す先がない。指す先のない太字は目印にならないので、そういう語は索引から外し、用語集の定義文で説明を届けるのが筋になる。`[-i]` はフラグの `i` だけを落とすので `g` は残る旨も書き添えた。あわせて、除外した語が候補に現れず 4 節「除外済みリスト」に集まること、そこで戻せることを凡例に足す。
- [Low] **索引の主要参照をゴシックにした**: 明朝の算用数字は太らせても見分けが付きにくく、字幅の差が小さいぶんとくに効かない。索引は拾い読みされるページなので、字面そのものが変わるほうが目印として届く。Zen Kaku Gothic New は 700 の実フォントを `@font-face` で読んでいるため、合成太字ではなく本来の Bold が出る。
- [Medium] **校正まわりの設定ファイル名を対にした**: `textlint_prh.yml` の `prh` はツール名（textlint-rule-**P**roof**R**eading**H**elper）であって何をするファイルかを表しておらず、相方の `textlint_allowlist.yml` が役割名なので**対になっていなかった**。`user_words.txt` に至っては接頭辞も拡張子も外れていた。**接頭辞＝どのツール／後半＝何をするか**へ揃える。

  | 旧 | 新 | 役割 |
  |---|---|---|
  | `config/textlint_prh.yml` | `config/textlint_rewrite.yml` | 表記を**直す** |
  | `config/textlint_allowlist.yml` | （変更なし） | 日本語校正で**指摘しない** |
  | `config/user_words.txt` | `config/spellcheck_allowlist.yml` | スペルチェックで**指摘しない** |
...
- [High] **`vs build` / `vs preflight` が、解決できない章指定で止まるようになった**（`Guards::ChapterTargetCheck`）: `vs build 11-install abc` は **`abc` を黙って捨てて 11-install だけ組み、成功として終わっていた**。著者は 2 章ぶんできたつもりでいるのに、成果物にも終了コードにも食い違いが出ない。**CLI 引数は設定ファイルと違って「今打った指示」なので、一部を黙って捨ててよい場面がない**——`vs lint` が既に採っていた「解決できない指定が 1 つでもあれば何もせず中断」に揃えた。
  - 理由で文面を分ける。**原稿が無い**（`abc`）なら綴りの確認先を、**原稿はあるが `catalog.yml` 未登録**（`21-draft`）なら「登録してください」を出す。後者を「章がありません」と言うと、著者は在るファイルを探し続けることになる。
...
- [Medium] **回転テーブルが版面いっぱいまで拡大されるようにした**（`table-colspan-spec.md` §6.2 を改訂）: 版面自動フィットの計算に**等倍（1.0）の上限**が入っており、表がその自然な大きさで頭打ちになっていた。回転テーブルは `break-before/after: page` で**専用ページを与えられている**のだから、そこに余白を残す理由がない。実測（A5・7 列 4 行の表）で **scale 90% → 130%**、版面高さの利用率が **68% → 96.8%** になった。PDF・クリーン EPUB・Kindle（PDF ページの画像化）のすべてに効く。
  - 代わりに `SCALE_MAX`（2.0）を新設した。行数・列数の少ない表は「版面に収める」だけだと何倍にでも伸びるため（実測: 4 列 2 行の表で **555%**）。本文の 2 倍を超える文字で組まれた表は、表というよりポスターに見える。
  - 拡大しても版面には収まっている（実測: 回転後 168.5mm / 版面 174.0mm・余白 5.6mm）。寸法推定は実測より約 6% 小さめに出るが、`SAFETY`（0.95）と 5% 刻みの切り捨てが吸収している。
  - `scale` / `shift-y` の著者指定が自動算出より優先される点は従来どおり。

- [High] **PDF 枝と EPUB/Kindle 枝を並列に走らせ、ビルドを約 176 秒短縮した**（`build-target-parallelization-spec.md`）: Markdown → HTML までは共通で、そこから先の `html → pdf → print_pdf` と `html → epub → kindle` は互いに独立している。共通前段のあと、臨界経路である PDF 枝をメインスレッド・EPUB/Kindle 枝を子スレッドで走らせるようにした。実測（A5・515 ページ・`targets: pdf, epub, kindle`）で **535.51 秒 → 359.55 秒（−32.9%）**。前付・奥付の 1 回レンダ化（−71 秒）と合わせると、同じ本が **660 秒台から 360 秒**になる。
  - **EPUB 枝がまるごと PDF 枝の陰に隠れる。** EPUB 枝 203.7 秒に対し PDF 枝 341.9 秒で、壁時計は 17.6 + max(341.9, 203.7) = 359.6 秒と実測に一致した。EPUB と Kindle をさらに割る案は壁時計を 1 秒も縮めない（どちらも PDF 枝より短いまま終わる）ので採らない。
  - **資源競合は穏当だった**——PDF 枝 +2.6%、EPUB 枝 +10.4%。同時に走る重量プロセスは Chromium 2 つ＋Kindle Previewer（JVM）で、10 コア機では `user 642 秒 / real 361 秒`＝並列度 1.78 と CPU は飽和していない。**4 コア級の機械では未計測**。
  - **`VIVLIO_BUILD_PARALLEL=0` で逐次へ戻せる。** 性能の保険ではなく切り分けの道具で、不具合が並列由来かを 1 コマンドで判定できる状態を保つ。`book.yml` には出していない——著者の本の性質ではなく実行機と作業の都合だから。
  - **ログは書き手を常に 1 つに保つ。** 子枝のログはいったん溜め、合流時にまとめて吐く（スピナーと 2 枝が同じ TTY へ書くと行が混ざる）。`--log=debug` の計時表には逐次合計と壁時計の**両方**を出す。壁時計だけにすると隠れた枝の劣化に気付けない。
  - **`Dir.chdir` はプロセス全体の cwd を動かす**という落とし穴があった。EPUB の zip 書き戻し 3 箇所がこれを使っており、並列にすると PDF 枝が起こす `npx vivliostyle` が一時ディレクトリを cwd として継ぐ——しかもタイミング次第なので再現しない壊れ方をする。子プロセスの cwd だけを変える `system(…, chdir:)` へ寄せた。同様にスピナーは `Common.emit` を経由せず `$stdout` へ直に書くため、出力先が差し替えられているスレッドでは回さないようにした。
  - **カバー資産の生成と `workspaceDir` を枝から剥がした**。前者は PDF 枝・入稿用・EPUB 枝の 3 箇所が同じファイルを書いており（先に走ったほうが作って後続は素通りする、という順序頼みの調停）、共通前段の 1 ステップへ引き上げた。後者は 5 つの生成 config が同じ `.vivliostyle/` を指しており、直下の `publication.json` を互いに上書きするので `.vivliostyle-pdf` / `.vivliostyle-epub` へ分けた（**深さは 4 階層のまま**——1 段深くすると資産への相対プレフィックスが合わなくなる）。
  - 成果物は逐次と等価であることを確認した。PDF は 515 ページ・5 バイト差（生成時刻）、EPUB は**バイト一致**、KPF は 1,041 バイト差（Kindle Previewer が埋める UUID/時刻）。
- [High] **前付・奥付を本文と同じ 1 回のレンダで組むようにし、ビルドを約 71 秒短縮した**（`front-back-matter-single-render-spec.md`）: 本扉・権利ページ・奥付のたかだか 3 ページのために vivliostyle を 2 回起動しており、実測 **73.17 秒 → 0.71 秒**になった。原因は組版の重さではなく**起動費**で、中身が空の 1 ページ HTML（CSS・フォント・画像なし）でも **21.7 秒**、フル CSS の奥付単体で 22.3 秒——**何を組んでも約 22 秒かかる**。本文スパインの末尾へ 3 ページを載せて 1 回で組み、結合時に qpdf のページ範囲指定で綴じ順へ並べ替える。成果物は 515 ページで変わらず、サイズは 96.54 MB → **96.44 MB** とむしろ減った。
  - **末尾に足すのが要点**。先頭に足すと本文のページ番号が 2 つずれ、`target-counter` で解決している目次のページ番号・索引・相互参照・dedup のページマップが軒並み動く。末尾なら本文のページ番号は 1 つも動かない。
  - **中間ファイルへは分割しない**。3 本に切り出してから結合する案も成立するが、切り出した各ファイルが**フォントを丸ごと抱え込む**（1 ページ切り出すだけで 2.91 MB）ため最終 PDF が +1.87 MB になる。埋め込みフォント量は本の厚さにあまり比例しないので、薄い本ほど比率が悪化する。結合を「ファイルの列」から**「ファイル＋ページ範囲の列」**（`PdfMerger::Segment`）へ一般化して回避した。qpdf の並べ替えは `/Dests` 3,978 件・注釈 10,040 件を保ち、アンカーの着地位置も **3,923 / 3,923 が期待どおり**（実測）。
  - **切り出し位置は「末尾 3 ページ」と決め打ちせず `/Dests` から実測する**。権利ページが 2 ページに溢れることも、`chapter_pagebreak: verso` で白紙が挟まることもあり、数え間違えると静かに隣のページを切り出す壊れ方をするため。
  - **`/Dests` に出るのは「リンクの飛び先になっている id」だけ**だった（`<body id>` は出ず、自己参照リンクは出る）。どこからもリンクされない前付・奥付は素のままでは 1 件も dest を持たないため、staged HTML（`pdf/` のコピーのみ）へ自己参照する空リンクを目印として埋める。`position: absolute` で流れから外すのは、本扉の `body` が `display: grid` で行を明示しており素の子要素を 1 つ足すと行割り当てがずれるから。`html/` の原本はクリーンなままなので EPUB / Kindle には現れない。
  - **柱とノド／小口を名前付きページで固定した**。単独ドキュメントだった頃は `@page :nth(1)` が柱を消していたが、本文の末尾では直前の章のタイトルが `string()` に残ったまま出る。ノド／小口も `@page :left` / `:right` が表裏で入れ替えるため、本文のページ数次第で余白が反転する。名前付きページは Paged Media の詳細度で `:left` / `:right` に勝つので、そこで綴じ側を固定した（実測: 奥付はレンダ上 513 ページ目＝右ページに組まれたが、最終 PDF の左端は 25.5mm ＝小口 18mm + padding で、固定が効いている）。
  - **前付・奥付の HTML 生成を共通前段へ前倒しした**。techbook 後処理が `html/` を一括で舐めるため個別再適用のコードが消え、EPUB が読む `_colophon.html` を PDF 側の工程が書く依存も断てた（`build-target-parallelization-spec.md` §3.3）。
- [Medium] **章・目次・部扉などの「右ページ始まり（改丁）」を `page.chapter_pagebreak` で選べるようにした**（`chapter-pagebreak-spec.md`）: 章・目次・部扉・付録・用語集・後書き・索引は常に右ページ（奇数）から始まっており、変えるには CSS を直接書き換えるしかなかった。値は **`recto`（既定）/ `verso` / `any`** の 3 つで、`@pagebreak` 記法の引数（裸 / `:recto` / `:verso`）と**ちょうど 1 対 1**に対応させてある——「原稿に書けばその場だけ、book.yml に書けば本全体」で説明が済む。実測では本書 368 ページ中 **20 枚が白紙**（5.4%）で、`any` にするとこれが消える。ページ単価で刷る同人誌や、ページ数の上限がある配布資料向け。
  - **原稿中の `@pagebreak:recto` / `:verso` は本設定より優先される**（局所が大域を上書きする一般則）。実装上も、打ち消すのは `body` / `body.toc` / `body.vs-header-simple h1` / `.part-title` の 4 セレクタだけで、記法が付ける `.vs-break-*` には触れないことで担保している。
  - `any` の置換先は `auto` ではなく **`break-before: page`**。`.part-title` と `body.vs-header-simple h1` は文書内の要素であり、`auto` にすると直前の内容へ流れ込んでしまう。`verso` には legacy `page-break-before` の対応値が無いので併記しない。
  - 奥付を偶数ページ（左）に置くための空白ページ挿入（`PdfMerger`）も `any` では行わない。改丁とは別の慣習だが、「どちら側でもよい」と宣言した本で白紙だけが残るのは一貫しないため。
  - **`section_pagebreak` とは値の語彙を共用しない**。2 つは違う問いを立てており（節＝改ページ**するか** / 章＝**どちら側の面か**）、共用すると `chapter_pagebreak: none` という**構造上あり得ない値**が構文上は書けてしまう（章は別文書として組まれ、PDF は章ごとの PDF を連結し EPUB は 1 章 = 1 XHTML なので、章が前ページの途中から続く状態を作れない）。節ごとの改丁が要るなら `@pagebreak:recto` を節見出しの前に書けば足りる。
  - **PDF 専用**。EPUB・Kindle はリフロー型で recto / verso の概念が無く、元から章頭でページが改まる。
- [Medium] **`page.section_page_break` を `page.section_pagebreak` へ改名した**: `@pagebreak` 記法が `pagebreak` を一語として定義しているのに、このキーだけが `page_break` と割れていた。**後方互換は取らない**——一般公開中のベータ版にこのキーは存在せず、救済すべき既存プロジェクトが無い（旧キーを書いても無視され、既定の `true` として扱われる）。定数 `SECTION_PAGE_BREAK_SELECTORS` も `SECTION_PAGEBREAK_SELECTORS` へ追随させた。
- [High] **章扉・節絵の意匠を実組版のフィードバックで追い込んだ**（`heading-metrics-spec.md` の後続調整・PDF / EPUB / Kindle）: 全ターゲットをビルドして通読した結果の是正 4 点。
  - **`theme.frontispiece.heading_offset` を新設**（既定 0mm・同梱 book.yml は 15mm）: 章扉の「章番号・章題・リード文」のまとまりを、まとめて下へずらす。扉絵は画像ごとに構図が違い、絵の余白と文字の位置が噛み合わないことがあるため、著者が追い込めるつまみを用意した。EPUB / Kindle の合成画像にも判型比として同じ量が効く。
  - **章番号と章題を詰め、章題とリード文の間を空けた**（近接の原則）: 3 要素が等間隔に並んで 1 つの塊に見えていた。章番号↔章題は同じ見出しの一部なので近づけ（h1 の `gap` を 6mm → 2mm、章題の上マージンを 0 に）、章題↔リードは負マージン（−10mm）をやめて正の余白（5mm）にした。EPUB / Kindle の合成画像も同じ比率へ（`number_y` 0.30 → 0.34、`title_mid` 0.50 → 0.46）。
  - **節絵の帯を 3 行の節題が収まる高さにした**: 長い節題（「ラベルIDの扱い（手動ID・自動ID・一意性・命名規則）」）が 3 行になると飾りの枠からはみ出して見えた（`pdf_h2_height.png` 実測）。帯を `aspect-ratio: 480/100` → `420/100`（A4 で 33.8mm → 38.6mm）とし、行送りを 1.35 → 1.25、飾りを `background-size` 150% → 160% に拡げて枠として成立させた。1 行・2 行の節題も同じ高さで組む（行数で帯の高さが変わると紙面が落ち着かない）。
  - **EPUB / Kindle の合成見出しの折返しを日本語組版の作法へ寄せた**: 2 行目以降を節番号のぶん字下げして番号の右で上下に揃うようにし（`epub_h2.png` 実測。以前は 2 行目が番号の下に潜り込んでいた）、行頭・行末禁則（小書き仮名・長音・約物・閉じ括弧を行頭に置かない／開き括弧で行を終えない）、語の境界を優先する折返し（「Markdown 執筆チュー／トリアル」→「Markdown／執筆チュートリアル」）、半角語を語中で割らない保護（「手動I／D・」→「手動／ID・」）、末尾 1 文字の泣き別れ回避を入れた。章扉の見出し画像から**用語集の † 記号を落とした**（画像化された † はクリックできず記号だけが残るため）。リード文は字面の下限を上げ（画像幅比 0.024 → 0.026）、位置を固定比から**章題の実際の下端から決める**方式へ変えた（2 行以上の章題と重なっていた）。飾りは右下にあるため、リード列を中央から左寄りへ移して行末の干渉を減らした。
- [High] **見出し・扉絵の寸法指定を mm から「1 行の文字数」へ移し、節絵の帯を約半分の高さに縮め、見出しの前後の余白比を正した**（`heading-metrics-spec.md` Phase 1・PDF）: `theme.frontispiece` に並ぶ 3 つの設定は同じ mm 表記なのに解釈が 3 通りに分かれており（`padding` はリテラル・`lead_width` はリテラル・`heading_width` は**上限のみ**）、`heading_width` を大きくしても効かなかった。`heading_width` だけが `clamp()` の第 3 引数に置かれていたためで、実幅は `paper-scale × 96mm` に固定されていた。**キー名を役割どおりに改め、寸法は文字数で受ける**（旧キーは残していない）。
  - **`theme.frontispiece.padding` → `edge_inset`**: 実体は「扉絵をページの端からどれだけ引っ込めるか」で、本文の余白とは無関係だった（使用箇所は `--frontispiece-background-size` の 1 箇所のみ）。`padding` は「内側の詰め物」と読めて誤解を招くため改名した。CSS 変数も `--frontispiece-padding` → `--frontispiece-edge-inset`。
  - **`heading_width` / `lead_width` → `heading_chars` / `lead_chars`**（＋ `theme.ornament.heading_chars` を新設）: **mm をやめて文字数（全角換算）で指定する**。文字の大きさは判型に追従するため、文字数なら「A4 の 8 文字」と「A5 の 8 文字」が同じ意味になり、mm 指定の「同じ値なのに判型で意味が変わる」問題が構造的に消える。実測でも既定値では**どの判型でも 1 行 7.4 字**に固定されていた（箱幅と文字サイズの両方に同じ `paper-scale` が掛かるため）ので、「クイックスタート」（8 字）は A4・B5・A5 のいずれでも 1 字だけ泣き別れていた。換算（文字数 × 1 字の送り）は**生成時にリテラル mm へ解いて焼き込む**——`clamp()` を `calc()` の中で掛け合わせると Vivliostyle が宣言ごと落とすため（`@page { size }`・綴じオフセットと同じ P3 方式）。CSS 側の `font-size` / `letter-spacing` と Ruby 側の換算がずれないよう、CSS の値を回帰テストで固定した。版面に収まらない字数は 🟡 が上限を添えて案内する（`heading_chars: 16 は版面幅 108mm に収まりません（最大 11 文字）`）。`theme.ornament` は画像名だけのスカラー形と、画像名＋字数のマッピング形の両方を受ける。**節題だけは箱幅ではなくフォントサイズが変わる**——節絵の帯は版面幅いっぱいで固定なので箱を広げる余地が無く、字数を減らすと字が大きくなる（20〜48Q に収める）。
  - **泣き別れの真因は `base.css` の `word-break: keep-all`** だった。CJK 文字間の分割点が全て消え、残るのは `.chapter-title` の `overflow-wrap: anywhere` による緊急分割だけになるため「入るところまで詰めて 1 字落とす」挙動になる。節題（h2）には既に `word-break: normal` の手当てが入っていたが**章題には入っていなかった**。両方に **`word-break: auto-phrase`**（Chromium 119+・日本語を文節で割る）を 2 段書き（`normal` → `auto-phrase`）で入れた。実測で `normal` の「Ruby のイ／ンストール」「新規プロジェ／クトの作成」が「Ruby の／インストール」「新規プロジェクトの／作成」になる。ただし **1 語が箱に入らないケースは救えない**（「クイックスタート」は分割点が無く緊急分割へ落ちる）ため、**箱幅の確保が先・割り位置の改善は後**という順序が必須。MeCab は使わない（組版の見た目が MeCab の有無で変わると同じ原稿が環境で別の本になる）。
  - **節絵の帯を 68mm → 約 33.8mm（ページ高の 23% → 11%）へ**: 飾りが左上・右下の**対角配置**で、飾り 1 つ分の高さが 2 段ぶん必要になっていた。`h2::before` の 1 層を**左右 2 層のスプライト**（各層が帯の半幅・`background-size: 150% auto`・`left top` / `right bottom`）に分けて同じ行に並べた。**専用アセットを作らないので同梱 12 種も著者の自作画像もそのまま使える**（`ImageGenerator` は不変）。左右の飾り避け（`padding-inline`）は「飾りの最大張り出し」ではなく「節題の行位置での輪郭」で決めた——角飾りは帯の上端付近で最大 26.3mm 張り出すが**垂直中央では 8.4mm**しかなく、最大値に合わせると横幅を約 20mm 捨てることになる（アセットのアルファ輪郭を実測）。結果、帯を半分に縮めながら**従来と同じ節題の字数を保てた**（最大値に合わせると 2 行に割れる節題が 1 行に収まる）。右の飾りは帯を飾り 1 つ分より高く取った差分（約 7.7mm）でわずかに下がる。実測: 12 章（節 4 つ）が 7 → 6 ページ（−14%）。
  - **見出しの前後の余白を「前 ≫ 後」にした**（近接の原則）: `.section-topic` に `margin-block-start` が無く、前の空きは直前の `pre { margin-block: 3mm }` だけ、後ろは `margin-block-end: 1em` ＋後続段落の余白で、比が **1 : 3 と逆**だった。そのため「前節の最後の段落＋コードブロック＋次節の見出し」が 1 つの塊に見えていた。前 16mm / 後 4mm に改め、実測で **16.0mm : 5.3mm ≒ 3 : 1** を確認した。名前付きグリッド行は節リードが無い節（大多数）でも空行ぶんの `row-gap` を残して「後ろの空き」を膨らませるため暗黙行へ変更し、ページ先頭に来た節では `margin-break: discard` で先頭余白を捨てる。節リードの 16mm 押し下げ（固定行からのはみ出しを見込んだ補正）は不要になったので撤去した。
  - **EPUB / Kindle の見た目は Phase 1 では変えていない**。`EpubBuilder#frontispiece_lead_ratio` が `lead_width` を直接読んでいたため、改名に合わせて新キーから同じ比率を導く形へ差し替え、値が改名前と一致することをテストで固定した。合成画像のレイアウトを PDF と揃えるのは Phase 2（下記）。
- [High] **EPUB / Kindle の扉絵・節絵も同じ寸法体系へ揃えた**（`heading-metrics-spec.md` Phase 2・§5）: `theme.style: image` の EPUB / Kindle では、リフロー環境（特に Kindle）で背景画像・固定寸法・重ね合わせが安定しないため、`HeadingImageComposer` が**飾りと見出し文字を 1 枚に焼いた合成画像**を配っている。ここが PDF とは別のパラメータ体系で動いていたため、同じ原稿の扉・節絵が両ターゲットで違う字数・違う高さに組まれていた。
  - **文字数指定を合成側の唯一の入口にした**。EPUB 側には既に「1 行の文字数（capacity）」と「入らなければ 8% 縮小」の仕組みがあったが、`ORNAMENT_FONT_RATIO`（画像高さ × 0.14）・`LEAD_FONT_RATIO`（0.024）・扉絵タイトルの `width × 0.072`（コメントに「11 字まで 1 行に収まる大きさ」＝実質の字数指定）というハードコードされた比率で動いていた。これを**`font_size = 領域の幅 ÷ 字数`** へ反転させ、3 箇所すべてを `heading_chars` / `lead_chars` / `ornament.heading_chars` から導くようにした（既定値も PDF 側 `BookSettingsCss::DEFAULT_*_CHARS` と共有）。
  - **節絵をコンパクト帯にした**。PDF と同じ 4.8:1 の帯へ、飾りを左右に並べて配置する。CSS の 2 層スプライトの SVG 等価物として `clipPath` で元アセットの左半分・右下半分を切り出す。実測: EPUB の節絵 SVG が 2880×1205 → **2880×600**、Kindle の JPEG が 1400×586 → **1400×292**、左飾りの張り出しは帯幅の **16.0%**（PDF 実測 16.2% と一致）。節題は中央寄せから**左寄せ**（PDF の `text-align: left` と同じ位置）へ。
  - **EPUB 側の余白も前後比 3:1 にした**。`h1/h2.vs-image-heading-epub` は `margin-block: 0.5rem` で**前後対称**、`article.vs-section-topic-epub` は後ろだけ `1em` で、PDF と同じ「見出しが前節の末尾と一体化して見える」問題を持っていた。リフローなので mm ではなく `em` で `1.8em / 0.6em` を与える。**あわせて Phase 1 の申し送りを訂正**——`body.vs-header-image .section-topic`（特異度 0,2,1）が `article.vs-section-topic-epub`（0,1,1）より強いため、Phase 1 の mm マージンは EPUB にも漏れていた（「EPUB は見た目不変」はこの点で不正確だった）。`body.vs-epub .section-topic` を同特異度・後段読込で置いて是正した。
  - **`frontispiece_lead_ratio` が preset 構成で常に既定 0.60 へ落ちていたのを直した**。`Common::CONFIG.page[:width]` は `page.use` のプリセット構成では nil なので、リード幅比が実測に基づかない固定値になっていた。解決済みの `BookSettingsCss.build_page_cfg` から版面幅を取り、実比（A4・24 字なら 0.508）を導くようにした。
  - **`LAYOUT_VERSION` を 2 → 3 に上げ、文字数指定をキャッシュ鍵へ追加した**。版数だけでは足りない——`heading_chars` を変えても鍵が変わらず、`--no-clean` 時に旧レイアウトの合成画像が使い回されていた。
  - **折返し位置だけは PDF と一致しない**。合成画像の中では CSS のテキスト整形（`word-break: auto-phrase`）が働かず、Ruby が表示幅だけで割るため。構造的な制約なので `KNOWN_ISSUES.md` に既知の制限として記録した（実用解は「`heading_chars` を見出しの実長以上にして 1 行に収める」——1 行の字数は両ターゲットで一致するため判断は共通）。

- **`vs metrics --all` の「一般・専門漢字の出現箇所」を初出の章・行順に並べるようにした**: 従来は「出現の少ない順（同数なら文字コード順）」で並んでいたため、`亘 → 第21章 / 俯 → 第11章 / 叩 → 第22章 …` と章が前後し、原稿にルビを書き足すには 15 字ぶんファイルを行き来する必要があった。**挙げる 15 字の選抜は従来どおり出現の少ない順**（稀な漢字ほどルビが要る）だが、**表示は初出順**にしたので、原稿を頭から開いていけば往復せずに直せる。

- [Medium] **確認プロンプトの見た目と応答解釈を統一**（`command-feedback-spinner-spec.md` §1.3）: 「本当に削除しますか？」のような確認は**12 箇所すべてが独自実装**で、絵文字（`❓` / `🟡` / なし）・括弧（`(y/N)` / `[y/N]` / `[Y/n]`）・出力方法（`print` / `$stdout.print` / 独自 `prompt`）がばらばらだった（例: `vs rename` は `  ❓ …を実行しますか？ (y/N):`、`vs delete` は `🟡 本当に … を削除しますか？ (y/N):`）。`Common.confirm?` に集約し、**`❓ <質問文> [y/N]:`**（既定 Yes のときは `[Y/n]`）へ統一した。応答は `y` / `yes` を受理し（大文字・前後の空白は無視）、Enter だけなら既定値、解釈できない入力は安全側（いいえ）へ倒す。読み取り先は `input:` で注入できるため、`vs upgrade` / `tool_upgrader` の `deps.stdin` による DI も維持している。**挙動が変わった箇所**: `vs new` の最終確認は従来「`n` のときだけ中断」で `abc` のような入力でも続行していたが、取り返しのつかない操作の前では安全側が妥当なため中断扱いに改めた。
- [Low] **ビルドのステップ名 2 つを実処理に合わせて修正**: `build front pages and tail` は扉・権利ページ（前付）と奥付（後付）を生成するステップだが、**「tail」が奥付を指す曖昧語**だったため `build front and back matter` に改めた（HTML のみ生成する対のステップも `build front and back matter html` へ）。`rename` は実際には**圧縮とリネームの両方**を行う（`run_compress_pdf_if_needed` ＋ `rename_output_pdfs!`）のに名前から圧縮が読み取れず、隣接する `compress, rename and final clean` とも語彙が揃っていなかったため `compress and rename` に改めた。ステップ名はログ（`[Timer] …`）・計時サマリー・スピナー表示の共通語彙なので、表示される名前と実処理が一致する。
- [Low] **廃止済み `vs index:build` のクラス定義を削除**: コマンド自体は以前に廃止済みだったが、`IndexBuildCommand`（`index_command.rb`）のクラス定義だけが残っていた。`RootCommand.command_map` に登録されていないため CLI からは到達できず、`vs --help` にも出ない完全なデッドコードだった（ドキュメント側からは削除済みで、クラス本体の削除が漏れていた）。索引ページ生成の実処理は `build/pipeline.rb` → `IndexCommands.process_index_for_build!` → `UnifiedIndexManager#build_index!` の経路で生きており、削除の影響がないことを確認したうえで撤去した。あわせて実態と食い違っていたコメント 2 箇所——`index_command.rb` の「index:build: 索引ページを生成（vs build から呼ばれる）」（実際は呼ばれていない）と `unified_index_manager.rb` の「vs index:apply は内部で vs index:build を実行しない」（参照先が存在しない）——を是正した。これで CLI から到達できない孤立コマンドクラスはゼロになった（登録済み 30 コマンド）。
- [Low] **図版 CSS の死んだ `.figure` クラスセレクタを廃止**: 図版スタイルの主語 `:is(.figure, figure)` のうち `.figure` クラス側は、`class="figure"` を生成する箇所がどこにも無い死に符号だったため、要素セレクタ `figure` のみへ簡素化した（`stylesheets/chapter-common.css` 図版セクション・`layout-utils.css` の `:not()` 除外リスト・`components.css` のコメント）。同じく死んでいた `.figure.right`（float 回り込み）ルールも削除。特異度がクラス相当（0,1,0）から要素（0,0,1）へ下がることで、layout-utils の汎用 `.align-*` ユーティリティ（`margin-block: 0.5em`）が配置指定付き図版の上下余白 4mm に勝ってしまうため、`figure:is(.align-left, .align-center, .align-right) { margin-block: 4mm }` を明示して従来の組版を保った。これで「図版＝`<figure>` 要素・テキスト枠＝`.output`/`.terminal`/`.diagram`・`.figure` クラスは不存在」という整理が完成し、名前の二義性が構造的に発生しなくなった。
...
- [Medium] **`.output` / `.terminal` / `.diagram` 内の逐語ブロックに行番号を付けないようにした**: 実行結果・端末転写・テキスト図は「コードの提示」ではないため、これらの枠に囲まれた `<pre>` は Prism 行番号の対象外とした（`prism_lines.rb` がコンテナ div 化の後に走るので、祖先に `.output`/`.terminal`/`.diagram` があれば免除できる）。従来は全 `<pre>` へ機械的に行番号が付き、出力例や端末ログにまで番号が振られていた（かつインデント量次第で図の一部だけコードブロック化して行番号の付き方が揺れていた）のを是正。言語付きフェンス（` ```ruby ` 等）は従来どおり行番号が付く。

- [High] **箇条書き拡張——fancy list（英字・ローマ数字マーカー）と複合番号 `:::{.outline-list}` を新設**: Pandoc `fancy_lists` 互換のマーカー種別（`a.` `A.` `i.` `I.`・片括弧 `a)`・両括弧 `(a)` `(1)` の計 13 様式）で番号リストを書けるようになった。**独自記法は作らず** Pandoc の記法をそのまま踏襲——先頭項目のマーカーがリスト全体の様式と開始番号（`E.` → 5 から・`(iv)` → 4 から）を決め、ネストした子リストにはレベルごとに同じ判定が適用されるため「`1.` の子に `(a)`」のような階層別マーカーも書くだけで組める。VFM は `A.`/`(a)` をただの段落にしてしまうため、前処理 `MarkdownTransformer.convert_fancy_lists`（定義リストと同型の Kramdown 経由方式）が fancy を含むリストブロックだけを HTML 化し `<ol>` へ様式クラス・`type`・`start`（括弧様式は `counter-reset` インライン指定も）を付与する。**fancy を含まないブロックはバイト一致で素通し**し、標準リストのルビ・脚注等 VFM 機能は不変。誤爆防止も Pandoc 準拠——大文字＋ピリオドのリスト開始行は空白 2 つを要求（`B. Russell は…` は地の文のまま）、`\(1)` エスケープで地の文化、単文字はローマ数字優先（`c.` = 100）。空行を挟んだ様式変更は別リストとして分裂（Kramdown の EOB マーカー `^` を注入。空行なしの様式変更は該当行と修正例つきで警告し先頭様式で続行）。**複合番号（1.1 / 1.2）** は標準の番号リストを `:::{.outline-list}` で囲むだけ（div 化は既存 CONTAINER_RULES・表示は CSS `counters()` で Ruby 実装ゼロ）。**番号なし箇条書きのレベル別マーカーを和文組版寄りの「● ○ ・」で明示**（記法なし・字下げするだけ。第 3 レベルの中黒は文字列 `list-style-type: "・"` が Vivliostyle で有効なことを PDF 実測で確認）。**Kindle 対応**: KFX は `::before` をルールごと破棄し括弧マーカー・複合番号が不可視になるため、`EpubBuilder#decorate_list_markers_for_epub!` が Kindle フレーバのみ実体マーカー `<span class="vs-li-marker">`（`(a) ` / `1.1. ` 等）を li 先頭へ注入（vs-adm-label と同じ実体注入パターン・冪等）。クリーン EPUB・PDF は `::before`＋CSS カウンタのまま無改変。ul 第 3 レベルは Kindle では ■ へ優雅に劣化。原稿 21 章（リストのルール・fancy list 基本）・22 章（「リスト装飾」に全様式一覧・outline-list・約束事）へ追記。仕様書: [nested-list-notation-spec.md](docs/archives/nested-list-notation-spec.md)。
- [High] **`vs upgrade` を「執筆環境の一括最新化」へ拡張（本体 gem・雛形・外部ツールの三段統合）**: `vs upgrade` 1 コマンドで ① vivlio-starter 本体の gem 更新 → ② プロジェクトの雛形追従 → ③ 導入済み外部ツールの一括更新を順に実行するようになった。「`gem update` → `vs upgrade` → ツール更新」と別コマンドを順序どおり覚えて実行する二度手間・三度手間を解消する。**① 本体の自己更新**: RubyGems 上の新版を検出したら確認（`--yes` でスキップ・非対話では安全側に更新しない）のうえ `gem update vivlio-starter` を実行し、**新しい版の vs で同じ upgrade を exec 再起動して続きを実行**する（`--skip-self-update` が再帰防止と更新オプトアウトを兼ねる。古い雛形で②を済ませてしまう事故が構造的に起きない。gem update は旧版を残すため失敗時も現行版がそのまま動く）。**③ 外部ツール更新**: brew formula / brew cask / npm -g / gem の 4 系統に散らばる導入済みツールを、**計画提示（現在版 → 最新版・系統を表形式で明示）→ 確認 → 実行 → 再診断** の 4 段構成で一括更新（不足分のインストールは再診断で既存 `--fix` 処理へ委譲）。最新版の取得は `brew outdated --json` / `npm outdated -g --json` の一括問い合わせのみで、オフライン時は中途半端な更新を避けて実行前に中断する。ツール単位で失敗しても続行し、末尾に手動復旧コマンド付きで失敗一覧を提示。**node を更新するときは @vivliostyle/cli を必ず連動更新**する（Node 26 × vivliostyle 10.6 の Chrome 展開デッドロックのような組み合わせ起因の実害の再発防止）。**Ruby 本体は自動更新せず**、新版の検出＋導入経路（rbenv/rvm/asdf/mise/Homebrew を prefix パスで判別）に合った更新手順と gem 再インストールの注意を「📣 お知らせ」で案内するに留める。brew/npm パッケージ名は新設 `ToolUpgrader::TOOLS` に一元化し、`--fix` のインストール処理も同表を参照（二重管理の解消）。ツール更新は macOS + Homebrew のみ対応で、他環境ではこのフェーズだけをスキップ（②は動く）。逆にプロジェクト外で実行した場合は②だけをスキップ（①③は動く）。waifu2x / rouge は更新対象外（計画表に明示）。②の対話（競合確認）を前半に集め、時間のかかる③を無人で流せる実行順とした。`vs doctor` は診断＋不足分導入（`--fix`）に純化（`--upgrade` オプションは導入しない）。原稿 13 章・51 章・README を更新。仕様書: [doctor-tool-upgrade-spec.md](docs/archives/doctor-tool-upgrade-spec.md)・[upgrade-unification-spec.md](docs/archives/upgrade-unification-spec.md)。
...
- [Low] **EPUB 表紙画像の拡張子を `.jpg` / `.jpeg` 両対応に**: `EpubBuilder#resolve_cover_image_path` が `cover_{theme}.jpg` 決め打ちで `.jpeg` を認識せず表紙なしになっていたのを、`.jpg` 優先で実在する方を採用するよう改修（`epub_builder.rb`）。EPUB 設定行生成（`build_cover_config_line`）とカバー生成スキップ判定（`generate_epub_cover_if_needed`）の双方に波及。あわせて `scripts/copy_to_scaffold.rb` の `covers/` 不要ファイル除去で `.png` / `.jpg` / `.svg` / `.md` を温存する処理を追加。
...
- **EPUB の索引・用語集リンクが行き先を失っていた（epubcheck RSC-012・実測 102 件）**: 索引・用語集の後処理は「参照先 id が EPUB に実在するか」を照合し、無いリンクを素のテキストへ解除する恒常対策を持っている。ところがこの照合が**共通フェーズの先頭**に置かれており、**あとから走る `inject_heading_images_for_epub!` が見出し・章リードの中身を合成画像へ差し替える**。章題・章リード・節題は `<img>` 1 枚に畳まれ、テキストは `alt` 属性へ移るので、そこにあった索引アンカーは要素として消える——照合を済ませたあとに消えるため、安全網をすり抜けた死リンクがそのまま残っていた。
  - 消える側を直した。`apply_image_heading!` が中身を捨てる前に索引・用語集のアンカー（`idx-…` / `gls-src-…`）を集め、**中身の無い `<span class="vs-epub-anchor">` として見出しに残す**。見出しは章・節の入口そのものなので、着地点としても主要参照の意図と一致する。対象を 2 種の接頭辞に絞るのは、脚注 id のように別のフェーズが意図して外した id をここで復活させないため。
  - 照合そのものも**共通フェーズの最後へ移した**。今後また id を消す処理が増えたとき、epubcheck を割らずリンク解除で済ませるための保険。
  - 主要参照はとくに影響が大きかった。`main_reference?` は「章題にしか無い語は、その章題を指すほかない」という分岐を持ち、**構造的に見出し・章リードへ着地しやすい**。実測では索引全体では 647 件中 64 件（9.9%）の死リンクだったが、**主要参照だけは 89 件中 27 件（30.3%）**が消えていた。用語集バックリンクも 167 件中 11 件。
  - 消えていた語（`Amazon` / `HTML` / `KPF` / `Kindle` / `ノンブル`）はいずれも、同リリースの「英字の索引語が日本語に接すると拾われなかった」修正で新たに拾えるようになった語。取りこぼしを直したことでこの経路が初めて踏まれた。
- **死リンク解除の照合が、日本語を含むアンカーで必ず外れていた**: 上記の安全網は href のフラグメントと id 属性を突き合わせるが、**href は URL エンコードされて出力される**のに対し id 属性は素の UTF-8 のまま。`gls-src-31-lint-css組版-1` は href では `gls-src-31-lint-css%E7%B5%84%E7%89%88-1` になるため、CJK を含むアンカーは**実在していても必ず不一致**になり、本来残すべきリンクを解除する側へ倒れていた。パーセント記号だけを解いてから照合する（`+` を空白に変える `unescape` ではなく `unescapeURIComponent` を使う）。見出しのアンカー保存によって安全網自体が発動しなくなったため実害は出ていないが、保険が誤作動する余地は残さない。
- **脚注の中の索引語で id が重複していた（epubcheck RSC-005）**: `footnote_converter` は段落内脚注をインライン `<span>`（画面用）と `<aside>`（印刷用）へ**同じ本文 HTML で二重に出力**する。脚注本文に索引語・用語集語があると、本文中で採番済みの `idx-…` / `gls-src-…` が両方へ複製され、XHTML の id 重複 ERROR になっていた（実測: 22 章「CSS」・33 章「ノンブル」）。EPUB 経路の `strip_inline_footnote_ids_for_epub!` は span 自身の id しか外しておらず、中身までは見ていなかった。span を丸ごと控えとみなし、中の id もすべて外す。残す側を `<aside>` にするのは、EPUB で読者に見えるのがそちらだから（`.page-footnote-inline` は `display: none`、`body.vs-epub aside.page-footnote` が `display: block`）。
- **節題の字送りが判型で溢れやすさを変えていた**: 節絵の帯は版面幅から左右の飾り避け（`padding-inline`）を引いた幅に節題を組む。CSS 側は `clamp(11mm, calc(var(--paper-scale) * 16mm), 16mm)` と**用紙比で縮む**のに、`--section-title-font-size` を解く Ruby 側は **34.0mm 固定**（＝`--paper-scale` が 1.0＝A4 のときの値）で見積もっていた。小さい紙ほど過大見積もりになり字送りが小さくなるため、**B5 だけが偶然 4% の余裕**を持ち `heading_chars: 14` の指定に 14.6 字入っていた。A4 では見積もりが一致して余裕ゼロになり、25 字の節題が 2 行に収まらず 3 行へ落ちる——「判型を大きくするほど溢れやすい」という直感に反する挙動の正体がこれで、`a4_*` を選んだ著者は誰でも踏むところだった。
  - CSS と同じ `clamp` の式で飾り避けを求め、余裕は `SECTION_TITLE_HEADROOM = 0.96` として**明示的に全判型へ配る**（節番号 `6-3` の幅・英数字の字幅の揺れ・禁則のぶん）。実測: A5 23.05Q・B5 29.51Q・A4 35.11Q で、同梱 12 プリセットすべてが 1 行 14.58 字に揃った（従来の余裕は A5 +13.6%・B5 +4.4%・A4 ±0%）。
  - A5 は過大見積もりが最も大きく、修正前から安全側だった（`a5_standard` の実ビルドで確認）。回帰テストは A5 / B5 / A4 の 3 判型が同じ余裕を持つことを固定する。

- **原稿（12〜96 章）を実装と突き合わせ、食い違いを直した**: 見直しの過程で見つかった主な誤りは次のとおり。いずれも「書いてあるとおりにすると期待と違う結果になる」たぐいのもの。
  - **設定の既定値が逆だった**（41 章）: `lint` の 5 キーすべて（`trim_long_vowel` ほか 2 つは実際には `true`、`disabled_rules` には `arabic-kanji-numbers` が入っている）と `output.pdf.techbook`（既定 `true`）、`output.cover`（既定 `master`）。CONFIG_KEYS が既定値の正典なので、そちらへ揃えた。`book.yml` のコメント「techbook（既定は無効）」も同様に古い。
  - **カバー生成の説明が generated-assets 移設に追随していなかった**（43・44 章）: 生成物は `covers/` 直下ではなく `.cache/vs/covers/` に出力され、名前も `frontcover_<テーマ>_<判型>_<色空間>.pdf` と自動で決まる。ファイル名を `output.pdf.cover.front` などで指定する節は、キー自体が撤去済みだったため削除。`directories.covers` を確認せよという案内も retired 済みのキーを指していた。
  - **`vs renumber` の採番規則**（23 章）: 「01 から順に振り直す」と書いてあったが、実装は**先頭章の番号を起点**に詰める（`regular_start_number`）。実行例も「11-intro → 01-intro」と、実際には起きない結果を載せていた。
  - **索引レビューの新着ラベル**（33 章）: 「直近 3 時間以内／今日抽出」ではなく、`NEW!` が「今回はじめて挙がった候補」、`Today` が「今日辞書に登録した語」。表記も `TODAY` ではなく `Today`。`target_terms` の既定も `standard` ではなく `light`。
  - **ビルド手順が Step 番号表のままだった**（44 章）: 61 章と同じ古い記述。実装は番号を持たず（実行条件で並びが変わるため）、`:shared` →（`:pdf` ∥ `:epub`）→ `:join` の 3 相で走る。流れの図へ置き換え、タイミング例も本書の実測（508 ページ・291.77 秒）に差し替えた。
  - **診断対象から mermaid が抜けていた**（51・92 章）: 実装は 17 種を検査するのに原稿は 16 種しか挙げておらず、mermaid 記法を使う人が「入れ忘れ」に気づけない。
  - **早見表の欠落**（90・91 章）: `vs index:plan` / `index:export` / `index:import` は public コマンドで 33 章にも解説があるのに一覧から抜けていた。記法早見表からは mermaid と `:::{.talk}` が抜けていた。`index:auto` が作るファイル名も `_index_review.md` ではなく `_index_glossary_review.md`。
  - **導入手順の陳腐化**（92・94 章）: rbenv で入れる Ruby が `4.0.2`（対応は 3.4.10 / 4.0.6）、手動手順から librsvg が欠落、CI 例の成果物名 `output_compressed.pdf` はワークスペース内の中間名だった。
- **裸 URL の指摘が別の行を指していた**: 行番号の付け替え（`correct_link_line_number`）が「その URL を含む最初の行」を探す実装で、同じ URL が複数回出てくる原稿では**常に最初の出現へ丸められて**いた。実測: 4 行目の裸 URL が 1 行目と報告される。付け替えが要るのは、検証が前処理の途中（フロントマター追加・HTML コメント除去・データ展開のあと）で走り、その時点の行番号が原稿とずれているため。
  - **`validate_links` と `validate_images` に分け、リンクは原稿に対して検証する**ようにした。前処理を通す前なら行番号はそのまま著者の手元の行であり、付け替え自体が要らない。`validate_directives!` が「パイプラインの最初に置くのは行番号がずれないため」と同じ理由でそこに居るのと揃えた形。画像の検証は相対パスの解決とデータ展開のあとでなければ実ファイルの有無を判定できないので、従来の位置のまま（同じ画像を何度も使う原稿で最初の行に丸まる制約は残る）。
  - 原稿を直接見るようになったぶん、前処理が担っていた除外を検証側へ移した。**HTML コメント内の URL**（コメントアウトした記述を指摘しても直しようがない。行数を保つため中身を空白へ潰す）と、**`@qr:https://…`**（QR コード記法の引数。前処理後は `<img>` になるので従来は自然に外れていた）。
- **参照リンクの定義行が「裸 URL」として指摘されていた**: `scan_bare_urls` は脚注定義行（`[^id]:`）を除外していたが、参照リンクの定義行（`[ref]: https://…`）は素通ししていた。CommonMark の標準記法で、その URL は `[テキスト][ref]` から参照される先なので、「リンク記法 `[テキスト](URL)` の使用を推奨します」と促されても直しようがない。`vs create` が配る雛形（`templates/chapter.md`）が参照リンクの実例を載せているため、**新しい章を作るたびに必ず 1 件 🟡 が出ていた**。
- **図とログの囲みで行番号が振られていた**（44・61 章）: ツリー図がコードブロック扱いになり行番号が付き、44 章では 2 行が版面からはみ出していた。`:::{.diagram}`（図）と `:::{.output}`（実行結果）で囲む——22 章に自分たちで書いた作法どおりに揃えた。

- **英字の索引語が、日本語に接すると拾われなかった**: 辞書は英数字だけの語に `/\bCSS\b/` のようなパターンを自動生成するが、Ruby の `\b` は日本語を語構成文字として扱う。そのため `CMYK版` `JPEG形式` `YAML形式` のように**和語が直接続く形では 1 件も拾われない**（`（CSS）` や `CSS、` のような約物は平気なので気づきにくい）。英数字の前後に空白を置く流儀でも複合語では自然にくっつくため、書き方の方針では防げない。
  - `TermPattern` が辞書の `\b` を **ASCII の語境界**として読むようにした。英字どうしの食い込み（`MATTR` の中の `TTR`、`CSS3` の中の `CSS`）は従来どおり弾く。辞書には読みやすい `\b` の綴りを残し、意味の解釈だけを 1 箇所に置く——既存の辞書を書き換えて回る必要はない。
  - 本文タグ付け（`IndexMatchScanner#index_term_pattern`）が `TermPattern` を通らず**同じ 3 行の写し**を持っており、広さ計測と主要参照の候補だけが新しい解釈になる食い違いを招くところだった。写しを消して委譲に変えた。用語集のみの語も同じ解釈へ揃えている（従来は境界なしの完全一致で、`CSS3` の中にも †リンクを付けていた）。
  - 実測: 地の文で **27 件のタグ付けが増え、失われたものは 0 件**（`CMYK` +8・`RGB` +4・`Vivliostyle` +4・`HTML` +3 ほか）。
- **索引の副次参照の絞り込みが、章を丸ごと落としていた**: 上限（`index.max_sub_references`）は出現**回数**で数える——ページ番号は組版が決めるので Ruby からは見えない。先頭から機械的に切っていたため、1 つの章に集中した語は他の章を丸ごと押し出していた。実測: `CMYK` が 43 章で 12 回出るようになった途端、44 章の 1 件（p.292）が 8 件の枠から溢れ、**索引から辿れなくなった**。章ごとに 1 件ずつ拾ってから残りの枠を埋めるよう改めた。索引のページ参照は全体で **866 → 960 件**（上限に達していた 64 語が、押し出していた章を取り戻した）。
- **短い索引語が長い索引語を食っていた**: 辞書の語を本文へ当てるとき、当てた語はタグごと退避されて後続の語から見えなくなる。辞書は読み順に並ぶので「マスター」が「マスター画像」より先に当たり、**`<dfn>マスター</dfn>画像` と組まれて長いほうが二度と一致しない**。用語集の「マスター画像」に本文リンクが 1 件も付かず、ビルドのたびに 🟡 が出ていたのはこれが理由だった。索引としても、実際には「マスター画像」と書いてある箇所が「マスター」の項目に集められる誤りになる。
  - 長い語から当てる（最長一致）よう順序を変えた。同じ長さなら辞書の並びを保つ——`sort_by` は安定ではないので添字を第 2 キーに置き、ビルドのたびに順序が揺れないようにしている。
  - 実測: 辞書 171 語のうち他の語に含まれるものが 15 組あり、うち **6 組が分断されていた**（`マスター画像` / `ラベルID` / `レビューファイル` / `バンドル` / `バンドル画像` / `バンドル` ⊃ `バンド`）。英字の語（`CSS組版` ⊃ `CSS` など）は `\b` 付きパターンのおかげで無傷だった。修正後は 12 語すべてが語全体でタグ付けされる。
  - 全章ビルドで **🟡 用語集語の警告が 2 件とも消滅**。`マスター画像` は用語集に `→ p.275, 279, 280, 281, 285`・索引に `275, 279–281, 285`、`ラベルID` は `→ p.114 …` と `114, 117, 120–122, 129, 319` を得た。短いほうの `マスター`（281, 335）も従来どおり単独出現を拾っている。総ページ数は 515 で不変。
- **`vs --help` に出ないコマンドがあった**: `index` / `index:plan` / `index:export` / `index:import` の 4 つ。33・91 章では解説しているのに、ヘルプから辿れないので著者は存在に気づけない。原因はコマンド一覧が二重管理だったこと——ルーティングの正典は `RootCommand.public_commands` なのに、`HelpCommand::COMMAND_CATEGORIES` がコマンド名も説明文も手書きで持っていた。契約テスト（CL-01）とドキュメント整合テスト（DC-02）はどちらも `public_commands` から動的に一覧を得ており「一覧のハードコードはしない」と明記されているので、ヘルプだけがこの型から外れていた形。
  - **一覧が持つのは分類と並び順だけにした**。説明文は各コマンドクラスの `self.description` から引く（`vs <command> --help` の見出しと同じ文言になる）。二重管理は実際にずれていて、**22 件中 14 件の説明が食い違っていた**。`index:auto` に至っては旧ファイル名 `_index_review.md` を案内したままだった（現行は `_index_glossary_review.md`）。`vs index --help` のワークフロー説明にも同じ旧名が残っていたので直した。
  - 載せ忘れが再発しないよう、**`public_commands` と一覧が双方向で一致する検査**を `help_spec_test` に足した。コマンドを追加して一覧に入れ忘れると `rake test` が落ちる。`help` だけは自分自身なので `UNLISTED_COMMANDS` として明示的に外す（`contract/docs_allowlist.yml` に同じ理由が先に書いてある）。`root_command.rb` のコメントが持っていた**三つめの手書き一覧**も、列挙をやめて正典の在り処を指す形に改めた。

- **`vs index:plan` が `vs index:auto` と違う候補を出していた**: `IndexPlanReporter` は「下見と本番で**同じ画面**を出す」ために切り出したクラスで、見え方が変わると下見の意味がなくなる。ところが `plan!` は生の候補をそのまま渡し、`auto_process!` は `selectable_candidates`（登録済みと棄却済みを落とした集合）を渡していた。**実測: 2,880 件 対 2,644 件**——差の内訳は登録済み 169 件・棄却済み 67 件。件数が水増しになるだけでなく、**著者が自分で `[r]` を付けて外した語を「推奨候補」として名指しで勧めてしまう**。plan 側も選べる候補だけを渡すよう揃えた。

- **`vs import` が動かなくなっていた**: Re:VIEW Starter の実プロジェクト（7 章・B5・持ち込み表紙 PDF）で試したところ、Step 5 で `undefined method 'save_catalog'` を出して**そこから先が丸ごと走らなかった**。`CatalogUpdater` が行単位編集へ書き換わったとき（部扉の実装時）に消えたメソッドで、`vs import` だけが取り残されていた。catalog.yml は変換されず、設定・表紙も未処理のまま。テストは `strip_re_extension` という補助メソッドしか触っておらず、変換そのものを一度も通していなかったので気づけなかった。
  - **catalog.yml の変換を行単位に改めた**。YAML として読み書きすると「まだ有効にしていない章」を書き残したコメント行（`# - lectures.re`）が消える——著者にとっては原稿の予定表そのものなので、キー名（`PREDEF`→`PREFACE` 等）と `.re` だけを置き換えて、部・並び・コメントは原文のまま残す。`.re` は後ろに英数字が続かないときだけ落とす（`.review` を巻き込まない）。
  - **表紙が撤去済みのキーへ書き込まれていた**: `output.pdf.cover.front` は `book.yml` にも `CONFIG_KEYS` にも無く、🟡 を 1 件出して終わっていた。現行の表紙設定は `output.cover`（`master` / `light` / `dark` のテーマ名）で、持ち込み表紙は `covers/frontcover_master.png` に置く決まり。取り込み後に `output.cover: master` を書くようにした——既定値と同じだが、`light` を選んだプロジェクトへ取り込むと持ち込んだ表紙が黙って無視されるため明示する。
  - **裏表紙が取り込まれていなかった**: `master` テーマは表・裏の 2 枚を要求するのに、`frontcover_pdffile` しか見ていなかった。結果として**雛形の見本画像が著者の本の裏表紙になる**。`backcover_pdffile` も同じ経路で取り込み、指定が無い本には差し替え先のパスを添えて 🟡 を出すようにした。あわせて Re:VIEW 側の書き方の揺れ（配列・`cover.pdf<2>`・`cover.pdf*`）を吸収する——従来は配列を渡されると `NoMethodError` で取り込み全体が止まっていた。
  - **判型の対応づけが恣意的だった**: `B5`→`b5_airy`、`A5`→`a5_compact`、その他はすべて `a4_standard`。行間や余白の好みは Re:VIEW 側の値からは決められないので、同じ判型の `_standard` へ寄せる。A5/B5 以外は**勝手に A4 にせず**、`page.use` の選び方を案内して雛形の判型を残す。
  - **`[centering]` が無効なクラスに化けていた**: `:::{.centering}` という CSS の無い名前へ変換していたため、中央寄せが**黙って効かない**。ブロックごと寄せる記法なので `.align-center` へ。`[flushright]` も同じ理由で `.text-right`（段落内の文字揃え用）から `.align-right` へ改めた。
  - **雛形の索引辞書が取り込んだ本に残っていた**: `vs new` は見本原稿 27 章と、それを説明する辞書 171 語を配る。`vs import` は原稿だけを入れ替えるので、**消した原稿の語が著者の本の用語集・索引に載ったまま**になり、ビルドのたびに「原稿のどこにも出現しません」が 17 件並んでいた（実測）。原稿と一緒に辞書も空へ戻す——`vs upgrade` が辞書の無いプロジェクトへ配るのと同じ初期形を使い、取り込んだ原稿からは `vs index:auto` で作り直す。
  - `book.isbn` の対応づけを足した（Re:VIEW の `config.yml` に `isbn` がある）。
  - **`//sideimage` の囲みが失われていた**: Re:VIEW の markdownbuilder（`_render_sideimage`）は `//sideimage` を「画像 ＋ 空行 ＋ 本文」へ平坦化し、**囲みの終わりを示す印を残さない**。変換後の Markdown だけを見ても、どこまでが画像の脇に置く本文なのか決められない——結果として画像と本文がただ縦に並ぶ形になっていた。
    - 章ごとに **`.re` 原稿を開き直して**、`//sideimage` … `//}` が何ブロックぶんの本文を抱えていたかを読み取り、同じ範囲を `:::{.sideimage}` / `:::{.sideimage-right}` で囲む。推測ではなく原文の構造をそのまま移すので、段落が複数あっても箇条書きを含んでいても崩れない。`//blankline` だけのブロックは数えない（Markdown では空行にしかならず、数えると本文を 1 つ多く巻き込む）。
    - 幅 `[30mm]` は版面幅（紙幅 − ノド − 小口）に対する比率へ換算して `{width=22%}` にする。Vivlio ではこの値が**そのまま画像列の比率**になる（`post_process` が `--sideimage-img-fr` へ写す）ため、mm をそのまま渡しても意味を持たない。
    - 実測: `book_c`（実プロジェクト）の **21 件すべてを復元**し、`side=R` の 4 件が `.sideimage-right`、残り 17 件が `.sideimage` と原文どおりに分かれた。`#@#` でコメントアウトされた指示は拾わない。置き場所が決まらなかったときは章名と画像名を挙げて 🟡 で知らせる。
  - **コードブロックの閉じフェンスが開きに化けていた**: `detect_code_block_languages` が `/^```\s*\n(.*?)^```/m` で拾っており、**言語付きで開いたブロックの閉じフェンス（裸の ```）を開きと誤認**していた。そこから次の開きまでを「言語未指定のブロック」と読むため、閉じフェンスが ` ```text ` に書き換わり、**以降の地の文とコードが総入れ替わり**になる。実測: `book_c` の取り込みで **68 箇所**が壊れ、`21-answers.md` は 105 個の開きに対し裸の閉じが **1 個**だけになっていた。行単位で開閉を追う実装へ改め、同章は 53/53 で整合。入れ子フェンス（```` の中の ```）も内側を独立したブロックとみなさない。
  - **`//output` が `//list` と区別できず、実行結果に行番号が付いていた**: Re:VIEW の `_render_codeblock` は `//list` も `//output` も同じ ```lang フェンスへ落とし、**ブロックの種別を残さない**。`.output` / `.terminal` の中の逐語ブロックは行番号を付けない決まり（22 章）なので、素のフェンスのままだと実行結果に番号が付く。1 命令＝1 フェンスなので、`.re` の逐語系命令の並びと `.md` のフェンスの並びを 1 対 1 で突き合わせ、`//output` → `:::{.output}`・`//cmd` → `:::{.terminal}` へ戻す。数が合わない章は**何もしない**（当て推量で違う種別を被せない）。実測: `book_c` の 14 件すべてを復元。
  - **`//clearpage` が消えていた**: markdownbuilder の `clearpage` は `blank2()`——**空行を 2 つ置くだけ**で印を残さない。変換後は段落間の空きと見分けがつかない。`.re` を読んで `@pagebreak` を戻す。`//clearpage` 自身に中身が無いので、**直後にくる素のテキスト**を目印にして位置を決め、その目印が囲み（`:::{.column}` など）の中にいるときは囲みの外へ出す——中に入れると改ページが囲みの内側で起きる。
  - **`contents/` を持たないプロジェクトへ取り込むと落ちていた**: `cleanup_existing_directories!` が「既にあるディレクトリ」しか作り直さず、無いままだと後段の move が `No such file or directory @ rb_file_s_rename` になる。常に作るよう改めた。
  - 94 章を実挙動に合わせた（表紙 2 面・判型・辞書の初期化・サイドイメージ・ログ例・トラブルシューティング表）。

- **`@pagebreak` が前書き・後書きで黙って無効だった**: 章の種別ごとに読み込む CSS が違い、`.vs-break-page`（`@pagebreak` の実体）は `chapter-common.css` にしか無い。前書きは `preface.css` を読むため規則が届かず、**改ページが何も起きない**。`---` のほうは `preface.css` が自前の `hr.pagebreak` を持つので効いており、「両者は等価」と説明している 22 章と食い違っていた。実測: 同じ位置で `---` は 5 → 6 ページ、`@pagebreak` は 5 ページのまま。
  - **改ページ記法の実体を `stylesheets/pagebreak.css` へ切り出した**。定義はこの 1 ファイルだけに置き、`chapter-common.css` と `preface.css` の 2 箇所から `@import` する。前者は `chapter.css` と `appendix.css` の両方が、後者は `postface.css` が読むので、**2 行で本文・付録・前書き・後書きに届く**。あわせて `hr.pagebreak` の**二重定義も解消**した（`chapter-common.css` と `preface.css` の両方にあった）。`preface.css` に残すのは、そこにしかない `hr` 装飾（1px の罫と前後アキ）の打ち消しだけ——装飾を足した側が消す責任も持つ、という切り分け。
  - **全章種別が読む `page-settings.css` へは入れなかった**。目次・索引・用語集・奥付・扉・部扉は**生成ページ**で、著者が改ページ記法を書く場所ではない。試しに置いたところ本書が 1 ページ増えた——22 章の節見出し「改ページ `@pagebreak`」が、目次ラベルでは**バッククォートの落ちた素のテキスト**になり、そこへ `@pagebreak` の置換ルールが当たって `<div class="vs-break-page">` に化けていた（本文側は `<code>` に守られて無傷）。目次ラベルの生成が `@` ディレクティブを解釈させてしまう**別の潜在バグ**で、記法を書ける章種別にだけ配ることで踏まずに済ませている。

- **表・コードのキャプションが本体と別ページへ割れていた**: `<p class="table-caption">` / `.code-caption` は表やコードの**兄弟要素**として先に置かれるが、改ページ制御が何も掛かっていなかった。見出しなら Vivliostyle が黙って次の内容と一緒に送ってくれる（`break-after: avoid` の有無で結果が変わらないことを B5・spacer 176〜196mm の 1mm 刻みで確認した）が、**`<p>` は送ってくれない**。残り高さが「キャプションは収まるが表の 1 行目は収まらない」帯に入ると、キャプションだけが前ページの末尾に残る。
  - A/B 実測で **規則なし 8/14・規則あり 0/14**（spacer 188〜200mm の 7 通り × 表／コード）。本書にも実在の分断が **2 件**あった（`リスト 6-1` `リスト 6-2`。後者はページ下部が丸ごと空いていた）。
  - `.table-caption` / `.code-caption` に `break-after: avoid` を掛けた。Re:VIEW Starter の `caption_needspace` に相当する。**包む `div` へ `break-inside: avoid` を掛けない**のは、版面より高い表やコードが丸ごと次ページへ押し出されて大きな空きを作るため——`index.css` に実測済みの失敗が記録してある。`break-after: avoid` なら中身は途中で分割できる。
  - 本文の見出し（h2〜h6）にも同じ宣言を足した。索引・前書き・目次には以前から入っていたが本文には無く、`page.section_pagebreak: false` を選べるようにした時点で前提が崩れていた。上記のとおり PDF では結果が変わらないので、これは EPUB / Kindle 向けの保険（組むのは読み手側のエンジン）。Kindle KFX は modern な `break-*` を解さないため legacy の `page-break-after` を併記している。

- [High] **`catalog.yml` から外した草稿が図表番号の章プレフィックスをずらしていたのを直した**: `vs create 15-draft` したあと `catalog.yml` から外しても原稿は `contents/` に残る。前処理はその `contents/` を舐めて本文章の並びを作っていたため、**組まれない草稿まで数に入り、以降の章の図表番号だけがずれていた**（章扉は第 4 章なのに図は「図 5-1」）。後処理側の並びは組み上がった `html/` を見るので草稿は入らず、**前処理と後処理で並びが食い違う**のが原因。
  - 章立ての正典は `catalog.yml` なので、前処理もそこから並びを起こす（`main_chapters_from_catalog`）。`contents/` のグロブはやめた。
  - 本書では `61-developer` が `catalog.yml` でコメントアウトされたまま `contents/` に残っており、この経路に乗っていた。**並びの末尾だったので実害は出ていなかった**が、途中の番号に草稿を作れば以降の全章がずれる。
- [High] **`contents/_README.md` が図表番号の章プレフィックスを 1 つずらしていたのを直した**: 著者向けの説明ファイルなので章から外れるはずだが、`TokenResolver` が `_README` に番号 `01` を割り当てるため `main_chapter_token?` を通り、**本文章の並びの先頭に居座っていた**。結果、`11-workflow` が第 1 章ではなく第 2 章として採番され、その章の図表が「図 2-1」になっていた。ビルド後半は `chapter_tokens_override` が入るのでずれないが、**クロスリファレンスの採番は前処理（Step 3）で行われるのでこちらが効いていた**。
  - `_` 始まりは章ではない（システムページか著者向けの説明ファイル）ので、`main_chapter_token?` が接頭辞で弾く。**判定点を 1 つにしたので `override` / `discovered` / `detect_main_chapters_from_files` のどの経路にも効く。** システムページ（`_titlepage` `_part1` など）は番号を持たないため従来から落ちており、挙動が変わるのは `_README` のような非システムの `_` ファイルだけである。
  - **既存の本では図表番号が 1 つ下がる**（例: 24 章の「図 8-1」→「図 7-1」）。これが正しい番号で、本文からの `@id` 参照も同じ番号を指すため整合は取れている。
- [High] **`vs build` / `vs preflight` が索引レビューファイルを消していたのを直した**: clean が `_index_glossary_review.md` を中間生成物として削除対象にしていた。`vs index:auto` が書き出すファイルではあるが、**そこから先は著者が編集する入力**である。「除外済みリストから戻す語を選ぶ」ような途中の判断がまるごと失われ、続く `vs index:apply` は「ファイルが見つかりません」で終わる。レビュー中に一度でもビルドやプリフライトを挟めば起きるので、除外した語を戻す操作が事実上できなくなっていた。掃除するのは意図が明示された `--purge` のときだけにする。
- [High] **レビューファイルの往復で辞書の出現箇所（contexts）が壊れていたのを直した**: 原因は 2 つある。(1) 主要参照の子行 `  - 主要参照: 21` は出現箇所行 `  - 章名: 文脈` と綴りが同型なので、文脈として取り込まれ `chapter: 主要参照` が辞書へ入っていた。往復のたび再出力・再取り込みされ、値が空へ潰れて残る（本書の辞書で 12 語）。(2) 章名から表示用の「（catalog 外）」注記を剥がす `sub` がその場で `$~` を上書きし、続けて読む `Regexp.last_match(2)` が nil になっていた——**辞書の contexts が軒並み空だったのはこれ**。綴りの前半を `MAIN_REFERENCE_PREFIX` として切り出して子行を先に弾き、出現箇所行は `MatchData` を受けてから読む形にした。
- [High] **索引の主要参照が章扉のページを指していたのを直した**（`index-main-reference-section-spec.md` Phase 1）: `main: 21-markdown-tutorial` と指定すると「その章での初出」が主要参照になるが、**章題（h1）そのものが初出**になるため、索引は章扉のページを太字にしていた。著者が指したいのは「## Markdown とは」がある次のページである。**章題は「章の名前」であって、その語を腰を据えて説明している箇所ではない。**
  - 落とし先を「節見出し（h2〜h6）にその語がある → そこ」「無ければ本文の初出」「本文にも無ければ（章題にしか無い）章題」の順で決める。実測（本書・`main:` 指定 42 件）で **23 件が節へ降り**、16 件が本文初出、章題に残るのは 3 件だけになった。
  - **行ごとの逐次処理では決められない**——本文の初出を採るかどうかが「後に節見出しが来るか」に依るため。章を読んだ時点で「節見出しにあるか／本文にあるか」だけ先に下見する 2 パスにした。

- [High] **目次・索引・用語集に book.yml の設定が一切届いていなかったのを直した**: この 3 ページは章 HTML と違って `FrontmatterGenerator` を通らず自前で `<head>` を組み立てており、**生成物 `book-settings.css` を読み込んでいなかった**（`toc.rb` / `unified_page_builder.rb` の 3 箇所）。`chapter_pagebreak` の実装中に、打ち消しが効かない 3 ページとして発覚した。波及は 2 つ。
  - **判型**: `page-settings.css` の `@page { size: 210mm 297mm }` は P3 以降どこからも書き換わらないフォールバックなので、**A5 / B5 の本でもこの 3 ページだけ A4 で組まれていた**。本書が A4 のため露見していなかった。`@page` 側のコメントも `css_updater が自動更新` という P3 以前の仕組みを指したままだったので、実態（生成 CSS が上書きする／ここは保険）に書き直した。**A4 以外での確認は未実施**。
  - **テーマ色**: `index.css` / `glossary.css` が 8 箇所で参照していた **`--theme-color` はどこにも定義されていない変数**だった（実在するのは `--theme-accent`）。索引・用語集だけが常にフォールバックの `#333` で描かれていた。`--theme-accent` へ、`--theme-color-light` は既存の `--color-column-bg` へ差し替え、Kindle 用のリテラル規則も追加した（`body.vs-kindle` の既存規則は章の body クラス前提で、`.index-section h2` などには届かない）。
- [Medium] **索引が五十音の行ごとに改ページされていたのをやめ、段へ流し込むようにした**: 改ページ指定は無く、原因は `.index-section` の `break-inside: avoid-column`（および `@media print` の `avoid`）だった。1 行ぶんの項目は段の高さを容易に超えるため、収まらない節が丸ごと次の段へ押し出され、段が尽きれば次ページ送りになっていた（索引 8 ページ中 4 枚で最終段の 6 割が空いていた）。`break-inside` を撤去して流し込み、行見出しが段末に取り残されるのだけ `break-after: avoid` で防ぐ。`dt`/`dd` の対を保つ規則は `@media print` の中にあり EPUB / Kindle へ届いていなかったため外へ出した。実測: 総 368 → 366 ページ（索引 8 → 7 ページ）。

- [Medium] **Kindle で回転テーブルが回転せず素の表に戻っていたのを直した**（`kindle-rotate-table-image-spec.md`）: `.rotate-table > table` は `position: absolute` ＋ `transform: rotate(-90deg)` で回転しているが、**KFX はそのどちらも解さない**ため宣言ごと無視され、Kindle では列が詰まって折り返し、横長の表は見切れていた（`kindle_rotate_table.png` 実測）。CSS では解決できないので、数式・mermaid・章扉と同じく画像へ劣化させる。**クリーン EPUB は据え置き**（あちらは 90 度回転が効いており、画像化すると拡大時に劣化する）。
  - **PDF で組まれたページをそのまま切り出す。** 回転テーブルは PDF では専用ページに組まれる（`break-before/after: page`）ので、そのページを画像化すれば **PDF と寸分違わぬ見た目**が手に入り、回転・センタリング・scale の追い込みが PDF 側 1 箇所で完結する。表だけを別レンダリングする案は Chromium の起動が表の数だけ増え、PDF と 1 対 1 の見た目も保証しにくい。
  - **切り出し元は dedup 前の 1 回目のレンダ。** dedup が消すのは用語集・索引のバックリンクと本文の † マークで、回転テーブルの中身は 1 文字も変わらない。dedup 後を待つと Kindle 枝の開始が 160 秒遅れ、PDF 枝の陰に収まらなくなる。**ページを引くのと切り出すのを同じ PDF に対して行う**限り、ずれようがない。
  - **左右は表裏を判定せずに切る。** 柱とノンブルは天地の余白に入るので、左右は inner / outer の**狭いほう**で対称に切れば版面を欠かない。recto / verso で inner と outer が入れ替わる問題を、判定せずに回避できる。
  - **`/Dests` に出るのはリンクの飛び先になっている id だけ**という落とし穴を、前付・奥付と同じく踏んだ。回転テーブルのラッパはどこからも参照されないため、staged HTML（`pdf/` のコピーのみ）へリンクを 1 本埋めて目印にする。`html/` の原本はクリーンなままなのでクリーン EPUB には現れない。
  - **枝をまたぐ待ち合わせの解放は 2 段構えにする。** Kindle 枝は PDF より 70 秒以上先行するため、待たないと「まだ無い」を「PDF が無い」と誤認して**静かに**素の表へ落ちる。実装当初は解放を「PDF 枝の終わり」1 箇所に集約したところ、間に挟まる `backlink dedup`（161 秒）と `apply outline`（34 秒）まで待ってしまい壁時計が **359.6 秒 → 485.7 秒**に悪化した。「作り終えた瞬間」＋「枝の終わり（保険）」の 2 箇所で解放して解消（実測 **361.2 秒**・機能追加ぶんは 1.7 秒）。
  - **KFX では `margin: auto` が効かず、画像がページ左端に寄る**（実機確認）。`text-align: center` ＋ インライン画像という古い組み合わせへ変えた（EPUB で画像を中央に置く定石）。画像を `display: block` にすると `text-align` が効かなくなるので display は触らない。
  - 抽出コストは**固定費 1 秒前後＋ 1 枚あたり 0.92 秒**（実測）。表が増えても PDF 枝と EPUB 枝が同じだけ伸びる（後者はラッチの待ち時間として）ため、**並列の余裕（71 秒）は減らない**——増えるのはビルド全体の時間だけ。
  - `output.targets` に `pdf` が無いビルドでは画像化できないため素の表へ縮退し、🟡 で「`targets` に `pdf` を足すと画像化されます」と案内する。
- [Medium] **`PdfMergerTest` が 1 件も実行されていなかったのを直した**: クラス冒頭の `private` が `test_` メソッドの定義まで巻き込んでおり、Minitest は public な `test_` しか集めないため、カバー結合の 3 件を含む 5 件が一度も走っていなかった。起こしてみると期待値が 3 点古びていた——`CONFIG` は `Data` なので生の `Hash` を `const_set` するとドット記法が壊れる、`setup` の `Marshal.dump(CONFIG)` は `wrap_config` が作る匿名クラスを dump できず必ず落ちる、結合対象のパスがワークスペース分離（P4）以前のルート直下のままだった。

- [High] **`page.section_page_break: false` の PDF で、節絵が前後の本文に重なり、章扉に本文が流れ込んでいたのを是正した**（`page-break-control-spec.md` §2.2）: 節（`##`）を本文に続けて組む設定にすると、`theme.style: image` の PDF だけが 2 通りに崩れていた（EPUB / Kindle は別レイアウトのため無事）。原因はいずれも「節が必ずページ先頭に来る」という既定の前提に依存していた点にある。
  - **節絵が直前の段落・コードブロックに重なる**: `image-header.css` の `.section-topic` は節絵用に `"section-title" 150px` の固定行を敷くが、h2 自身は `aspect-ratio: 239/100`（版面幅 162mm なら約 68mm）を持つため、行より背の高い箱が `align-self: center` で置かれ、節絵は行の**上下へ 14mm ずつはみ出していた**。既定では上のはみ出しがページ上端の余白へ逃げるので誰も気づかなかったが、続けて組むと直前の本文へ食い込む。`false` のときだけ行を `auto`（実寸）へ戻し、はみ出しを見込んで節リードを 16mm 押し下げていた補正も併せて外した。
  - **章扉に最初の節が混入する**: `theme.style: image` の章扉は `@page :nth(1)` のページ全面背景（扉絵）で成立しているため、最初の節が同じページへ流れ込むと絵柄の上に節見出しと本文が重なる。**章扉だけは節の改ページを止めても独立したページとして残す**ようにした（2 つ目以降の節は従来どおり本文に続けて組まれる）。判定は `section.level2:first-of-type` で、`body.vs-epub` を除外して PDF に限定する。
  - 章扉保護で最初の節の改ページが復活したことに伴い、`PageBreakNormalizer` の対象範囲も広げた。従来は `section_page_break: false` なら「h2 は改ページしない＝冗長性が消える」として一切正規化しなかったが、それでは `---` を節の前に書く習慣の原稿で**章の最初の節にだけ空白ページが 1 枚挟まる**。改ページする h2 の範囲を `:all` / `:first_section_only` / `:none` の 3 値で判定し、章扉保護が効く最初の節の前だけを正規化する。image スタイルの判定は `theme.style` ではなく **body クラス（`vs-header-image`）**で行う——付録は `theme.style: image` でも常に `vs-header-simple` なので、CSS が実際に見る印と同じものを見るのが唯一ずれない。
  - 是正はすべて `false` のときだけ `book-settings.css` へ出す追加規則で、**既定（`true`）の組版はバイト単位で不変**。上書きは `image-header.css` の「固定 150px 行＋節リード 16mm 押し下げ」を後勝ちで打ち消す前提に立つため、その前提が消えたら気づけるよう回帰テストで固定した。

- [Medium] **`vs metrics` の漢字レベル判定が、2020 年度施行より前の学年別漢字配当表を使っていたのを更新した**（`furigana-level-spec.md` §4）: 当初の同梱データは kanjiapi.dev（KANJIDIC2 由来）から取得したもので、**2017 年 3 月告示・2020 年度施行の改訂が反映されていなかった**（kanjiapi.dev は現在も未反映で、`茨` を `grade: 8` と返す）。都道府県名の 20 字（井・佐・埼・奈・媛・岐・岡・崎・栃・梨・沖・滋・潟・熊・縄・茨・阜・阪・香・鹿）が「中学漢字（L1）」に分類されており、小学生向けの本で**本来ルビの要らない漢字がルビ候補に挙がって**いた。文化庁「常用漢字表」と文部科学省「学年別漢字配当表」に基づくデータへ差し替え、公式の字数（1 年 80 / 2 年 160 / 3 年 200 / 4 年 202 / 5 年 193 / 6 年 191・中学以降 1110・常用漢字 2136）と一致することを 2 つの独立した出典の突き合わせで検算した。**収録字は 2140 字のまま不変**で、変わったのは 57 字の学年ラベルのみ（中学→小4 が 20 字、学年間の移動が 37 字）。常用漢字表の外にある簡易慣用字体 剥・叱・填・頬 の 4 字は、実際の原稿で使われるのはこちらのため従来どおり併載している（外すと符号位置判定に落ちて「一般漢字(L2)」に誤分類される）。
- [Low] **HTML タグの属性値に書いた URL を「裸 URL」として警告しないようにした**: `scan_bare_urls` はインラインコードしか除外していなかったため、原稿に直書きした `<a href="https://…">` や、前処理が生成する `<img alt="URL">`（`@qr`）の URL まで 🟡 の対象になっていた。タグ（`<…>`）の中は本文への直書きではないので走査対象から外した。
- [Low] **存在しないコマンドを打ったとき、主要コマンド一覧を表示するようにした**: `vs hogehoge` のような未知のコマンドでは `RootCommand` の `nested` が既定の `help` を選ぶため、`help [-h/--help] / Vivlio Starter の主要コマンド一覧を表示します / [-h/--help] ヘルプを表示` という **help 自身の usage** が出ていた。打ち間違えた利用者にとって何の手がかりにもならないため、`vs --help` と同じ主要コマンド一覧を出すようにした。既知コマンドのオプション誤り（`vs build --unknown-option`）は、そのコマンドの使い方を見せたいので従来どおり。終了コードは POSIX 慣習どおり 1 のまま。
- [High] **`vs rename` が変更予定を見せないまま実行確認を求めていた問題を修正**: 一括連番（`vs rename` 引数なし）は**全章の番号が動く**操作なのに、既定では `❓ 連番付け直しを実行しますか？ [y/N]:` の 1 行しか出ず、**何がどう変わるか分からないまま y/N に答える**状態だった（変更予定の一覧はコード上は存在したが `log_info`＝既定では非表示だったため）。単章指定（`vs rename 71 72`）も同様。実行予定を `log_always` へ格上げし、既定ログレベルで必ず表示するようにした。**変更のない章も含めて全件並べる**——`--step=2` のような飛び番指定でも「2 つ置きの連番になる」ことが一目で読み取れるため。実行前に予定が出るので dry-run も兼ねる。あわせて `🔴 N章のファイルが複数見つかりました:` のあと**どのファイルが競合したかが見えなかった**問題も是正した（`log_error(detail:)` へ移動）。`vs lint --register` の `ユーザー辞書へ N 語を登録しました` も既定で見えていなかったため `log_result` へ格上げした。
- [Low] **出力される機会のない `log_info` 群を削除**（`cli-argument-parsing-spec.md` §10.3）: `--log` を配らないコマンド（D3）では `log_info` 系が永久に表示されないため、応答メッセージへの統合が済んだ 37 箇所を撤去した（`rename` 12・`delete` 6・`import` 18・`lint` 3）。**`clean` / `resize` / `create` / `index` / `pdf` の計 118 箇所は残している**——これらは `vs build --log` の実行中に実際に表示される（Step 0 の clean、Step 1 の resize、索引処理など）ため、削除すると `vs build --log=debug` の情報が失われる。「`--log` を持たないコマンドのコードだから未使用」とは限らない点を仕様書に明記した。あわせて `rename.rb` の dry-run 分岐 4 箇所を撤去——Samovar のオプション定義に `--dry-run` が無く `options[:dry_run]` は常に nil で到達不能だった（昔の実装の名残）。実行前に予定が表示されるようになったため機能としても不要になった。
- [Medium] **ログレベルの決め方を一本化した（`--log` を宣言しないコマンドで「エラーになるのに閾値だけ効く」捻れの解消）**（`cli-argument-parsing-spec.md` Part 2）: `Common.current_log_level` はログ 1 行ごとに `ARGV` をパターンマッチしており、Samovar のオプション宣言とは無関係に動いていた。そのため **`--log` を宣言していない 16 コマンド（`vs lint --log=debug` 等）は 🔴「Could not parse token」になるのに、エラーメッセージだけは debug 詳細で出る**という、原因の見当がつかない挙動になっていた（`options[:log_level]` は実質どこからも読まれておらず、宣言は事実上ヘルプ表示専用だった）。**解析済みコマンドの `--log` を唯一の入力とし、`CLI.start` で 1 回だけ解決してモジュールが保持する**方式へ改めた。これにより「`--log` を宣言しないコマンドでは単に無効なオプション」と一貫する。あわせて 2 点を是正——(1) **大文字の扱いを記法によらず統一**（従来は `--log=DEBUG` が debug になる一方 `--log DEBUG` は info に落ちていた）、(2) **不正な値を 🟡 で知らせる**（`--log=verbose` は黙って info になっていたためタイプミスに気づけなかった。指定できる値を添えて info で続行する）。状態は `module_function` の影響を受けないようモジュール自身に置いたので、include 経由の呼び出しでも同じ値を見る。副次的に **テストが `ARGV` を汚さずに `Common.log_level=` でレベルを制御できる**ようになった。`--log` を受け付けるのは従来どおり `build` / `preflight` / `new` の 3 コマンド——他コマンドの `log_info` は「どのキャッシュを消したか」といった内部トレースで著者向けの情報ではないため、⑤ command-feedback-spinner の応答メッセージへ委ねる。
- [High] **オプションを位置引数の前後どちらに置いても動くようにした**（`cli-argument-parsing-spec.md` Part 1）: `vs build 10 --no-clean` は通るのに **`vs lint 10 --fix` は 🔴** という、規則が推測できない状態を解消した。調査の結果、原因は Samovar の解析順（`Table#parse` は行を宣言順に 1 回ずつ処理し、`Options` は `input.first` が既知フラグである間しか消費しない）と、**コマンドごとに宣言順がバラバラだった**ことにあり、制約の向きが 3 種類に分かれていた——(A) 位置引数を先に宣言した 8 コマンド（build 等）は「オプションは後置きのみ」、(B) options を先に宣言した 9 コマンド（`lint` / `metrics` / `open` / `resize` / `cover` / `index:auto` 等）は**真逆で「位置引数は後置きのみ」**、(C) `one` を先に宣言した pdf 系 3 コマンドは `Samovar::One` の既定パターンが `//`（何にでもマッチ）のため**オプションを黙って食っていた**。`OptionTokenNormalizer` に「トークンをオプション群と位置引数群へ仕分け、そのコマンドの宣言順に合わせて並べ替える」処理を追加して解決した。並べ替えは Samovar へ渡す内部形式の話であり著者の打ち方は縛らない——どちらの順で打っても同じ形へ揃うため結果が一致する。寄せる向きは `table.merged` の行順から自動導出するため**宣言順を変える必要がなく、usage 表示も従来のまま**（pdf 系 3 コマンドのみ §5.3 のとおり是正）。あわせて**オプションが位置引数を挟んで分かれていても効く**ようになった（`vs lint --fix a.md --register` は従来 🔴）。綴り間違い（`--no-cleen`）と、値を取らないフラグへの `=` 付き指定（`--no-clean=1`）は従来どおり 🔴 のままとし、黙ってオプション扱いしない。**全公開コマンドについて「オプションを前後どちらに置いても解析結果が変わらない」ことを検証する契約テスト**を追加したので、新コマンドで宣言順を取り違えても検知できる。
- [Medium] **すべての公開コマンドで `--help` が使えるようにした**: `vs pdf:compress --help` / `vs pdf:pages --help` / `vs pdf:rasterize --help` は、`one` がオプションを食う問題（上記 (C)）により**ヘルプが表示されず**、`-h` が入力 PDF のファイル名として扱われていた。`pdf:compress` に至っては `options` 定義そのものが無かった。pdf 系 3 コマンドの宣言順を `options` 先へ是正して根治し、対症療法（食われた位置引数が `-h` かを判定する `help_requested?` / `help_flag_argument?`）を撤去して `options[:help]` へ統一した。公開 26 コマンドすべてで `--help` / `-h` が有効であることを契約テストで固定した（内部コマンド 4 個は対象外）。副次的に `vs pdf:pages --dpi 200`（入力 PDF 省略）も通るようになった。
- [Medium] **`vs preflight` の終了コードを画面の 🔴 と一致させた（裸 URL の警告だけで CI が落ちる食い違いの解消）**: 判定が `LinkImageValidator.any_issues?`（severity を見ず「レポートに issue があるか」だけ）だったため、**裸 URL の警告しかない場合でも 1** を返していた（「⚠️ Preflight 完了: 警告 N 件（ビルドは可能です）」と表示しながら CI は失敗、という食い違い）。判定を `IssueRegistry.counts.errors` へ寄せ、**🔴 が 1 件でもあれば 1／🟡 だけなら 0** に統一した。あわせて**危険スキーム（`file://` / `javascript:`）を 🟡 から 🔴 へ昇格**——ローカルファイル漏洩・スクリプト注入の第一防衛線であり CI を通してはならない指摘のため（逐次ログも `log_error` に変更）。副作用として、これまで終了コードに反映されていなかった 🔴（**ラベルID の重複・QueryStream の展開エラー**）も 1 を返すようになり、「画面の絵文字＝CI の成否」が例外なく成立する。
- [Medium] **索引・用語集スキャンの警告を章別サマリーに反映**: 索引系の 🟡 は `IssueRegistry` へ未ブリッジで、全章 `vs preflight` で死語や `vs index:auto` 未実施の案内が出ても「✅ 良好な状態です」と総括される食い違いがあった。ビルド経路で出る 3 種を `:index` カテゴリで積むようにした——(1) 用語集語が原稿のどこにも出現しない（死語・全章走査時のみ発火するため誤検知にならない・章に紐付かないので「章に紐付かない指摘」へ）、(2) 索引候補の抽出が未実施の章（**章ごとに**積むので章別サマリーの該当行に出る）、(3) 索引語辞書が無い。`vs build` でも `print_summary` 後の「ほか警告 N 件」に数えられる。実行範囲に由来する指摘（走査対象外の章のスキップ等）と環境・設定の指摘（MeCab 不在・辞書の読み込み失敗）は従来どおり積まない（`vs doctor` の領分）。
- [High] **索引スキャンが `\` を含むインラインコードの行を壊していた問題を修正**（原稿 3 章で実害）: 退避した内容を戻す際に `gsub!(token, original)` を使っていたため、置換文字列中の `` \` `` が Ruby の特殊参照（マッチ前の文字列）として解釈され、**行頭のテキストが重複して混入**していた。実際に `21-markdown-tutorial`（`` `\` `` が消える）・`61-developer`・`90-notation-cheatsheet`（表のセルに行頭が二重に入る）の 3 行が毎回のビルドで壊れていた。復元を必ずブロック形式（`gsub(token) { content }`）で行い、逐語で戻すようにした。回帰テスト付き（改修前のコードでは失敗することを確認済み）。
- [Medium] **索引スキャンを 8.3 倍高速化した**（実測 3.92s → 0.47s・全 28 章 481KB）: 行ごとに用語数（153 語）だけ「4 本の保護 gsub ＋ 用語パターンの `Regexp` 生成」を繰り返しており、地の文 7,421 行 × 153 語 = **113 万反復**がスキャン時間の大半を占めていた。(1) 用語パターンと読みは辞書由来で不変なので**初回だけ**組み立てて使い回す、(2) 退避（既にタグ付けされた要素・HTML タグ・振り仮名・インラインコード）は**行ごとに 1 回**だけ行い、生成したタグは即座に退避して後続の用語から隠す——という 2 点の整理による（新設 `LineMask`）。従来の「用語ごとに保護し直す」方式と遮蔽の効果は等価で、**全 28 章のタグ付け結果・`_index_matches.yml`（アンカーID・出現番号・初出判定）・用語集バックリンクが 1 バイトも変わらない**ことを黄金マスタ比較で確認した（唯一の差分が上記 `\` 破損 3 行の是正）。`vs build` の索引ステップは 5.32s → 0.95s、全章 `vs preflight` は 10.8s → 6.3s になった。
- [Medium] **`vs preflight <章>` で用語集警告が大量に出る問題を解消し、`vs build` と挙動を揃えた**（`preflight-glossary-warning-scope-report.md`）: 章を絞ると「🟡 用語集語がビルド対象章に出現しません」が最大 14 件出た直後に「✅ 良好な状態です」と表示されていた。原稿は正常で、**警告件数は対象章数に反比例する**（1 章→14 件 / 5 章→5 件 / 17 章→1 件 / 全 28 章→0 件）＝実行範囲の副作用だった。用語集の照合（R4）が「今回スキャンした章に出現したか」で判定するため、範囲を狭めるとほぼ全滅するのが原因。**preflight は「vs build が報告することを先に見る」機能なので、build が言わないことは言わない**という原則で揃えた: (1) `run_step4_index_processing` に走査範囲の判定（`full_catalog_scope?`）を足し、対象章が catalog 全章を覆っていなければ索引処理そのものを行わない——`vs build <章>`（single mode）が Step 4 を持たないことと一致し、誤検知は構造的に消える（スキップ理由は `--log` で表示。実行時間も 1.4s → 1.15s へ短縮）。(2) R4 自身にも同じガードを置き、`vs index:build <章>` など他の入口が同じ罠を踏まないようにした（catalog を読めない場合は従来どおり警告する）。(3) 出現先の案内を **`catalog 外の …` → `catalog 未登録の …`** へ修正——旧文言は「ビルド対象外」の集合を指していたため、部分実行では catalog **登録済み**の章を「catalog 外」と呼ぶ誤報になっていた（ガードにより、この分岐へ来るのは contents/ にあって catalog.yml に無い原稿だけと保証される）。
- [Medium] **`vs preflight` が索引の案内メッセージを握り潰していた問題を修正**: `IndexCommands.flush_post_build_messages` は `build_command.rb` からしか呼ばれておらず、preflight は一度も flush していなかった。そのため Step 4 が積む R7（`vs index:auto` 未実施の章の案内）と「索引語辞書が見つかりません」は**表示されないまま捨てられていた**（＝従来の preflight の索引処理は誤検知だけを見せ、有用な案内は捨てる状態だった）。preflight でも flush するようにし、build との非対称を解消した。
- [Medium] **Guard の 🟡 警告が `vs preflight` の最終判定に反映されない非対称を解消**: 最終行は `LinkImageValidator.any_issues?` **だけ**を見ていたため、`ContainerClassCheck` が未知クラスを 🟡 警告しても「✅ Preflight 完了: 良好な状態です」になる一方、裸 URL 警告は `link_issues` に積まれるため「課題あり」に転ぶ——同じ「警告」なのに最終判定への影響が発生源次第で変わっていた。`IssueRegistry` 経由で全発生源の警告を数えるようにし、Guard 警告のみの場合も `⚠️ Preflight 完了: 警告 1 件（ビルドは可能です）`（終了コード 0）と報告する。
- [Medium] **単章ビルドが `output.targets` 次第で無言終了し、PDF も開かなかった問題を修正**: 単章ビルドは `output.targets` によらず**常に閲覧用 PDF だけ**を生成する仕様（`register_single_mode_steps`）なのに、成果物の報告（`📚 … を作成しました`）と macOS の自動オープンだけは `output.targets` を見ていた。そのため `targets: kindle`（または `epub` 単独）の本で `vs build 11` を実行すると、`11-workflow.pdf` は正しく出来ているのに**完了メッセージも所要時間も出ず、PDF も開かない**（既定ログレベルでは「単章ビルドが完了しました」も info 扱いで非表示のため、成功したのかどうか分からない）状態だった。報告・オープンとも「実際に生成された PDF（`pipeline.generated_pdf_name`）が存在するか」で判断するよう変更し、フルビルドと同じ `📚 11-workflow.pdf を作成しました (31.3s)` が常に出るようにした。あわせて、単章では作られない成果物（`_compressed.pdf` / `_print.pdf` / `.epub`）を `output.targets` から推測して列挙していた死にコード（`get_created_files_list_for_single_mode`）を撤去。フルビルドの報告・オープン判定は従来どおり `output.targets` 依存で不変。
- [Medium] **`--opt=value` 記法を CLI 全体で使えるようにし、トークン正規化を 1 実装へ共通化**（`cli-option-parsing-report.md` §10）: Samovar は `--opt=value` を解さない（`Options#parse` がフラグ文字列の完全一致でしか引けず `=` を分割しない）ため、各コマンドが独自にトークンを正規化していた。対応していたのは `build` と `preflight` だけで、**`vs new mybook --log=debug` / `vs pdf:pages a.pdf --dpi=200` / `vs rename --step=2` は 🔴「Could not parse token」**になっていた（「`=` が使えるコマンドと使えないコマンドがある」という一貫性のない CLI）。正規化を新設モジュール `OptionTokenNormalizer` へ一本化し、`build` / `preflight` / `new` / `pdf:pages` / `pdf:rasterize` / `rename`（`renumber` は継承）の 6 コマンドへ `prepend` 1 行で展開した。**対象フラグはコマンドの `options` 定義から自動導出する**（`table.merged` を走査し、値プレースホルダを持つ `ValueFlag` のみを短縮形ごと拾う）ため、案 A の原案にあった「値を取るオプションの表」の手書きが不要になり、**今後オプションを追加しても `=` 記法が自動で効く**（オプション定義と正規化表の二重管理が構造的に消える）。`type: Integer` のオプションは正規化後に Samovar が型変換する（`--dpi=200` → `200`）。継承コマンドで親のオプションを拾うには `table` ではなく `table.merged` が要る点、正規化結果は新しい配列で返さず `input.replace` で元の配列へ書き戻す必要がある点（`Nested#parse` が親と同じ配列を子へ渡し、親は「空になったか」で未解釈トークンを検出するため）は、いずれも実測で確認してコメントに残した。調査報告 [cli-option-parsing-report.md](docs/archives/cli-option-parsing-report.md) の §5 の表は `rename --step/-s` を落としていたが、自動導出によりこの種の見落としは起こらなくなった。
- [Low] **`vs preflight --log` の直後に置いたオプションが黙って捨てられる問題を修正**: 値を伴わない `--log`（= info）のトークン正規化でインデックスを二重に進めていたため、`vs preflight --log --no-verify` の `--no-verify` が Samovar へ渡らず**検証が走ってしまっていた**（エラーも警告も出ないため気づけない。`--log=info --no-verify` と書けば効くという分かりにくい非対称）。同じバグは 2026-04-13 に `build_command.rb` から複製されたもので、build 側は 2026-07-21 に修正済み・preflight 側だけが残っていた。上記の共通化で複製実装ごと撤去して解消（回帰テスト付き）。
- [Low] **`vs new` の `--log` のオプションキーを `:log_level` へ統一**: `build` / `preflight` が `:log_level` なのに `new` だけ `:log` で、同じ `--log` の参照名が揃っていなかった。`NewCommand#debug?` と `new.rb` の `log_debug` の 2 箇所を追随させた（利用者から見た挙動は不変）。
- [High] **Kindle で本のテーマ（アクセント）色が出ない問題を根治（テーマ色のリテラル焼き込み）**: Kindle(KFX) は `var()`（CSS カスタムプロパティ）・`color-mix()` を一切解さないため、テーマ色（`var(--theme-accent)`）で塗っていた本文アクセント——太字強調 `strong`・強調下線・見出しマーカー・コラム/TIP/注記の枠・付録の章/節見出し——が Kindle では色指定ごと脱落して**黒**になったり、静的フォールバックの**グレー #888**・**くすんだ金 #b8860b** に固定されたりして、本のテーマ色がどこにも正しく出ていなかった（前書き/後書きの見出し下線・左罫・リンク色も同様に消失、または `preface.css` の静的 `#4f46e5` 固定で `preface_color` に非追従だった）。ビルド時に `book.yml` の `theme.color`・`appendix_color`・`preface_color` を**リテラル hex に解決**し、章 HTML で最後に読み込まれる生成 CSS `book-settings.css` へ `body.vs-kindle` 規則として焼き込むことで、静的フォールバックを後勝ちで上書きしテーマ色を復活させる。KFX 非対応の `color-mix()`（コラム地色の淡いテーマ色）も Ruby 側で事前計算してリテラル化。`body.vs-kindle` スコープのためクリーン EPUB（Kobo/Apple）・PDF は無変更。`theme.style` が `image`（付録が simple）でも `simple`（全章 simple）でも、本文・付録・前書き/後書きすべてでテーマ色が出る。`theme.color`・`appendix_color`・`preface_color` を変えれば Kindle のアクセントも追従する（固定色でない）。色解決は新設の純粋モジュール `VivlioStarter::CLI::ThemeColor`（色名/hex→リテラル hex・3桁展開・8桁 alpha 切詰・不正値の既定色フォールバックで CSS 注入耐性）に一元化し、`techbook` の色決定（旧 `THEME_COLOR_HEX`）も同モジュールへ集約した（挙動不変）。付録色未指定時は `appendix.css` の静的既定に合わせて yellow（PDF の実カスケードと一致）、前書き色未指定時はテーマ色へフォールバック（`--color-preface-accent` の既存挙動と一致）。前書き/後書きの規則は `preface.css`（後書きは `postface.css` 経由）を読む `body.preface` / `body.postface` にスコープする。あわせて `book_settings_css` の統合テストが作業用 `book.yml` の `theme.style: simple` で落ちていた脆弱性（テストが image 前提をハードコード）を、style で期待変数を条件化して修正。仕様書: [kindle-theme-color-literalize-spec.md](docs/specs/kindle-theme-color-literalize-spec.md)。
- [High] **Kindle でインライン数式のサイズがフォント変更に追従しない問題を根治（単純式のテキスト化）**: Kindle(KFX) はリフロー環境で画像を本文フォント相対サイズにできず、SVG 画像の数式は読者がフォントサイズを変えると相対的に極大/極小化していた（KNOWN_ISSUES の 2 件）。**Kindle フレーバ限定で、単純なインライン数式を SVG 画像から本文テキスト（`<i>`/`<sup>`/`<sub>`＋Unicode 記号）へ変換**し、フォントサイズに 100% 追従させるようにした。テキスト化は新設の純粋モジュール `MathTextRenderer`（`lib/vivlio_starter/cli/build/math_text_renderer.rb`）が担い、LaTeX の単純サブセット（ラテン文字＝イタリック・ギリシャ＝立体・上下付き・`\frac`＝`A/B`・`\text`/`\mathrm`＝立体・`\times`/`\approx`/`\langle` 等の記号・単位）を**ホワイトリスト方式**で変換する。1 トークンでも解釈できない式（`\sqrt`・`\sum`・積分・2 段入れ子の上下付き・入れ子分数など）は**式全体を無変換**とし、従来どおり SVG 画像＋px フォールバックのまま残す（部分変換はしない）。組み込みは `EpubBuilder#textify_simple_math_for_kindle!` が `alt` の元 LaTeX を読み `<span class="vs-math vs-math-text">` へ置換する形で、Kindle 専用フェーズの `convert_math_units_for_epub!` の**直前**に配置（残った複雑式に px フォールバックが従来どおり効く順序）。ディスプレイ数式（`$$…$$`）・クリーン EPUB・PDF は一切不変。原稿 `contents/` の現行インライン数式 26 種はすべてこのサブセットに収まり、表セル内の数式も同時に解決する。MathML は KDP のデバイス間表示が非一貫なため不採用。原稿 44 章に Kindle の数式表示の説明を追記。仕様書: [kindle-inline-math-textify-spec.md](docs/specs/kindle-inline-math-textify-spec.md)。
- [High] **EPUB/Kindle の章扉で右下の桜が次ページへ落ちる問題を根治（ファクシミリ合成）**: リフロー型 EPUB には「複数要素を同一ページに固定する」手段が無く、旧実装（原画を 62% で上下分割し、裾の桜を `img.vs-frontispiece-tail` として chapter-lead 直後へ注入）では、章扉の合計高さが 1 画面を超えると裾の桜だけが次ページへ送られて「白紙に桜だけ」という破綻を起こしていた（分割比をどう調整しても画面サイズ・読者フォント設定次第で再発する原理的な問題。Kindle KFX は `break-inside: avoid` も非対応）。**PDF と同じ縦長章扉（左上飾り→章番号→タイトル→リード文→右下飾り）をリード文まで含めて 1 枚の portrait 合成画像に焼き込む**方式へ全面変更し、分割不能な単一 `<img>` として h1 に配置することで、どの端末・フォント設定でも千切れない完全な章扉ページを実現した。焼き込み成功時は二重表示を避けるため HTML 側の `div.chapter-lead` を除去する（合成失敗時＝librsvg/ImageMagick 不在時は実テキストのまま simple 縮退）。リード全文は `<img alt>` と SVG `aria-label` に保持（読み上げ・検索・画像非表示時のフォールバック）。リード内のインライン数式（SVG）は alt の元 LaTeX をデリミタ除去して焼き込み＋著者向け警告。生成キャッシュのキーにレイアウト版数ソルト（`LAYOUT_VERSION`）とリード本文を混ぜ、`--no-clean` ビルドで旧 62% 帯画像を掴む事故を防止。h1 の再処理はクラス有無で冪等化（再入によるリード消失を防ぐ）。ラスタライズ幅は 1000→1400px へ引き上げ（リード文字の可読性）。旧 `frontispiece_tail` 系（定数 `FRONTISPIECE_SPLIT`・メソッド `frontispiece_tail_svg`/`inject_frontispiece_tail!`・CSS `img.vs-frontispiece-tail`・`svg_wrapper` の `view_y`）は全廃。PDF 経路・節絵（ornament）・前付/付録/後付・目次は不変。仕様書: [epub-frontispiece-facsimile-spec.md](docs/specs/epub-frontispiece-facsimile-spec.md)。
- [Low] **`:::{.showcase}` が「未知のコンテナクラス」と誤警告される問題を修正**: コンテナクラス検証（`Guards::ContainerClassCheck`）の許可リストは CSS のクラスセレクタから自動抽出するが、showcase は前処理の `ShowcaseTransformer` がブロックごと消費して `figure.vs-showcase` へ変換するため CSS に `.showcase` セレクタが存在せず、正しい原稿にもかかわらず preflight 警告が出ていた。前処理消費クラスの明示リスト `PREPROCESSED_CLASSES` に `showcase` を追加して解消。
...
- **`Metrics::LiveDisplay` を撤去した**: 章別リストを ANSI 制御でその場書き換えするライブ UI クラスだが、`Runner` は自前の逐次出力を持っており `LiveDisplay` を**一度も呼んでいなかった**（`require` すら無く、参照は自身のテストのみ）。実装（約 200 行）とテストを削除。あわせて `contents/32-metrics.md` の「ライブプレースホルダーを描画して解析完了次第リアルタイムで差し替える」「ANSI 制御でグラフ部分だけを差し替える」「サマリを章の解析前に表示する」という記述も、実際の挙動（章番号順の逐次出力 → 全体集計 → `--all` の参考資料）に合わせて書き直した。
...
- [Low] **`book.yml` の死蔵設定キー `metrics.sentence_length` / `metrics.clause_length` を削除**: 読解難度を建石式リーダビリティー（`readability`）へ刷新した際に、平均文長・平均節長を閾値判定する経路が失われ、これら 2 ブロックは `Metrics::ConfigLoader` からも一切読まれない死蔵設定になっていた（コメントは「これ未満: 短すぎて幼稚な印象」等と判定閾値のように書かれたままで誤解を招く／長文ワースト抽出は `runner.rb` の `LONG_SENTENCE_MIN = 80` ハードコード）。`config/book.yml` の「文構造の基準」節ごと撤去（scaffold へは `scripts/copy_to_scaffold.rb` で反映）。マニュアル `41-book-yml.md` の metrics 節も、プリセットが切り替えるのは `chapter`/`section` の分量のみで、語彙難度・語彙多様度・読解難度・`labels` はプリセット外の共通設定である旨に修正。
...
- **RC 前のビルド成果物不具合 6 件を修正** (`docs/specs/build-output-bugfix-spec.md`):
  ...
  - **(①) frontispiece/ornament 画像の中間生成物が `bundled/` に残存** (`lib/vivlio_starter/cli/pre_process/image_generator.rb`, `clean.rb`, `scripts/copy_to_scaffold.rb`, `stylesheets/images/bundled/`, `lib/project_scaffold/stylesheets/images/bundled/`): 残存していた `*_alpha*.webp` / `*_color*.webp` / `*_merged*.webp`（旧実装が webp 中間を生成していた時代の git 追跡済み遺物）を root/scaffold から削除（scaffold の最終バリアント `sakura_portrait/landscape.webp` も生成物のため追跡解除）。再発防止として `generate_variant_output` の中間 PNG を `Dir.mktmpdir` に隔離し（例外中断時も自動削除され `bundled/` を汚さない）、WebP 変換を `convert_to_webp` に抽出。`clean_bundled_variant_images` と `scripts/copy_to_scaffold.rb` の除去パターンに中間生成物（webp/png）を保険として追加。
