...
- **辞書どうしが逆を向いていないかを調べるスクリプトを置いた**（`scripts/dict_conflicts.rb`）。自前の `prh_*.yml` と上流の `technical-word-rules` を突き合わせ、「A → B」と「B → A」が同時に立っている組み合わせだけを挙げる。落ち着く先のある連鎖（「A → B」と「B → C」）は数えない——そこまで拾うと件数に埋もれて、本当に困るものが見えなくなる。`lint.trim_long_vowel` が落とす分と `BUILTIN_ALLOWLIST` で決着済みの分は分けて数える。辞書を足したときと、上流を更新したときに回す。
