...
## 既知の不具合（Known Issues）
- **frontispiece/ornament 画像の中間生成物が削除されず残存する** (`stylesheets/images/bundled/`): `sakura.webp` 等の元画像から `*_landscape.webp` / `*_portrait.webp` を生成する過程で、`*_alpha.webp` / `*_alpha_x2.webp` / `*_color_x2.webp`（`sakura_portrait_alpha.webp`, `sakura_portrait_alpha_x2.webp`, `sakura_portrait_color_x2.webp`, `sakura_landscape_alpha.webp` 等）といった中間ファイルがビルドパイプライン序盤で作成される。これらは最終成果物ではなく削除されるべきだが、現状クリーンアップされずに `bundled/` 配下へ残ってしまう。`clean.rb#clean_bundled_variant_images` の削除パターンは `*_portrait.webp` / `*_landscape.webp` の2種のみで中間生成物を網羅していない。後日デバッグ予定。
- target: print_pdfでビルドすると、生成されたpdfのファイルサイズが僅か140kBであり、titlepage, legalpage, colophon, colophon の4つのページのみである。本文が全く出力されていない。アウトラインは付与されているので、vivlio-starter-pdf の影響かも知れない。（原因は要調査）
- target: epubでビルドすると、生成されたepubに扉絵(frontispiece/ornament)が含まれていない。表紙画像cover.jpgが一回り小さいように見受けられる。また、pdf（A4出力で374ページ）と比べると分量が少ない（epub版は137ページ）。epub版にはcolophonが含まれていない。用語集や索引の同一ページ番号併合処理が行われていない。
- **frontispiece/ornament 画像の中間生成物が削除されず残存する** (`stylesheets/images/bundled/`): `sakura.webp` 等の元画像から `*_landscape.webp` / `*_portrait.webp` を生成する過程で、`*_alpha.webp` / `*_alpha_x2.webp` / `*_color_x2.webp`（`sakura_portrait_alpha.webp`, `sakura_portrait_alpha_x2.webp`, `sakura_portrait_color_x2.webp`, `sakura_landscape_alpha.webp` 等）といった中間ファイルがビルドパイプライン序盤で作成される。これらは最終成果物ではなく削除されるべきだが、現状クリーンアップされずに `bundled/` 配下へ残ってしまう。`clean.rb#clean_bundled_variant_images` の削除パターンは `*_portrait.webp` / `*_landscape.webp` の2種のみで中間生成物を網羅していない。後日デバッグ予定。
