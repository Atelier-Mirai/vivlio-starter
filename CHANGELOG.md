...
- **`vs lint --fix` と表示の食い違いを網羅的に洗い、残っていた 5 件を直した**。前の 5 件（下記）を受けて、解析パスで働く抑止を全部数え上げ（14 種）、修正パスに対応する守りがあるかを一つずつ実測した。
  ...
  - **相互参照のラベルを壊していた**。`@ruby-sample` が `@Ruby-sample` に、`@pageref:javascript-intro` が `@pageref:JavaScript-intro` になり参照先が見つからなくなる（出力例の外でも起きる）。ラベルは識別子で地の文ではないので、解析パスでは落とし、修正パスでは退避する。綴りはビルドの置換器（`CrossReferenceProcessor::ReferenceReplacer`）と**同じ定義**を使う——数式と同じ流儀で、別に書くと lint が守る範囲とビルドが参照として扱う範囲がずれる。メールアドレスはこの定義で除かれる。
