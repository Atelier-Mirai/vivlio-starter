...
- **開発者向けガイド（61 章）を書き直した**: 1,095 行・42,489 字 → 444 行・17,212 字。冒頭で「フォークして改造・拡張したい開発者向け」と宣言しながら、実態は全機能のカタログになっていた。**他章と重複する約 400 行**——コマンド別解説 6 節（12・23・31〜33・43〜45・51 章）、`book.yml` / `catalog.yml` の解説（41・23 章）、著者向け記法の再掲（22・24・25・33 章）——を各章へ委ね、「どこに何があるか」と「手を入れるならどこを触るか」に絞った。章内の重複（gem のディレクトリ構造と `vs new` が作る構造が別々に「ディレクトリ構造／構成」として並ぶ、バージョン表が 2 箇所）も解消した。
  ...
  - **開発者が最初に踏むのに書かれていなかったもの**を追加: `docs/specs/NOTES.md`（症状から引く知見メモの索引）、`copy_to_scaffold.rb` によるスキャフォールド同期（ルートが正本）、PDF プロバイダの MIT / AGPL 二系統、記法・設定キー・ビルドステップそれぞれの拡張手順。
