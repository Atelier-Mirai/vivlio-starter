...
- **[Medium] 目次：節数が多い章（例：開発者向けガイド）で目次が次ページに押し出される**: `li.toc-chapter` が `display: flex` + `flex-wrap: wrap` のため、Vivliostyle の `break-inside: auto` / `break-before: auto` が効きにくく、章ブロック全体が1ページに収まらない場合に次ページへ押し出される。`display: flex` を維持したまま解決するのは困難で、`display: block` への変更と番号欄レイアウトの再設計が必要。
- **[Medium] 目次：「はじめに」（`toc-chapter-no-number`）の直後で改ページされる**: 目次の `h2`（「目次」タイトル）の高さが大きく、「はじめに」エントリが次ページに押し出される。`h2` の `block-size: 2lh` を縮小するか、`li.toc-chapter-no-number` の前余白を調整することで改善できる可能性がある。
...
### Added
- **`post_replace_list.yml` 専用スタイルシート `stylesheets/replace-list.css` を新設** (`stylesheets/replace-list.css`, `stylesheets/chapter.css`, `stylesheets/appendix.css`, `stylesheets/preface.css`, `lib/project_scaffold/stylesheets/`): `post_replace_list.yml` の置換ルールが付与する「隠れクラス」専用のスタイルシートを標準 CSS として新設。`.codered-right`（二重矢印コメント右寄せ赤）、`.hen-comment`（編集者コメント）、`.kaiwa` / `.kaiwa.sense` / `.kaiwa.deshi`（会話文）、`li.aokome`（青コメ ▶）、`li.akakome`（赤コメ ❶❷❸）のスタイルを定義。`chapter.css` / `appendix.css` / `preface.css` から `@import` されるため、著者の `custom.css` に依存せず標準でスタイルが適用される。プロジェクトスキャフォールド（`vs new`）にも同期し、新規プロジェクトにも自動配置される。`stylesheets/_README.md` も更新。
