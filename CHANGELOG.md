...
### Added
- [Medium] **既存プロジェクトのアップグレード専用コマンド `vs upgrade` を新設**: gem 更新で改良された雛形（CSS・テンプレート等）を既存プロジェクトへ取り込めるようになった。`vs new` が展開時に生成する雛形マニフェスト `config/scaffold.lock`（展開時点の雛形ハッシュ・自動生成・要 git コミット）との三者比較（雛形旧版/新版/プロジェクト現物）で各ファイルを「追加／更新（未カスタム→自動適用可）／競合（雛形も著者も変更→diff 提示・y/n/d の個別確認）／保持／最新」に分類し、計画表を提示してから適用する。`--dry-run`（計画表示のみ・一切書き込まない）・`--yes`（競合以外を確認なしで適用）。上書き前に必ず `.cache/vs/upgrade-backup/<timestamp>/` へ元ファイルをツリー構造で退避。**著者データ領域（`contents/`・`images/`・`covers/`・`codes/`・`data/`・`book.yml`・`catalog.yml`・索引/用語集辞書・`user_words.txt`・`textlint_allowlist.yml`）はハッシュ判定以前に一律対象外**で、著者の辞書が雛形サンプルで置き換わる事故を構造的に排除（辞書が存在しない場合のみ空の辞書を用意する）。lock のない旧プロジェクトは現物と雛形の直接比較にフォールバックし、差分ありは安全側に倒してすべて競合扱い（適用後は lock が記録され次回から通常動作）。diff は外部 `diff` コマンド非依存の自作 LCS（プレビュー 20 行・`[d]` で全文）。あわせて `vs new --add-missing` は非推奨化（警告表示・次リリースで削除予定）。原稿 13 章・README にアップグレード手順を追記。仕様書: [project-upgrade-command-spec.md](docs/archives/project-upgrade-command-spec.md)。
- [Medium] **索引候補の抽出が未実施の章をビルドが検知して案内**: `vs index:auto` が走査した章集合を辞書トップレベル `scanned_chapters:` へ和集合で記録し（contents/ に実在する章のみ・改名残骸は自動掃除）、ビルド Step 4 で「ビルド対象 − 走査済み」が非空ならビルド末尾に `🟡 索引候補の抽出が未実施の章があります: 71-appendix → vs index:auto を実行してください` を案内する。新章を catalog へ追加したとき、その章にしか出ない語が索引から黙って漏れる問題の恒久対策。`scanned_chapters` キーが無い旧辞書では判定しない（誤警告なし）。仕様書: [index-glossary-consistency-spec.md](docs/archives/index-glossary-consistency-spec.md)（R7）。
- [Medium] **ビルド対象章に出現しない用語集語の 🟡 警告**: 用語集生成時、今回のスキャンで 1 回も出現しなかった g 語を「catalog 外の 61-developer に出現」「原稿のどこにも出現しません（語の変更・削除？）」等の手掛かり付きで列挙する。掲載自体は維持——定義は書籍の語彙資産であり、外すか否かは著者が flags（`-g`）で決める（同仕様書 R4）。
- [High] **図解注釈記法 `:::{.showcase}` を新設（スクリーンショットへの枠・引き出し記号・丸数字を Markdown だけで）**: 画像編集ソフトを使わずに、画像へ「囲み枠（`rect`）」「ホームベース型の引き出し記号（`pointer`）」「丸数字バッジ」「トリミング（`crop`）」を重ねられるようにした。座標は**画像の解像度に依存しないパーミル系**（左上 (0,0)〜右下 (1000,1000)）で、実測しやすいよう `120px` 形式（元画像の実ピクセル）も混在可。`crop` は画像属性 `![](shot.png){width=100% crop="40px 30px 20px 10px"}`（CSS shorthand 準拠の 1/2/4 値）として書き、**座標系は crop 前の元画像で固定**されるため crop を後から調整しても全注釈の座標が生き残る。`{pos=right}`（バッジ位置）・`{dir=up}`（矢印の向き）・`{label="検索"}`・`border` / `background` / `color` / `font-size` を拡張属性で調整でき、`:番号` を省略すればバッジ無し、番号付き著者コメントは `<img alt>` へ「① コメント ②…」形式で集約される。実装は**前処理で「元画像＋注釈を焼き込んだ合成 SVG」を生成**し `<figure class="vs-showcase"><img>` へ置換する方式（数式 SVG 化と同型）——CSS の重ね合わせはリフロー型 EPUB（特に Kindle）が描画しないため、重ねを SVG 内で完結させて **PDF・EPUB・Kindle が単一の生成物を共有**する（ゆえに Kindle 向けの CSS 劣化対策が不要）。PDF は SVG のままベクタで鮮明に出し、EPUB/Kindle は対で焼いたラスターへ `EpubBuilder` が参照を差し替える（Kindle は SVG 内 base64 を非対応）。ラスター形式は**元画像のユニーク色数から自動判定**し、スクリーンショット（平坦色）は PNG・写真は JPEG とする——JPEG はスクリーンショットの文字端にリンギングを出すうえ PNG より大きく（実測 2600px 幅で PNG 6.7KB / JPEG 19KB）、逆に写真は PNG が可逆ゆえに数倍太る（同 PNG 3.0MB / JPEG 412KB）ため。参照先は `data-vs-raster` 属性で前処理が明示し、EpubBuilder は拡張子を推測しない（`--no-clean` ビルドで前回形式の残骸を拾わないため。属性は差し替え後に除去）。生成物は画像内容＋注釈のハッシュでキャッシュされ、撮り直せば自動再生成される。ImageMagick / librsvg 不在時は**注釈なしの通常画像へ縮退**し原稿はビルド可能なまま（`vs doctor` の案内対象）。原稿 `contents/22-extentions.md`（「図解注釈の方法」節を新設）・`contents/94-sample.md`（バイオリンの逸話に沿った作例）に追記。仕様書: [explanatory-diagram-spec.md](docs/archives/explanatory-diagram-spec.md)。
- [Medium] **コードインクルード範囲指定時の行番号を元ファイルの実行番号で表示**: `` ```include:prime.rb:22-25``` `` で取り込んだコードの行番号が、従来の `1` 始まりではなく元ファイルでの実際の行番号（`22, 23, 24, 25`）で表示されるようになった（PDF・クリーン EPUB・Kindle の全ターゲット対応）。開始行の情報は pre_process で消失していたため、フェンス情報文字列に GitHub パーマリンク風マーカー `` ```ruby:prime.rb#L22-L25 ``（終了行はクランプ後の実効値）を載せて VFM の figcaption 経由で post_process へ伝搬し、`PrismLinesCommands` が `pre[data-start]`＋インライン `counter-reset: linenumber N-1` へ変換する（figcaption の表示テキストはパスのみへ復元）。著者が直接 `` ```ruby:foo.rb#L5 `` と書く手書きマーカーも公開仕様（R9）。全文取り込み・救済不能フォールバックは従来どおり `1` 始まり。原稿 `contents/22-extentions.md`（行番号節・コードインクルード節）に追記。仕様書: [code-include-line-number-spec.md](docs/archives/code-include-line-number-spec.md)。

### Changed
- [High] **ビルドの索引スキャンを辞書読み取り専用化（用語集バックリンクの導出データ化）**: 辞書は「語彙の一次データ」だけを持ち、出現情報（頁番号・バックリンク・context）は原稿とビルド対象章から毎回導出する——という設計原則へ逸脱 2 箇所を寄せた。`IndexMatchScanner` が git 管理の `config/index_glossary_terms.yml` へ書き込んでいた `backlink_sources` を廃止し、スキャン中間ファイル `_index_matches.yml`（ワークスペース）の `glossary_backlinks:` セクションへ移動。`UnifiedPageBuilder` はそこからバックリンクを描画する。これにより **`vs build` は辞書を一切書き換えず**、catalog をトグルして印刷版・全章版を往復しても git 作業ツリーは汚れない。旧辞書に残置した `backlink_sources` はビルドでは読まれず（無害）、次の `index:auto`/`apply` 等の保存の機会に黙って消える。root/scaffold 同梱辞書からも除去済み。仕様書: [index-glossary-consistency-spec.md](docs/archives/index-glossary-consistency-spec.md)（R1〜R3）。
- [Medium] **`vs index:auto` が辞書へ書いた変更を既定ログレベルで必ず要約表示**: 従来は手動マークアップ登録・高スコア候補の自動承認がレビュー前に無言で辞書へ入っていた（既定レベル warn ではレポートが一切出ない）。`📝 辞書を更新しました: 手動マークアップ 2 語（特殊相対性理論, 重力）・自動承認 1 語（クロスリファレンス）` の形式で常時表示し、候補抽出の総括行（自動承認/推奨/一般の件数とレビューファイル案内）も既定レベルへ昇格。何も書かなかった実行では無言（無言＝無変更が成立）（同仕様書 R8）。
- [Medium] **EPUB/Kindle のコード行番号を「行ブロック＋ぶら下げインデント」方式（F 案）へ全面刷新**: Kindle の 2 列テーブル方式（KFX がセル `width`/`nowrap` を尊重せず、2 桁番号の縦折返し・行間不均一が是正不能）と、クリーン EPUB の「長行折返しで Prism ガターの番号がずれる」未解決問題を、同じ構造で同時に解消。`EpubBuilder` が `pre.line-numbers` を「1 論理行 = 1 `div.vs-code-line`」の容器 `div.vs-code-epub` へ変換し（両フレーバ共通フェーズへ移動）、折返しは `padding-left`＋負の `text-indent`（ぶら下げインデント）でコード開始位置へ揃える。行番号はクリーン EPUB では CSS カウンタ `::before`（選択・コピーに混入しない）、Kindle では nbsp 右詰めの実テキスト `span.vs-code-ln` を注入（`vs-adm-label` と同じ実体注入パターン・等幅で桁が揃い縦折返しが原理的に起きない）。範囲 include の `data-start` は容器へ引き継がれ両フレーバで開始値として消費される。`code.css` の Kindle テーブル CSS は F 案 CSS へ置換（要 `ruby copy_to_scaffold.rb` 同期）。PDF の Prism ガターは不変。仕様書: [epub-code-line-numbers-spec.md](docs/archives/epub-code-line-numbers-spec.md)。

### Fixed
- [High] **用語集の「幽霊バックリンク」——catalog から外した章への † 参照が印字され得た問題**: 今回のビルドで出現しなかった g 語の `backlink_sources` が前回ビルドの値のまま辞書に残り、用語集ページが存在しない章へのバックリンクを描画し得た（例: 61-developer を catalog からコメントアウトした印刷版）。バックリンクをビルド毎のスキャン結果から導出することで根治——除外ビルドでは空になり、catalog を戻せば完全復活する。仕様書: [index-glossary-consistency-spec.md](docs/archives/index-glossary-consistency-spec.md)（§2）。
- [Medium] **索引辞書の context（本文抜粋）が原稿推敲に追従せず stale 化していた問題**: `enrich_terms_with_context` が「context が空のときだけ再抽出」だったため、推敲済みの古い抜粋がレビュー画面に残り続けた（実測 15/724 件・報告書 [index-context-staleness-report.md](docs/archives/index-context-staleness-report.md)）。「現原稿に無い context を捨て、空になったら出現する複数章から補充（上限 3 章・候補抽出側と同数）」へ変更。stale 判定は全空白除去＋部分文字列一致の 1 実装（`context_live?`）に集約し、今回読み込んでいない章（catalog 外・部分実行 `vs index:auto 21-23` の対象外）を参照する context は判定せず温存する——印刷トグル運用を壊さない。レビュー md では catalog 外の章の抜粋に `（catalog 外）` を注記（表示のみ・apply のパースで剥がされ辞書へは戻らない）（同仕様書 R5/R6）。
- [Medium] **`[用語]`（読みなし）手動マークアップが単位・フラグ表記を索引語として誤登録していた問題**: 表セルの `[eV]` `[Hz]`（単位）・`[g]`（フラグ解説）が `vs index:auto` で辞書に入っていた——特に `g` は pattern `/\bg\b/` となり本文中の英字 g 全部にタグが付く。ASCII 可視文字のみ 2 文字以下の語は登録対象外とし、`🟡 [eV] は単位・記号表記とみなし索引登録しません（94-sample）→ 索引に載せる場合は読み付きで [eV|よみ] と書いてください` の形で章名つき警告を出す。同梱原稿の誤登録（`eV`・`Hz`）は棄却リストへ移動済み（`g` は棄却済みだった）。逃げ道（読み付き記法）は原稿 33 章に明記（同仕様書 R9）。
- [High] **`vs lint` が VFM 記法（機械データ）を日本語の文として誤検出していた問題（記法ガード機構の新設）**: `:::{.showcase}` の座標行を 1 文とみなして `sentence-length` が文長を数え、座標のカンマを `max-comma` が読点と数えるなど、showcase ブロックだけで 9 件・allowlist が肩代わりしていた分を含め 85 件規模の誤検出構造があった。従来は `config/textlint_allowlist.yml` の「VFM 記法」正規表現 5 エントリで抑え込んでいたが、allowlist は語彙除外リストであり記法の抑止は誤用（文脈を持てないため `sentence-length` 系は消せず、グローバルに効くため地の文への正当な指摘まで飲み込んでいた）。新設の `Lint::NotationGuard` が textlint へ渡す前の一時ファイル上で記法を中和する: showcase など機械データ・ブロックは全行空行化（宣言的な `MACHINE_DATA_CONTAINERS` へ 1 語足せば新記法も追従）、コンテナのマーカー行は空行化（内部の地の文は検査対象のまま）、ふりがな `{親文字|よみ}` は**親文字を残し**、クラス属性 `{.aki}` は除去。行数は必ず保存（指摘の行番号はずれない）、コード領域には触れない（フェンス内の記法解説例・`` `{.aki}` `` は不変）。スペルチェック側も同じガードを通り、`rect`/`pos` 等が未知語として湧く経路も閉じた。allowlist の VFM 5 エントリは撤去（撤去前後で指摘は完全同一＝ガードが完全置換。副次効果として allowlist が飲み込んでいた「。で終わらない段落」への正当な指摘 5 件が復活）。マーカー判定は「コロン＋属性ブレースで行全体が構成される行」に限定——`:::` で始まるだけの行（例: コメントアウトされた表の閉じ `:::-->`）まで空行化すると HTML コメントが未閉じになり textlint が暴走（CPU 99%）する実害を実装中に確認したため。仕様書: [lint-notation-guard-spec.md](docs/archives/lint-notation-guard-spec.md)、調査報告: [lint-notation-guard-report.md](docs/archives/lint-notation-guard-report.md)。
- [High] **`vs lint` が GC のタイミング次第で一部ファイルを黙って検査しないことがあった潜在バグ**: textlint へ渡す一時ファイルを `Tempfile.new` で作りパス文字列だけ保持していたため、Tempfile オブジェクトが GC されるとファイナライザが textlint 実行前にファイルを削除していた。textlint は存在しないパスを黙って無視するため、エラーも警告も出ずに「一部のファイルだけ検査されない」（実測では全章 25 ファイル中 20 ファイルが未検査・出力は正常風）。記法ガード導入によるメモリ割り当て増で GC が確実に発火するようになり発覚したが、従来も textlint 実行中に GC が走れば起こり得た。`Tempfile.create`（ファイナライザ登録なし・削除は後始末処理のみが担う）へ変更して根治。
- [High] **`vs lint --fix` が原稿を一切修正していなかった不具合（完全な no-op）**: `--fix` は vs-lint コメント変換用の**一時ファイルに対して**渡されており、textlint が直した内容は一時ファイルごと `ensure` で捨てられていた（原稿は 1 バイトも変わらないまま「そのうち N 箇所は自動修正可能です → `vs lint --fix`」と案内し続けていた）。`--fix` 指定時は「修正パス → 解析パス」の 2 段構成とし、修正パスが一時ファイルを textlint に直させたうえで、**変更があったファイルだけ** vs-lint コメント形へ逆変換して原稿へ書き戻すようにした（未変更ファイルには触れないので mtime も保たれる）。書き戻しは同一ディレクトリの一時ファイル＋`rename` によるアトミック置換で、原稿は「旧内容のまま」か「新内容へ完全置換済み」のどちらかにしかならない。textlint プロセス実行中の Ctrl+C で原稿が壊れない従来の性質（堅牢性テスト 5-6-2）は維持している。解析パスからは `--fix` を外した（付けても解析用の一時ファイルを直して捨てるだけのため）。仕様書: [lint-notation-guard-spec.md](docs/archives/lint-notation-guard-spec.md) §2。
- [Medium] **PDF の長い節見出しが飾り帯と版面を突き抜けていた不具合（自動折返し対応）**: `.section-topic h2` のタイトルが 1 行のまま帯からはみ出していた（pdf_section3-2 実測）。原因は 3 段重ね——(1) `white-space: nowrap`、(2) タイトル span が inline-flex で内側の匿名 flex アイテムが `min-width: auto` のため縮まない、(3) **`base.css` の見出し共通 `word-break: keep-all` が CJK 文字間の折返しまで禁止**（最後の 1 つが決定打で、CSS を正しく直しても折れない）。h2 の `nowrap` 廃止＋箱へ幅上限（`inline-size: 100%`）を与え、タイトルをブロック化して `word-break: normal` へ戻すことで、EPUB/Kindle の節絵と同様に長い節題が帯の中で自動折返しされるようにした（`text-wrap: balance` の継承で 2 行の行長も揃う）。短い節題の見た目は不変。
...
- [Medium] **Planned のガイド線マクロ（`@lu`/`@ld`/`@ru`/… 系の絶対配置＋SVG ガイド線）を完全廃止**: 正式サポートに至らないまま撤去し、`RESERVED_MACRO_POSITION_PREFIXES` と `.figure-guides` の痕跡を削除。**後継は図解注釈記法 [explanatory-diagram-spec.md](docs/archives/explanatory-diagram-spec.md)（`.showcase` 記法）**。
