...
- **前後に空きのある ` / ` を `slash-between-japanese` の対象から外した**。「直後に続くブロック要素（コードブロック / テーブル / 画像）」に「`・` か全角 `／` に」と指摘していたが、`・` に直すと**意味が変わる**。本書では「・」が「と」、「 / 」が「どれか一つ」を表していた——和文の並列は中黒が 544 件で、空きありのスラッシュは全件が「どれか一つ」の意味だった（上の例は直後に来るどれかが対象、`章番号 / 範囲` は引数がどちらかの形、`高品質 / 標準` はどちらかを選ぶ）。三つ並べて `・` にすると「全部が対象」と読める。
  ...
  残る案の全角 `／` との違いは字幅だけで、前後に空きがあれば半角でも間合いが取れる。この規則が欧文の `EPUB / Kindle` を見逃しているのと同じ理由である（詰めると一語に見える、の裏返し）。詰まって見えるのは空きのない `有効/無効` の形（片側だけ空いた形を含む）に限るので、そちらは引き続き指摘する。見出しも「並べるなら `・`、どれか一つなら前後に空きを入れた ` / ` か全角 `／` に」と、**意味で直し方が分かれる**ことを示す文言に改めた。同じ並びの中で `・` と `/` が混ざる形（`文字数・行数・文/節統計`）は表記の不統一なので、これも引き続き拾う。本書の指摘は 15 → 8 件。
