...
箇条書き・見出し・表・引用の項目も対象外です。項目に句点を付けないのは一般の作法ですし、`**独自の装飾を追加する**` のように段落全体が強調ひとつの行（小見出しとして組まれます）も同じ扱いです。`:::{.output}` や `:::{.terminal}` の中身も見ません——機械が出力した文字列に句点を求めても、著者には直しようがないためです。
...
```
📄 contents/95-further-inspiration.md  (日本語校正)
    4件  [prh] CSS組版 => CSS 組版
         行: 11, 13
```
