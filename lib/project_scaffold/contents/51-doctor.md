...
:::{.chapter-lead}
Vivlio Starter の動作には、いくつかの外部ツールが必要です。`vs doctor` を使うと、必要なツールがすべて揃っているかを一括で確認でき、不足していれば `--fix` オプションで自動インストールできます。すでに動いている環境をまとめて最新へ引き上げるのは `vs upgrade` の担当です。この章では、環境を整える二つのコマンドを続けて解説します。
:::
...
:::{.section-lead}
`vs doctor` は診断のみ、`vs doctor --fix` は診断＋自動インストールです。まず診断だけ実行して状況を確認するのがお勧めです。
:::
...
:::{.section-lead}
`vs upgrade` は、執筆環境をまとめて最新化するコマンドです。プロジェクトの直下で 1 回実行するだけで、本体・雛形・外部ツールの三つを順に更新します。
:::
...
「合流」は、あなたがスタイルシートの末尾に自分の規則を足し、雛形は先頭のほうを改良した——というときに起こります。**変えた場所が離れていれば、どちらか一方を捨てる必要はありません。** 同じ場所を変えていたときだけ「競合」として尋ねます。この場合は適用するとあなたの変更が失われるため、そう明示したうえで確認します（退避はされるので元に戻せます）。

合流には「前回追従したときの雛形」が要ります。これは古い版の gem（`gem update` は旧版を消しません）か、`.cache/vs/scaffold-base/` に控えた写しから調達します。どちらも見つからなければ、従来どおり「競合」として尋ねます。
...
`vs` コマンド自体は `Gemfile.lock` を見ずに動くので、この更新に失敗しても執筆は続けられます。その場合は 🟡 で手元での実行方法を案内します。
...
二つのコマンドは、医者にたとえると**`vs doctor` は健康診断、`vs upgrade` は治療・整備**です。答える問いが違います。
