...
:::{.chapter-lead}
`vs` のコマンドとオプションをまとめた早見表です。詳しい使い方と実行例は「解説章」列の章を参照してください。全コマンド共通の `-h` / `--help` は表から省いています。対象章を受け取るコマンドには、章番号（`21`）・範囲（`11-21`）・スラッグ（`21-markdown`）を指定できます。
:::
...
原稿（`.re`）を直接読み込んで変換します。Re:VIEW と Re:VIEW Starter の実行環境は不要です。取り込むのは `catalog.yml` に記載された章だけです。
...
`vs index:plan [対象章]`（省略時は全章。語数の目安と候補の分布を表示し、辞書やレビューファイルは変更しない）
...
`vs index:export [パス]` / `vs index:import [パス]`（省略時は `index_library.yml`。用語集の定義、棄却の判断、読みの補正を別の書籍へ引き継ぐ）
