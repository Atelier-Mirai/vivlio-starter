...
各行はもうひとつ、**単位**を持ちます。`:chapter` は章ごとに意味を持つ処理、`:whole_book` は全章そろってはじめて成立する処理です。

**単章ビルドはこの表から導出されます。** `register_single_mode_steps` が同じ表を走査し、`:chapter` の行だけを引いて単章の手順を組み立てます。モードごとの一覧は持ちません。

:::{.note}
かつては単章と全章で表が 2 つあり、どちらも手で維持されていました。片方への追加がもう片方へ届かないため「単章では出るのに全章では出ない」が繰り返し起き、そのたびに個別の補償コードが増えていきます（実際に 3 ファイル 5 箇所ありました）。単位を宣言する形にしたのは、この食い違いを**構造として起こらなくする**ためです。詳しくは `build-mode-parity-spec.md`。
:::
...
前の 3 つは `cli/pre_process/`、`MathTextRenderer` は `cli/build/` にあります。SVG の生成は `mathjax_to_svg.mjs` を呼び、結果は `GeneratedAssetCache` に載ります。
