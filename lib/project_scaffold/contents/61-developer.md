...
PDF が WebP を格納できないのは、ISO 32000 に該当するフィルタが無いためです。渡すと Chromium がデコードして Flate へ入れ直します。可逆圧縮では写真がほとんど縮まないため、**素材 280 KB が PDF 内で 1,963 KB になります**。JPEG なら `DCTDecode` でストリームがそのまま入り、素材のサイズがそのまま PDF のサイズになります。
