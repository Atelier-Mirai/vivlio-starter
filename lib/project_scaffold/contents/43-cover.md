...
:::{.chapter-lead}
`vs cover` は、カバー画像から PDF・印刷・EPUB 用の画像を生成するコマンドです。用意済みの `light` / `dark` テーマの SVG を使うことも、独自の SVG や PNG を使うこともできます。本章では、カバー画像の準備、生成、管理を説明します。
:::

`vs cover` は、一つのソース画像から用途別のカバー画像を生成します。PDF 閲覧用、商業印刷用、電子書籍用の画像を作れます。
...
基本の使い方は、引数を付けずに `vs cover` を実行することです。
...
- Photoshop などで CMYK プレビューを確認しながらデザインするのが理想的です
...
`vs cover` は、一つのマスター画像から用途別のカバー画像を生成します。同じデザインを保ったまま、各用途に合う画像を作れます。
