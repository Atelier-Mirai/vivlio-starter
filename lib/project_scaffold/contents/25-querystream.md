...
:::{.notice}
**画像ファイルの置き場所**：`cover:` などで指定した画像ファイル（例: `joyful_ruby.webp`）は、`data/` ではなく **その QueryStream 記法を書く章の画像ディレクトリ**に置きます。たとえば `25-querystream.md` で展開するなら `images/25-querystream/` 配下に画像を置いておく必要があります（通常の `![](画像)` 記法とまったく同じ扱いです）。
:::
