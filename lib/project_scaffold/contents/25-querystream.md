...
ここまでの流れを、実際のデータで試してみましょう。ここでは Ruby と C の入門書を `data/books.yml` に用意します（`title` / `author.name` / `desc` / `cover` を持つ配列です）。
...
あとは原稿に `= books` と 1 行書くだけです。

```markdown
= books
```

これだけで、全件がテンプレートで展開されます。実際の出力は次のようになります。{.aki}
...
スタイルを追加したい場合は、`templates/_book.my_style.md` を一つ作るだけで `= books | :my_style` が有効になります。
...
QueryStream は英語の複数形を単数形へ自動変換してテンプレート名を決めます。`books` → `_book.md` のような素直なケースはもちろん、末尾に数字やアンダースコアが付いていても単数形化されます。以下の対応表を目安にしてください。
...
データ展開機能は**設定より規約**の方針で設計されています。新しいデータ種別を追加するのに必要なのは、二つのファイルを置くだけです。
