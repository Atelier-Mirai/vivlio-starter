...
LaTeX の書き方を覚えなくても大丈夫です。**見たままの形で書けば、そのまま数式として組まれます**。上付き文字や根号は、日本語入力の変換や記号パレットから直接入力できます。
...
書ける表記の主なものを挙げます。右の列は同じ式を LaTeX で書いた場合で、**どちらで書いても同じ結果になります**。{.aki}

| 書きたいもの | 素のまま書く | LaTeX で書く |
| :--- | :--- | :--- |
| 上付き・下付き | ``x²`` ``10⁻³⁴`` ``H₂O`` | ``x^2`` ``10^{-34}`` ``H_2O`` |
| 冪乗 | ``2^(1/3)`` ``a^(p-1)`` | ``2^{1/3}`` ``a^{p-1}`` |
| 根号 | ``√2`` ``√(x+1)`` | ``\sqrt{2}`` ``\sqrt{x+1}`` |
| 総和 | ``Σx²`` ``Σ(k=0 to ∞)`` | ``\sum x^2`` ``\sum_{k=0}^{\infty}`` |
| 積分・極限 | ``∫[0, π/2]`` ``lim(n→∞)`` | ``\int_0^{\pi/2}`` ``\lim_{n \to \infty}`` |
| 関数 | ``sin(x)`` ``arctan(1/5)`` | ``\sin(x)`` ``\arctan(1/5)`` |
| 剰余 | ``a mod n`` | ``a \bmod n`` |
| ギリシャ文字 | ``π`` ``θ`` ``λ`` | ``\pi`` ``\theta`` ``\lambda`` |
| 記号 | ``×`` ``÷`` ``±`` ``≒`` ``≠`` ``≦`` | ``\times`` ``\div`` ``\pm`` … |
...
言語を書かないコードブロックも、中身が数式ならディスプレイ数式として組まれます。

:::{.note}
**判定は控えめです。** `puts` や `array[i]` のようなコードが数式になることはありません。

日本語を変数名に使った式は、**数式だとはっきりわかる手がかりがあるときだけ**拾います。
``平文^e mod n`` は肩の `^` があるので数式に、``距離 × tan(仰角)`` は関数 `tan` があるので数式になります。
一方 ``楕円の周の長さ = 4a × E(e)`` や ``行計 × 列計 ÷ 総計`` は手がかりが `×` `÷` だけなので、
コードのまま表示します。``A社 × B社`` のように**数式ではない文**も同じ形をしていて、
機械には見分けられないからです。

こうした式を数式として組みたいときは、次の「思ったとおりにならないとき」のように `$…$` で囲んでください。
:::
...
見た目は同じ等幅の書体なので、記法そのものを解説したいときはバッククォートを 2 つにしてください。
