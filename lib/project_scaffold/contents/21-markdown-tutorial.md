...
数式は、`$…$`（インライン）と `$$…$$`（別行立て）で囲んで書きます。ビルド時に画像化して埋め込むため、PDF でも EPUB・Kindle でも同じように表示されます。

:::{.note}
**「別行立て」は、行を改めて独立させる**という意味の日本語組版の用語です。数式に限らず、引用などにも使います。LaTeX の *display math* をそのまま訳した「ディスプレイ数式」も同じものを指します。本書では「別行立て」で通します。
:::
...
書ける表記の一覧は、巻末の「数式記法早見表」にまとめました。バッククォートで囲んだ数式の扱いと、インラインと別行立ての使い分けは、次章「便利な拡張記法」で解説します。
