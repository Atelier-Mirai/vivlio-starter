...
数式の基本（`$…$` と `$$…$$`、素の表記でも書けること）は前章で解説しました。ここでは**バッククォートで書いた数式**と、**インラインと別行立ての使い分け**を扱います。

### バッククォートで囲んだ数式も解釈します

数式の書き方を知る前は、``` ` ``` で囲んで数式を書いていた方も多いはずです。等幅の書体になって本文と区別が付くからです。**その原稿はそのままで構いません**——中身が数式であれば、数式として組みます。
...
言語を書かないコードブロックも、中身が数式なら別行立ての数式として組まれます。複数行なら、関係演算子の位置で揃います。

:::{.note}
**判定は控えめです。** `puts` や `array[i]` のようなコードが数式になることはありません。

日本語を変数名に使った式は、**数式だとはっきりわかる手がかりがあるときだけ**拾います。``平文^e mod n`` は肩の `^` があるので数式に、``距離 × tan(仰角)`` は関数 `tan` があるので数式になります。一方 ``楕円の周の長さ = 4a × E(e)`` や ``行計 × 列計 ÷ 総計`` は手がかりが `×` `÷` だけなので、コードのまま表示します。``A社 × B社`` のように**数式ではない文**も同じ形をしていて、機械には見分けられないからです。
:::
...
**総和・積分・大きな分数は、別行立て（`$$…$$`）で書いてください。** インラインに置くと、TeX の決まりで上下限が記号の横へ寄り、高さも圧縮されて読みにくくなります。

総和は $\sum_{i=1}^{n} i$、積分は $\int_{0}^{1} x^2 dx$。——インラインに置いた場合です。{.aki}

同じ式を別行立てにすると、上下限が記号の上下へ開きます。{.aki}
...
**別行立ての数式は画像のまま**です。総和や積分の組版を保つためで、行を占有するので崩れません。詳しくは巻末の「数式記法早見表」を参照してください。
:::
