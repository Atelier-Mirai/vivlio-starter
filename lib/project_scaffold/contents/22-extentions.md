...
本筋からは外れるけれど書き添えておきたいこと——細かい注意点や、知っておくと得をする背景——は、`:::{.small}` で囲むと本文より一段小さく、左右を内側へ寄せて組まれます。枠もラベルも付かないので、`.memo` や `.notice` ほど身構えずに読み流してもらえます。
...
1 段落だけのときは、`{.aki}` と同じように段落末へ `{.small}` と書くこともできます。
...
文字は小さくなりますが、**行送りは本文のまま保たれます**。版面の行グリッドから外れないため、隣のページと行の位置が揃ったままになります。{.aki}

### 脚注 — ページ下部への注記

本文の流れを止めずに補足を添えます。書き方は 2 通りあり、どちらも参照した箇所と同じページの下部に組まれます。

**参照形式**は `[^ラベル]` を本文に置き、定義を別の行に書きます。同じ脚注を本文中の複数箇所から参照でき、長い説明や出典を書くのに向きます。

**インライン形式**は `^[補足]` と書き、その場で内容を完結させます。定義を別に書く必要がないため、一度きりの短い注釈に向きます。

```markdown
Vivliostyle は CSS 組版エンジンです[^css]。
インライン形式なら定義は要りません^[その場で完結する短い注釈に向きます]。

[^css]: W3C の CSS 仕様に沿って、ブラウザの組版技術で本を組みます。
```
...
:::{.memo}
脚注番号は**章の中の出現順**に自動で振られます。参照形式のラベル（`[^css]` の `css`）は番号ではなく識別子なので、原稿の途中に脚注を挿しても付け直す必要はありません。
:::
...
:::{.memo}
`.output`・`.terminal`・`.diagram` の中の逐語ブロックには行番号が付きません（実行結果・端末・図は「コードの提示」ではないためです）。行番号がほしいソースコードは、これらの枠の外にコードブロックとして書いてください（言語名を付ければハイライトと行番号が付きます）。
:::
...
白背景のスクリーンショットや図版は、そのまま置くと紙面の余白に溶けて輪郭が分からなくなります。`{.bordered}` を付けると細い枠が付きます。

```markdown
![](screenshot.webp){.bordered width=60%}
```

キャプション（`** タイトル @ラベル **`）を付けた画像には**はじめから枠が付く**ので、`.bordered` が要るのは**キャプションのない画像**と、`:::{.sideimage-left}` / `:::{.sideimage-right}` の中に置いた画像です。囲みとして `:::{.bordered}` … `:::` で使うこともできます。枠の色は `--color-figure-border`（既定 `#ccc`）で変えられます。
...
図番号は表・リストにも同じ要領で付けられます。たとえば上で定義した図は、本文から参照すると @einstein-portrait のように図番号付きのリンクへ展開されます。自動ID・参照の書き方・重複チェックなど詳しい仕組みは「クロスリファレンス（相互参照）の使い方」の章で解説します。
...
座標系の模式図、構造の対応表、値の推移——スクリーンショットでは示せない図は、自分で描くことになります。**SVG で描いてください。** PNG と違い、拡大しても輪郭が崩れず、ファイルも軽く、あとから数値を書き換えて直せます。

Vivlio Starter は SVG をベクタのまま PDF と EPUB へ運びます。文字も画像に焼かず、**書籍の書体で組みます**——図の中に書いた `<text>` は、ビルド時にその字だけを抜き出したフォントが図自身へ埋め込まれるので、読者の環境に何が入っていても同じ字面で出ます。

:::{.note}
Kindle だけは事情が違います。KFX 形式が SVG を扱えないため、ビルドが PNG または JPEG へ焼きます。このとき書体は組んだ機械のものになるので、**図の書体を確かめるなら PDF か EPUB で見てください。**
:::
...
`viewBox` の数値そのものに意味はありません。効くのは **viewBox の幅と font-size の比**だけです。そこで「4 単位 = 1mm」になるよう viewBox の幅を判型に合わせておくと、以降の数値をすべてミリメートルで考えられます。
...
:::{.tip}
**ルートの `<svg>` に `width="100%"` を必ず書いてください。** 画像の CSS は `max-inline-size: 100%`——つまり上限であって指定幅ではありません。`viewBox` しか持たない SVG は本来の寸法を持たないため、ブラウザ既定の 300px（約 79mm）で組まれ、「160mm のつもりが半分の大きさで出る」ことになります。
:::

図を半分の幅で置くときは、原稿側で幅を絞ったうえで viewBox の幅も半分にします。原稿の指定を忘れると `width="100%"` が版面全幅に効いて、文字が倍の大きさで出ます。

```markdown
![キャプション](fig.svg){width=50%}
```
...
`sans-serif` のような汎用名しか書かなかった場合も、書籍の書体へビルドが寄せます——`sans-serif` は見出し書体、`serif` は本文書体、`monospace` はコード書体です。ただし**名指ししたほうが意図は残る**ので、迷ったら書いてください。

**太字は使えます。** Regular と Bold の両方が埋め込まれるので、`font-weight: 700` を指定しても紙面で細く出たり、にじんだ疑似太字になったりしません。

:::{.notice}
**外部フォントを参照する書き方は効きません。** `@import` も、相対パスの `@font-face` も、図の中では必ず失敗します。`<img>` から参照される SVG は独立した文書として扱われ、外のファイルを読めないためです。書体は名前で指定するだけにしてください——実体はビルドが埋め込みます。
:::
...
`.s3`（170）と `.area-pale`（208）は白地の上では薄く、細い線にすると印刷で飛びます。このふたつは**面か太い線にだけ**使ってください。線の上に文字を重ねるときは、`.plate`（白）を下に敷くと読めます。

確かめ方は簡単です。PDF を開いて、プレビューや印刷ダイアログで白黒表示にしてみてください。区別のつかない線があれば、色ではなく線種を変えます。
...
![パーミル座標系。画像の左上が (0, 0)、右下が (1000, 1000)。横は幅の千分率、縦は高さの千分率で、右へ・下へ増える。rect:1 530, 335, 175, 165 の枠を重ねてあり、起点だけでなく幅・高さも同じ千分率であることを示している](permille-coordinates.svg)
...
[mermaid](https://mermaid.js.org/) のコードフェンスで書いた図を、ビルド時に図へ変換します。標準の mermaid 記法をそのまま使い、独自記法はありません。**PDF はベクタ（印刷解像度で鮮明）、EPUB・Kindle はラスター**で書き出され、すべてのターゲットで同じ図が出ます。フローチャートやシーケンス図など、罫線図（`.diagram`）では手描きになりがちな図を、テキストだけで正確に描けます。
...
`mmdc` が対応する範囲の全ダイアグラム種別を受け付けます（flowchart / sequenceDiagram / classDiagram / stateDiagram / erDiagram / gantt / pie など）。図ソースの 1 行目に種別を書きます。書き方は mermaid 公式ドキュメントを参照してください。

### 見た目の調整

図中テキストのフォントは**本書の設定に自動で従います**（`config/book.yml` の見出しフォント）。図だけに特殊なフォントを使うことはできません。テーマ色は第一段階では**ライト固定**です。曲線の形など mermaid 固有の見た目は、図ソース側の `%%{init: {'flowchart': {'curve': 'stepBefore'}}}%%` で著者が制御します（Vivlio Starter は関与しません）。
...
:::{.notice}
`.text-right` は `text-align: right` のみを適用します。ブロックの幅は変わらないため、段落テキストや表のセルなど、幅を維持したまま文字だけ右寄せしたい場面に適しています。
:::

プレーンなテキストでは両者は同じに見えます（どちらも文字が右端に並ぶため）。違いがはっきり出るのは「枠の見える要素」や「短いブロック」に当てたときです。枠付きの `.column` に当てて見比べると、`.align-right` は**枠そのものが内容の幅に縮んで右端へ寄り**、`.text-right` は**枠が全幅のまま中の文字だけ右に揃う**のが分かります。{.aki}
...
対話形式のページは、`:::{.talk}` ブロックの中に**「キー: 発話」**を並べて書きます。

まず話者を `config/talk.yml` に定義します。色だけの簡易形と、表示名・アバター・左右を指定する詳細形の両方が使えます。
...
sensei:
  ...
  color: indigo        # テーマ色名 / HEX（省略時はテーマのアクセント色）
  avatar: sensei.webp  # images/characters/ 内のファイル名（省略可）
...
アバターの絵を用意するのが大変なときは、`avatar: auto` と書くと**話者色に名前の頭 1 文字を白抜きした簡易アバターが自動生成されます**。まずはこれで書き進めて、絵ができたらファイル名に差し替えられます。`display:` に `avatar: auto` と書けば、画像を指定していない話者すべてに適用されます。
...
表示は**`style`（形式）・`name`（話者名）・`avatar`（アバター）**の 3 つで決まります。本全体の既定は `config/talk.yml` の `display:` に書きます。
...
ブロックごとに上書きすることもできます。指定しなかった項目は `display:` の値を引き継ぎます。
...
数式について、``` ` ``` で囲まれた数式も**中身が数式**であれば、数式として組みます。
...
日本語を変数名に使った式は、**数式だとはっきりわかる手がかりがあるときのみ**拾います。``平文^e mod n`` は肩の `^` があるので数式に、``距離 × tan(仰角)`` は関数 `tan` があるので数式になります。一方 ``楕円の周の長さ = 4a × E(e)`` や ``行計 × 列計 ÷ 総計`` は手がかりが `×` `÷` だけなので、コードのまま表示します。``A社 × B社`` のように**数式ではない文**も同じ形をしていて、自動判定が困難な為です。
:::
...
自動判定には限界があります。次の 2 つのうち、数式として組まれるのは上だけです。{.aki}
...
上には `θ` と `²` という**数式にしか出てこない字**があります。一方、下は英字・数字・記号だけで `total = price - tax` と字面が同じです。数式かどうか、機械には判断できません。{.aki}

言語に `math` と書けば、しるしが無くても数式として組まれます。次の 3 つは**中身がまったく同じ**で、言語指定だけが違います。{.aki}
...
総和は $\sum_{i=1}^{n} i$、積分は $\int_{0}^{1} x^2 dx$。——インラインに置いた場合です。{.aki}
...
水平線の記法 `---` を 1 行置くと、その位置で改ページされます（前章「水平線（改ページ）」参照）。同じことを `@pagebreak` と書いても実現できます（両者は等価。どちらを使うかは好みです）。

さらに、印刷本でよく使う**「必ず右ページ（奇数ページ）から始める」**も指定できます。
...
| 記法 | 動作 |
| :--- | :--- |
| `@pagebreak` | その位置で改ページする（`---` と等価） |
| `@pagebreak:recto` | 次の内容を**右ページ（奇数）**から始める。必要なら空白ページが 1 枚入る |
| `@pagebreak:verso` | 次の内容を**左ページ（偶数）**から始める。同上 |

部扉や章扉を必ず右起こしにしたいときに使ってください。EPUB・Kindle はリフロー型で左右ページの概念が無いため、いずれも**単純な改ページ**へ自動的に劣化します。

引数を書き間違えると（`@pagebreak:rect` など）ビルド前チェックが 🔴 で指摘します。

なお、章・目次・部扉・付録・用語集・後書き・索引をどちら側の面から起こすかは、`config/book.yml` の `page.chapter_pagebreak` が**本全体をまとめて**決めます。値は `recto`・`any`・`verso` の 3 つで、記法の引数とちょうど同じ言葉です（既定は `recto` ＝右起こし）。原稿に書いた `@pagebreak:recto` / `:verso` は**この設定より優先されます**——本全体では面を問わないことにしておいて、この 1 箇所だけは右から、といった使い分けができます。
...
原稿の中に、`config/book.yml` の設定値やビルドした日付を差し込めます。奥付やコラムで「いつ時点の情報か」を明示するときに便利です。
