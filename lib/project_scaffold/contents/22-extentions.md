...
:::{.chapter-lead}
Vivlio Starter には、基本の Markdown を補う出版向けの拡張記法があります。本章では、コラム、注記、画像レイアウト、表、装飾など、よく使う記法をリファレンスとして説明します。相互参照・索引・データ展開は、それぞれの専用章で扱います。
:::
...
`:::` の開始行にクラス名を指定すると、対応するスタイルが適用されます。`:::` と `{` の間にはスペースを入れても構いません。
...
本文に添える細かな注意点や背景は、`:::{.small}` で囲めます。本文より一段小さく、左右を内側へ寄せて組まれます。枠とラベルが付かないため、`.memo` や `.notice` より控えめに示せます。
...
:::{.memo}
脚注番号は**章内の出現順**に付きます。参照形式のラベル（`[^css]` の `css`）は番号ではなく識別子なので、原稿の途中に脚注を挿しても書き直す必要はありません。
:::
...
中身は連続空白がそのまま保たれ、コードブロックのような行番号は付きません。桁を確実にそろえるため、**図はコードフェンスで囲んでください。**
...
操作手順を説明するときは、スクリーンショットの対象を示す必要があります。ボタンを枠で囲む、矢印で指す、丸数字を本文に対応付けるといった作業には、従来は画像編集ソフトを使っていました。画像を修正するたびに元データを探して編集し、書き出し直す必要があります。

`:::{.showcase}` は、注釈を**原稿に文字で書く**ための記法です。注釈は原稿とともにバージョン管理でき、数値を書き換えるだけで修正できます。元画像は変更しません。
...
この形式なら、画像を高解像度版に差し替えたり、ビルド時に画像を最適化したりしても、注釈の位置は変わりません。
...
:::{.tip}
`px` で実測して位置を決め、必要に応じてパーミルへ直せます。`px` のまま使うこともできます。
:::
...
`pointer` は、ホームベース型の記号で一点を示します。ボタン、アイコン、小さな部品のように、枠で囲むほどの広がりがない対象に使います。

```markdown
pointer:[番号] [X], [Y] {オプション} 著者コメント
```

指定する座標は記号の左上ではなく、**矢印の先端が指す点**です。記号本体は `dir` で指定した向きに従い、先端の反対側へ伸びます。対象に記号が重ならないよう、**対象の外側の境界**を指定してください。
...
**座標の基準は、切り落とす前の元画像のままです。** `crop` は表示する範囲を変えるだけで、座標系は変えません。注釈を書いた後に `crop` を追加・調整しても、**注釈の座標を書き直す必要はありません。**
...
図中のテキストには、**本書の設定に従うフォント**（`config/book.yml` の見出しフォント）を使います。図だけに別のフォントを指定することはできません。テーマ色は第一段階では**ライト固定**です。曲線の形など mermaid 固有の見た目は、図ソース側の `%%{init: {'flowchart': {'curve': 'stepBefore'}}}%%` で制御します。
...
長いテキストを、雑誌のような二段組（二列）で組みます（`column-count: 2` / `column-gap: 1.2em`）。各要素は `break-inside: avoid` により、段をまたいで分割されません。
...
:::{.note}
横結合や複数行ヘッダーを含む表だけが拡張変換の対象です。通常の表（GFM テーブル）は、従来どおり扱われます。
:::
...
倍率（`scale`）と垂直位置（`shift-y`）は、**版面の大きさと表の内容から算出**し、ページに収まるようにします。多くの場合、オプションを指定せずに組めます。
...
アバター画像を用意しない場合は、`avatar: auto` と書きます。**話者色に名前の先頭一文字を白抜きした簡易アバターが生成されます。** 後から画像のファイル名へ差し替えられます。`display:` に `avatar: auto` と書けば、画像を指定していない話者すべてに適用されます。
...
範囲を指定して取り込むと、行番号は `1` ではなく**元ファイルの行番号**（この例では `10, 11, …, 20`）で表示されます。紙面から元ファイルの該当箇所を確認できます。ファイル全体を取り込む場合は、従来どおり `1` から始まります。
...
次の機能は、それぞれ専用の章で説明しています。ここでは、記法の概要だけを示します。
...
本章にない装飾は、`:::{.myclass}` の汎用コンテナ記法と `stylesheets/custom.css` を組み合わせて作れます。任意のクラス名でブロックを囲み、CSS で見た目を定義します。
...
`stylesheets/custom.css` に `.marker-box { background: #ffeb3b; }` のように定義すると、装飾が反映されます。CSS で定義した時点でクラスは使えるため、別の登録は要りません。ビルド前チェックは `stylesheets/` からクラス名を集め、CSS にないクラス名を使ったときだけ警告します。
:::
