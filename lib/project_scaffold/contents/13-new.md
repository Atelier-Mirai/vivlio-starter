...
:::{.chapter-lead}
クイックスタートで `vs new` をひと通り体験した方向けの詳細リファレンスです。`--yes` や `--force` などのオプション、ディレクトリ構成の意味やプロジェクト名のルールなどを確認したくなったときに戻ってきてください。急いでいる方は読み飛ばして次の章へ進んでも構いません。
:::
...
`codes/sample.rb`のソースコードを全文取り込んで表示するには次のように書きます。

```markdown
```include:sample.rb```
```

`codes/ruby/hello.rb`のソースコードの10-20行目を取り込むことも可能です。
