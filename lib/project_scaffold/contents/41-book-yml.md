...
:::{.chapter-lead}
`config/book.yml` は Vivlio Starter プロジェクトの中心となる設定ファイルです。書籍情報、テーマ、出力形式、各機能の詳細な設定を、ここでまとめて管理します。

本章では、設定項目を「**必ず設定する**」「**必要に応じて調整する**」「**機能別の詳細設定**」の三層に分けて説明します。
:::
...
`airy` は行間を広く取るレイアウト、`compact` は文字を多く組むレイアウトです。`custom` を選ぶと、`page_presets.yml` で独自の版面を定義できます。

#### `section_pagebreak` — 節で改ページするか

節見出し（`##`）でページを改めるかどうかの選択です。既定の `true` では節ごとに新しいページが始まります。`false` にすると節が本文の流れの中に続けて組まれ、ページ数を抑えられます。ページ数の上限が決まっている配布資料や、章あたりの節が多い薄い本で有効です。

`false` にしても**章扉（章の 1 ページ目）はそのまま独立したページとして残ります**。`theme.style: image` の章扉はページ全面の扉絵で成り立っているため、そこへ本文や節見出しが流れ込むと絵柄と文字が重なってしまいます。最初の節だけは扉の次のページから始まり、二つ目以降の節が本文に続けて組まれます。

#### `chapter_pagebreak` — 章をどちら側の面から始めるか

章・目次・部扉・付録・用語集・後書き・索引は、既定では**必ず右ページ（奇数）から始まります**。製本した本らしい体裁になる代わりに、前の章が奇数ページで終わると白紙が 1 枚入ります。これを「改丁」と呼びます。
...
`any` にすると白紙が消えます。本書を `recto` のまま組んだときは、**全体の約 5% が白紙**でした。ページ単価で刷る同人誌や、ページ数の上限がある配布資料では、この差が効いてきます。本書は `section_pagebreak: false` と併せて**ページ数が 1 割以上減りました**。
...
電子書籍は、配信先によって必要な形式が異なります。Vivlio Starter では、これらを別のターゲットとして扱います。
...
`pdf.techbook` は、絵文字をカラーの SVG 画像へ置き換えるモードです。Chromium の PDF エンジンは絵文字を Type 3 フォントとして埋め込むため、そのままでは印刷所へ入稿できません。既定で有効です。詳細は「ビルド（vs build）」を参照してください。
...
`pdf_preview` セクションでは、`vs build` 後に表示する PDF のウィンドウ位置を設定できます。デュアルモニター環境で、サブモニターに表示する場合などに使います。
