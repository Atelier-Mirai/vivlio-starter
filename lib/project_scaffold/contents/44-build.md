...
:::{.chapter-lead}
原稿を組版するには、`vs build` を実行します。閲覧用 PDF、印刷入稿用 PDF、電子書籍（EPUB）、Amazon Kindle 用ファイル（KPF）を、一つのコマンドで生成できます。

`vs build` は Vivlio Starter の中核となるコマンドです。原稿の前処理、画像の最適化、Markdown から HTML への変換、Vivliostyle による組版、PDF の結合、アウトラインの付与までを自動で実行します。
:::
...
`true` にすると、原稿のカラー絵文字を Twemoji の SVG 画像へ置き換え、入稿に適した PDF を出力します。**既定で有効**です。
...
印刷入稿用の表紙は本文とは別ファイルとして出力されます。Japan Color 2001 Coated の ICC プロファイルで CMYK に変換し、出力インテントを埋め込んだ PDF/X-1a:2001 として書き出します（`@vivliostyle/cli` 同梱の ICC を自動利用）。別のプロファイルを使いたいときだけ、パスを指定してください。
...
`targets: pdf, print_pdf` と指定すると、両方を一度にビルドできます。閲覧用 PDF と入稿用 PDF を同時に生成します。
:::
...
クリーン EPUB は WebP や SVG をそのまま格納できるため活かし、Kindle 版は端末での確実な表示を優先して画像形式やレイアウトを変換します。`targets: epub, kindle` と指定すれば、両方を一度に生成できます。

Kindle では、画像をリーダーのフォントサイズ変更に追従させられません。そのため、**インライン数式は Kindle 版だけ本文テキストへ変換して**埋め込みます。``E=mc^2`` のような式は上付き文字を使った整形済みテキストに、``√(GM/R)`` のように整形しきれない式は**入力した文字列**になります。どちらも画像ではないため、文字の大きさは本文に追従します。

ディスプレイ数式（`$$ … $$`）は Kindle でも画像のままです。総和や積分を含む大きな式を独立した行に組む、という書き手の意図をそのまま保つためで、行を占有する分、文字の大きさが変わっても崩れません。**複雑な式はディスプレイ数式で書く**のが、Kindle でもっとも安定します。
...
`.kpf` ファイルは Kindle Previewer 3 で開くと、実機に近いプレビューで表示を確認できます。KDP にアップロードする前に、扉絵・コード・表・数式などが意図どおりに表示されることを確認してください。
:::
...
執筆中は `vs preflight` でエラーを確認し、`vs build 21` のように章を絞って体裁を確認します。入稿前の仕上げで、書籍全体をビルドしてください。
...
:::{.column}
**使い分け**: 執筆中は `targets: pdf` で内容を確認し、入稿前に `targets: pdf, print_pdf` へ切り替えて入稿用 PDF を生成します。電子書籍も同時に生成する場合は、`targets: pdf, epub, kindle` を指定してください（クリーン EPUB と Kindle 用 KPF の両方が出力されます）。
:::
...
:::{.section-lead}
Vivlio Starter は、原稿内の特殊記号、絵文字、波ダッシュなどを出力時に処理します。印刷所でのフォントエラーを防ぐためです。
:::
...
- **インライン絵文字の画像化（Techbookモード）**:
  `config/book.yml` の設定で `techbook: true`（技術書典向けモード）を有効にしておくことで、文章中に書かれた一般的な絵文字を、印刷に適した高品質な画像（TwemojiのSVG画像）へと自動で変換します。画像化されることで、文字化けや表示崩れの心配もなく、すっきりときれいに表示されます。
  ※絵文字の画像素材に関するクレジット表記は、奥付に自動的に挿入されます。
- **図版の書体の埋め込み**:
  著者が `images/` に置いた SVG 図版は、そのままでは書体が解決されず、お使いのパソコンのフォントで組まれて Type 3 の原因になります。ビルド時に、図に出る字だけを抜き出したフォントを図そのものへ埋め込むので、書籍と同じ書体で、かつ入稿できる形で書き出されます。図はベクタのまま運ばれるため、拡大しても輪郭が崩れません。

<!-- no-lint -->
著者が特別な設定や画像を準備する必要はありません。原稿に書いた内容を、印刷に適した PDF として出力します。
...
Vivlio Starter は、表紙、本文、奥付を含むすべてのページの波ダッシュを「`〜`」へ統一します。環境による文字化けを防ぐためです。


## トラブルシューティング

:::{.section-lead}
ビルド時によくある問題とその解決方法をまとめます。エラーの原因が分からないときは、まずここを確認してください。
:::
...
**PDF は 2 回組まれます。** 索引や用語集のページ番号は、実際に組版してみるまで決まりません。そこで 1 回目（`build overall pdf`）でどの語が何ページに来るかを測り、番号を確定させてから 2 回目（`backlink dedup`）を組み直します。本書（26 章・346 ページ）ではそれぞれ約 2 分かかり、**合わせてビルド時間の 8 割以上**を占めます。スピナーがこの二つで長く止まるのは、そのためです。
...
原稿の組版と出力ファイルの処理は、`vs build` が行います。
