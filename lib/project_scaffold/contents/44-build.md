...
**PDF は 2 回組まれます。** 索引や用語集のページ番号は、実際に組版してみるまで決まりません。そこで 1 回目（`build overall pdf`）でどの語が何ページに来るかを測り、番号を確定させてから 2 回目（`backlink dedup`）を組み直します。本書（26 章・346 ページ）ではそれぞれ約 2 分かかり、**合わせてビルド時間の 8 割以上**を占めます。スピナーがこの 2 つで長く止まるのは、そのためです。
