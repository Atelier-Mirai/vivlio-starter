    ...
    module LintCommands
      ...
      class LintRunner
        ...
        BUILTIN_ALLOWLIST = [
          ...
          '/プレビューア/',
          '/レビューワーク/',
          # `リーダ([^ー]) => リーダー$1`（読み手・指導者の意）が「リーダビリティー」（可読性）の
          # 頭に当たる。別語なので、長音の文体をどちらに決めた本でも誤検出になる
          '/リーダビリティ/',
          # 「使うべきではない言葉」の項目。**`expected` が置換語ではなくメッセージ文**で、
          # spellcheck-tech-word はそれをそのまま修正テキストに使う。`--fix` を当てると
          # 「手酷い失敗」が「手使うべきではない言葉なので修正してください失敗」になる（実測）。
          #
          # 語の選び方の良し悪しという価値判断で、綴りの誤りでも表記ゆれでもない。文脈も
          # 見ないので「Rust は知らない」と「知らない記法」を区別できず、「常識的な判断を
          # すると」のようなごく普通の文まで叩く。全部まとめて黙らせる。
          # 誇大表現を見たいなら、`--fix` を持たない独自ルールとして実装するのが筋
          '/知らない|知らなさそう|常識的|酷い|最強|最も優れた/',
          # `チェーン => チェイン`。ツールチェーン・ブロックチェーンのように、技術書では
          # 「チェーン」が定着している（外来語の表記としても長音を添えるのが原則）。
          # 打ち消すだけでなく、逆向きの `チェイン => チェーン` を prh_web_technology.yml
          # に置いて正しい綴りを主張している。ここはその一方の口を塞ぐためのもの
          '/チェーン/',
          # `インタフェイス|インターフェイス|インターフェース|インターフェィス => インタフェース`。
          # **同梱の辞書どうしが逆を向いている**——prh_web_technology.yml は
          # `インターフェース|インタフェース => インターフェイス` と言う。直しても直しても
          # もう一方に叱られるので、どちらを採るにせよツール側で決着を付けるしかない。
          #
          # 採るのは prh 側（インターフェイス）。テクニカルコミュニケーター協会の
          # 「外来語（カタカナ）表記ガイドライン 第 3 版」が、長音は「ー」で書くという
          # 原則の**例外**として `インターフェイス（interface）[59.1%]` を名指しで挙げている
          # （数字は利用者アンケートの支持率。内閣告示第二号 2Ⅲ3 注 2 の「『エイ』と書く慣用が
          # ある場合はそれによる」に当たる）。上流の出どころは WEB+DB PRESS 編集部の
          # 用語統一ファイルで、JIS 寄りの雑誌のハウススタイル。
          #
          # JIS 表記で通す本は prh_web_technology.yml の項目を逆へ向ける
          '/インターフェイス/',
          # `\bRuby\b => Ruby`（大文字小文字を無視）。正規表現の `\b` はハイフンを語の
          # 切れ目とみなすので、`ruby-build`・`ruby-vips` のような**小文字が正しい名前**の
          # 頭に当たる。gem や Homebrew の formula には `ruby-` で始まるものが多く、
          # 語を 1 つずつ足しても追いつかないため、継ぎ目の形で書く。
          # 素の `ruby` は引き続き `Ruby` へ直すよう促される
          '/ruby-[a-z]/',
        ...
        #
        # 上流 22 項目のうち、末尾（-er / -or）の長音を足すものはこの 5 つ。`スタンドアロン` →
        # `スタンドアローン` や `パタン` → `パターン` のような語中の綴りの違いは、長音の文体の話では
        # ないので入れない。
        #
        # **語幹に長音を含む語を数え落とさないこと。** はじめは「見出し語に `ー` を含まないもの」で
        # 絞って 3 件としたが、`リーダ` `メーカ` はそれ自体が `ー` を含むため漏れていた（実測で発覚）。
        # 見出し語の綴り（先読みと捕獲を外したもの）に `ー` を足すと期待の綴りになるか、で数える。
