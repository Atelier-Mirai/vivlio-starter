  ...
  module CLI
    module PreProcessCommands
      ...
      module PlainMathTranspiler
        ...
        # それ以外は 1 かたまりを取る。
        #
        # **数字と英字は混ぜて取らない。** `H_2O` は LaTeX では「H の添字 2」に続く「O」で、
        # 混ぜて取ると `H_{2O}` になり O まで添字へ落ちる（実測: 本書の早見表で
        # `H₂O` が `H₂ₒ` と出た）。一方 `a^560` は 560 全体が指数でなければ
        # `a^5` の後ろに `60` が並んでしまう。数字の連なり／英字の連なりを
        # それぞれ 1 かたまりと見れば、どちらも正しくなる。
