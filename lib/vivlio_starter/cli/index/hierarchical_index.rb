...
#
# 【本番から未接続】このクラスを呼ぶ経路は現在ひとつも無い。実装済みで動くが、
# 使い道が来るのは index-main-reference-spec.md §R8（ページ範囲表記「12-15」・
# 親子項目）である。読む人が「索引の重複排除はここ」と誤解しないよう明記しておく:
#   - 同一ページの重複排除は BacklinkDeduplicator が PDF のページマップで行う
#     （HTML のファイル名でまとめる deduplicate_same_page! とは判定材料が違う）
#   - 索引ページの組み立ては UnifiedPageBuilder が @index_data から直接行う
# 消して R8 で書き直すより、動く実装を残して所在を書くほうが安いという判断。
#
# 提供する機能:
