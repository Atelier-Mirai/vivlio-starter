...
async function waitForRenderComplete(page, timeoutMs) {
  ...
  let previousCount = 0;
  let stableCount = 0;
  const STABLE_THRESHOLD = 5; // 5回連続で同数なら完了とみなす
  const POLL_INTERVAL = 500; // 500ms間隔
...
const CONTAINER_POLL_INTERVAL = 1000; // 1秒間隔
...
async function extractPageMapping() {
  ...
  try {
    ...
    // ページコンテナが出現するまでポーリング待機
    let renderTarget = await waitForRenderFrame(page);
    ...
    if (!renderTarget) {
      ...
      console.error(`[debug] body snippet:\n    ${bodySnippet}`);
