  ...
  module Starter
    module CLI
      ...
      module RenameCommands
        ...
        def included(base)
          base.class_eval do
            desc 'rename OLD NEW', '章のスラッグ/番号を変更します (Thor)'
            long_desc <<~DESC
              指定した章のスラッグ（名前）と、必要に応じて番号を変更します。
            ...
            def rename(old_arg, new_arg)
              ...
              if old_number != new_number && File.exist?(old_css)
                if File.exist?(new_css)
                  ...
                  Common.update_css_counter(new_css, new_number.to_i)
              ...
              Common.log_info("vs build #{new_number}-#{new_slug}")
            end
          end
        end
      end
