...
#   を解析し、各参照先 (href="#fn1") の直後に
#   <aside role="doc-footnote" class="page-footnote" id="fn1"> ... </aside>
#   を挿入。末尾の footnotes セクションは除去する。
def convert_endnotes_to_page_footnotes!(html)
  ...
  # 参照直後に脚注ノードを挿入（<p> 内では inline 要素を使う）
...
task :post_process do |t, _args|
  ...
  html_files.each do |html_file|
    ...
    if converted != content
      ...
      BookBuild.log_success("#{html_file}: 章末脚注をページ脚注に変換")
    end
  end
