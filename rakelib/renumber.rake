...
def renumber_single_chapter(old_arg, new_arg)
  ...
  if old_number == "98" || old_number == "99"
    BookBuild.log_error("#{old_number}はあとがきまたは奥付のため、変更できません")
    exit 1
  end
  
  if new_number == "98" || new_number == "99"
    BookBuild.log_error("#{new_number}はあとがきまたは奥付のため、変更先として使用できません")
    exit 1
  end
  
  BookBuild.log_action("章番号変更: #{old_number} → #{new_number}")
  ...
  if old_md_files.length > 1
    ...
    old_md_files.each { |f| BookBuild.log_info("- #{File.basename(f)}") }
    exit 1
  end
  ...
  if File.exist?(new_md_file)
    BookBuild.log_error("#{new_number}章のファイルが既に存在します: #{File.basename(new_md_file)}")
    exit 1
  end
  
  BookBuild.log_info("#{old_basename}.md → #{new_basename}.md")
  ...
  unless response == 'y' || response == 'yes'
    BookBuild.log_warn("章番号変更をキャンセルしました")  
    return
  end
  
  # 1. Markdownファイルのリネーム
  FileUtils.mv(old_md_file, new_md_file)
  BookBuild.log_success("Markdownファイル変更完了")
  ...
  if File.exist?(old_css)
    if File.exist?(new_css)
      BookBuild.log_warn("#{new_css} が既に存在するため、CSSファイルは手動で統合してください")
    else
      FileUtils.mv(old_css, new_css)
      update_css_counter(new_css, new_number.to_i)
      BookBuild.log_success("CSSファイル変更完了")
    end
  end
  ...
  old_img_dirs.each do |old_dir|
    if File.directory?(old_dir)
      ...
      if File.exist?(new_dir)
        BookBuild.log_warn("#{new_dir} が既に存在するため、画像ディレクトリは手動で統合してください")
      else
        FileUtils.mv(old_dir, new_dir)
        BookBuild.log_success("画像ディレクトリ変更完了: #{File.basename(new_dir)}")
      end
    end
  end
  ...
  old_generated_files.each do |file|
    if File.exist?(file)
      File.delete(file)
      BookBuild.log_info("#{file} を削除")
    end
  end
  
  BookBuild.log_success("章番号変更完了")
  BookBuild.log_info("変更を反映するには以下を実行してください:")
  BookBuild.log_info("rake build #{new_basename.split('-')[0]}-#{new_basename.split('-', 2)[1]}")
end
