...
- **数式は見たまま書ける** — `x²` `√(a+b)` `Σ` のように素のまま書けば、LaTeX を知らなくても綺麗に組まれます
- **テーマで簡単デザイン** — `book.yml` でアクセントカラーや扉絵を選ぶだけで、統一感あるデザインに
- **環境構築も自動** — `vs new` でプロジェクトを作成し、`vs doctor` で必要なツールを自動セットアップ

### 執筆を支える機能

- **文章校正（`vs lint`）** — 表記揺れ・冗長表現・文体の混在を検出。`だ円`（楕円）のような**交ぜ書き**も約 2,000 語の辞書で指摘し、`--fix` で漢字へ直せます。英単語の綴りも 50 以上の技術辞書で確認します
...
- **成果物は自動で軽くなる** — 出力ごとに適した画像形式へ変換し、紙面に対して過剰な解像度は自動で詰めます。著者が用意した `images/` は書き換えません
...
**主対象は macOS + Homebrew です。** 開発と検証はこの環境で行っており、`vs doctor --fix` による外部ツールの自動導入も Homebrew を前提としています。

Linux / Windows では**未検証**です。本体は Ruby と Node で書かれているので、[追加ツール](#追加ツールのインストールpdf-操作)を各環境のパッケージ管理（apt / dnf / winget など）で手動導入すれば動作する見込みですが、確認は取れていません。非 macOS で `vs doctor --fix` を実行すると、**自動導入は行わず、必要なツールの一覧を示して止まります**。手動導入の道しるべとしてお使いください。
...
- **画像最適化**: 出力ごとの派生を生成（PDF は JPEG・EPUB は WebP）。実効解像度を測って過剰な画像を縮小
- **クロスリファレンス**: 図・表・コードリストへの参照を自動解決
- **フロントマター生成**: book.yml の設定を各章に自動反映
- **ソースコード読み込み**: `codes/` からコードを埋め込み、行番号を付与
- **脚注変換**: 外部リンクをページ脚注に自動変換
- **数式の組版**: 素の表記（`x²`・`√n`・`Σ`）を TeX へ起こし、MathJax で SVG 化。Kindle ではインライン数式を本文テキストへ落として文字サイズに追従させます
