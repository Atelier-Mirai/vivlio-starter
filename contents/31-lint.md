...
:::{.section-lead}
書いたつもりと Markdown の解釈がずれる箇所を 3 つ見つけます。いずれもビルドは成功するので、紙面を見るまで気づけません。
:::
...
指摘するのは**読みを添えていない、半角 1〜2 文字**の綴りだけです。`[CSS]` や `[PDF]` のような 3 文字は、索引語として自然なので黙っています。直し方は 2 つあります。
...
:::{.notice}
3 つとも自動修正しません。`[g]` はコードだったのか索引語だったのか、`---` は改ページだったのか見出しだったのか——正解が二択で、決められるのは著者だけだからです。
:::
