...
すでに紹介した Flexbox（フレックスボックス）は、「1 本の軸（横一列、または縦一列）」に沿ってボックスを並べる仕組みで、従来のボックスモデルを発展させた **一次元レイアウト** です。

それに対して CSS グリッドレイアウトは、ページ全体に「行」と「列」からなる格子（グリッド）を先に用意し、その上に要素を配置していく **二次元のレイアウトシステム** です。
いわば「ボックスをどう並べるか」ではなく、「ページ全体の二次元テンプレート（設計図）をどう描くか」を決めるための技術と言えます。

このテキストでは、一次元・二次元のどちらのレイアウトであっても、原則として **CSS グリッドだけ** を使ってコードを書きます。Flexbox は「一次元レイアウトのための仕組みがある」という参考情報として名前を紹介するにとどめ、読者が両方の書き方を覚えなくてもよいようにしています。
