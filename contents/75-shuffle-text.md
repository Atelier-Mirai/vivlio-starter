...
[手軽にテキストシャッフル演出ができるJavaScriptライブラリ「shuffle-text」を公開](https://ics.media/entry/15498/) という記事がございます。
...
元記事は、`NPM` を利用して使う方法が説明されておりましたが、
https://github.com/ics-ikeda/shuffle-text で公開されている examples から、HTML と JavaScript を使った基本的な使い方をご紹介いたします。

マウスカーソルをのせると、文字がシャッフルされる演出がなされます。
[動作例](https://wave-improve.netlify.app/shuffle_text/index.html)と[ソースコード](https://github.com/Atelier-Mirai/wave-improve/tree/master/shuffle_text) です。ご参考になれば幸いです。

![作成例](shuffle_text.png)
...
`<li class="shuffle">01/01 「トップページ」更新</li>`と、**シャッフルしたい要素に `class="shuffle"` とクラス名を付与**する点がポイントです。
...
`.shuffle`クラスに、簡単に効果を適用するための `shuffle-text-custom.js` です。
