...
:::{.chapter-lead}
前章のクイックスタートで `vs new` をひととおり体験した方向けの詳細リファレンスです。`--yes` などのオプション、ディレクトリ構成の意味やプロジェクト名のルールなどを確認したくなったときに戻ってきてください。急いでいる方は読み飛ばして次の章へ進んでも構いません。
:::
...
入力した書籍名・著者名などは `config/book.yml` に自動的に書き込まれます。後から `book.yml` を直接編集して変更できます。
...
記述は `PREFACE`（前書き）・`CHAPTERS`（本文）・`APPENDICES`（付録）・`POSTFACE`（後書き）の4欄からなり、各章は拡張子 `.md` を省いたファイル名で登録します。`CHAPTERS` の下にタイトル（末尾に半角コロン `:`）を置くと、その配下が一つの「部」にまとまり、部扉ページが生成されます。
...
`--yes` オプションをつけると、すべての質問をスキップして既定値でプロジェクトを作成します。`book.yml` は後から自由に編集できるため、上級者にはこちらが快適です。
...
同名のディレクトリがすでに存在する場合はエラーになります。`vs new` は新しいプロジェクトを作るコマンドで、既存のプロジェクトには手を出しません。
...
`vs upgrade` は、本体 gem・プロジェクトの雛形・外部ツールをまとめて最新化するコマンドです。不足しているファイルの追加もここで行います。使い方は「環境の診断と更新」の章で解説します。
