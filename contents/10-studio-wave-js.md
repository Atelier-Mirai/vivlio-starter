...
:::{.chapter-lead}
この章では、Chapter 7・8 で作ってきた Wave Studio のトップページを題材に、実際のウェブサイトの中で JavaScript がどのように使われているかを見ていきます。

第9章では、変数・条件分岐・繰り返し・関数・DOM・イベントリスナといった文法を一通り学びました。この章では、その知識を使って「キラキラ光るキャッチコピー」と「先頭へ戻るボタン」という 2 つのしかけを読み解きます。

HTML・CSS・JavaScript がそれぞれどんな役割を持ち、どのように連携しているのかを、コードを追いながら確認していきましょう。
:::


## Wave Studio の JavaScript 概観

まずは `index.html` にどのように JavaScript が読み込まれているかを確認しましょう。
...
どちらにも `defer` 属性が付いています。`defer` は「HTML の読み込みが終わってから JavaScript を実行する」という指定でした。これにより、

- まず HTML と CSS でページの見た目を組み立て
- そのあとに JavaScript で「動き」や「インタラクション」を追加する
...
CSS は「shine クラスが付いたら光る」「各文字を個別にアニメーションできるようにしておく」ところまでを担当し、
**いつ shine クラスを付けるか・各文字のタイミングをどうずらすかは JavaScript 側が決めます。**


### JavaScript：テキストを 1 文字ずつ分解する
...
2. 各要素について：
   ...
   - 各文字ごとに、`<span style="animation-delay:〇〇s">文字</span>` の HTML を作る。
     - 文字の順番 `i` に応じて `animation-delay` を少しずつ増やし、
       一文字ずつ時間差で光り始めるようにしている。
   - 最後に `element.innerHTML = textbox` で、元のテキストを
     「たくさんの `<span>` に分解されたテキスト」に置き換える。
3. 最後に `addShineClassName()` を呼び出し、最初の状態を反映する。

これで、CSS 側に用意しておいた `.glow.text.shine span` のアニメーションと組み合わさり、
...
CSS と JavaScript は、この `#page_top` に対して「表示・非表示」と「アニメーション」を制御します。
...
- スクロール量が `200px` 以上なら：
  ...
  - → CSS 側の `upward-animation` が効いて、ボタンが下からふわっと現れる。
...
これを最初から自分だけでコーディングしても良いのですが、**AI に「こういう動きを作って」と頼んでしまう**のも 1 つのやり方です。大まかな仕様さえ伝えれば、AI が JavaScript と CSS のひな形を用意してくれます。
...
- **Vegas 風スライドショーを作る**
  ...
  > studio-wave プロジェクトの `work3` をコピーして `work4` を作りました。`work4/images` に `sea01.webp`〜`sea07.webp` という縦長の海の写真を置いてあります。これらを使って、トップページの `.hero` 要素に Vegas 風の全画面スライドショーを実装してください。jQuery ではなくバニラ JavaScript で、画像をフェードで切り替えるようにしてほしいです。

- **ヒーロー画像を画面いっぱいに表示する**

  > いま実装したスライドショーの画像は 1920×1440 の縦長画像です。ヘッダー部分を除いた画面全体を覆うように、CSS を調整してください。縦横比は保ったまま、上下左右に余白が出ないようにしたいです。

- **スクロール後に固定ヘッダーを表示する**

  > `header` と `nav` をまとめた `.site-header` という要素を作り、最初は非表示にしておきたいです。200px くらいスクロールしたら、画面上部に固定表示されて、フェード＋少し下にスライドして現れるようにしてください。JavaScript からスクロール量を見てクラスを付け替える実装をお願いします。
...
なお、この章で扱った Studio Wave の JavaScript / CSS は、実際のプロジェクトでは次のファイルに分かれています。興味があれば、エディタで開いてコードの全体像を眺めてみてください。
...
- Wave Studio のトップページでは、**HTML** が土台、**CSS** が見た目とアニメーション、**JavaScript** が「いつ・どのように動かすか」の制御を担当しています。
- `glow_text.js` では、キャッチコピーのテキストを 1 文字ずつ `span` に分解し、`animation-delay` を変えながら CSS アニメーションを適用することで、文字がキラキラ現れる表現を実現しています。
- `hero_slideshow.js` と `_hero.css` では、複数の海の画像を全画面で切り替えるスライドショーを実装し、画像サイズのリサイズや簡単な遅延読み込みで表示の軽さも両立させています。
- `page_top.js` と `_site_header.css` では、スクロール量に応じてクラスを付け替え、CSS のアニメーション（`upward` / `downward`）と `scroll-behavior: smooth` を組み合わせることで、「一定量スクロールしたら出てくる先頭へ戻るボタン」と「スクロール後に現れる固定ヘッダー」を実現しています。

今後、自分で JavaScript を書くときにも、

1. まず HTML で「フック」となる id / class を決める
2. 次に CSS で「どんな見た目・アニメーションになるか」を用意する
3. 最後に JavaScript で「いつ・どの状態でクラスを付け替えるか／中身を書き換えるか」を実装する
