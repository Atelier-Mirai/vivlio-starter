...
:::{.chapter-lead}
Vivlio Starter には、いくつかの外部ツールが必要です。`vs doctor` は必要なツールをまとめて確認し、不足分を `--fix` で導入できます。`vs upgrade` は、すでに使っている環境をまとめて更新します。この章では、この二つのコマンドを説明します。
:::

## vs doctor とは

:::{.section-lead}
`vs doctor` は、Vivlio Starter が使う外部ツールを診断するコマンドです。ビルドや lint が予期せず失敗したときは、最初に `vs doctor` を実行してください。
:::

新しい環境でプロジェクトを始めるときや、コマンドが予期せず失敗するときに実行してください。不足しているツールを一覧で確認できます。

なお、Vivlio Starter gem 自体のインストール手順や Ruby 環境の構築については、「インストール詳細」の章を参照してください。
...
`--fix` を付けて実行すると、不足している設定ファイルや辞書を scaffold（`vs new` の雛形）から復元します。読み込めない `book.yml` がある場合は、**書名・著者名など復元できる値を可能な限り取り出してから**初期状態のテンプレートへ書き戻します。元の破損ファイルはバックアップします。

## 基本的な使い方

:::{.section-lead}
`vs doctor` は診断のみを行い、`vs doctor --fix` は診断後に不足分を自動で導入します。初めは診断だけを実行して、状況を確認してください。
:::
...
診断後、不足しているツールを導入します。macOS では Homebrew を使います。Homebrew 自体や Xcode Command Line Tools が未導入の場合は、導入前に確認プロンプトを表示します。
...
`--yes`（または `-y`）は `--fix` と組み合わせて使うオプションです。Xcode Command Line Tools や Homebrew の導入確認を省き、自動で進めます。CI/CD 環境やセットアップスクリプトで使えます。
...
macOS では、ツールの種類に応じて三つの経路のいずれかで導入します。
...
npm 経由のツールには node が必要です。node が未導入なら Homebrew で先に導入し、その後に npm パッケージを導入します。
...
各ツールの詳細な導入方法と最新の手順は、各ツールの公式サイトと公式ドキュメントを参照してください。バージョンや手順は変更されることがあります。

## vs upgrade — 環境をまとめて最新化する

:::{.section-lead}
`vs upgrade` は、執筆環境を更新するコマンドです。プロジェクトの直下で実行すると、本体、雛形、外部ツールを順に更新します。
:::
...
2. **プロジェクトの雛形追従** — バージョンアップで改良された雛形（スタイルシート・テンプレートなど）を既存プロジェクトへ取り込み、仕上げに `Gemfile.lock` を新しい版へ合わせます
3. **外部ツールの更新** — Homebrew（formula / cask）・npm・gem で導入したツールを、更新計画の確認後に更新します
...
不足ツールは更新後の再診断で導入するため、`vs doctor --fix` を別途実行する必要はありません。
...
| 合流 | 雛形もあなたも変更・**別の場所** | 両方を残して自動で合流します（尋ねません） |
| 競合 | 雛形もあなたも**同じ場所**を変更 | diff を提示し、1 件ずつ確認します（y/n/d） |
| 保持 | 著者データ領域 | **絶対に触りません** |
...
「合流」は、著者がスタイルシートの末尾へ規則を追加し、雛形が先頭付近を改良した場合などに起こります。**変更箇所が離れていれば、両方の変更を残せます。** 同じ場所を変更していたときだけ「競合」として確認します。この場合は適用すると著者の変更が失われるため、そのことを示して確認します。適用前に退避するため、元に戻せます。

合流には、前回追従したときの雛形が必要です。古い版の gem（`gem update` は旧版を消しません）か、`.cache/vs/scaffold-base/` に保存した写しを使います。どちらも見つからなければ、「競合」として確認します。
...
雛形の追従が終わると、`Gemfile.lock` が指す本体の版を、いま動いている版へ合わせます（`bundle update vivlio-starter` に相当します）。lock だけが古い版を指したままだと、`bundle exec vs` や `bundle install` を使うときに「その版が見つかりません」と止まってしまうためです。すでに版が一致していれば何もしません。

`vs` コマンド自体は `Gemfile.lock` を見ずに動くため、この更新に失敗しても執筆は続けられます。その場合は 🟡 で実行方法を表示します。

:::{.note}
**`vs` が `Bundler::GemNotFound` で起動しないとき**

1.0.0 より前の版で作ったプロジェクトでは、`vs` を実行しても Ruby のエラーだけが出て止まることがあります。`Could not find vivlio-starter-… in locally installed gems` というものです。当時の `vs` が、プロジェクトの `Gemfile.lock` を読んで起動していたためです。そこに書かれた版が手元から消えていると（古い版を整理した・別の機材へ移した・その版の公開が取り下げられた）、起動そのものができません。同じ場所で止まるので、`vs upgrade` や `vs doctor` も使えません。

このときは、`vs` を介さずに本体を入れ直してください。

```bash
gem update vivlio-starter
```

1.0.0 以降の `vs` は `Gemfile.lock` を見ずに起動するので、一度これを通せば同じことは起こりません。あとは `vs upgrade` だけで最新化できます。
:::
...
二つのコマンドは目的が異なります。**`vs doctor` は状態を診断し、`vs upgrade` は環境を更新します。**
...
ビルドが失敗したときは、原因を確認せずに `vs upgrade` を実行しないでください。バージョンを更新すると問題の切り分けが難しくなることがあります。最初に `vs doctor` で不足や異常を確認してから対処します。
...
:::{.column}
**ヒント**  
`vs doctor` は何度実行しても構いません。ツールを手動で導入した後に再実行すると、認識されているか確認できます。
:::
