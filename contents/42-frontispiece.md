...
:::{.chapter-lead}
Vivlio Starter では、章の扉ページの背景画像（frontispiece）と、節見出しの装飾画像（ornament）を設定できます。画像を使うデザインと、画像を使わない簡潔なデザインを選べます。
:::

## frontispiece と ornament とは

:::{.section-lead}
frontispiece は章の扉ページに置く縦長の背景画像、ornament は節見出しに置く横長の装飾画像です。設定すると、書籍全体で画像の使い方をそろえられます。
:::

### frontispiece（扉絵）

frontispiece は、各章の扉ページ（章タイトルを表示するページ）の背景に置く画像です。縦長（portrait）の画像を使います。
...
ornament は、節見出し（`## 見出し`）の背景に置く装飾画像です。横長（landscape）の画像を使います。
...
画像は、独自に配置した画像を優先し、見つからない場合にバンドル画像を検索します。同じ名前の独自画像を置くと、バンドル画像を上書きできます。
...
章扉に載る「章番号・章題・リード文」をまとめて下へ移動します。扉絵は画像ごとに構図が異なるため、絵の余白と文字の位置をこの値で調整します。
...
3. **拡張子の不一致**: 画像ファイルの拡張子が `.webp`, `.png`, `.jpg`, `.jpeg` のいずれかになっているか確認
...
- frontispiece: 版面と同じ縦横比（A 判・B 判なら √2:1）の縦長画像を使用
...
:::{.section-lead}
`theme.style: simple` を設定すると、章扉や節見出しに背景画像を使わず、色とタイポグラフィを中心に組みます。
:::
