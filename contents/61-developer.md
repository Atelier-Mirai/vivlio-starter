...
| Step E | EPUB 生成（`epub` ターゲット時のみ。CSS サニタイズ・フォント非埋め込み・epubcheck 準拠調整を含む） |
| Step 14 | 最終クリーンアップ（中間生成物の削除） |

`output.targets` の設定によってステップの組み合わせが変わります。`pdf` のみ・`print_pdf` のみ・`epub` のみ・複数の組み合わせそれぞれで不要なステップがスキップされます。単章ビルド（`vs build 01-intro`）の場合は Step 6〜12 をスキップし、Step 5 で entries.js と PDF を直接生成します（最終クリーンは単章モードでも実行）。
...
`output.targets` に `epub` が含まれる場合、PDF 生成の完了後に `Build::EpubBuilder` が EPUB を生成します（実装は `cli/build/epub_builder.rb`）。PDF とは出力要件が異なるため、HTML/CSS に EPUB 専用の後処理を加えてから Vivliostyle CLI の EPUB 出力を呼び出し、生成後の `.epub`（ZIP）に対して再パッケージングを行います。
...
### 堅牢性テスト（`test/vivlio/starter/robustness/`）

通常のユニットテスト（`test/vivlio/starter/cli/**`）とは別に、「想定外の入力・環境・操作」に対する振る舞いを検証する回帰テストを `test/vivlio/starter/robustness/` 配下に集約しています。`docs/specs/vivlio_starter_robustness_test_spec.md` の 🔴 高優先度項目を Minitest 化したもので、1.0.0-alpha リリース時点で 18 項目すべてをカバーしています。
...
```bash
# 堅牢性テストのみ実行
bundle exec rake test TEST='test/vivlio/starter/robustness/**/*_test.rb'

# 個別テスト
bundle exec ruby -Ilib -Itest test/vivlio/starter/robustness/<filename>_test.rb
```
...
3. **回帰テストを `test/vivlio/starter/robustness/` に追加**: 仕様書 `docs/specs/vivlio_starter_robustness_test_spec.md` の項番を冒頭コメントに明記し、双方向リンクを維持
