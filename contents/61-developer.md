...
| Step 6 | 目次生成・ビルド用 entries / config 生成（`.cache/vs/build/pdf/` 内） |
...
`output.targets` の設定によってステップの組み合わせが変わります。`pdf` のみ・`print_pdf` のみ・`epub` のみ・`kindle` のみ・複数の組み合わせそれぞれで不要なステップがスキップされます。単章ビルド（`vs build 01-intro`）の場合は Step 6〜12 をスキップし、Step 5 で単章用の entries / config と PDF を直接生成します（最終クリーンは単章モードでも実行）。
...
8. `{.text-right}` 記法変換
9. book-card / テーブル変換（コンテナ `long-table` / `rotate-table` ＋拡張テーブル横取り）（`markdown_transformer.rb` / `table_converter.rb`）
10. リンク脚注化
...
- 通常の閲覧用 PDF（`PdfCommandRunner`）: ワークスペースの生成 config（`.cache/vs/build/pdf/vivliostyle.config.sections.js` 等）を `-c` で渡して `vivliostyle build` を実行
...
ビルドで生成された中間ファイル（ワークスペース `.cache/vs/build/` と、索引レビュー用などのルート作業ファイル）を削除します。最終 PDF はデフォルトでは保持されます。
