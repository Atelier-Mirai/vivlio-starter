...
:::{.chapter-lead}
クイックスタートで `vs new` をひと通り体験した方向けの詳細リファレンスです。`--yes` や `--add-missing` などのオプション、ディレクトリ構成の意味やプロジェクト名のルールなどを確認したくなったときに戻ってきてください。急いでいる方は読み飛ばして次の章へ進んでも構いません。
:::
...
### 既存ディレクトリへの不足ファイル追加（--add-missing）

同名のディレクトリがすでに存在する場合、通常はエラーになります。

```
エラー: ディレクトリ "mybook" はすでに存在します。
既存ディレクトリに不足ファイルだけを追加する場合は --add-missing オプションを指定してください（既存ファイルは保持されます）。
  vs new mybook --add-missing
```

`--add-missing` を付けると、既存ファイルはそのまま保持し、**不足しているファイルだけを追加**します（非破壊）。原稿や設定の編集内容が上書きされることはありません。逆に言うと、既存ファイルが新しい雛形で**更新された場合でも上書きはしません**。あくまで「足りないファイルの補完」専用と捉えてください。
...
Vivlio Starter のバージョンアップで新しいテンプレートやスタイルシートが追加された場合、`--add-missing` で既存プロジェクトに不足ファイルだけを補完できます。なお、既存ファイルの更新分は取り込まれない点にご注意ください。
:::

両方のオプションを同時に指定することもできます。

```bash
vs new mybook --yes --add-missing
```
