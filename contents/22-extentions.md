...
21 章の Markdown フレーバー系統図も、この mermaid 記法で描いています。
...
なお、章・目次・部扉・付録・用語集・後書き・索引は、**本全体の既定として右ページ始まりになっています**。この既定は `config/book.yml` の `page.chapter_pagebreak` で変えられます（`recto` / `any` / `verso` の 3 つで、記法の引数とちょうど同じ言葉です）。原稿に書いた `@pagebreak:recto` / `:verso` は**この設定より優先されます**——本全体では面を問わないことにしておいて、この 1 箇所だけは右から、といった使い分けができます。
...
ページ数を抑えたい薄い本などで、そもそも節を続けて組みたいときは `config/book.yml` の `page.section_pagebreak` を `false` にします（「book.yml 設定リファレンス」の章を参照）。
