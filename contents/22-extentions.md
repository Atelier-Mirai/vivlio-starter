...
単位を省略した `@vspace:10` は `10mm` と同じです。従来の `@posi:N`（＝ `@vspace:N`）／ `@nega:N`（＝ `@vspace:-N`）も同じ意味で使えます。
...
あとは `stylesheets/custom.css` に `.marker-yellow { background: #ffeb3b; }` のように定義すれば、原稿中で `@mark:重要@markend` と書くだけで蛍光ペン風の装飾が反映されます。ルールは上から順に適用されるため、既存ルールとの干渉に注意してください。
:::
