...
次のように出力されます。図や表、コードへ自動発番されていることを確かめて下さい。
...
```markdown
[Ruby|るびー]は[まつもとゆきひろ|松本行弘]氏によって開発されました。
...
:::{.note}
**注意**: 索引・用語集の記法は `[]` を使用します。ルビ記法 `{単語|読み}` と区別して下さい。
:::
...
用語とその説明を、標準 Markdown（pandoc / Markdown Extra 風）の定義リスト記法で記述します。用語を 1 行で書き、次の行から `: `（コロン＋スペース）で説明を続けると、`<dl>` / `<dt>` / `<dd>` に変換されます。
...
QueryStream は **データ（`data/*.yml`）** と **テンプレート（`templates/_*.md`）** を組み合わせて展開します。たとえば物理学の名著を `data/physics_books.yml` に並べておきます。

```yaml
# data/physics_books.yml（抜粋）
- title: 相対性理論
  author:
    name: アルベルト・アインシュタイン
...
1 件ぶんの見た目は、テンプレート `templates/_physics_book.md`（データ名の単数形・先頭 `_`）で決めます。`= title` のように `=` ＋スペース＋キー名で値を、`= author.name` のようにドットでネストした値を埋め込めます。

```markdown
<!-- templates/_physics_book.md -->
::: {.book-card}
![](cover)
**= title**
= author.name 著（= year年）
...
:::{.notice}
`cover:` で指定した表紙画像（例: `relativity.webp`）は、`data/` ではなく **その記法を書く章の画像ディレクトリ**に置きます。たとえばこの章（`22-extentions.md`）で展開するなら `images/22-extentions/` 配下に 4 冊ぶんの画像を置いておく必要があります（通常の画像記法と同じ扱いです）。
:::

あとは原稿に `= physics_books` と 1 行書くだけで、全件がテンプレートで展開されます。
...
抽出条件・並び替え・件数・スタイルの組み合わせ、テーブル形式テンプレート、ネスト構造データの扱いなど、QueryStream の詳しい使い方は「データ展開機能の使い方」の章でたっぷり解説します。お楽しみに。
...
水平線の記法 `---` を 1 行置くと、その位置で改ページされます（Vivlio Starter では `---` は水平線ではなく改ページになります）。
...
段落の下に少し余白を入れたいだけなら `{.aki}` / `{.aki2}`、1 行ぶんの改行なら `@vspace:1lh` や `<br>` を使います。
