...
:::{.memo}
mermaid の描画には `mmdc`（`@mermaid-js/mermaid-cli`）が必要です。未導入の環境では ` ```mermaid ` は**コードブロックのまま出力**され、ビルドが止まることはありません（そのときは警告でお知らせします）。`vs doctor --fix` で導入できます。図 1 枚ごとに Chromium の起動コストがかかりますが、生成物はキャッシュされるため再ビルドは高速です。
:::
