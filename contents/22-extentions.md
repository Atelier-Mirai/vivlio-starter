...
本文の流れを止めずに補足を添えます。書き方は二通りあり、どちらも参照した箇所と同じページの下部に組まれます。
...
プロンプト記号（`$` / `%` / `#`）は著者が書きます。どの行がコマンドでどの行が出力かは著者だけが知っているので、コマンドと実行結果を一つのブロックに混在させても構いません。
...
次の例では、先程の表を `.rotate-table` で90度回転して表示しています。
...
日本語を変数名に使った式は、**数式だとはっきり分かる手がかりがあるときのみ**拾います。``平文^e mod n`` は肩の `^` があるので数式に、``距離 × tan(仰角)`` は関数 `tan` があるので数式になります。一方 ``楕円の周の長さ = 4a × E(e)`` や ``行計 × 列計 ÷ 総計`` は手がかりが `×` `÷` だけなので、コードのまま表示します。``A社 × B社`` のように**数式ではない文**も同じ形をしていて、自動判定が困難な為です。
:::
...
言語に `math` と書けば、しるしが無くても数式として組まれます。次の三つは**中身がまったく同じ**で、言語指定だけが違います。{.aki}
...
部扉や章扉を必ず右起こしにしたいときに使ってください。EPUB・Kindle はリフロー型で左右ページの概念がないため、いずれも**単純な改ページ**へ自動的に劣化します。

引数を書き間違えると（`@pagebreak:rect` など）ビルド前チェックが 🔴 で指摘します。

なお、章・目次・部扉・付録・用語集・後書き・索引をどちら側の面から起こすかは、`config/book.yml` の `page.chapter_pagebreak` が**本全体をまとめて**決めます。値は `recto`・`any`・`verso` の三つで、記法の引数とちょうど同じ言葉です（既定は `recto` ＝右起こし）。原稿に書いた `@pagebreak:recto` / `:verso` は**この設定より優先されます**——本全体では面を問わないことにしておいて、この 1 箇所だけは右から、といった使い分けができます。
...
あとは `stylesheets/custom.css` に `.marker-box { background: #ffeb3b; }` のように定義すれば装飾が反映されます。**CSS を書くこと自体が登録**なので、別途どこかに宣言する必要はありません。ビルド前チェックは `stylesheets/` の中を読んでクラス名を集めており、CSS にないクラス名を使ったときだけ「綴りを間違えていませんか」と警告します。
:::
