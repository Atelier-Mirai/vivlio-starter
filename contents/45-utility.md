...
:::{.chapter-lead}
この章では、執筆環境の整備と成果物の作成を補助するコマンドを扱います。いずれも既定の設定で使えます。PDF を開く位置、紙面の見た目、章の雛形は必要に応じて調整してください。`templates/` ディレクトリの使い方も補足します。
:::
...
:::{.section-lead}
`vs open` は、ビルドで生成した PDF を macOS の Preview.app で開くコマンドです。ファイル名を指定して任意の PDF も開けます。
:::
...
このコマンドは、主に**ネットワーク経由で共有する PDF**のために使います。
...
印刷所へ提出する入稿用データには、このコマンドを使わないでください。`vs pdf:compress` は画像解像度やフォントを調整してサイズを減らすため、印刷品質へ影響する可能性があります。入稿には、`vs build` で生成した圧縮前の PDF を使ってください。
:::
...
処理時間が増えるため、通常は `false` に設定し、必要なときだけ `vs pdf:compress` を使います。自動圧縮が有効な場合でも、`vs build --no-compress` で一時的に省けます。
...
:::{.section-lead}
`vs clean` はビルドで生成された中間ファイルを削除します。`vs build` は既定でビルド後に自動でクリーンアップを実行するため、通常は手動で実行する必要はありません。
:::
...
:::{.section-lead}
Vivlio Starter では、`config/book.yml` のテーマ設定で色やフォントを切り替えられます。リンクの色やコードブロックの角などを細かく調整するには、`stylesheets/custom.css` を使います。
...
:::{.column}
**設定の順序**: `book.yml` の `theme.color` や `page.use` で設定できる項目は、まず `book.yml` で設定してください。`custom.css` は `book.yml` では設定できない細かな調整に使います。
:::
