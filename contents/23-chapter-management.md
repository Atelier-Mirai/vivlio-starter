...
通常章の雛型 `templates/chapter.md` は、章 1 本ぶんの骨組みです。リード文・節・小見出し・箇条書き・コードブロック・画像・まとめまでが一通り並んでいるので、書き出しで手が止まりません。
...
:::{.tip}
雛型は自由に編集できます。よく使う書き出しや決まり文句、章末の定型文を `templates/chapter.md` に書き足しておけば、以降の `vs create` にその内容が引き継がれます。
:::
...
:::{.memo}
索引辞書のうち、レビューファイルに表示される「文脈」（本文からの抜粋）は追随の対象外です。こちらは `vs index:auto` を実行するたびに本文から取り直されるため、古い章名が残っても自動的に解消します。
:::
