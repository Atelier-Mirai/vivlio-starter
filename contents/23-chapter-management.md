...
:::{.chapter-lead}
Vivlio Starter では、章ファイルの作成・削除・名前変更・連番の整理をコマンドで行えます。`catalog.yml` と画像ディレクトリも、操作に合わせて更新されます。
:::

## vs create — 章ファイルを作成する

:::{.section-lead}
`vs create` は章の Markdown ファイルと画像ディレクトリを生成し、`config/catalog.yml` へ追加します。
:::
...
番号のみの指定も可能です。スラッグなしのファイル名（`11.md`）が生成されます。
...
テンプレートファイルはプロジェクトの `templates/` ディレクトリにあり、自由に編集できます。`{{TITLE}}` プレースホルダーは、ファイル名のスラッグに置き換えられます。

テンプレートが見つからない場合は、最小限の骨子が生成されます。

### 生成されるファイルの中身

通常章の雛形 `templates/chapter.md` には、章一つ分の構成が入っています。リード文、節、小見出し、箇条書き、コードブロック、画像、まとめを並べています。

```markdown
# {{TITLE}}

:::{.chapter-lead}
各章の冒頭に置く短い導入文です。この章で扱うテーマと、読み終えたときに
何ができるようになるのかを伝えます。
:::

章の書き出しです。リード文で示したテーマを、もう少しだけ具体的に開いて
いきます。

## 節のタイトル

:::{.section-lead}
節の導入文です。この節で何を扱うのかを数行で示します。
:::
```

不要な部分は消し、必要なところを書き換えていけば章になります。

:::{.tip}
雛形は自由に編集できます。よく使う書き出しや章末の定型文を `templates/chapter.md` に加えると、以後の `vs create` で生成する章にも反映されます。
:::

#### 記法を思い出したいとき

表・数式・図解注釈・会話文・ダイアグラム・相互参照・索引・データ展開まで、使える記法の実例は `templates/markdown_snippet.md` にまとめてあります。「あの記法、どう書くのだったか」と思ったらこのファイルを開き、必要な部分をコピーしてください。

原稿ではないので `catalog.yml` には登録されておらず、ビルドの対象にもなりません。置いたままでも本には出ないので、消さずに残しておいて構いません。

## vs delete — 章を削除する

:::{.section-lead}
`vs delete` は章の Markdown ファイル、画像ディレクトリ、`catalog.yml` のエントリを削除します。削除前には確認プロンプトが表示されます。
:::
...
:::{.section-lead}
`vs rename` は章の Markdown ファイル名、画像ディレクトリ名、`catalog.yml` のエントリを変更します。
:::
...
`vs rename` と `vs renumber` は、章のファイル名を変えるだけでなく、その章名を参照している箇所も自動で書き換えます。

| 追随するもの | 内容 |
| :--- | :--- |
| `config/catalog.yml` | 章の並び順の定義 |
| `images/<章名>/` | 章専用の画像ディレクトリ |
| 索引辞書の主要参照 | `main:` が指す章名（→「索引・用語集機能」） |

一方、**章番号で書いた設定は自動では書き換えません**。
...
`90-98` が「付録すべて」という意味なのか特定の章を指しているのかは、機械には判断できないためです。こうした設定を書いている場合は、改番の後に見直しを促すメッセージが表示されます。

:::{.memo}
索引辞書のうち、レビューファイルに表示する「文脈」（本文からの抜粋）は追随の対象外です。`vs index:auto` を実行するたびに本文から取り直すため、古い章名は残りません。
:::

## vs renumber — 章番号を一括で付け直す

:::{.section-lead}
`vs renumber` は全章の番号を 1 から順に振り直します。章の追加・削除で番号が飛んだときに使います。`vs rename` の別名コマンドです。
:::
...
`contents/` 内の通常章（01-89）を先頭章の番号から順に、付録（90-98）を 90 から順に振り直します。番号が変わらない章も含めて全件を並べたうえで、確認プロンプトが表示されます。
...
通常の章:
  11-intro      →  11-intro
  13-setup      →  12-setup
  15-advanced   →  13-advanced
付録:
  91-appendix-a →  90-appendix-a
❓ 連番付け直しを実行しますか？ [y/N]:
```

この例では先頭章が `11` なので、そこを起点に `11, 12, 13` と詰められます。`01` から振り直されるわけではありません。
...
- 画像ディレクトリが移動先にすでに存在する場合は、手動での統合が必要です
