...
`false` にしても**章扉（章の 1 ページ目）はそのまま独立したページとして残ります**。`theme.style: image` の章扉はページ全面の扉絵で成り立っているため、そこへ本文や節見出しが流れ込むと絵柄と文字が重なってしまいます。最初の節だけは扉の次のページから始まり、2 つ目以降の節が本文に続けて組まれます。
