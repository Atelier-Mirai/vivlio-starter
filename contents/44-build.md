...
:::{.section-lead}
`vs build` の前に原稿のエラーだけを素早く確認したい場合は、`vs preflight` コマンドが便利です。PDF を生成せず、約6秒でチェックを完了します。
:::
...
`vs build` でも同じ検証は行われますが、エラーに気づくのが600秒後になります。`vs preflight` を先に実行しておくことで、ビルド前に問題を修正できます。
...
```
vs preflight 11
⚠️  11-workflow.md:20 - 画像 'workflow.svg' が見つかりません（代替画像を使用します）
⚠️                      画像の場所: images/11-workflow/workflow.svg
🔍 リンク・画像検証の結果:
   画像: 1 件の問題（存在しない画像: 1）
   外部URL到達性チェック: スキップ（--verify-links で有効化）
❌ Preflight 完了: 問題あり (0.2s) — 詳細は上記を確認してください
```

`⚠️` は警告（ビルドは続行できるが要注意）、`❌` はエラー（修正が必要）を表します。問題がなければ次のように表示されます。

```
✅ Preflight 完了: 問題なし (1.1s)
```
...
| 種別 | 記号 | 例 |
|:---|:---|:---|
| 画像ファイル不在 | ⚠️ | `11-workflow.md:20 - 画像 'workflow.svg' が見つかりません` |
| コードインクルードファイル不在 | ❌ | `ファイルが見つかりません: codes/sample.rb` |
| QueryStream 展開エラー | ❌ | `QueryStream 展開エラー: テンプレートファイルが見つかりません` |
| クロスリファレンス未定義ラベル | ⚠️ | `21-markdown-tutorial.md:926 - 未定義のラベルID: @basic-request` |
...
章を書き終えるたびに `vs preflight <章番号>` を実行する習慣をつけると、画像の置き忘れやコードファイルのパスミスにすぐ気づけます。`vs build` の600秒を待たずに済むので、執筆のリズムが保ちやすくなります。
:::
