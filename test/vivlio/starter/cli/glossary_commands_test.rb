  ...
  module Starter
    module CLI
      class GlossaryCommandsTest < Minitest::Test
        # glossary:lint で用語集違反が無い場合に成功メッセージが出力されることを確認
        def test_glossary_lint_passes_with_clean_content
          within_temp_dir do
            ...
            output = capture_io { glossary_command.glossary_lint }.first
        ...
        def test_glossary_lint_exits_with_violations
          within_temp_dir do
            ...
            error = assert_raises(SystemExit) { capture_io { glossary_command.glossary_lint } }
        ...
        def glossary_command
          @glossary_command ||= build_glossary_command
        end

        def build_glossary_command
          Class.new do
            # Thor DSL のスタブ
            def self.desc(*) = nil
