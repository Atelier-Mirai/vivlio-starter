...
::: {.chapter-lead}
Vivlio Starter で使える記法を、実例つきで一通り並べた見本帳です。`vs create` では使われません——「あの記法、どう書くのだったか」を思い出したいときに、このファイルを開いて必要な部分をコピーしてください。
:::

::: {.notice}
このファイルは**見本**です。原稿ではないので `config/catalog.yml` には登録されておらず、ビルドの対象にもなりません。中に出てくる画像（`image.png` など）も実在しないので、コピーしたら自分の画像名へ置き換えてください。
:::

ここからが記法の実例です。**太字**、*斜体*、***太字+斜体***、~~取り消し線~~、`インラインコード`
