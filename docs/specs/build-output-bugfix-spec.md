...
| # | 症状 | 重大度 | 状態 |
| --- | --- | --- | --- |
| ① | `bundled/` に画像の中間生成物が残存 | 低（見た目・リポジトリ衛生） | ✅ 修正済 |
| ② | `print_pdf` ビルドで本文が欠落（4ページのみ） | **高（成果物が壊れる）** | ✅ 修正済 |
| ③-b | `epub` に奥付が含まれない | 中 | ✅ 修正済 |
| ③-c | `epub` の索引・用語集でページ番号併合がされない | 中 | ✅ 修正済 |
| ③-a | `epub` で扉絵が表示されない | 中〜高（EPUB 品質） | ⏸ 保留（Kindle 互換性・別途検討） |
| ③-d/e | `epub` カバー比率・分量 | 低 | ③-e は欠落なし／③-d は要実機確認 |
| ④-A | EPUB で段落・ブロック数式が生 LaTeX 露出 | 中 | ⏸ 保留（Kindle で MathML 不安定・別途検討） |
