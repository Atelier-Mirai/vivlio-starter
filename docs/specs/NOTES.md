...
: **`vs metrics` の「文字数」が用途と合っていないと感じたとき。** 内部に 2 通りの数え方があり、著者に見えるのは生（コード・記法込み）だけで、地の文はその半分——本書実測で 2.03 倍の開き。章別の分量警告も生で判定している。Heaps 則による索引語数の目安表と、別書 `workbook` の較正データを併載。
  状態: 調査のみ・論点 4 つ未決 ／ 次: 表示と分量基準をどちらの数え方に揃えるかを決める（`index-term-selection-spec.md` §3 の文字数基準がこの結論を待っている）
