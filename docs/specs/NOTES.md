...
: **`metrics.*.chapter` の分量基準を疑うとき・「1 章 ○○ 字が目安」と言いたくなったとき。** 同じ 10 冊を章別に分解した実測（9 冊 104 章）。既定値は git 履歴・仕様書のどちらにも導出が無く、生／地の文のどちらで数えた値かも決められていなかった、という調査結果つき。**章の分量を決めるのは本の厚さでなく章数**——総量がほぼ同じ 3 冊で 1 章あたりが 3 倍違うので、「技術書なら 1 章 ○○ 字」とは言えない。PDF 抽出と `Masking` の系統差（+25.4%・実体はコード内の日本語コメント行）の補正係数も記録。§3.4 に節（H2）の実測 408 節。§7 が決定事項——**4 プリセット（`compact`/`handy`/`standard`/`commercial`）を実測値に置き換え、総本文字数でも選べるようにする**、判定は絶対値（既定）と相対（選択可）の二本立て、記号は分量 **💡**・文章の質 **🤔**。**「外挿するな・内挿はよい」の実例が §4.3 と §7.2.1 に対で入っている。**
  状態: 実測データ（仕様ではない）・§7 に決定事項 ／ 次: 実装（表示の本文主体化・💡/🤔 の分離・帯の入れ替え・`handy` 新設・相対モード）。`handy` は内挿値で実測 0 冊
