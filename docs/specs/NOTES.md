...
: **`vs metrics` の「文字数」が用途と合っていないと感じたとき。** 内部に 2 通りの数え方があり、著者に見えるのは生（コード・記法込み）だけで、地の文はその半分——本書実測で 2.03 倍の開き。章別の分量警告も生で判定している。Heaps 則による索引語数の目安表と、別書 `workbook` の較正データを併載。
  状態: 論点 1（著者に見せる文字数＝本文を主体・コードと記法は外数）は 2026-08-05 決定。論点 2〜4 は未決 ／ 次: 分量基準の判定をどちらの数え方にするか決める（実測は `chapter-volume-calibration-data.md` にある）
...
: **`metrics.*.chapter` の分量基準を疑うとき・「1 章 ○○ 字が目安」と言いたくなったとき。** 同じ 10 冊を章別に分解した実測（9 冊 104 章）。既定値は git 履歴・仕様書のどちらにも導出が無く、生／地の文のどちらで数えた値かも決められていなかった、という調査結果つき。**章の分量を決めるのは本の厚さでなく章数**——総量がほぼ同じ 3 冊で 1 章あたりが 3 倍違うので、「技術書なら 1 章 ○○ 字」とは言えない。PDF 抽出と `Masking` の系統差（+25.4%・実体はコード内の日本語コメント行）の補正係数も記録。§3.4 に節（H2）の実測 408 節。§7 が決定事項——**5 プリセット（`compact`/`handy`/`standard`/`commercial`/`heavy`）を実測値に置き換え、総本文字数でも選べるようにする**、判定は絶対値（既定）と相対（選択可）の二本立て、記号は分量 **💡**・文章の質 **🤔**。**「外挿するな・内挿はよい」の実例が §4.3 と §7.2.1 に対で入っている。**
  状態: 実測データ（仕様ではない）・§7 に決定事項（相対モードの仕様は §7.2.0） ／ 次: 実装（表示の本文主体化・💡/🤔 の分離・帯の入れ替え・`handy`/`heavy` 新設・`use: relative`・死蔵していた `ideal` を ✅ として復活）。`handy` は内挿値で実測 0 冊
