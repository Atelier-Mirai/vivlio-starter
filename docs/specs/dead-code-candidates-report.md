...
> **追記（2026-09-12）: `included` 4 件は誤検出ではなく、本当に死んでいた。**
> 4 つとも本体が空で、かつ `include DoctorCommands` のような **`include` 側が
> どこにも無い**——フックである以上 Ruby が呼ぶ可能性はあるが、呼ぶ側が無ければ
> 起動され得ない。ここでの判断は「フックかどうか」で止まっており、**「呼ばれるか」
> まで見ていなかった**。4 件とも撤去した。
> 走査の分類に一段足すこと: フックは「Ruby が呼ぶ」で終わらせず、**起動条件
> （include / prepend される箇所）があるか**まで確かめる。
