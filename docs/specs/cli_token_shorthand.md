...
- 本仕様は `Common.normalize_tokens` を土台としつつ、catalog との連携や新規章作成の扱いまでを含む「CLI token shorthand」のガイドラインを提供する。
...
- **Normalized Token**: CLI 入力を Resolver に渡す前段でゼロ埋め・レンジ展開済みにしたトークン（例: `"01"`, `"02"`, `"03"`）。TokenShorthand namespace の正規化ロジックがこの役割を担う。
- **Catalog Entry**: `catalog.yml` から得られる章情報。`{ number: '01', slug: 'life', path: 'contents/01-life.md', kind: :chapter }` のような構造を想定。
- **Resolver**: 正規化済みトークンと catalog entry を突き合わせ、CLI が最終的に扱う章レコードへ変換するコンポーネント（仮称 `TokenShorthandResolver`）。

## 4. 基本要件
1. **ゼロ埋め規則**: `0-9` の入力は `00-09` に読み替える。`1-foo` → `01-foo`, `1` → `01`。
2. **レンジ展開**: `1-3` → `01,02,03`。降順（`10-8`）や混在（`1-3,5,8-10`）も展開する。
3. **slug 付き指定**: `01-life`, `01-life.md`, `01-life/foo` などの亜種も `01-life` に正規化する。`create` など slug を新規に与える場合は `TokenShorthandResolver` が slug 抽出を補助する。
...
raw tokens (ARGV, positionals)
        ↓ Common.normalize_tokens (range, zero fill, slug 分離)
normalized tokens ("01", "02", "03", ... / "01-foo" ...)
        ↓ TokenShorthandResolver.resolve(
              tokens: normalized tokens,
              catalog_entries: CatalogLoader.load(...),
              allow_new: true/false,
              allow_slug_only: true/false)
resolved entries (Data/Struct)
```
...
- **ポリシーフラグの吸収**:
  ...
  - `allow_new: true` → catalog 不在でも `exists: false` の Struct を生成し、slug が無ければ `TokenNormalizer.extract_slug` 等で補う。
...
- `TokenShorthand::Resolver` は `Entry = Data.define(...)` を同ファイル内に宣言し、返却値をこの不変データで統一する。CLI 側は Common を経由せず `TokenShorthand::Resolver.resolve(...)` を直接呼び出す。
- 旧来 Common が担ってきた CLI トークン加工（`normalize_tokens`/`normalize_chapter_token`/`expand_range_token` など）や `get_file_type`/`get_chapter_number` といった補助関数は、全 CLI が Resolver を使い終わったタイミングで削除する。これらは TokenShorthand 名前空間の内部実装に吸収し、Common には設定・ログなど歴史的ユーティリティのみを残す。
- `Common` 側に一時的な Facade（例: `Common.resolve_shorthand(...)` → Resolver 呼び出し）を置く場合でも、最終的には TokenShorthand namespace を唯一の API とし、Spec どおりの構成を完了条件とする。
...
## 6. エラーハンドリング指針
1. **存在しない章**: `allow_new: false` の場合 `Vivlio::Starter::Errors::UnknownChapterToken`（仮）を投げ、CLI 側で rescue して usage を表示。
2. **slug 重複**: 新規章作成時に、既存 slug と衝突したら警告を出す。Resolver は `catalog_entries.group_by(:slug)` を参照して検知できる。
3. **フォーマット不正**: `abc` や `01--foo` など規約外文字列は `Common.normalize_tokens` の段階で弾く。将来的に `--strict` オプション等で挙動切り替え可。
4. **特殊ファイル指定**: `_titlepage_legalpage.pdf` や `.cache/metrics/*.yml` など `special?` が true のエントリを通常章向け CLI が受け取った場合は `UnsupportedSpecialFile`（仮）を投げ、clean / cache 系コマンドへの誘導メッセージを共通化する。
...
- `TokenShorthandResolver.resolve` は常に同一フィールド構成の不変データを返す。`Data.define(:number, :slug, :kind, :basename, :path, :ext, :exists, :catalog_entry, :special?)` のような定義を想定。
...
4. **Common のクリーンアップ**: 全 CLI が Resolver を通る状態になったら、ブランチ上で `rake test` を通過させたうえで main/master にマージし、旧 `Common.normalize_tokens` 系ユーティリティを削除して終了する。

## テスト戦略
1. **Resolver 単体テスト**: ゼロ埋め、降順レンジ、混在トークン、slug-only、`allow_new` / `allow_slug_only` / `special?` 判定などを網羅するユニットテストを追加。
