...
: 索引辞書 `index_glossary_terms.yml` の `context:` 抜粋が原稿推敲に追従せず stale 化する問題の調査報告。`enrich_terms_with_context` が「context が空の語だけ再抽出」する設計が原因（実測 15/724 が stale）。context はビルド非依存の表示専用データなので毎回本文から再計算してよい、という前提で設計案 3 つを比較。付随して「再抽出が 1 章分しか返さない既存バグ（§5.1）」「`vs index:auto` が黙って辞書へ書き込む（§5.2）」も報告。
  状態: 調査報告・仕様化待ち（2026-07-16 起票）
  次のアクション: 本書レビュー → 仕様書へ昇格（推奨は案 A：`enrich` で stale を捨てて補充・§5.1 の複数章修正とセット）
