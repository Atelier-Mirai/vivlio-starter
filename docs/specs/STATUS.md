...
| 仕様書 | 内容 | 状態 |
| :--- | :--- | :--- |
| `index-apply-rejected-consistency-spec.md` | `vs index:apply` が、登録済みの語を**手書きの定義文ごと**削除する不具合の修正。`[-i]` が「用語集には残す」語を除外リストへ書き込み（矛盾の発生）、次回の Section 4 同期がそれを削除で解決していた（時限式） | **実装済み**（2026-08-21・§8）。修正を戻すと新テスト 4 件が落ちることを確認。既存の矛盾は次回の `apply` で自動解消される |
