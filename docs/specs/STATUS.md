...
| 仕様書 | 内容 | 状態 |
| :--- | :--- | :--- |
| `upgrade-conflict-review-spec.md` | `vs upgrade` の競合 diff が途中で切れ、**見ていない変更まで `y` で適用される**。真因は diff がハンクに分かれていないこと。あわせて、競合が多数になったときレビューが終わらない問題も扱う | 実装待ち |
...
`upgrade-conflict-review-spec.md` は単独で実装できる。§4 の順序に「1〜2 を入れれば事故は防げる。3 以降は体験の改善」と切れ目を書いてあるので、リリースとの兼ね合いで途中まででも成立する。
