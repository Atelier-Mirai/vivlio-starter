...
`index-glossary-consistency-spec.md`
: 索引・用語集の整合性仕様。設計原則「辞書＝語彙の一次データ／出現情報（頁番号・バックリンク・context）＝ビルド毎に導出」に基づき、(1) `vs build` が辞書へ `backlink_sources` を書き込み出現しなかった語は前回値のまま残る幽霊バックリンクの根治（中間 YAML へ移設・**ビルドは辞書を書かない**）、(2) context stale 化の恒久対策（報告書の案 A＋複数章補充）、(3) **catalog.yml の章追加・削除（61-developer トグル運用）への追従**（未走査章の検知 `scanned_chapters`・未出現 g 語の警告）、(4) `index:auto` の書き込み可視化と `[eV]`/`[g]` 誤登録ガード（ASCII 2 文字以下除外）。調査報告: `index-context-staleness-report.md`。
  状態: 確定仕様・未着手（2026-07-16 策定。report からの昇格＋catalog 整合性の論点を統合）
  次のアクション: Phase 1（可視化＋誤登録ガード・小粒先行）から実装

`index-context-staleness-report.md`
: 上記仕様の元になった調査報告（context stale 化の実測 15/724・原因特定・設計案比較）。
  状態: 仕様書へ昇格済み（調査記録として保存）
  次のアクション: なし（実装完了時に仕様書と一緒にアーカイブへ移動）
