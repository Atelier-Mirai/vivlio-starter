...
| 仕様書 | 内容 |
|---|---|
| `index-main-reference-section-spec.md` | 主要参照を節へ降ろす（章扉を指してしまう問題）／節指定 `21#見出し`／推測を 65% へ／`[igm33]` 記法とフラグ解析の集約。**Phase 1〜2（節へ降ろす・節指定 `21#見出し`）は実装済み**。残りは Phase 3（推測 65%・対象を全語へ）と Phase 4（`[igm33]`） |
