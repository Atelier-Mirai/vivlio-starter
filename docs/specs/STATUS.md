...
| 仕様書 | 内容 | 状態 |
| :--- | :--- | :--- |
| `plain-math-notation-spec.md` | LaTeX を知らない著者が素で書いた数式を、ビルド時に判定して数式として組む。バッククォートを `$` の代用と見なす（`workbook` 実測: インライン 361 出現・ディスプレイ 70 件）。**原稿に LaTeX は書かない**のが不変条件 | **①〜⑤ 実装済み**（2026-08-19〜20・§13）。残るのは ⑥ 日本語ラベルの分割（将来）と実機確認 |
...
`plain-math-notation-spec.md` は ①〜⑤ が実装済み（§8）。残るのは ⑥ 日本語ラベルの分割で、①②の実地結果を見てから判断すると決めている。他の実装待ちは無いため、外向きの順序制約は無い。
