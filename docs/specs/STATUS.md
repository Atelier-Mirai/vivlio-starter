...
| 仕様書 | 内容 | 状態 |
| :--- | :--- | :--- |
| `markdown-notation-collision-spec.md` | CommonMark / GFM の記法を全数で実測した結果、**参照リンク・タスクリスト・4 スペース字下げコードブロックの 3 つが壊れていた**。壊しているツールを直す T-1（参照リンク）・T-2（タスクリスト）と、伝えるにとどめる L-1（短い ASCII の `[g]`）・L-2（字下げは非対応）・L-3（段落直後の `---` `===` は見出しになる） | **未実装**。T-1 を入れずに lint だけ入れると本書が自分の教え（21 章・早見表の参照リンク）を警告するので順序を違えない。§11 に原稿の加筆 3 点 |
