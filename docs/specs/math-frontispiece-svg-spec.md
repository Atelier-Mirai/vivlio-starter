...
> - `docs/specs/epub-glossary-index-xhtml-report.md` — 用語集/索引の epubcheck ERROR（RSC-005 / RSC-012）。
>
...
> **実装済み（2026-06-15）。** 合成画像生成器 `build/heading_image_composer.rb` を新設し、
> `EpubBuilder#generate_epub_entries!` の Step E に `inject_heading_images_for_epub!`
> （扉絵 h1・節絵 h2）を追加。`theme.style == 'image'` かつ**本文章（番号 1..89）**のときのみ、
> `theme.css` の `--frontispiece-image`（portrait）/ `--section-bg-image`（landscape）を実画像として読み、
> 飾り画像を白フラット化＋縮小 JPEG の data URI として埋め込み、見出し番号・タイトルを `<text>` で
> 重ねた合成 SVG を組む。**ただし配る実体は SVG ではなくフラット JPEG にラスタライズする**
> （`rsvg-convert` で PNG 化 → ImageMagick で白フラット JPEG 化）。`images/headings/<kind>-<hash>.jpg`
> に書き出し `<img>` で差し込む。見出しテキストは `<img alt>` に格納する。
>
> **重要な設計変更（2026-06-15・Kindle 実機検証）**: 当初は合成 SVG をそのまま `<img src=".svg">` で
> 配ったが、**Kindle は「SVG 内に base64 data URI で埋め込んだ画像」を非対応**（Kindle Previewer の
> 変換でブロッキングエラー＝出版不可）。このため**ビルド時に JPEG へラスタライズ**して配る方式へ変更。
> フラット JPEG は Kindle を含む全リーダーで確実に表示される。あわせて、当初の隠しテキスト span
> （`clip: rect(...)`）も撤回——`clip: rect()` は Kindle が非対応（無視される）であり、かつ**EPUB 目次は
> 各章 HTML の `<title>`（entries.epub.js）から生成され h1 テキストに依存しない**と判明したため、
> 隠し span は不要（見出しテキストは `<img alt>` に保持で読み上げ・検索に足りる）。
>
> EPUB 用 CSS（`vs-image-heading-epub` / `vs-section-topic-epub`）は `components.css` に追加
> （PDF 経路は当該マークアップが無いため無害）。`images/headings/` は `clean.rb` のクリーン対象。
> **付録（90-98）・前付（00）・後付（99）・特殊ページは対象外で simple 版**（PDF と整合）。
> rsvg-convert / magick 未導入・画像未解決・合成失敗時は注入をスキップし simple 相当へ縮退（§B-5）。
> `rsvg-convert`（librsvg）は `vs doctor` のチェック対象に追加（`--fix` で `brew install librsvg`）。
> 未決事項: フォント焼き込み（非埋め込み）のため `<text>` 字形はビルド機のフォント依存（§B-7・v2.0 で対応）。
