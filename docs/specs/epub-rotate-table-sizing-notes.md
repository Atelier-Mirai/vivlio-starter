...
**コミットは残っていない。** `e7ff7f20`（版面いっぱいまで拡大）の直後に作業ツリーで
試作し、実機確認のうえ `git checkout` で捨てたため、git 履歴には現れない。

再着手するときは **§3 を読めば足りる**——前処理・CSS とも全文を転記してある。
差分は 3 ファイル（`table_converter.rb` / `components.css` /
`table_converter_test.rb`）計 143 行だった。
