...
| T6 | ID + EP（release 補強） | 低 | T1 | ✅ 実装済み（2026-06-12。DC-03 は §11.3 のとおり初期導入では見送り）。2026-06-13: epubcheck を導入し EP-02 で生成 EPUB の実 ERROR を検出（当初 35 件＋取りこぼし計）→ `epub-pipeline-fix-spec.md` の Fix-1〜7 で解消し **FATAL/ERROR/WARNING 0 件**を達成。**EPUB は RC ゲートへ正式復帰**（EP-02 は `test:manual`→`test:release` に内包） |
