...
| T6 | ID + EP（release 補強） | 低 | T1 | ✅ 実装済み（2026-06-12。DC-03 は §11.3 のとおり初期導入では見送り） |
| T7 | CN（`rake test:canary`）+ CI 方針の別途検討 | 低 | T1 | ✅ テスト実装済み（2026-06-12。ネットワーク必須のため定期実行は CI 整備時に運用へ載せる） |
