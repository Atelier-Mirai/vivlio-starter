...
| T4 | DG + NF（robustness 拡張） | 中 | — | ✅ 実装済み（2026-06-12。NF-02 が実不具合を検出→修正。DG-03/04 は理由付き skip・§8.3） |
| T5 | CL + DC-01/02（contract 新設） | 中 | — | ✅ 実装済み（2026-06-12。CL-01 が --help 不具合 3 件を検出→修正。DC が文書残骸を検出→許容リストで課題化） |
