...
### 15.1 派生ファイルの復元（2026-06-13 追加）

`vs build` は `config/book.yml` のページ設定から `stylesheets/page-settings.css` と
`vivliostyle.config.js` を再生成する。判型プリセットを切り替えてビルドすると、
これらは最終プリセットの値で上書きされるため、`BookYmlPatcher.apply` が book.yml だけを
復元しても作業ツリーにこの 2 ファイルの差分が残る（既存挙動）。

`BookYmlPatcher.apply` のブロック終了時に、book.yml と一緒にこれら派生ファイル
（`BookYmlPatcher::BUILD_GENERATED_FILES`）も退避前の内容へ復元する。退避時に存在しなかった
ファイルは削除して元に戻す。これにより `rake test:layout` 実行後の `git status` が汚れない。
...
| ステップ | 内容 | 優先度 | 前提 | 状態 |
|---|---|---|---|---|
| T0 | マニュアルビルドの警告棚卸し（§4.3）+ 共通ヘルパー抽出（§15）+ Rakefile タスク整備（§3） | 最高 | — | ✅ 実装済み（2026-06-12。棚卸し結果: 🟡 2 件＝索引案内のみ→許容リスト化） |
| T1 | MB + FT（`rake test:manual` の中核） | 高 | T0 | ✅ 実装済み（2026-06-12） |
| T2 | PK（`rake test:package`） | 高 | T0 | ✅ 実装済み（2026-06-12） |
| T3 | FZ（`rake test` へ追加） | 高 | — | ✅ 実装済み（2026-06-12。FZ-01/03 が実不具合 2 件を検出→修正） |
| T4 | DG + NF（robustness 拡張） | 中 | — | ✅ 実装済み（2026-06-12〜13。NF-02 が実不具合を検出→修正。DG-03 は exit(1) 矛盾を検出→文脈分離（pipeline はスキップ続行 / 単体は 🔴 exit 1）で修正。DG-04 も magick 生成の実画像で有効化し、DG-01〜04 全件 skip なしで稼働） |
| T5 | CL + DC-01/02（contract 新設） | 中 | — | ✅ 実装済み（2026-06-12〜13。CL-01 が --help 不具合 3 件を検出→修正。CL-02 の終了コード規約は無効入力 exit 1 へ変更し有効化。DC が検出した文書残骸（cover:a4 系 / index:build）はマニュアル修正済みで許容リストから削除） |
| T6 | ID + EP（release 補強） | 低 | T1 | ✅ 実装済み（2026-06-12。DC-03 は §11.3 のとおり初期導入では見送り）。⚠️ 2026-06-13: epubcheck を導入し EP-02 を実行したところ、生成 EPUB に実 ERROR 35 件を検出（CSS-008 26 / RSC-005 7 / RSC-007 2）。これは EPUB 生成側の不具合のため EP-02 は緩めず、調査記録を `docs/specs/epub-validation-findings.md` に作成。EPUB 修正は別仕様書で対応し、当面 EPUB は RC ゲート外 |
| T7 | CN（`rake test:canary`）+ CI 方針の別途検討 | 低 | T1 | ✅ 実装済み（2026-06-12）。実機確認済み（2026-06-13: `@vivliostyle/cli` **11.0.1**＝ピン留め 10.5.0 からのメジャー更新でマニュアルが正常ビルド・Type 3 再発なし・新規警告なし）。ネットワーク必須のため定期実行は CI 整備時に運用へ載せる |
...
- [x] `rake test` が従来どおり高速（実ビルドなし）のまま、FZ / DG / NF / CL / DC を含んで green
- [x] `rake test:release` 一発で RC 前総点検が完了する
- [x] 各テストファイル冒頭に対応する本仕様の ID（MB-01 等）を記載（robustness の既存様式に倣う）
- [x] 発見された本体側の不具合の扱い: 数行で完結し原因が自明な 4 件（--help 不具合 3 件・不正バイト列・NFD 誤警告）は検出経緯を CHANGELOG / コードコメントに明記の上で同時修正した。設計判断を要する 3 件（gs 不在時の exit(1)・未知コマンドの終了コード・マニュアルの旧コマンド表記）は skip / 許容リストへ理由付きで記録し、別タスクとして残した
- [x] CHANGELOG.md に記録
