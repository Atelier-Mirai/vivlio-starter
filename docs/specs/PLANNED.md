...
- [Medium] **`vs lint` が `$…$` の数式を保護していない**: コードスパン（`` `foo(1)` ``）は保護されるのに、数式（`$(4/3)πr³$`）は地の文として検査される。実測（2026-08-20）: `球の体積は $(4/3)πr³$ です。コードは \`foo(1)\` です。` を lint すると、prh が数式の中の `(4/3)` だけを「全角にせよ」と指摘する。
  - **素の表記を数式として組む機能（`plain-math-notation-spec.md`）が入って表面化した。** 著者が数式を書くほど、数式の中身が校正されて誤指摘が増える。しかも `--fix` が当たれば `（4/3）` になって**数式が壊れる**。
  - 直し方: `Masking` は既にフェンス・インラインコードを解釈する唯一の実装なので、そこへ数式（`$…$` / `$$…$$` / `\(…\)` / `\[…\]`）の退避を足し、textlint へ渡す前に外すのが筋。判定器が起こした `$…$` も同じ経路に乗るので、追加の手当ては要らないはず。
  - 暫定: 本書は `config/textlint_allowlist.yml` に `(4/3)` `(GM/R)` を入れて黙らせている（あの 2 語のためだけの登録なので、直ったら消す）。
