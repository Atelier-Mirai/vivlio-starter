...
- [Low] **交ぜ書き語を原稿から集めて辞書を育てる**: 辞書は gem 側にあり著者が足せないため、**「この語も交ぜ書きだ」と気づいても行き先がない**。140 語は出発点にすぎず、語彙は実際の原稿からしか育たない。集め方には 3 つの向きがある。
  ...
  - **外部辞書をそのまま `--fix` に載せると原稿が壊れる**（2026-08-18 実測）。上記 Mozc 辞書の 4720 語（既存 140 語との重複を除いた分）を**`vs lint` の実対象**——`contents/` 27 ファイル・**地の文 22 万字**（`Masking` でコード領域を除いた分）——へ当てたところ、**17 種・31 箇所**が当たり、真の交ぜ書きはほぼ無かった。
    - `が解 → 瓦解` 8 箇所（「これ**が解**析」）／`合し → 合祀` 4 箇所（「結**合し**た」）／`も本 → 摸本` 4 箇所／`目と → 目睹` 2 箇所。`--fix` を掛ければ「結合した」が「結合祀た」になる。
    - **明確な誤りと、古い表記への差し戻しも混じる。** `素晴らしい → 素薔薇しい`（当て字）、`見なす → 見做す`・`この節 → 此の節`・`当てはめ → 当て嵌め`・`塗りつぶし → 塗り潰し`——後者は現代の標準表記を旧表記へ戻す向きで、交ぜ書きの是正ではない。
    - 参考: `docs/` と Ruby コード注釈まで含めた **450 ファイル・のべ 438 万字**（うち約半分は Ruby ソース）では **59 種・439 箇所**。`側が → 側臥` が 52 箇所（「呼び出し**側が**」）など、母数を広げるほど誤検出も比例して増える。
