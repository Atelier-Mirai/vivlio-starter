...
- [Medium] **版面の分断を機械が見つける**: `page.section_pagebreak: false` を選ぶと、見出しがページ末尾に取り残される「泣き別れ」や、図版・表とその見出しが別ページへ割れる事故が起こりうる。節が必ずページ先頭から始まっていた頃は**原理的に起きなかった**ので、いま防御が無い。本書自身が 2026-08-09 に `false` へ切り替えており（515 → 449 ページ）、実害の出る条件は揃っている。
  - **まず CSS で防ぐ**。本文の見出し（h2 / h3 / h4）に `break-after: avoid` が**掛かっていない**。索引（`index.css`）・前書き（`preface.css`）・目次（`toc.css`）には入っているのに本文だけ抜けているのは、h2 が常に改ページしていて孤立しようがなかったため。`section_pagebreak` を選べるようにした時点でこの前提は崩れている。あわせて、キャプションの下に最低 n 行ぶんの高さを要求する仕組み（Re:VIEW Starter の `caption_needspace` に相当）も要る。
