...
> **索引・用語集の仕上げ（RC 対象）は仕様書 3 本すべて実装完了**。
>
...
> - `index-main-reference-spec.md`（参照をどう見せるか）… **2026-08-03 完了・全 6 フェーズ**
>
...
>
> 「索引の目安に使う文字数の基準を `vs metrics` と揃える」は **2026-08-05 に解消**したため削除した。`vs metrics` の表示・判定とも本文（コードと記法を除いた地の文）基準になり、索引側の `IndexSizeEstimator.prose_chars_of` と**同じ `Analyzer.prose_length`** を見るようになったので、著者が画面で見る「文字数: ○○ 文字（本文）」がそのまま目安語数の根拠になる。経緯は `metrics-char-count-basis-report.md`、実測は `chapter-volume-calibration-data.md`。
>
