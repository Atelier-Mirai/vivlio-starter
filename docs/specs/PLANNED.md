...
- [Medium] `vs lint` に「ら抜き言葉」検出機能を設ける。
- [Medium] `vs lint` 「BはAと同じようにXしない」という対句は、日本語として二通りに読めるため、書き換えを促す機能を設ける。
  「BはAと同様にXしない」は(Aも本来Xしない)のか、「Aとは違ってBはXしない」（Aは実はXする）なのか、文脈だけでは判別できない。
対比を書きたいときは、`AはXする。一方Bは（Aとは異なり）Xしない。`のように、両者の性質を独立した文でそれぞれ明示する。
（例：「スレッドのようにメモリを共有もしない」→「スレッドはメモリを共有するが、Ractorは共有しない」）。
