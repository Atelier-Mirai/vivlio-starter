...
- [Medium] **版面の分断を機械が見つける**: `page.section_pagebreak: false` を選ぶと、見出しがページ末尾に取り残される「泣き別れ」や、図版・表とその見出しが別ページへ割れる事故が起こりうる。節が必ずページ先頭から始まっていた頃は**原理的に起きなかった**ので、いま防御が無い。本書自身が 2026-08-09 に `false` へ切り替えており（515 → 449 ページ）、実害の出る条件は揃っている。
  - **見出しの孤立は PDF では起きない**（2026-08-10 実測）。本文の見出しに `break-after: avoid` を足したが、**PDF ではこの宣言の有無で結果が変わらない**——Vivliostyle が既に見出しを次の内容と一緒に送るためで、「見出しは収まるが続く 1 行が収まらない」高さを B5・spacer 176〜196mm の 1mm 刻みで探しても孤立する組み合わせが無かった。宣言は EPUB / Kindle 向けの保険として残してある（組むのは読み手側のエンジン）。**残る穴は見出し以外**——図版・表とキャプションの分断で、キャプションの下に最低 n 行ぶんの高さを要求する仕組み（Re:VIEW Starter の `caption_needspace` に相当）が無い。検出機能を作るなら狙いはこちら。
  - **検出は描画結果に対して行う**。`pdftotext` の抽出順は視覚順と一致せず**この用途には使えない**——「ページ末尾の行が原稿の見出しと一致するか」で 449 ページを走査すると 2 件挙がったが、**2 件とも誤検出**だった（見出しの後ろに内容があるのに、抽出順の都合で見出しが最後に来ていた。実測 2026-08-09）。Vivliostyle の組版結果か、PDF のテキスト描画座標を見るしかない。
  - 出し方はビルド後の 🟡 か `vs preflight` の一部。指摘には**ページ番号と見出し名**を添え、`@pagebreak` で個別に逃がす道を示す。
