...
- [High] **`:::{.terminal}` をリテラル化する**: 中身が素の Markdown として VFM に渡っているため、`$ cp *.png *.bak` の `*` が斜体化する・連続空白が消えて桁揃えが崩れる・`---` の行が改ページになる、等の不具合がある。`<pre>` 化して解決する。詳細は [terminal-literal-spec](terminal-literal-spec.md)。
  - **プロンプト記号の設定機構（旧 `prompt-setting-spec.md`）は却下**。どの行がコマンドかは著者だけが知っており、著者が `$` を書けば足りる。同仕様書は破棄し、経緯は上記仕様書の「背景」に記録した。
