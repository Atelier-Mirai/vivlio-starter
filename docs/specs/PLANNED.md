...
- [High] **ビルドパイプライン全般の見直し**: `targets`（pdf / print_pdf / epub / kindle の単独・複合）×（単章 / フルビルド）の組み合わせが多く、`register_*_steps` が継ぎ接ぎになっている。ステップ登録ロジック・メソッド責務・テストカバレッジを再設計して共通化する（pdf と print_pdf の差は本来カバー・トンボの有無のみ。なお「print_pdf を pdf から導出して高速化」は 2026-07-10 実装完了 → [docs/archives/print-pdf-derivation-spec.md](../archives/print-pdf-derivation-spec.md)）。
- 
...
- **`@` ディレクティブ拡張の検討**（未確定・ブレスト段階）: 残る `@` 記法が `@vspace` のみになったのを機に、追加候補を整理した叩き台。`@pageref`（ページ番号参照・最優先候補）、`@pagebreak:recto`、`@version`/`@today`、`@qr`、`@hspace` ほか。略語展開 `@abbr` は `@id` 参照と衝突するため単独 `@` 記法としては見送り（用語集統合 or 標準 `*[…]:` `<abbr>` で検討）。判断基準・各候補の実装感・未決論点は → [at-directive-ideas.md](at-directive-ideas.md)
- 
...
- [Medium] **QueryStream のデータ用画像を `data/` 配下にも置けるようにする**: 現状、`data/*.yml` の `cover:` 等で参照する画像は、QueryStream 記法を書く**各章の画像ディレクトリ**（例 `images/22-extentions/`）に置く必要がある。データ（文字情報）は `data/` に一元化されているのに、画像だけ利用する章ごとに複製するのは煩雑で一体感に欠ける。`data/` 配下（例 `data/images/` や `data/physics_books/` のようにデータ単位のフォルダ）に画像も同居させ、展開時に解決できるようにすると、データ一式が自己完結して扱いやすい。要設計の論点: (1) 画像解決の探索順（章の画像ディレクトリ → `data/` 配下、の優先順位）、(2) ビルド時に最終出力先（`images/` 配下）へどうコピー・正規化するか（`ImagePathNormalizer` との整合）、(3) 同名画像の衝突回避、(4) EPUB/Kindle 同梱時のパス、(5) 複数章で同じデータを使い回す場合の重複排除。→ **仕様書作成済み（2026-07-12）**: `docs/specs/querystream-data-images-spec.md`（実装待ち）。
- 
...
- [Done] **Post-processing 単体テスト整備**: 組み込み置換ルール（`ReplacementRules`）の主要ルール（段落クラス付与、見出し・body クラス、各種クレンジング）のグループ別スナップショットテストを整備済み（`test/vivlio_starter/cli/post_process/replacement_rules_test.rb`）。想定外パターン（複数クラス、`<pre>` 内非置換、属性値保護等）の回帰も含む。→ [post-replace-list-retirement-spec.md](post-replace-list-retirement-spec.md)
