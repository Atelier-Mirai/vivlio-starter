...
- [High] **索引を「出現箇所」でなく「説明箇所」を指すものにする**: 現在は語が出るたびに索引項目ができるため、「Markdown」のような頻出語は**ほぼ全ページを拾ってしまい**、読者が「で、どこを読めばいいのか」を判断できない。索引の本来の役割は所在の網羅ではなく**説明の在り処への案内**で、商業書の索引が主要参照（その語を腰を据えて説明している箇所）を太字などで区別しているのはそのためである。頻出語ほど索引としての価値が下がる、という現状の逆転を正したい。
  - **土台は既にある**: `IndexMatchScanner#process_term` は初出を判定して `is_definition` を立て、タグも `<dfn>` と `<span>` で書き分けている（`index_match_scanner.rb:443`）。ところが **`is_definition` はどこでも消費されていない**——索引ページは全出現を同格のページ番号として並べる。まずここを繋ぐだけでも「初出だけ太字」は成立する。
  - ただし**初出＝説明箇所とは限らない**。前書きで軽く触れてから第 3 部で腰を据えて説明する、という構成のほうが普通である。判定材料の候補: 語が見出し（h1〜h3）に現れる箇所／`.column` や `.memo` など説明ブロックの中／同一ページでの出現密度／`scoring_engine.rb` が既に持つ `score` の局所版／著者が原稿に明示する記法（`@index:main` のような主要参照マーク）。**著者の明示を最優先し、自動判定はその補助**という順序が本プロジェクトの他の記法と揃う。
  - 出し方は 2 通り考えられる。**(a) 主要参照だけを太字にして他も残す**（読者は網羅性も得る）／**(b) 主要参照以外を落とす**（索引が短くなりページ数も減る）。(a) が索引の作法としては素直だが、同人誌ではページ単価の都合で (b) を選びたい本もある。`book.yml` の設定で選べるのが妥当か、仕様化のときに詰める。
  - **副次効果としてビルドも軽くなる可能性がある**: 索引項目が減れば Step 8 の dedup 対象（実測 1,750 件のページ番号重複を除去している）も減る。ただし本文の再レンダは避けられないので、効くのは dedup の判定部分だけ。
