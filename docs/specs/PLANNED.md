...
- [Low] **交ぜ書き語を原稿から集めて辞書を育てる**: 辞書は gem 側にあり著者が足せないため、**「この語も交ぜ書きだ」と気づいても行き先がない**。140 語は出発点にすぎず、語彙は実際の原稿からしか育たない。集め方には 3 つの向きがある。
  ...
  - **外部辞書をそのまま `--fix` に載せると原稿が壊れる**（2026-08-18 実測）。上記 Mozc 辞書の 4720 語（既存 140 語との重複を除いた分）を、手元の日本語テキスト **235 ファイル・地の文 204.8 万字**（`contents/` + `docs/` + ルートの `*.md`。`Masking` でコード領域を除いた分）へ当てたところ **44 種・314 箇所**が当たり、**本文中の実際の交ぜ書きは 0 件**だった。`vs lint` の実対象である `contents/` だけに絞ると 17 種・31 箇所。
    - `合し → 合祀` **71 箇所**（「結**合し**直した」）／`が解 → 瓦解` **56 箇所**（「これ**が解**析」）／`も本 → 摸本` 30 箇所／`側が → 側臥` 25 箇所（「呼び出し**側が**」）／`目と → 目睹` 18 箇所（「項**目と**して」）／`は種 → 播種` 2 箇所（「後書き**は種**別が」）。`--fix` を掛ければ「結合し直した」が「結合祀直した」になる。
    - **古い表記への差し戻しも多い。** `取りこぼし → 取り溢し` 32 箇所、`この方 / この前 / この節 / この内 → 此の…` 計 24 箇所、`当てはま / 当てはめ → 当て嵌` 12 箇所。現代の標準表記を旧表記へ戻す向きで、交ぜ書きの是正ではない。`素晴らしい → 素薔薇しい` のような当て字も混じる。
    - **正しく当たった 6 箇所も自己言及だった**——`う回`（4）と `語い`（2）で、いずれも「こちらが誤検出を理由に載せなかった語」を本仕様と原稿が説明している箇所である。**本書に実在する交ぜ書きは、この辞書では 1 件も見つからなかった**（こちらの 140 語は `しきい値` 2 件・`ひな型` 2 件・`かぎ括弧` 1 件を検出している）。
    - **一部は丁寧に作られている。** `らちが明か → 埒が明か` は「らち」単独ではなく複合語で持っており、こちらが危惧した誤爆（「拉致が明かない」）を避けている。機械抽出でも複合語を残す判断が入っている箇所はある。
  - **形態素境界を見れば誤検出の 9 割が消える**（2026-08-18 実測）。上記の 316 箇所へ「**マッチの開始位置と終了位置がともに形態素の境界と一致すること**」を課すと **27 箇所**まで落ちた（MeCab の `-Owakati` で分かち書きし、累積文字数から境界の集合を作って判定）。**外部辞書を取り込む道はここにある。**
    - **消えるのは「語の途中から始まる／終わる」型。** `合し`（結**合し**直した → `結合|し|直し`）、`が解`（これ**が解**析 → `これ|が|解析`）、`は種`（後書き**は種**別 → `後書き|は|種別`）、`側が`、`目と`。Mozc 辞書の誤検出の大半がこれである。**交ぜ書き語そのものは形態素として認識される**ので取りこぼさない——`だ円|の|面積`、`完ぺき|な|実装`、`靖国神社|に|合し|さ|れ`（合祀の交ぜ書きを正しく 1 形態素と見る）。
    - **消えないのは「境界は合うが意味が違う」型**で、こちらは正規表現の制約が引き続き要る。`だ円`（囲ん**だ円** → `囲ん|だ|円`。`だ` が独立形態素）、`べき乗`（予約す**べき乗**り換え → `予約|す|べき|乗り換え`）、`も本`（置いたままで**も本**には出ない → `も|本`）。**最後の例は原理的に曖昧**で、「も本／には出ない」と「も／本には出ない」のどちらにも読める——`ambiguous-comparison` が扱う構造的曖昧さと同じ性質のもので、解析器を替えても解けない。
    - **既存の制約のうち一部は境界チェックで代替できる。** `かく乱`（とに**かく乱**暴 → `とにかく|乱暴`）と `き裂`（引**き裂**く → `引き裂く|と`）は境界だけで落ちる。`だ円`・`べき乗`・`きょう体` は落ちないので残す。
    - **MeCab は必須にしない。「あれば使う、無ければ機能低下」に揃える**（2026-08-18 決定）。既存の 3 機能——`vs metrics`（`check_mecab_available` が `LoadError` / `RuntimeError` を捕まえて簡易トークナイザへ落ちる）・`vs index`・見出し分割——がすべてこの形で、`doctor` も MeCab を**任意ツール**（「MeCab (索引機能用)」）として案内している。`vs lint` だけ必須にすると方針が割れ、`doctor` の必須／任意の分類も作り直しになる。
      - なお `natto`（MeCab の Ruby バインディング）は**既に gemspec の必須依存**である。ただし本体（C ライブラリ）が無くても gem のインストールは通り、`Natto::MeCab.new` が実行時に落ちるだけなので、依存を足す話にはならない。
      - **実装形は「辞書を 2 段に分ける」。** `if mecab then 検出する` という分岐を各所へ書くのではなく、**MeCab が要らない語（現在の 140 語＝単独で安全か、正規表現で守れるもの）と、境界チェックなしでは危険な語（`合祀`・`摸本` など）を別の定数に持ち、起動時にどちらを使うか決める**。分岐は `check_mecab_available` 相当を呼ぶ 1 箇所で済む。
      - **この形なら、MeCab の有無で「指摘が減る」ことはあっても「別の指摘が出る」ことはない。** 140 語ぶんの結果は誰の環境でも同じで、MeCab を入れた人が追加で拾えるだけになる。**再現しない指摘を出さないことは lint の信用に直結する**——同じ原稿で結果が変わると、著者はどちらを信じてよいか判断できない。
      - **急がない**——現在の 140 語は誤検出 0 件で、この仕組みが要るのは「単独では危険な語まで載せたい」場合である。
    - **実装時の注意**: 検証は `Masking.protect_code` を通した行（インラインコードが `__CODE_SPAN_0__` に置き換わったもの）を MeCab へ渡したため、分割位置が元の文とずれた箇所がある。上の 27 箇所にはそのぶんの取りこぼしが含まれるので、**実際の削減率は 9 割より高い**。実装では素の文を解析すること。
