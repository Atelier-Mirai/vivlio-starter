...
### 8.6 表が増えても「枝の余裕」は減らない（2026-08-01 実測）

抽出コストの内訳を測った（A5・515 ページ・101MB の PDF に対して）。

| 対象 | 実測 | 性質 |
|---|---|---|
| `/Dests` 走査（`pages_for`） | 0.9〜4.7s | **固定費**（表の枚数に依存しない） |
| qpdf でページを抜く | 0.46s | ほぼ固定 |
| ラスタライズ＋切り出し | **0.92s / 枚** | 枚数に比例 |

30 枚なら抽出は約 28 秒。**それでも EPUB 枝の余裕（71 秒）は減らない。**
抽出は PDF 枝にあるので、コストが増えると**両方の枝が同じだけ伸びる**——PDF 枝には
直接、EPUB 枝にはラッチの待ち時間として。

```
30 枚のとき: PDF 枝 343.4 + 26.8 = 370.2s / EPUB 枝 272.1 + 26.7 = 298.8s → 差 71.4s（不変）
```

余裕を削るのは **EPUB 枝側の per-image 作業**（PNG のコピー・KPF が扱う画像点数）だけで、
これはごく小さい。増えるのは**ビルド全体の時間（1 枚あたり約 0.9 秒）**である。

なお `/Dests` 走査が固定費である以上、表が 1 枚でも 30 枚でも「最低 1 秒前後」はかかる。
表が 0 枚のビルドではステップごと登録されないので、この固定費は発生しない。

### 8.7 KFX では `margin: auto` が効かない（実機確認・2026-08-01）

差し替えた画像が**ページ左端に寄って描画された**（垂直方向は中央）。
当初は `display: block` ＋ `margin-inline: auto` で中央寄せしていたが、KFX は
論理プロパティも `margin: auto` も当てにできない。

**`text-align: center` ＋ インライン画像**という古い組み合わせへ変えた。これは EPUB で
画像を中央に置く定石でもある。画像を `display: block` にすると `text-align` が効かなく
なるため、display は既定のまま触らないこと。`kindle-css-compatibility-notes.md` に並ぶ
既知の制約（`var()` / `color-mix()` / flex / 擬似要素）と同じ性質の落とし穴。

### 8.8 残件

- **Kindle Previewer での再確認**。§8.7 の中央寄せが効いているか。画像は 289×1535 と
  縦長で、`.rotate-table` の箱（`88vh`）に収める CSS も入れてあるが、KFX が `vh` と
