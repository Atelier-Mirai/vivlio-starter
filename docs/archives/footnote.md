...
>    脚注番号もドキュメント出現順に並ぶ。
>
> **【再検証 2026-06 / Vivliostyle CLI 11.0.2・core 2.43.2】** メジャーアップ後、不可視 span ハックを
> 外して `href="#fnN"` を `aside` 自体へ解決させる実験ビルドを行ったところ、**脚注の重複描画が再発**した
> （`00-preface` で脚注1が3回描画・後続脚注が脱落・総ページ数が膨張）。core 2.43.2 でも本バグは未解消のため、
> 不可視 span 二重ノードハックは引き続き必須である。詳細は `docs/specs/footnote-simplification-spec.md` §8 を参照。
>
> 以下は調査当時の記録として残す。
