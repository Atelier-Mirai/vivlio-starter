...
### 10.3 出力機会のない `log_info` 群の整理 — ✅ 完了（2026-07-28）

D3 により `--log` を配らないコマンドでは `log_info` 系が出力されない。⑤ で応答メッセージへの統合が済んだので整理した。

**残したもの（ビルド経由で生きている）**

`clean.rb`(28) / `resize.rb`(15) / `create.rb`(11) / `index/`(45) / `pdf.rb`(19) の計 118 箇所は、`vs build --log` の実行中に**実際に表示される**（Step 0 の clean、Step 1 の resize、索引処理など）。削除すると `vs build --log=debug` の情報が失われるため残す。**「`--log` を持たないコマンドのコードだから未使用」とは限らない**——ビルドから呼ばれるかを必ず確認すること。

**削除したもの（37 箇所）**

`rename.rb`(12) / `delete.rb`(6) / `import.rb`(18) / `lint`(3)。いずれもビルド経路から呼ばれず `--log` も持たないため、出力される機会がなかった。

**削除ではなく格上げしたもの — 整理中に見つかった UX 上の欠陥**

`log_info` を精査したところ、**確認プロンプトの判断材料が既定で見えていない**箇所があった。

| 箇所 | 問題 | 対応 |
|---|---|---|
| `rename` 一括連番の変更予定一覧 | 全章の番号が動く操作なのに `❓ 連番付け直しを実行しますか？` しか見えなかった | `log_always` へ格上げ。**全件を並べる**——`--step=2` のような飛び番指定でも「どういう連番になるか」が読み取れるため（実行前に予定が出るので dry-run も兼ねる） |
| `rename` 単章の変更予定 | 同上（`71-talk-sample → 72-talk-sample` が見えなかった） | `log_always` へ格上げ |
| `rename` の章番号重複エラー | `🔴 N章のファイルが複数見つかりました:` の後、**どのファイルが競合したかが見えなかった** | `log_error(detail:)` へ移して必ず表示 |
| `lint --register` の登録実績 | `ユーザー辞書へ N 語を登録しました` が `log_success`（info）で見えなかった | `log_result` へ格上げ |

**あわせて撤去したデッドコード**: `rename.rb` の dry-run 分岐 4 箇所。Samovar のオプション定義に `--dry-run` が無く `options[:dry_run]` は常に nil で到達不能だった（`vs index:build` と同じパターン・昔の実装の名残）。実行前に予定が表示されるようになったため、機能としても不要になった。
