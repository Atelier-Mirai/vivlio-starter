...
### 7.1 build 側の「ほか警告 N 件」（§2.4）を撤去した

`⚠️ ほか警告 N 件（詳細は vs preflight で確認できます）` は「リンク・画像以外の警告が
埋もれないよう件数だけ知らせる」ためのものだったが、**実際には常に再掲になっていた**。

想定と実態のずれは 2 つ。

1. **他の発生源も逐次ログで本文を出している。** Guard・クロスリファレンス・QueryStream・
   索引はいずれも `log_warn` を出しており、埋もれてはいなかった。件数を足すと、
   同じ 1 件が本文と件数の 2 回語られる。
2. **案内先でも件数しか出ない。** 「詳細は vs preflight で」と誘導しても、
   preflight の章別サマリーは件数の表なので、辿った先で同じ件数に行き着くだけだった。

実例（索引の主要参照の警告）: 本文 2 行 → build の「ほか警告 1 件」→ preflight の
「章に紐付かない指摘 1 件」と、1 件が 3 回顔を出していた。

**`IssueRegistry` は集計専用に戻す。** 総括（`⚠️ Preflight 完了: 警告 N 件`）にだけ
効かせ、メッセージも件数も再表示しない——§2.1 の
「registry は集計専用で表示を一切行わない（二重表示を避けるための責務分離）」が本来の設計で、
§2.4 はそこから外れていた。

一度は「件数だけでは何の警告か分からない」と読んで**内訳を出す方向で直したが、これも誤り**
だった。分からなかったのは内容ではなく、なぜ同じことを何度も言われるのかである。
表示を足すのではなく、重複を消すのが正しい向きだった。

### 7.2 章別サマリーの「章に紐付かない指摘」行も撤去した

`章に紐付かない指摘  🟡 警告 1 件` という行は、**著者に何も伝えていなかった**。
何について警告されていて、どう直せばよいかが分からない。指摘するなら
その 2 つが要る（`warning-messages-actionable` の流儀）。

そして**それは既に満たされている**。この枠に入る指摘（Guard 警告・索引まわり）は
すべて発生源が本文で言い切っており、直し方まで添えている:

```
🟡 主要参照が未指定の索引語が 55 語あります（索引が引きにくくなります）
🟡  vs index:auto を実行すると、章の候補付きでレビューファイルに一覧できます
```

表に件数を足すと、この 1 件を二度語ることになる。**章別サマリーは章の話に徹する。**
章に紐付かない指摘は集計としては最終行（`⚠️ Preflight 完了: 警告 N 件`）に効いており、
「画面に 🟡 が出ているのに『良好』と総括しない」という §0-2 の目的は保たれる。

これに伴い `UNATTACHED_LABEL` は削除した。全章 ✅ かつ横断的な警告があるときは
`✅ 全 27 章: 問題なし` ＋ `⚠️ Preflight 完了: 警告 1 件` と並ぶ——章としては問題なく、
横断的な警告が 1 件ある、という事実そのままである。

### 7.3 preflight の進行表示が「ビルド中」と名乗っていた

スピナーの見出しが `UnifiedBuildPipeline` 共通で「ビルド中: …」固定だったため、
原稿を検査するだけの preflight でも「ビルド中」と出ていた。出力物ができると
誤解させ、実行を止める判断を誤らせる。`mode: :preflight` では「点検中: …」と出す
（`pipeline.rb` の `PROGRESS_VERB`）。
