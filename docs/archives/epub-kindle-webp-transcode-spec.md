...
改名案は「危険文字を**削除**し、連続する `_` は 1 つに畳む」素朴な正規化で生成する（提示のみ。実ファイルは変更しない）。削除方式により `Einstein's` → `Einsteins` のように自然な名前になる（`_` 置換だと `Einstein_s` になる）。
...
`guards/`（既存 `images_dir_check.rb` の隣）に新ガード（例: `ImageFilenameCheck`）を置き、`build` / `preflight` コマンド冒頭の `Guards::Guard.run!` に加える（warn-only なので非ブロッキング。既存の guard 実行作法に整合）。

**`vs import` の取り込み時サニタイズ（実装済み）**: Re:VIEW Starter から取り込む `vs import` でも同じ基準でファイル名を正規化し、「今後の取り込み」への恒久防御とする。判定・除去の基準は `lib/vivlio_starter/cli/image_filename_sanitizer.rb`（`ImageFilenameSanitizer`）に一元化し、検出（ガード）と取り込み（import）で共有する。import では**実体と参照の双方**に同一 sanitize を掛けて食い違いを防ぐ:
- `import/image_processor.rb` の `copy_images_to_local`: コピー先ファイル名の stem をサニタイズ。
- `import/markdown_converter.rb` の `convert_img_tags` / `normalize_image_paths`: markdown 画像参照のファイル名をサニタイズ（実体と一致）。
