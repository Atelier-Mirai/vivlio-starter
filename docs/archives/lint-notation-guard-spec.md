...
> 関連: `lib/vivlio_starter/cli/lint.rb`, `lib/vivlio_starter/cli/masking.rb`, `lib/vivlio_starter/cli/lint/tokenizer.rb`, `config/textlint_allowlist.yml`, `test/vivlio_starter/robustness/lint_fix_interrupt_test.rb`, `explanatory-diagram-spec.md`
...
| G2 | コンテナのマーカー行 | `/\A[ \t]*:{3,}[ \t]*(?:\{[^{}\n]*\})?[ \t]*\r?\n?\z/`（コロン＋任意の属性ブレースで**行全体が構成される**もの。G1 のブロック外） | 行ごと空行化 | `:::{.column}` / `::::` / 閉じ `:::`。記法であって文ではない。**コンテナ内部の地の文は検査対象のまま**（マーカー行だけを消す）。「`:::` で始まる行」まで広げてはならない——理由は §7.1 |
...
## 7. 実装時の追記（2026-07-16・実装完了）

実装・検証の過程で仕様の 2 点を修正し、隣接する既存バグを 1 件掘り当てた。

### 7.1 G2 の厳格化（textlint 暴走インシデント）

当初案の G2（`/\A[ \t]*:{3,}/` = 「`:::` で始まる行はマーカー」）は広すぎた。
94 章にはコメントアウトされた表 `<!--::: {.long-table}` 〜 `:::-->` があり、閉じ行
`:::-->` を空行化すると **HTML コメントの `-->` が消えて未閉じコメント**になる。
その中の表＋数式を読んだ textlint が返らなくなり（CPU 99%・タイムアウトなし）、
`vs lint` が永久にハングする実害が出た。

G2 は §3.3 の表のとおり「コロン＋任意の属性ブレースで行全体が構成される行」へ厳格化した。
判定に迷う行はガードせず素のまま textlint へ渡す（＝ガード導入前と同じ扱い）が常に安全側、
という原則を不変条件に加えた（**ガードは文書構造——コメントの開閉——を変えてはならない**）。
回帰テスト: `test_should_not_blank_commented_out_container_closer`。

### 7.2 一時ファイルの GC 削除（既存の潜在バグを同時修正）

`convert_vs_lint_comments` は `Tempfile.new` で作ったオブジェクトを保持せずパス文字列だけ
返していたため、**GC がファイナライザ経由で一時ファイルを削除**し得た。textlint は存在しない
パスを黙って無視するので、「一部のファイルだけ検査されない」事故になる（全章 25 ファイル中
20 ファイルが未検査になる形で顕在化。ガードの割り当て増で GC が発火するようになり発覚したが、
従来も textlint 実行中に GC が走れば起こり得た flaky 爆弾）。
`Tempfile.create`（ファイナライザ登録なし・削除は `cleanup_temp_files` のみ）へ変更した。
回帰テスト: `test_converted_tempfiles_survive_garbage_collection`。

### 7.3 検証結果（§3.9 の実測）

- 手順 1: showcase 由来の誤検出 9 件（22 章 7・94 章 2）が 0 件になった。消えた指摘は
  すべてブロック内のもので、地の文への指摘は 1 件も減っていない（I3 成立）。
  22 章で `ja-no-mixed-period` が 2→7 件に増えたが、全件目視で「。で終わらない段落への
  正当な指摘」と確認（allowlist の `":::"` / `/:::$/` が段落末尾に癒着したマーカー越しに
  飲み込んでいた分の復活。報告書 §3 問題 3 の実例）。
- 手順 2: allowlist の VFM 5 エントリ撤去後、指摘は**完全に同一**（+76 件の再発なし・
  `":::"` エントリの残渣もなし。報告書 §7 論点 4 は「残渣なし」で決着）。
- 手順 3: 全章 25 ファイルが 8.4 秒で完走（765 箇所・textlint プロセス残留なし）。
