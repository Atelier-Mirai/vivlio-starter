...
> 関連: `nested-list-notation-ideas.md`（検討経緯）, `kindle-css-compatibility-notes.md`（KFX 制約の実測記録）, `post-replace-list-retirement-spec.md`（`:::` div 化の現行実装）, `lib/vivlio_starter/cli/pre_process/markdown_transformer.rb`（定義リスト＝実装前例）, `lib/vivlio_starter/cli/build/epub_builder.rb`（ADMONITION_LABELS＝注入前例）
