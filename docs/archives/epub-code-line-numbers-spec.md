...
> 関連: `lib/vivlio_starter/cli/build/epub_builder.rb`（`convert_code_blocks_for_epub!` ほか・行スプリッタ）, `stylesheets/code.css`, `stylesheets/prism.css`, `kindle-css-compatibility-notes.md`, `code-include-line-number-spec.md`（**§8 連携必読**・`data-start` と `--code-font` 修正はあちら側）

## 0. 現状と問題（2026-07-12 時点）

- **PDF**: Prism `.line-numbers-rows`（絶対配置ガター）。ページ幅固定のため崩れない。**本仕様の変更対象外**。
  - ⚠️ **2026-08-01 追記: この前提は誤りだった。** 「ページ幅が固定」と「行が折り返さない」は別で、`code.css` が全 `pre` に `white-space: pre-wrap` を掛けているため PDF でも長行は折り返し、クリーン EPUB と同じ番号ずれが起きていた。PDF にも F 案を敷いて解消 → `pdf-code-line-numbers-spec.md`
