...
| `lib/vivlio/starter/cli/index/index_match_scanner.rb` | 22 | Metrics/AbcSize: 6件, Layout/LineLength: 4件, Metrics/CyclomaticComplexity: 2件 |
...
| `lib/vivlio/starter/cli/build/utilities.rb` | 5 | Naming/PredicateMethod: 2件, Metrics/ModuleLength: 1件, Style/SelectByRange: 1件 |
| `lib/vivlio/starter/cli/build/backlink_deduplicator.rb` | 5 | Style/ReduceToHash: 2件, Metrics/ClassLength: 1件, Metrics/AbcSize: 1件 |
...
| `lib/vivlio/starter/cli/metrics/analyzer.rb` | 4 | Metrics/AbcSize: 2件, Metrics/ClassLength: 1件, Style/TallyMethod: 1件 |
...
自動修正（`--autocorrect` + `--autocorrect-all`）で1595件から887件に削減。RuboCop 1.86.1 + TargetRubyVersion 4.0 に更新後、Ruby 4.0 新cop（`Style/ReduceToHash`・`Style/TallyMethod`・`Style/SelectByRange` など）が新たに検出されている。

残存違反の大半は Metrics 系（AbcSize, CyclomaticComplexity, PerceivedComplexity）であり、メソッド分割等の手動リファクタリングが必要。
