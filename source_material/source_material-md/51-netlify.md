...
ウェブサイト一式を、例えば「`study`フォルダ」に作成したのであれば、
...
Netlifyは、小規模利用 [^190]なら無料でサイトを公開できるサービスです。
...
[^190]: Starterプラン(無料プラン)は、転送量: 100GB/月まで、容量: 100GB までです。作成した桜吹雪は約700kBの容量があります。単純計算ですが、毎日5000人が見るようになったら、有料プランへの移行を考えると良いでしょう。
...
- 2. しばらくすると、デプロイ（配備・配信）が完了します。`steady-paletas-13bb99`というサイト名で作成されたようです。 <br>
...
- 6. サイト名を `sakura-fubuki-netlify.app` に変更することが出来ました。

![](netlify38.png)
---
`sakura-fubuki-netlify.app` をクリックすると、今まで手元のコンピュータ上で動作確認していた「桜吹雪」が、今や世界中の人々に見られるように公開されています。
...
- 2. `Sites` をクリックします。今まで公開しているサイトの一覧が表示されますので、削除したいサイト(ここでは sakura-fubuki のみですが)を選び、クリックします。
