...
[注文住宅なら京都市で設計施工を行う工務店 garDEN](https://gar-den.jp/) を元に適宜改変いたしました。 下の作成例のように、画面下にメニューが配置されます。[動作例](https://wave-improve.netlify.app/follow_menu/index.html)をごらんになってください。また [ソースコード](https://github.com/Atelier-Mirai/wave-improve/tree/master/follow_menu) もございます。
...
追随メニューは、フッターではないので`<footer id="fixed_footer">`が無くなりました。そして CSS から扱いやすいよう、 `<nav class="follow_menu">` とクラス名を付与しています。
...
少し前の記事とはなりますが、[日本らしさを表現 CSSで文字の縦書きに挑戦](https://www.webcreatorbox.com/tech/writing-mode) により詳しい解説がございますので、ご参考にしていただければと思います。
