...
![](dom11.png)
もとの HTML は、`<li class="software">エディタ <span style="display: none;">(Pulsar / Zed / VS Codeなど)</span></li>` と書かれていましたが、 `textContent` メソッドにより `<li class="software">エディタ (Pulsar / Zed / VS Codeなど)</li>` と、元あったの `<span>` タグも消去され、`textContent` での設定文字になることにも注目してください。
