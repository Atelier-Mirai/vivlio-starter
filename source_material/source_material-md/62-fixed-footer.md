...
[注文住宅なら京都市で設計施工を行う工務店 garDEN](https://gar-den.jp/) を元に適宜改変いたしました。 下の作成例のように、画面下にメニューが配置されます。[動作例](https://wave-improve.netlify.app/fixed_footer/index.html)をごらんになってください。また [ソースコード](https://github.com/Atelier-Mirai/wave-improve/tree/master/fixed_footer) もございます。
...
文学作品の `<p>`タグはもう少し必要ですが、紙幅の関係で割愛しています。
...
また、作成例では隠れていますが、ある程度スクロールすると、「電話をかける」ためのリンクも出現します。それを実現しているのが、11-14行目です。 `active`クラスの有無で位置を調整するようにしています。
