...
@<href>{https://e-words.jp/w/関数.html, IT用語辞典} より、その定義をご紹介いたします。

//tip{
**関数(@<ruby>{function, ファンクション**)}

関数とは、コンピュータプログラム上で定義されるサブルーチンの一種で、数学の関数のように与えられた値（@<ruby>{引数,ひきすう}）を元に何らかの計算や処理を行い、結果を呼び出し元に返すもののこと。

//blankline
プログラム上で関連する一連の命令群を一つのかたまりとしてまとめ、外部から呼び出せるようにしたサブルーチンやプロシージャ（手続き）の一種である。呼び出し時に@<ruby>{引数,ひきすう}と呼ばれる値を指定することができ、この値をもとに内部で処理を行って、結果を@<ruby>{返り値,かえりち}あるいは@<ruby>{戻り値,もどりち}として呼び出し元に通知する。
...
このテキストでは、@<code>{function} というキーワードの存在が、始めての方に分かりやすいかと考え、従来型の関数宣言でコーディングしていきますが、後の章ではアロー関数も使って行きます。

「アロー関数」は、従来型の関数宣言と全く同一のものではありませんが、簡潔に書けるよう、これからの主流として導入され、広く用いられていますので、ぜひ使ってみてください。

//blankline
またより詳しい説明が、MDN @<href>{https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Functions/Arrow_functions,アロー関数式} にございますので、ご興味を持った方はお読みになってください。
