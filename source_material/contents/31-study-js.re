= 速習 @<ruby>{JavaScript,ジャバスクリプト} 入門
...
== JavaScript の 使用方法
JavaScriptは、HTMLに組み込まれて使われることが多いプログラミング言語です。 @<fn>{tantai}

//footnote[tantai][@<ruby>{Node.js, ノード・ジェイエス}など、ウェブサイトに読み込まれるのではなく、サーバサイドでも実行される環境もあります。]

次に紹介するのは、画面に「こんにちは。JavaScript」と表示するためのプログラムです。

まず、土台となる @<file>{index.html} を以下のように記してください。
...
@<code>{<head>}内で、スタイルシートとJavaScriptを読み込むよう、記述しています。
...
これにより @<file>{hello.js} という JavaScriptのプログラムを読み込んで実行できるようになります。
...
そして @<file>{hello.js} という JavaScriptのプログラム は次のように書きましょう。
...
書き終わりましたら、ブラウザから @<file>{index.html} を開いてみましょう。画面には「こんにちは。JavaScript」と言うメッセージが表示されます。

//image[hello_js][][width=80%]
...
一行目の @<code>{//} と書かれた部分は、コメントです。 コードの説明書きに使います。
...
@<code>{alert('表示させたいメッセージ')} と書くことで、@<code>{alert関数}が使えます。
...
//note[=(イコール)記号について]{
@<code>{=(イコール)}は、数学ですと、**ageと1が「等しい」**ということを意味する記号ですが、プログラミングの世界では **左の変数に右の値を入れる（代入する）** という記法になります。
...
始めのうちは **@<code>{age ⇦ 1**} のことだと、頭の中で置き換えると良いでしょう。
//}
...
//sideimage[shortcut][60mm][sep=5mm,side=R]{
...
//image[console0][][width=80%]

左側には記述したHTML文書が表示されています。@<code>{<h1>JavaScript練習</h1>} をクリックして選択すると、中程の欄には今選択した要素@<code>{<h1>}に適用されているCSSが表示されます。右側の欄は、グリッドやボックスモデルなどCSSに関する詳細が表示されています@<small>{(配置場所や大きさなどは調整可能ですので、画面と異なる配置等となっているかもしれません。)})。
---
...
//list[][][10]{
    <!-- JavaScriptの読み込み -->
    <script src="age.js" @<b>{defer}></script>
//}
...
後ほどご紹介いたしますが、JavaScript を使って、HTML の各要素を操作することも出来ます。例えば @<code>{<h1>}の文字を変更することも出来るのですが、15行目にある @<code>{<h1>} はまだ読み込まれていないので、実行出来ずにエラーとなります。

@<code>{@<b>{defer}} は「遅延読み込み」を意味しており、HTMLの読み込み完了を待ってから、JavaScript を実行させることができます。

それでは、ブラウザを再読み込みして、コンソール画面を見てみましょう。
警告も消え、すっきりしました。

//image[console2][][width=80%]


=== 値を表示させる

読み込んだ @<file>{age.js} には次のように書いていました。
...
ブラウザで読み込むと、現在の @<code>{age} の値 @<code>{0} が、右のように表示されます。

コンソール画面でも、@<code>{age} の値 を表示させて見ましょう。
//}
...
現在の @<code>{age} の値 @<code>{0} が、コンソール画面でも表示されます。

//image[console3][][width=100%]


//sideimage[console5][60mm][sep=5mm,side=R]{
プログラムファイル @<file>{age.js} から、読み込んだ @<code>{age} の値 @<code>{0} を表示させるのみならず、直接、コンソール画面で変数を宣言したり、その値を確認することも出来ます。
...
//sideimage[console6][80mm][sep=5mm,side=R]{

ちなみに同じ変数名で二回宣言することは出来ません。

@<code>{age} は、プログラムファイル @<file>{age.js} で宣言したものを読み込んでいますし、@<code>{height} は先ほど宣言しました。
...
先程の変数の例では、@<code>{age = 0}や@<code>{height = 50}と書いていました。@<code>{age}という変数に@<code>{0}を代入しており、「年齢が零歳である」、「身長が50cmである」とすぐ理解することができます。

他の例を挙げて見ましょう。例えばじゃんけんゲームを作ることを考えて見ます。プレイヤーの手とコンピュータの手を比べて、強かったほうが勝ち というゲームです。

グーを @<code>{0}、チョキを @<code>{1}、パーを @<code>{2} で表すことにすると、
...
ここで、この @<code>{0} という数値は一体どういう意味なのでしょうか。グーを意味しているのか、あるいはチョキなのか、パーなのか、覚えておくのも大変です。
...
@<code>{let computer = GUU}と、プログラムを書くことが出来るようになりました。@<code>{let computer = 0} と書くよりも文字数は増えますが、@<code>{0}, @<code>{1}, @<code>{2} の単なる数字とは異なり、その意味するところが明確で分かりやすくなりますので、活用してきましょう。
...
@<code>{age} という名前の変数や、@<code>{GUU} という名前の定数を宣言しましたが、変数や定数にはどのような名前を付けられるのでしょうか。
 ...
 * @<code>{let player_name;       // _ で繋げても良いです @<small>{(JavaScriptでは一般的ではありません)}}
 ...
 * @<code>{let 20th_century_pear; // 数字から始まる変数名は使用出来ません}
 * @<code>{let twentieth-century; // -(ハイフン)は使えない記号です}@<br>{}
...
正負を反転させるための演算子である、@<code>{-} は、一つの項のみを必要とするので、単項演算子と呼ばれます。
...
JavaScriptでは、冪乗を行う演算子として、@<code>{**} を使います。@<code>{*(アスタリスク)} を空白を開けずに続けて書いてください。2つでひとまとまりの冪乗の為の演算子になります。
...
@<code>{scores = []} と書き、中身が空っぽの配列を作成しています。次に、@<code>{scores = [85, 95, 85, 75]} と書き、 空っぽであった @<code>{配列 scores} の中に、 四つのの要素 @<code>{85, 95, 85, 75} を格納しています。@<fn>{array}

//footnote[array][容器だけで中身は空っぽの幕の内弁当と、おかずとして 85, 95, 85, 75 が入っている幕の内弁当を想像すると分かりやすいでしょうか。]
...
@<fn>{function2}
//footnote[function2][例えば、@<code>{if文}を学ぶ際に登場した@<code>{isSpecialDay()}関数もそうです。]
...
//blankline
またより詳しい説明が、(準)公式サイトMDN @<href>{https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Functions/Arrow_functions,アロー関数式} にございますので、ご興味を持った方はお読みになってください。
...
四角形ですので、幅 @<code>{width} と 高さ @<code>{height} があります。そして面積 @<code>{area}を求める為の関数も用意します。
...
一個の四角形の雛形となるもの、それが「クラス（分類）」です。 @<fn>{class}
...
=== 初めてのDOMツリー - @<code>{HTML}要素を取得・表示する

それでは、理解の為に、例を使って練習しましょう。

練習(practise)用に、@<file>{index.html} という HTMLファイルと @<file>{practise.js}
...
@<file>{index.html} に関しては、11行目で @<file>{practise.js} を読み込むようにしています。

@<file>{practise.js} は次のように書きましょう。
...
HTML では、複数の@<code>{<li>}要素や、@<code>{class}属性、@<code>{id}属性も登場しています。
@<code>{<span style="display: none;">} と 表示させないようインラインCSSが記述されていたり、@<code>{JavaScriptは、<br>「相互作用・動き・制御」の為の言語です。} と @<code>{<br>} タグで改行されています。
...
//}
  ...
  「**探す要素の ID**」とありますので、
  @<code>{document.getElementById()} を使って、DOMツリーから ID が @<code>{"html"} の要素 を選択したい場合には、@<code>{document.getElementById("html")} と書けば良いことが分かります。
...
「@<code>{selectors} の文字列は妥当な CSS セレクタでなければなりません。」とありますので、CSSで @<code>{<h1>}要素を選択する際に@<code>{h1}と書くのと同じように、@<code>{document.querySelector()} を使って、DOMツリーから @<code>{<h1>}要素 を選択したい場合には、@<code>{document.querySelector("h1")} と書けば良いことが分かります。

 : @<code>{document.querySelector("li")}
    単一のセレクタで指定すると、@<br>{}
    (複数の@<code>{<li>}が該当しますが、最初に見つかった)@<br>{}
    @<code>{<li class="hardware">コンピュータ <span>(Mac / Linux / Windows など)</span></li>} を取得できます。
 : @<code>{document.querySelector("ol li span")}
    (@<code>{li}のような単一のセレクタではなく) 複数のセレクタを並べると、@<br>{}
    @<code>{<span>(Mac / Linux / Windows など)</span>} を取得できます。
 : @<code>{document.querySelector(".hardware")}
    クラス名を指定すると、@<br>{}
    @<code>{<li class="hardware">コンピュータ <span>(Mac / Linux / Windows など)</span></li>} を取得できます。
 : @<code>{document.querySelector("#html")}
    ID属性を指定すると、@<br>{}
    @<code>{<li id="html">HTMLは、「文書構造」の為の言語です。</li>} を取得できます。@<br>{}
...
ですので、@<code>{document.querySelector("li")} と書くと、六つの @<code>{<li>} 要素の内、最初の一つを得ることが出来ます。

六つの @<code>{<li>} 要素 全てを取得するには、@<code>{document.querySelectorAll()} メソッドを使います。

@<code>{document.querySelector**All**("li")} と書くと、六つの @<code>{<li>} 要素、「**全て**」を取得できます。
...
先に学んだ @<code>{document.querySelector()}メソッドと合わせて、要素内の HTML を取得したり、設定したりしてみましょう。
...
それでは、次に、 @<code>{<h1>}要素に、マークアップを設定して見ましょう。
...
コンソール画面で入力すると、@<code>{JavaScript<br>練習} と、@<code>{<br>} を入れましたので、改行されて表示されるようになりました。
...
もとの HTML は、@<br>{}
　　　@<code>{<li class="software">エディタ <span style="display: none;">(Pulsar / Zed / VS Codeなど)</span></li>} @<br>{}
と書かれていましたが、 @<code>{textContent} メソッドにより@<br>{}
　　　@<code>{<li class="software">エディタ (Pulsar / Zed / VS Codeなど)</li>} @<br>{}
と、もともとあった @<code>{<span>} タグなどは消去されて、@<code>{textContent} で設定した文字のみになることにも注目してください。
...
コンソール画面に入力した結果は次の通りです。
//image[className1][][width=80%]

@<code>{.hardware} クラスには、 @<code>{.hardware { border-bottom: solid 3px lime; }} と黄緑の下線を引くよう指定していましたが、クラス名が @<code>{.effect} に変更したことに伴い、 @<code>{.effect { font-weight: bold; font-style: italic; }} と、太字の斜体で表示されるようになりました。

「インスペクタ」画面で確認すると、class名が @<code>{.effect} に更新されていること、CSS として、@<code>{{ font-weight: bold; font-style: italic; \}} が適用されていることが確認できます。

//image[className2][][width=80%]
...
先に取得しておいた @<code>{cName} には、もともとのクラス名 @<code>{hardware} が保存されていますので、「式展開（テンプレートリテラル・テンプレート文字列）」を使い、クラス名を追加しています。 @<small>{``(バッククォート)で囲みます}
