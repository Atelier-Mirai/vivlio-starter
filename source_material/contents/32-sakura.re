...
@<code>{</body>} 直前に桜吹雪のための CSS @<file>{hanabira.css}と JavaScript @<file>{hanabira.js} を読み込んでいます。

== 桜の花びらの為のCSS
@<file>{hanabira.css} は、桜の花びらの色やひらひら回転するCSSアニメーションのためのCSSです。
...
最初(@<code>{from})は、回転角0度(@<code>{rotate(  0deg)})で、@<br>{}
中ほど(@<code>{50%})は、回転角270度(@<code>{rotate(  270deg)})となり、@<br>{}
最後に(@<code>{to})は、回転角1度(@<code>{rotate(  1deg)})になるまで回転します。@<br>{}
そして、@<code>{scale(.9)}とありますので、 @<code>{.hana} で作成した花びらの大きさより少し小さ目(@<code>{90%})の花びらとなります。
//blankline
...
と書いて、ブラウザを再読み込みさせてください。左上でゆらゆらと回転する一枚の桜の花びらが見られるはずです。@<code>{t1} に代えて @<code>{t5} にすることで、花びらの色が濃くなります。@<code>{a1} に代えて @<code>{a5} にすることで、花びらの形が小さく、また回転も変わりますので、確認してみましょう。
...
今回の例で言えば、手書きで、五十枚の花びらを @<code>{<div class="hana t1 a1"></div>} のように書くのではなく、JavaScriptに 描画させたら良いですし、それぞれの花びらが、一定時間ごとに右や左、下へ動いていくよう、記述できます。
...
それぞれの花びらが区別できるように、IDに関する配列、 @<code>{X}座標に関する配列、 @<code>{Y}座標に関する配列等、それぞれの属性ごとに配列を用意すれば良いでしょう。
...
ですが、 このような設計ですと、1枚の花びらに関する情報が、 @<code>{X}座標に関する配列、 @<code>{Y}座標に関する配列などに分散することとなります。色や動きに関する特性も用意したいですし、落下速度をそれぞれのはなびらに設定しようと思った場合に、また新しい配列を用意することとなります。
...
一枚の花びらの雛形となるもの、それが「クラス（分類）」です。 @<fn>{class}
...
四角形クラスから生み出されてくる四角形には、例えば幅が100ピクセルで高さが20ピクセルものもあるでしょうし、幅が30ピクセルで高さも30ピクセルのものもあるでしょう、そして、それぞれ生み出されたオブジェクト（インスタンス・実例）としての個々の四角形は、面積を求めるための @<code>{area}と言うメソッド（機能・関数）を持っていますので、ある四角形の面積は2000平方ピクセルですし、別の四角形の面積は900平方ピクセルといった具合に、個々のオブジェクトに備わっているメソッドを呼び出すことで、求めることができます。


=== 「花びらクラス」を設計する

それでは、オブジェクト指向の考え方に基づいて、花びらクラスを設計してみましょう。花びらクラスを@<ruby>{鋳型,いがた}として生成される実例（インスタンス）は、自分自身の情報データ(属性)とその属性の操作方法(メソッド・関数) @<fn>{method}を持ちます。
...
まずたくさんの花びらが作られますので、どの花びらかわかるよう、他の花びらと区別するためのID属性が必要です。次に画面上のどの位置に表示されているのか、横方向(X座標)と縦方向(Y座標)それぞれの位置を表す属性が必要です。 @<fn>{zahyo}

//footnote[zahyo][(通常) 画面の左上がX座標、Y座標ともに0となる点(0, 0)です。X座標は画面の右へ行くほど大きくなります。またY座標は画面の**下**に行くほど大きくなります。]

また花びらの重なり順を表すためにZ軸方向つまり画面の手前にあるのか奥にあるのかを表すための属性が必要です。CSS の @<code>{z-index} に倣って、手前にあるものほど大きな値を持つとしましょう。
...
まず、様々な花びらクラスのインスタンス（実例）を生成することになるわけですが、その際に、それぞれの花びらの各属性(id, x, y, z など)の初期値を設定する必要があります。初期値を設定するために、JavaScriptではコンストラクタと呼ばれるメソッド(関数)が用意されています。コンストラクタは日本語では「@<ruby>{構築子,こうちくし}」と翻訳されます。
...
作成した花びらクラスのインスタンスの各属性を更新する為の関数と、そして実際のHTML上のdiv要素 @<code>{<div class="hana t1 a1"></div>} に動いた位置を反映する為の関数を用意しましょう。
...
//sideimage[UML_Class_Diagram][50mm][sep=5mm, side=R]{
  右は、**UMLクラス図** と呼ばれる図で、システム設計の際などに用いられます。@<small>{(今回のさくらの花びらが舞い散るプログラムでは、あまり必要となることはありませんが)} より大きなプログラムを作成される際には、@<href>{https://cacoo.com/ja/,cacoo} などのツールの利用や手書きされるなど、いきなりコードを書く前に、必要となるクラス等を整理する助けにもなってくれることでしょう。
//}
...
花びらを不規則に配置し、その位置を変化させる為には「乱数」を用いると便利です。JavaScriptには、 @<code>{Math.random()関数} が用意されており、**0以上1未満の浮動小数点数(≒小数)**を取得することが出来ます。
...
@<code>{rand(1, 5)} で、1以上5以下の乱数が得られます。 @<code>{rand(0.2, 0.7, "float")} では、0.2以上0.7未満の乱数が得られます。

@<code>{=>} が「矢印(arrow)」のように見えることから、「アロー関数」と呼ばれる書き方で、従来の @<code>{function} を使うと、以下のようになります。
...
整数と浮動小数点数、どちらでも使えるようにしたいのですが、整数を使うことが多いので、 引数に @<code>{type="integer"} と書き、整数を既定値にしています。次に @<code>{if}文の中で条件判定を行い、整数、浮動小数点数、それぞれの型(@<code>{type})に応じた乱数を返すようにしています。

@<code>{Math.floor()}は、「床関数」と呼ばれる、小数点以下を切り捨てるために用意されているJavaScriptの関数です。

#@# 数値を廃止ようにしますもし がいいでしょうであった場合には、まずフロアは 塩酸結果の小数部を切り捨てるための切り捨てるために用意されているジャバスクリプトの関数で、床関数です。最大値から最小値を引いた値、つまり、空間の幅に乱数値をかけ、それを最初兆足すことでシフトさせ、Siri温館の乱数求めています。
...
//}
 ...
 : 定数宣言
    　例えば「50枚」の花びらを舞わせることにします。プログラム中に直接「50」と言う数字をしまってももちろん動きますが、後から「100枚」に変更したい場合、何百行、何千行ものコード中から「50」と言う数字を探し出し、「100」に変更する必要があります。 @<br>{}
    　これはとても大変な作業になります。変更漏れがあるかもしれませんし、あるいは誤って、他の意味で使われていた「50」と言う数字を書き換えてしまうかもしれません。 そこで、ソースコード中に「50」と直接書く代わりに、 @<code>{NUMBER_OF_HANABIRAS} と定数を使って書くことをお薦めします。枚数を変更したくなったときには、その定数を宣言した行を参照して、 @<code>{const NUMBER_OF_HANABIRAS = 50} から @<code>{const NUMBER_OF_HANABIRAS = 100} と変更するだけで済みます。 @<br>{}
    　修正も容易ですし、プログラムを見たときにも、「50」と言う数字の代わりに @<code>{NUMBER_OF_HANABIRAS} と書かれていますので、「花びらの枚数である」と意味がはっきり分かり、詠みやすく理解しやすいコードを書くことが出来ます。
 ...
 : イベントリスナの設定
    　一般に、ウェブサイトのページが読み込まれた時(load)、画面がスクロールされた時(scroll)、ボタンが押された時(click)など、様々なイベントが発生します。様々なイベントが発生したときにどのような関数・処理を実行するのかを指定できるよう、JavaScriptには、イベントリスナと呼ばれる機能が備わっています。 @<br>{}
    　ここでは、画面がスクロールされたときに、花びらが画面のウインドウ内に納まるよう、スクロール位置を取得するイベントリスナを準備します。
 ...
 : 時々刻々、花びらの位置を更新して、ひらひら舞わせる処理
    　50枚の花びらそれぞれへの繰り返し処理を行う為には @<code>{for文}を用います。また、一定時間ごとに定期的に処理を行えるよう、JavaScript には @<code>{setInterval()} 関数が用意されていますので、これを使います。


=== hanabira.js のご紹介
それでは、花びらクラスを用いたコードのご紹介です。

@<code>{class}構文を用いて、花びらクラスを作成しています。花びらの位置情報などの属性と、それを操作するためのメソッド(関数)が実装されています。また画面表示するには、それぞれの花びらインスタンスが持つ位置情報を、実際の花びらdiv要素(DOM)に反映させるに反映させるようにしています。
...
@<href>{https://developer.mozilla.org/ja/docs/Glossary/IIFE, 即時実行関数式} に概要が記載されておりますので、紹介いたします。
...
即時実行関数を簡単な例 @<fn>{750}で確認してみましょう。

//footnote[750][ある意味、簡単ではない例となっていますが、@<href>{https://ja.wikipedia.org/wiki生命、宇宙、そして万物についての究極の疑問の答え} に説明がございますので、ご覧ください。]

以下のHTMLを書きます。

//list[][answer_to_life_the_universe_and_everything.html]{
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
...
@<code>{<h1>}で見出しを設定、@<code>{<script>}タグで、解答を与えてくれる JavaScript ファイルを読み込んでいます。

//blankline

読み込まれる JavaScript ファイル は次のように書きます。

//list[][answer_to_life_the_universe_and_everything.js][1]{
function forty_two() {
  alert("42")
}
//}

それではブラウザで、@<file>{answer_to_life_the_universe_and_everything.html} を読み込んで、「生命、宇宙、そして万物についての究極の疑問の答え」を見てみましょう。

何か解答が表示されたでしょうか。いえ、そのままです。

@<code>{forty_two関数}は、定義されたのみで、実行されてはいません。関数を呼び出すためには、次のように書く必要があります。
...
今度は「生命、宇宙、そして万物についての究極の疑問の答え」として「42」が表示されたはずです。JavaScriptでは、関数を呼び出し実行する為には、関数名 @<code>{forty_two} に続けて、関数を呼び出し実行することを示す @<code>{()} (丸括弧・関数呼び出し演算子)を付ける必要があります。

//blankline
さて、1行目から3行目までで定義した関数に@<code>{forty_two}と名前が付けられていましたので、5行目で、関数名 @<code>{forty_two} に続けて、呼び出し演算子@<code>{()}を続けて、 @<code>{forty_two()} と書くことで、関数を実行することが出来ました。

もし、@<code>{forty_two} と関数名が付けられていなかった場合、つまり以下のように書かれていた場合には、どうやって呼び出せば良いのでしょうか。
...
@<code>{function 〜 \}} までの関数全体を、グループ化演算子 @<code>{()丸括弧}で囲むことで呼び出したい関数を指し示します。そして、その指し示した関数を呼び出すために、関数呼び出し演算子@<code>{()}を適用させることで、指し示した無名関数を実行することができます。

//list[][answer_to_life_the_universe_and_everything.js]{
function () { alert("42") }
~~~~~~~~~~~~~~~~~~~~~~~~~~~
  この部分が関数の本体
...
#@# //sideimage[white_space][50mm][sep=5mm,side=R]{
先に紹介した @<file>{hanabira.js} は、次のように書かれていました。
#@# //}

//sideimage[Answer_to_Life][50mm][sep=5mm,side=R]{
//list[][]{
(
() => {
