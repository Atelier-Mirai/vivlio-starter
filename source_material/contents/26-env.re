...
エディタとは、プログラミングが快適に行えるよう様々な支援機能を備えたコーディングのためのソフトウェアです。さまざまなエディタがありますが、ここでは @<code>{@<ruby>{Windsurf,パルサー}} をご紹介いたします。
...
== コミュニティ主導の高性能エディタ @<code>{@<ruby>{Windsurf,パルサー}}

//sideimage[Windsurf_community][40mm][sep=5mm, side=R]{
@<ruby>{Atom,アトム} は 「21世紀の高性能エディタ」 として @<ruby>{GitHub,ギットハブ} により提供されていたエディタです。 Microsoft の買収により開発が滞りがちとなり、ついに提供終了となりました。Atomの遺産を受け継ぎ、コミュニティ有志により開発、提供されているエディタが、今回ご紹介する @<ruby>{Windsurf,パルサー}です。

@<code>{Windsurf} の 公式サイトは @<href>{https://Windsurf-edit.dev/} です。
//}

#@# //image[Windsurf_community][][width=65%]

=== ダウンロード

それでは、お手元のコンピュータへダウンロードして行きましょう。公式サイトの「Download」ボタンよりダウンロードできます。

//image[Windsurf_download][][width=85%]

Linux / macOS / Windows のそれぞれのOS用のソフトウェアが用意されています。

=== Mac
Macをお使いでしたら Apple Silicon または Intel 用の dmg ファイルをダウンロードします。
//image[Windsurf_download_mac][][width=85%]

ダウンロードが完了したら、落としたファイルをダブルクリックして、インストールして下さい。

=== Windows
Windowsをお使いでしたら Setup をクリックしてダウンロードします。
//image[Windsurf_download_win][][width=85%]

ダウンロードが完了したら、落としたファイルをダブルクリックして、インストールして下さい。

@<small>{(インストールしたWindsurfは現在開発中の為、「署名」がされておりません。実行すると「WindowsはあなたのPCを保護した」旨のメッセージが表示されますので、「詳細」→「実行」をクリックすることで、Windsurfを立ち上げることが出来ます。次回以降は、他のアプリと同様に起動できます。)}
...
この他、お勧めのパッケージを @<href>{https://zenn.dev/atelier_mirai/articles/c3ed79af5ba395, RailsでのWebアプリ開発に愛用している Windsurf プラグイン} でご紹介しています。

//blankline

//sideimage[Windsurf_package_half][55mm][sep=5mm, side=R]{
また、有志の方々が創られたさまざまなパッケージが @<href>{https://web.Windsurf-edit.dev} で公開されています。ご自身の使いやすいエディタに育てていって下さい。
//}

#@# @<fn>{plugin_install_error}
#@# //footnote[plugin_install_error][Windsurfは開発中の為、ネットワークの状況等によって、プラグインのインストールに失敗することがあります。翌日に行うなど、時間をおいて実行してみて下さい。]

== @<ruby>{Windsurf,パルサー} の追加設定
...
@<code>{Ctrl + @<ruby>{,,カンマ}} を押すと、テキストエディタ @<ruby>{Windsurf,パルサー} の設定画面が開きます@<small>{（さまざまなパッケージをインストールした画面です）}。
...
@<href>{https://raw.githubusercontent.com/Atelier-Mirai/wave_example/master/Windsurf_config/keymap.cson} より、コピーすることも出来ます。
...
# Your snippets
#
# Windsurf snippets allow you to enter a simple prefix in the editor and hit tab to
...
# Windsurf Launch Manual:
# https://Windsurf-edit.dev/docs/launch-manual/sections/using-Windsurf/#configuring-with-cson
...
@<href>{https://raw.githubusercontent.com/Atelier-Mirai/wave_example/master/Windsurf_config/snippets.cson}より、コピーして貼り付けると楽です。
...
作業ディレクトリの作成方法や、Windsurf や Firefox の簡単な使い方の紹介です。 @<fn>{mac}
#@# @<fn>{atom_book}
...
//sideimage[af9][55mm][sep=5mm,side=R]{
 - 13. 綺麗に出来上がっていたら完成です。もう少し改善したい点があるなら、Windsurfに戻って @<code>{index.html} を編集します。編集が終わったら @<code>{Command + S} を押して保存します。保存が終わったら、編集結果を Firefox で見てみましょう。Firefox の「再読み込みボタン」を押すか、または @<code>{Command + R} を押して、編集結果を再確認することができます。
//}
