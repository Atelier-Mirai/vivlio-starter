...
ここでは、二番目の @<code>{<li>}要素である @<code>{<li class="software">エディタ <span style="display: none;">(Pulsar / Zed / VS Codeなど)</span></li>} に着目します。 HTML上では @<code>{<span>} タグに Pulsar など具体的なエディタ名が書かれているのですが、@<code>{<span style="disdplay: none;"} と、インラインCSSにより、非表示となっています。
...
コンソール画面で入力すると「エディタ (Pulsar / Zed / VS Codeなど)」と全て表示されます。

//image[dom11][][width=80%]

もとの HTML は、@<code>{<li class="software">エディタ <span style="display: none;">(Pulsar / Zed / VS Codeなど)</span></li>} と書かれていましたが、 @<code>{textContent} メソッドにより @<code>{<li class="software">エディタ (Pulsar / Zed / VS Codeなど)</li>} と、元あったの @<code>{<span>} タグも消去され、@<code>{textContent} での設定文字になることにも注目してください。
