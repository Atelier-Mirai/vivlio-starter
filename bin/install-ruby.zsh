...
#   bin/install-ruby.zsh -v 3.3.5        # 特定の Ruby バージョンを導入
...
if [[ "$VERSION" == "latest" ]]; then
  ...
  latest="$(rbenv install -l 2>/dev/null | awk '{print $1}' | grep -E '^[0-9]+\.[0-9]+\.[0-9]+$' | tail -1 | tr -d ' ')"
  if [[ -z "$latest" ]]; then
  ...
  if [[ -n "$latest" ]]; then
    ...
    echo "⚠️  最新版を自動取得できませんでした。暫定的に 3.4.4 を使用します"
    VERSION="3.4.4"
  fi
fi
